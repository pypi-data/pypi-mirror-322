...
async def get_ulist_item_lseq_on_reorder(db, email, ulist_code, id1, id2, insert_mode=False):
    """Produces the new lseq for a re-order operation for ulist_items."""
    l = LexIdx()
    # If no re-order is required, return None of updates and insert in the end for inserts
    if id1 is None and id2 is None:
        if insert_mode:
            res = await db.execute(f"select max(lexseq) from ulist_items where email='{email}' and ulist_code='{ulist_code}'")
            lseq = oget(res,[0,'max'])
            return l.next(lseq)
        return None
