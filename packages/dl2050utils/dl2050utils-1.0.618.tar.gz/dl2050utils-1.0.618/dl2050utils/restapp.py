...
class App():
    ...
    async def ulists_insert(self, request):
        ...
        data = await request.json()
        kwargs = get_optional_args(data, ['ulist_code', 'ulist_group', 'ulist_name', 'ulist_payload', 'id1', 'id2'])
        # If ulist_code is provided
        ulist_code = oget(kwargs, ['ulist_code'])
        if ulist_code is not None:
            rows = await ulists_select(self.db, email, ulist_code=ulist_code)
            # If record exists, do not insert and return existing record
            if len(rows)!=0: return rest_ok(rows[0])
        row = await ulists_insert(self.db, email, **kwargs)
        if row is None: 
    ...
    async def ulists_update(self, request):
        ...
        email = await self.auth.get_email_from_uid(u['uid'])
        data = await request.json()
        args = enforce_required_args(self.LOG, data, ['ulist_id'], label='REST', label2='ulists_update')
        ulist_id = itemgetter('ulist_id')(args)
        kwargs = get_optional_args(data, ['ulist_code', 'ulist_group', 'ulist_name', 'ulist_payload', 'id1', 'id2'])
        if await ulists_update(self.db, email, ulist_id, **kwargs): 
    ...
    async def ulists_delete(self, request):
        ...
        email = await self.auth.get_email_from_uid(u['uid'])
        data = await request.json()
        args = enforce_required_args(self.LOG, data, ['ulist_id'], label='REST', label2='ulists_delete')
        ulist_id = itemgetter('ulist_id')(args)
        if await ulists_delete(self.db, email, ulist_id):
    ...
    async def ulists_select(self, request):
        ...
        email = await self.auth.get_email_from_uid(u['uid'])
        data = await request.json()
        kwargs = get_optional_args(data, ['ulist_id', 'ulist_group', 'ulist_code'])
    ...
    async def ulist_items_insert(self, request):
        ...
        args = enforce_required_args(self.LOG, data, ['ulist_id'], label='REST', label2='ulists_delete')
        ulist_id = itemgetter('ulist_id')(args)
        kwargs = get_optional_args(data, ['item_code', 'item_name', 'item_payload', 'id1', 'id2'])
        if await ulist_items_insert(self.db, email, ulist_id, **kwargs): 
    ...
    async def ulist_items_update(self, request):
        ...
        email = await self.auth.get_email_from_uid(u['uid'])
        data = await request.json()
        args = enforce_required_args(self.LOG, data, ['item_id', 'ulist_id'], label='REST', label2='ulists_update')
        item_id,ulist_id = itemgetter('item_id')(args),itemgetter('ulist_id')(args)
        kwargs = get_optional_args(data, ['item_code', 'item_name', 'item_payload', 'id1', 'id2'])
        if await ulist_items_update(self.db, email, item_id, ulist_id, **kwargs): 
    ...
    async def ulist_items_delete(self, request):
        ...
        email = await self.auth.get_email_from_uid(u['uid'])
        data = await request.json()
        args = enforce_required_args(self.LOG, data, ['id'], label='REST', label2='ulist_items_delete')
        ulist_item_id = itemgetter('id')(args)
        if await ulist_items_delete(self.db, email, ulist_item_id):
            log_and_raise_rest(self.LOG, label='REST', label2='ulists_delete', status_code=400)
        return rest_ok()
    ...
    async def ulist_items_select(self, request):
        ...
        email = await self.auth.get_email_from_uid(u['uid'])
        data = await request.json()
        args = enforce_required_args(self.LOG, data, ['ulist_id'], label='REST', label2='ulist_items_select')
        ulist_id = itemgetter('ulist_id')(args)
        rows = await ulist_items_select(self.db, email, ulist_id)
        return rest_ok(rows)
