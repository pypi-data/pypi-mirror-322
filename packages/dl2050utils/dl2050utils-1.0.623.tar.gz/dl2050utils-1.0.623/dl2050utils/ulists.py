...
async def get_ulist_item_lseq_on_reorder(db, email, ulist_code, id1, id2, insert_mode=False):
    """Produces the new lseq for a re-order operation for ulist_item."""
    # If no re-order is required, return None of updates and insert in the end for inserts
    if id1 is None and id2 is None:
        if insert_mode:
            res = await db.execute(f"select max(lexseq) from ulists where email='{email}' and ulist_code='{ulist_code}'")
            lseq = oget(res,[0,'max'])
            return l.next(lseq)
        return None
    ...
    lseq1 = await get_lseq(db, 'ulist_items', 'item_id',id1)
    lseq2 = await get_lseq(db, 'ulist_items', 'item_id',id2)
...
async def ulists_insert(db, email, ulist_code, ulist_name=None, ulist_payload=None):
    ...
    """
    # If ulist_code is provided, check if UList record exists, and returns it along with all items
    if ulist_code is not None:
        res = await db.select('ulists', filters={'email':email, 'ulist_code':ulist_code}, limit=1)
        rows = oget(res,['data'],[])
        if len(rows)!=0:
            row = rows[0]
            res = await db.select('ulist_items', filters=[{'ulist_code':row['ulist_id']}])
...
async def ulist_items_insert(db, email, ulist_code, item_name=None, item_payload=None, item_id_1=None, item_id_2=None):
    ...
    """
    # Create the record
    row = {'email':email, 'ulist_code':ulist_code, 'item_name':item_name,
           'item_payload':item_payload, 'created_at': now()}
    # Manage order: either interpolate item_id_1, item_id_2 or insert at the end
    row['lexseq'] = await get_ulist_item_lseq_on_reorder(db, email, ulist_code, item_id_1, item_id_2, insert_mode=True)
...
async def ulist_items_update(db, email, item_id, ulist_code, item_name=None, item_payload=None, item_id_1=None, item_id_2=None):
    ...
    lexseq = await get_ulist_item_lseq_on_reorder(db, email, ulist_code, item_id_1, item_id_2)
...
async def ulist_items_select(db, email, ulist_code):
    ...
    """
    filters = {'email':email, 'ulist_code':ulist_code}
