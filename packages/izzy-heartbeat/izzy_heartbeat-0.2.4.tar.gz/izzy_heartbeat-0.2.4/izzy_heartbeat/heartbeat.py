...
class HeartbeatServerThread(threading.Thread):
    ...
    _event (Event)
        A threading event to control pausing and resuming the pulse, if necessary.
    ...
    def run(self):
        """
        Starts the ``HeartbeatThread``, which will run indefinitely. Sends the message and after waiting the
        specified interval, in a loop. Setting and clearing `_event` controls execution of the loop.
        """
        while True:
            self._event.wait()
