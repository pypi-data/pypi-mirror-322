...
class NotebookEngineServiceApi(object):
    ...
    def notebook_engine_service_get_notebook_engine(
        ...
        **kwargs
        ...
        >>> thread = api.notebook_engine_service_get_notebook_engine(name_16, async_req=True)
        >>> result = thread.get()

        Args:
            name_16 (str): NotebookEngine resource name. Format: workspaces/{workspace}/notebookEngines/{notebook_engine}  Authorization may require the following permission on the engine's parent workspace: * actions/enginemanager/notebookEngines/GET
    ...
    def notebook_engine_service_update_notebook_engine(
        ...
        **kwargs
        ...
        Args:
            ...
            update_mask (str): The list of paths referencing which fields to update. Update mask must be non-empty.  Allowed field paths are: {\"profile\", \"cpu\", \"gpu\", \"memory_bytes\", \"display_name\", \"max_idle_duration\", \"max_running_duration\"}. Paths are case sensitive (must match exactly).  To update all allowed fields, specify exactly one path with value \"*\".
