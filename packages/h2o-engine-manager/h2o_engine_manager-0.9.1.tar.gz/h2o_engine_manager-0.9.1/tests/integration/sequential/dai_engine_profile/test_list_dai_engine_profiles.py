...
def test_list_dai_engine_profiles(
    ...
    profiles = dai_engine_profile_client_admin.list_all_dai_engine_profiles(parent=GLOBAL_WORKSPACE)
    assert len(profiles) == 2
    assert profiles[0].name == "workspaces/global/daiEngineProfiles/p3"
    assert profiles[1].name == "workspaces/global/daiEngineProfiles/p1"

    # user can list only p3 (has disabled roles).
    profiles = dai_engine_profile_client.list_all_dai_engine_profiles(parent=GLOBAL_WORKSPACE)
