...
def test_list_h2o_engine_profiles(
    ...
    profiles = h2o_engine_profile_client_admin.list_all_h2o_engine_profiles(parent=GLOBAL_WORKSPACE)
    assert len(profiles) == 2
    assert profiles[0].name == "workspaces/global/h2oEngineProfiles/p3"
    assert profiles[1].name == "workspaces/global/h2oEngineProfiles/p1"

    # user can list only p3 (has disabled roles).
    profiles = h2o_engine_profile_client.list_all_h2o_engine_profiles(parent=GLOBAL_WORKSPACE)
