...
def test_list_notebook_engine_profiles(
    ...
    # admin can list only p1 (has assigned its role) and p3 (has disabled roles).
    profiles = notebook_engine_profile_client_admin.list_all_notebook_engine_profiles(parent=GLOBAL_WORKSPACE)
    assert len(profiles) == 2
    assert profiles[0].name == "workspaces/global/notebookEngineProfiles/p3"
    assert profiles[1].name == "workspaces/global/notebookEngineProfiles/p1"

    # user can list only p3 (has disabled roles).
    profiles = notebook_engine_profile_client.list_all_notebook_engine_profiles(parent=GLOBAL_WORKSPACE)
