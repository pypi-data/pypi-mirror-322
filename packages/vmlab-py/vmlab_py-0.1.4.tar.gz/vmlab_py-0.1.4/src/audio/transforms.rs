...
pub struct Audio2MelSpectrogramRT {
    pub sample_rate: u32,
    pub fft_size: usize,
    pub hop_size: usize,
    pub center: bool,
...
impl Audio2MelSpectrogramRT {
    ...
    pub fn initialize(&mut self, audio: Vec<f32>, chunks: usize) -> usize {
        // PRE-PROCESS
        let (wav_length, frame_count) = parse_audio_length(audio.len(), self.sample_rate, 25);

        let mut audio = crop_pad_audio(
            audio,
            wav_length,
            if self.center { 0 } else { self.fft_size },
        );
        ...
        // Preemphasis
        if let Some(preemp) = self.preemphasis {
            preemphasis(&mut audio, preemp);
        }
        self.spectrogram.clear();
        self.mel_index = 0.0;
        self.mel_outputs.clear();
        self.audio = audio.chunks(self.hop_size).map(|v| v.to_vec()).collect();
    ...
    pub fn next<F>(&mut self, norm_fn: F) -> Option<(Array4<f32>, usize, Option<f32>)>
    where
        F: Fn(&Array2<f32>) -> Array2<f32>,
    {
        ...
        // Crop or pad the audio samples
        let begin_idx = self.mel_index.round() as usize;
        let target_mel_len = (self.mel_index + 3.2 * (self.chunk_size - 1) as f32).round() as usize;

        if (self.mel_index / 3.2) as usize > self.frame_count {
            return None;
        }

        while self.mel_outputs.len() < target_mel_len + 16 {
            if self.audio.is_empty() {
                break;
            }

            let samples = self.audio.pop_front().unwrap();

            let Some(fft) = self.spectrogram.add(samples.as_slice()) else {
                continue;
            };
            ...
            let mel = norm_fn(&mel);
            self.mel_outputs.push(mel);
        }

        // fetch the mel from begin_idx to target_mel_len as slice
        // if the mel_outputs is less than target_mel_len, then pad the mel_outputs.
        let mel_slice =
            self.mel_outputs[begin_idx..self.mel_outputs.len().min(target_mel_len + 16)].to_vec();

        let mut stacked_mel = ndarray::Array4::<f32>::zeros((1, 1, 80, mel_slice.len()));

        for (i, mel) in mel_slice.into_iter().enumerate() {
            stacked_mel
                .index_axis_mut(ndarray::Axis(3), i)
                .assign(&mel.to_shape((80,)).unwrap());
        }

        let cur_idx = self.mel_index.fract();

        self.mel_index += 3.2 * self.chunk_size as f32;
        let tg_frame_count = self.chunk_size.min(
            ((self.mel_outputs.len().min(target_mel_len + 16) - begin_idx) as f32 / 3.2) as usize,
        );

        Some((stacked_mel, tg_frame_count, Some(cur_idx)))
    }
}
