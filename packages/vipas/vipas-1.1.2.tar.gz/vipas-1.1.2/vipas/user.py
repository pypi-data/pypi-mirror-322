...
class UserClient:
    ...
    def _configure_decorators(
        ...
        self.generate_token = self._apply_decorators(
            self.generate_token
        )
    ...
    def _call_api(
        ...
        """
        try:
            response_data = self.rest_client._get_token(
                method=method,
                url=url,
                params=params,
                body=body,
    ...
    def generate_token(self, username: str, password: str) -> Dict[str, Any]:
        self.logger.info(f"Generating Temporary Access Token for user {username}.")
        body = {
            "username": username,
            "password": password
            }
        
        url = urljoin(self.configuration.host, "get_token")
        
        _header_params: Dict[str, Optional[str]] =  {}
        self.logger.info(f"Fetching Temporary Access Token from Environment Variables.")
