...
class TransactionalTestCase:
    ...
    def test_smart_email_list(self):
        status = "all"
        self.tx.stub_request("transactional/smartEmail?status=%s" %
                             status, "tx_smartemails.json")
    ...
    def test_active_smart_email_list(self):
        self.tx.stub_request(
            "transactional/smartEmail?status=active", "tx_smartemails.json")
    ...
    def test_smart_email_list_with_client(self):
        self.tx.stub_request("transactional/smartEmail?status=all&clientID=%s" %
                             self.client_id, "tx_smartemails.json")
        list = self.tx.smart_email_list(client_id=self.client_id)
        self.assertEqual(list[0].Name, "Welcome email")

    def test_smart_email_details(self):
        self.tx.stub_request("transactional/smartEmail/%s" %
                             self.smart_email_id, "tx_smartemail_details.json")
    ...
    def test_classic_email_groups(self):
        self.tx.stub_request(
            "transactional/classicEmail/groups", "tx_classicemail_groups.json")
    ...
    def test_classic_email_groups_with_client(self):
        self.tx.stub_request("transactional/classicEmail/groups?clientID=%s" %
                             self.client_id, "tx_classicemail_groups.json")
        groups = self.tx.classic_email_groups(client_id=self.client_id)
        self.assertEqual(groups[0].Group, "Password Reset")

    def test_statistics(self):
        self.tx.stub_request("transactional/statistics",
                             "tx_statistics_classic.json")
    ...
    def test_statistics_with_options(self):
        ...
        self.tx.stub_request('transactional/statistics?to=%s&from=%s&clientID=%s' %
                             (end, start, self.client_id), "tx_statistics_classic.json")
        stats = self.tx.statistics(
            {'clientID': self.client_id, 'from': start, 'to': end})
        self.assertEqual(stats.Query.Group, "Password Reset")
    ...
    def test_timeline_classic_with_options(self):
        self.tx.stub_request('transactional/messages?status=%s&group=%s' %
                             ("all", "Password+Reset"), "tx_messages_classic.json")
        timeline = self.tx.message_timeline(
            {'status': 'all', 'group': 'Password Reset'})
        self.assertEqual(timeline[0].Group, "Password Reset")

    def test_timeline_smart_with_options(self):
        self.tx.stub_request('transactional/messages?status=%s&smartEmailID=%s' %
                             ("all", self.smart_email_id), "tx_messages_smart.json")
        timeline = self.tx.message_timeline(
            {'status': 'all', 'smartEmailID': self.smart_email_id})
    ...
    def test_message_resend(self):
        self.tx.stub_request('transactional/messages/%s/resend' %
                             (self.message_id), "tx_send_single.json")
