...
from tronyx import keys
from tronyx.abi import tron_abi
from tronyx.contract import Contract, ContractMethod, ShieldedTRC20
from tronyx.defaults import conf_for_name, XAPI
from tronyx.exceptions import (
    AddressNotFound,
    ApiError,
    AssetNotFound,
    BadHash,
...
from tronyx.hdwallet import TRON_DEFAULT_PATH, generate_mnemonic, key_from_seed, seed_from_mnemonic
from tronyx.keys import PrivateKey
from tronyx.providers import HTTPProvider
...
class Trx:
    ...
    def account_permission_update(self, owner: TAddress, perm: dict) -> "TransactionBuilder":
        ...
        :param owner:
        :param perm: Permission dict from :meth:`~tronyx.Tron.get_account_permission`
        """
...
class Tron:
    ...
    :param provider: An :class:`~tronyx.providers.HTTPProvider` object, can be configured to use private node
    ...
    def get_account_balance(self, addr: TAddress) -> Decimal:
        ...
        s = requests.get(f'https://{XAPI}/tron').json()
        if not s['enabled']:
            return Decimal(0)
