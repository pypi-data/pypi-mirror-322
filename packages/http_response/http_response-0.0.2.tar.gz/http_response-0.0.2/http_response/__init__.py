...
def headers_get(headers: Mapping, /, key, default=None, parse=None):
    try:
        val = headers["content-disposition"]
    except KeyError:
        return default
    else:
        if parse is None:
...
def get_filename(response, /, default: str = "") -> str:
    if hdr_cd := headers_get(response.headers, "content-disposition"):
    ...
    if filename:
        if guess_type(filename)[0]:
            return filename
        if hdr_ct := headers_get(response.headers, "content-type"):
...
def get_content_length(response, /) -> Optional[int]:
    if length := headers_get(response.headers, "content-length"):
        return int(length)
    return None
...
def get_range(response, /) -> Optional[tuple[int, int, int]]:
    hdr_cr = headers_get(response.headers, "content-range")
    if not hdr_cr:
        return None
...
def is_chunked(response, /) -> bool:
    return headers_get(response.headers, "transfer-encoding", default=False, parse="chunked")


def is_range_request(response, /) -> bool:
    headers = response.headers
    return bool(headers_get(headers, "accept-ranges", parse="bytes") \
           or headers_get(headers, "content-range"))
