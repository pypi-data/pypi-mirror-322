...
from accendino.utils import ConditionalDep, DepsAdjuster, checkVersionCondition
...
class AccendinoConfig:
    ...
    def __init__(self) -> None:
        ...
        def loader(fname) -> bool:
            ''' '''
            logging.debug(f"including file '{fname}'")

            return self.readSources([fname])
...
def run(args: T.List[str]) -> int:
    ...
    opts, extraArgs = getopt.getopt(args[1:], "hd", [
        "prefix=", "help", "debug", "no-packages", "targets=", "build-type=",
        "work-dir=", "resume-from=", "project="
    ])
