...
class BuildArtifact(DepsBuildArtifact):
    ...
    def __init__(self, name: str, deps, srcObj, extraEnv={}, provides=[], pkgs={}, prepare_cmds = [], build_cmds=[]) -> None:
    ...
    def runCommands(self, runItems, env, config):
        with open(self.logFile, "at", encoding='utf8') as flog:
    ...
    def setMakeNinjaCommands(self, config, cmd='ninja', build_target='all', install_target='install', parallelJobs=True, runInstallDir='{builddir}'):
...
class CustomCommandBuildArtifact(BuildArtifact):
    ...
    def __init__(self, name: str, deps, srcObj, extraEnv={}, provides=[], pkgs={}, prepare_src_cmds=[], prepare_cmds=[], build_target='all',
                 install_target='install') -> None:
...
class CMakeBuildArtifact(BuildArtifact):
    ...
    def __init__(self, name, deps, srcObj, cmakeOpts=[], parallelJobs=True, extraEnv={}, provides=[], pkgs={}) -> None:
        '''
            name:
            deps:
            srcObj:
...
class QMakeBuildArtifact(BuildArtifact):
    ...
    def __init__(self, name, deps, srcObj, extraEnv={}, provides=[], pkgs={}) -> None:
        '''
            name:
            deps:
            srcObj:
...
class AutogenBuildArtifact(BuildArtifact):
    ...
    def __init__(self, name, deps, srcObj, autogenArgs=[], noconfigure=False, isAutogen=True, bootstrapScript='bootstrap.sh',
                 configureArgs=[], runInstallDir='{builddir}', extraEnv={}, provides=[], pkgs={}) -> None:
        '''
            name:
            deps:
            srcObj:
...
class MesonBuildArtifact(BuildArtifact):
    ...
    def __init__(self, name, deps, srcObj, mesonOpts=[], parallelJobs=True, extraEnv={}, provides=[], pkgs={}) -> None:
        BuildArtifact.__init__(self, name, deps, srcObj, extraEnv, provides, pkgs)
