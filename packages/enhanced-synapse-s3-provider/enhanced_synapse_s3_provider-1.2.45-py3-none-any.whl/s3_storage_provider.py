    ...
    async def store_file(self, path: str, file_info: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if self.store_remote:
                ...
                try:
                    logger.info("Uploading %s to S3 bucket '%s' at path '%s'", path, self.bucket, s3_path)
            ...
            # Case 1: store_local is False - delete immediately
            if self.store_local is False:
                should_delete = True
                logger.info("store_local is False, will delete local file: %s", local_path)

            # Case 2: store_local is True with retention=0 - delete immediately
            elif self.store_local is True and self.local_media_lifetime == 0:
                should_delete = True
                logger.info("store_local is True with retention=0, will delete local file: %s", local_path)

            # Case 3: store_local is True with positive retention - schedule deletion
            elif self.store_local is True and self.local_media_lifetime and self.local_media_lifetime > 0:
