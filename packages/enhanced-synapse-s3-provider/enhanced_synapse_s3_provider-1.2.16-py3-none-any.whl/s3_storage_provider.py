...
class S3StorageProviderBackend(StorageProvider):
    ...
    def __init__(self, hs, config):
        ...
        self.store_local = config.get("store_local", True)  # Default to True if not specified
        self.store_remote = config.get("store_remote", True)
        self.store_synchronous = config.get("store_synchronous", True)
        
        # Get retention settings from media_retention config
        media_retention = getattr(hs.config, 'media_retention', None)
        if media_retention:
            self.local_retention_ms = getattr(media_retention, 'local_media_lifetime', None)
            self.remote_retention_ms = getattr(media_retention, 'remote_media_lifetime', None)
        else:
            self.local_retention_ms = None
            self.remote_retention_ms = None

        logger.info(f"Store settings - store_local: {self.store_local}, store_remote: {self.store_remote}")
        logger.info(f"Retention settings - local: {self.local_retention_ms}ms, remote: {self.remote_retention_ms}ms")

        # Initialize S3 configuration
        self.s3_config = config.get("config", {})
        ...
        self.extra_args = self.s3_config.get("extra_args", {})
        self.api_kwargs = {}

        if "region_name" in self.s3_config:
            self.api_kwargs["region_name"] = self.s3_config["region_name"]

        if "endpoint_url" in self.s3_config:
            self.api_kwargs["endpoint_url"] = self.s3_config["endpoint_url"]

        if "access_key_id" in self.s3_config:
            self.api_kwargs["aws_access_key_id"] = self.s3_config["access_key_id"]

        if "secret_access_key" in self.s3_config:
            self.api_kwargs["aws_secret_access_key"] = self.s3_config["secret_access_key"]

        if "session_token" in self.s3_config:
            self.api_kwargs["aws_session_token"] = self.s3_config["session_token"]
    ...
    async def store_file(self, path, file_info):
        """Store a file in S3 and handle local retention based on configuration."""
        ...
        if self.store_remote:
            # Upload to S3
            s3_key = self._get_s3_key(path)
            logger.info(f"Starting S3 upload process for {path}")
            logger.info(f"S3 configuration - Bucket: {self.bucket}, Key: {s3_key}")
            
            try:
                await self._upload_to_s3(path, s3_key)
                logger.info(f"Successfully uploaded file to S3: s3://{self.bucket}/{s3_key}")
                
                # Verify the upload
                s3_client = self._get_s3_client()
                head_response = await defer_to_thread(self.thread_pool, s3_client.head_object, Bucket=self.bucket, Key=s3_key)
                logger.info(f"Verified S3 upload - Size: {head_response['ContentLength']} bytes, ETag: {head_response['ETag']}")

                # Handle local file based on store_local and retention settings
                if not self.store_local:
                    # If store_local is false, delete the file immediately
                    try:
                        os.unlink(path)
                        logger.info(f"Deleted local file (store_local=false): {path}")
                        self._cleanup_empty_directories(os.path.dirname(path))
                    except Exception as e:
                        logger.error(f"Error deleting local file {path}: {e}")
                elif self.local_retention_ms is not None and self.local_retention_ms > 0:
                    # Schedule deletion after retention period
                    retention_seconds = self.local_retention_ms / 1000.0
                    logger.info(f"Scheduling local file deletion in {retention_seconds} seconds: {path}")
                    self.hs.get_reactor().callLater(retention_seconds, self._delete_file_after_retention, path)

            except Exception as e:
                logger.error(f"Error uploading file to S3: {e}")
                if not self.store_local:
                    # If we're not storing locally and S3 upload failed, raise the error
                    raise
                return None

        return None

    def _delete_file_after_retention(self, path):
        """Delete a file after its retention period has expired."""
        try:
            if os.path.exists(path):
                os.unlink(path)
                logger.info(f"Deleted local file after retention period: {path}")
                self._cleanup_empty_directories(os.path.dirname(path))
            else:
                logger.warning(f"File not found for deletion after retention: {path}")
        except Exception as e:
            logger.error(f"Error deleting file after retention period {path}: {e}")
    ...
    def fetch(self, path, file_info):
        ...
        self.thread_pool.callInThread(_get_file)
