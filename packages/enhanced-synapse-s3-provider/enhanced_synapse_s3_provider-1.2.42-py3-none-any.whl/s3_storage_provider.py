...
class S3StorageProviderBackend(StorageProvider):
    ...
    def __init__(self, hs, config):
        ...
        raw_store_local = config.get("store_local")
        raw_store_remote = config.get("store_remote")
        raw_store_synchronous = config.get("store_synchronous")

        # Parse boolean values with defaults
        self.store_local = parse_bool(raw_store_local)
        self.store_remote = parse_bool(raw_store_remote)
        self.store_synchronous = parse_bool(raw_store_synchronous)

        # Apply defaults only if values are None
        if self.store_local is None:
            self.store_local = False
        if self.store_remote is None:
            self.store_remote = True
        if self.store_synchronous is None:
            self.store_synchronous = True
        ...
        # Clean up None values from api_kwargs
    ...
    async def store_file(self, path: str, file_info: Dict[str, Any]) -> None:
        ...
        try:
            logger.info("Processing file %s with store_local=%s, store_remote=%s", path, self.store_local, self.store_remote)
            ...
            # Upload to S3 if store_remote is True
            ...
            if self.store_remote:
                ...
                try:
                    logger.info("Attempting to upload %s to S3 bucket '%s' at path '%s'", path, self.bucket, s3_path)
                    ...
                    try:
                        ...
                        logger.info("Successfully verified upload of %s to S3 at %s", path, s3_path)
                    except Exception as e:
                        logger.error("Failed to verify S3 upload for %s: %s", path, str(e))
                        raise Exception(f"S3 upload verification failed for {path}")
                except Exception as e:
                    logger.error("Failed to upload %s to S3: %s", path, str(e))
                    raise

            # Only proceed with local file deletion if S3 upload was successful
            if self.store_local is False:  # Explicitly check for False
                if not self.store_remote or (self.store_remote and s3_upload_success):
                    logger.info("store_local is False, deleting local file: %s", local_path)
                    try:
                        os.remove(local_path)
                        logger.info("Successfully deleted local file: %s", local_path)
                        self._cleanup_empty_directories(local_path)
                    except Exception as e:
                        logger.error("Failed to remove local file %s: %s", local_path, str(e))
                        raise
                else:
                    logger.warning("Not deleting local file %s as S3 upload was not successful", local_path)
            elif self.local_media_lifetime is not None:
                if self.local_media_lifetime == 0:
                    if not self.store_remote or (self.store_remote and s3_upload_success):
                        try:
                            os.remove(local_path)
                            logger.info("Removed local file (retention=0): %s", local_path)
                            self._cleanup_empty_directories(local_path)
                        except Exception as e:
                            logger.error("Failed to remove local file %s: %s", local_path, str(e))
                            raise
                        ...
                        logger.warning("Not deleting local file %s as S3 upload was not successful", local_path)
                elif self.local_media_lifetime > 0:
                    retention_seconds = self.local_media_lifetime / 1000.0
