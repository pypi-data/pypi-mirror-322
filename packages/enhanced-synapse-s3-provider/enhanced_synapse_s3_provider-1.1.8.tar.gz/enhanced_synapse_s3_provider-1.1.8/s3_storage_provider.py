...
class S3StorageProviderBackend(StorageProvider):
    ...
    async def store_file(self, path: str, file_info: Dict[str, Any]) -> None:
        ...
        try:
            ...
            is_local = abs_path.startswith(abs_uploads) or (
                abs_path.startswith(abs_media_store) and 
                any(p.startswith("local") for p in os.path.relpath(abs_path, abs_media_store).split(os.sep))
            )
