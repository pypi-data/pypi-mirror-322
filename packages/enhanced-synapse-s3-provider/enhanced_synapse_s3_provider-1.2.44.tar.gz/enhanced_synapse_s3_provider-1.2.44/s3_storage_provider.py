...
class S3StorageProviderBackend(StorageProvider):
    ...
    async def store_file(self, path: str, file_info: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Handle local file based on store_local setting
            if self.store_local is False:  # Explicitly check for False
                if not self.store_remote or s3_upload_success:
                    try:
                        ...
                        raise
            elif self.store_local is True and self.local_media_lifetime is not None:
