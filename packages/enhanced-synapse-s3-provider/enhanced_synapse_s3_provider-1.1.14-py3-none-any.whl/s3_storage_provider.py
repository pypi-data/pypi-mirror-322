...
class S3StorageProviderBackend(StorageProvider):
    ...
    def __init__(self, hs, config):
        # Get media paths from Synapse config if they exist
        self.media_store_path = getattr(hs.config.media, 'media_store_path', None)
        self.uploads_path = getattr(hs.config.media, 'uploads_path', None)
        ...
        # Get media retention settings from provider config
        retention_config = config.get("media_retention", {})
        # Convert from milliseconds to seconds
        self.local_media_lifetime = retention_config.get("local_media_lifetime", 0) / 1000 if retention_config.get("local_media_lifetime") else None
        self.remote_media_lifetime = retention_config.get("remote_media_lifetime", 0) / 1000 if retention_config.get("remote_media_lifetime") else None
        
        if self.local_media_lifetime or self.remote_media_lifetime:
            logger.info("Media retention configured - Local: %s seconds, Remote: %s seconds", 
                      self.local_media_lifetime or "disabled", 
                      self.remote_media_lifetime or "disabled")
            # Start cleanup task
            self._start_cleanup_task()
        else:
            logger.info("No media retention configured, files will be deleted immediately after upload")
        ...
        self._pending_files = {}  # Dict to track files and their deletion times

        threadpool_size = config.get("threadpool_size", 40)
        self._s3_pool = ThreadPool(name="s3-pool", maxthreads=threadpool_size)
        self._s3_pool.start()

        # Manually stop the thread pool on shutdown
        reactor.addSystemEventTrigger(
            "during", "shutdown", self._cleanup_and_stop,
        )
        ...
        for path, is_local in files_to_delete:
            try:
                if os.path.exists(path):
                    ...
                    base_dir = self.uploads_path if (is_local and self.uploads_path) else self.media_store_path
    ...
    async def store_file(self, path: str, file_info: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Only proceed with local cleanup if paths are configured
            if not (self.media_store_path or self.uploads_path):
                logger.info("No media paths configured, skipping local cleanup for %s", path)
                return

            abs_path = os.path.abspath(local_path)
            
            # Determine if media is local based on path and configured paths
            is_local = False
            ...
            # If retention period is set, schedule the file for deletion
            if retention_period:
            ...
            # Only delete if no retention period is set
            logger.info(
