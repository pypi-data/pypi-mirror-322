    ...
    async def store_file(self, path: str, file_info: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if os.path.exists(local_path):
                ...
                logger.warning("Local file not found for cleanup: %s", local_path)
        except OSError as e:
            logger.error("Failed to remove local file %s after S3 upload: %s", local_path, str(e))
            # We'll raise the error here to ensure we know if cleanup fails
            raise
