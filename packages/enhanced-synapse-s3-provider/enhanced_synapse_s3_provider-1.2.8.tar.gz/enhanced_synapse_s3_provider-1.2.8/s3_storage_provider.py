...
class S3StorageProviderBackend(StorageProvider):
    ...
    def __init__(self, hs, config):
        ...
        # Convert store_local to boolean, default to True if not specified
        self._store_local = str(config.get("store_local", True)).lower() == "true"
        logger.info(f"store_local setting (after bool conversion): {self._store_local}")
        ...
        # Handle media_retention settings
        self._media_retention = getattr(hs.config, "media_retention", {})
        self._local_media_lifetime = self._parse_duration(self._media_retention.get("local_media_lifetime"))
        self._remote_media_lifetime = self._parse_duration(self._media_retention.get("remote_media_lifetime"))
    ...
    def store_file(self, path, file_info):
        ...
        This is called after Synapse has already stored the file locally.
        We need to handle the upload to S3 and then clean up the local file
        if store_local is False.
        """
        ...
        def _store_file():
            with LoggingContext(parent_context=parent_logcontext):
                ...
                logger.info(f"Starting S3 upload for {local_path} with store_local={self._store_local} (type: {type(self._store_local)})")
                ...
                try:
                    # Use upload_file which handles multipart uploads and is more reliable
                    ...
                    logger.info(f"Successfully uploaded {local_path} to S3 with key {s3_key}")

                    # Verify the upload by checking if file exists in S3
                    try:
                        self._get_s3_client().head_object(Bucket=self.bucket, Key=s3_key)
                        logger.info(f"Verified S3 upload for {s3_key}")
                        
                        # Delete local file if store_local is explicitly False
                        if self._store_local is False:
                            try:
                                os.remove(local_path)
                                logger.info(f"Deleted local file {local_path} after S3 upload (store_local is explicitly False)")
                        ...
                        logger.error(f"Failed to verify S3 upload for {s3_key}: {e}")
                        raise
        ...
        # Run in thread pool and make deferred yieldable
    ...
    def parse_config(config):
        ...
        # New: Add store_local to the config
    ...
    def _cleanup_empty_directories(self, path):
        ...
        while directory and directory.startswith(self.cache_directory):
            try:
