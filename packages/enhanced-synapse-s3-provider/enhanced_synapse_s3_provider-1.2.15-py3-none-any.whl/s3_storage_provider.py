...
class S3StorageProviderBackend(StorageProvider):
    ...
    def __init__(self, hs, config):
        ...
        self._local_media_lifetime = getattr(hs.config.media, "local_media_lifetime", 0)
        self._remote_media_lifetime = getattr(hs.config.media, "remote_media_lifetime", 0)
            ...
            with LoggingContext(parent_context=parent_logcontext):
                ...
                try:
                    ...
                    if not self._store_local:
                        # If store_local is false, we don't delete immediately
                        # Instead, we let Synapse handle the file with its retention settings
                        logger.info(f"File will be handled by Synapse retention (store_local=false): {local_path}")
                    elif self._local_media_lifetime > 0:
                        # If store_local is true and retention is set, let Synapse handle retention
                        logger.info(f"File will be retained locally for {self._local_media_lifetime} seconds: {local_path}")
                    else:
