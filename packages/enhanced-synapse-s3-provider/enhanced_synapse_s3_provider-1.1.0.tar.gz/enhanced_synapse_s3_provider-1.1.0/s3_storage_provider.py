...
class S3StorageProviderBackend(StorageProvider):
    ...
    def __init__(self, hs, config):
        # Get media paths from Synapse config
        ...
        # Get media retention settings from Synapse config
        try:
            # Access the media retention settings through the homeserver config
            retention_config = hs.config.server.media_retention
            self.local_media_lifetime = retention_config.local_media_lifetime if hasattr(retention_config, 'local_media_lifetime') else None
            self.remote_media_lifetime = retention_config.remote_media_lifetime if hasattr(retention_config, 'remote_media_lifetime') else None
            
            if self.local_media_lifetime or self.remote_media_lifetime:
                logger.info("Media retention configured - Local: %s, Remote: %s", 
                          self.local_media_lifetime or "disabled", 
                          self.remote_media_lifetime or "disabled")
        except Exception as e:
            logger.warning("Could not access media retention settings: %s. Defaulting to immediate cleanup.", str(e))
    ...
    async def store_file(self, path: str, file_info: Dict[str, Any]) -> None:
        ...
        # Determine if this is local or remote media based on the full path
        abs_path = os.path.abspath(local_path)
        abs_media_store = os.path.abspath(self.media_store_path)
        abs_uploads = os.path.abspath(self.uploads_path)
        
        # Check if path is under uploads directory (local media) or media_store (could be either)
        is_local = abs_path.startswith(abs_uploads)
        if not is_local and abs_path.startswith(abs_media_store):
            # If in media_store but not in uploads, check path structure
            # Local media typically has a different path structure than remote media
            path_parts = os.path.relpath(abs_path, abs_media_store).split(os.sep)
            is_local = len(path_parts) > 0 and path_parts[0] == "local_content"
        ...
        should_retain = bool(retention_period)
        
        if should_retain:
            logger.info(
                "Keeping file %s for retention period %s",
                local_path,
                retention_period
            )
        else:
            logger.info(
                "No retention period set for %s media, will remove local file: %s",
                "local" if is_local else "remote",
                local_path
            )
            try:
                ...
                base_dir = self.uploads_path if is_local else self.media_store_path
                
                while current_dir.startswith(base_dir):
                    try:
    ...
    def cleanup_local_content(self):
        ...
        local_content_dir = os.path.join(self.media_store_path, "local_content")
