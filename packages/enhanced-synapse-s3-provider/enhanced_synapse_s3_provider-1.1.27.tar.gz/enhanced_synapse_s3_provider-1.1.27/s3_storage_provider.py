...
class S3StorageProviderBackend(StorageProvider):
    ...
    async def store_file(self, path: str, file_info: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Upload to S3 using the thread pool
            def upload_to_s3():
            ...
            # Run upload in thread pool and wait for completion
            success = await defer.ensureDeferred(threads.deferToThread(upload_to_s3))
                ...
                if not self.store_local:
                    try:
                        if os.path.exists(local_path):
                            ...
                            logger.info("store_local=false: Removed local file after S3 upload: %s", local_path)
