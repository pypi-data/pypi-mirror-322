...
class S3StorageProviderBackend(StorageProvider):
    ...
    def __init__(self, hs, config):
        ...
        # Get settings with no defaults, so we use the values from config
        self.store_local = parse_bool(config.get("store_local"), False)
        self.store_remote = parse_bool(config.get("store_remote"), True)
        self.store_synchronous = parse_bool(config.get("store_synchronous"), True)
