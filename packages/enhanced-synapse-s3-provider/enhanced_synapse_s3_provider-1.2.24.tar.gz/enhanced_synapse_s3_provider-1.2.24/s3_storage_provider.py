...
class S3StorageProviderBackend(StorageProvider):
    ...
    def __init__(self, hs, config):
        ...
        def parse_bool(value):
            ...
            return bool(value)  # Handle other types by converting to bool

        # Get settings from config
        self.store_local = parse_bool(config.get("store_local", False))  # Default to False
        self.store_remote = parse_bool(config.get("store_remote", True))  # Default to True
        self.store_synchronous = parse_bool(config.get("store_synchronous", True))  # Default to True

        # Get media retention settings from Synapse config
        media_retention = getattr(hs.config.media, "media_retention", None)
        if media_retention:
            self.local_media_lifetime = getattr(media_retention, "local_media_lifetime", None)
            self.remote_media_lifetime = getattr(media_retention, "remote_media_lifetime", None)
        else:
    ...
    async def store_file(self, path: str, file_info: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if self.store_local is False:
                ...
                # If store_local is True or None, handle retention
