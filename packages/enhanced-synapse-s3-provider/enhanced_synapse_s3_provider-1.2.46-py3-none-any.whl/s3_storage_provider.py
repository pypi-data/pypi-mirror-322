        ...
        try:
            ...
            if not self.store_local:
                try:
                    ...
                    raise
            elif self.store_local and self.local_media_lifetime is not None:
                if self.local_media_lifetime == 0:
                    # Delete immediately if retention is 0
                    try:
                        os.remove(local_path)
                        logger.info("Successfully deleted local file (retention=0): %s", local_path)
                        self._cleanup_empty_directories(local_path)
                    except Exception as e:
                        logger.error("Failed to remove local file %s: %s", local_path, str(e))
                        raise
