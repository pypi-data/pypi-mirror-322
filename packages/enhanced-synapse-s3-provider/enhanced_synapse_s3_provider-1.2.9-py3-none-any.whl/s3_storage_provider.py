...
class S3StorageProviderBackend(StorageProvider):
    ...
    def store_file(self, path, file_info):
        ...
        logger.info(f"store_file called for path: {path}, file_info: {file_info}")
        logger.info(f"store_local: {self._store_local}, store_remote: {self._store_remote}")
        ...
        def _store_file():
            with LoggingContext(parent_context=parent_logcontext):
                ...
                logger.info(f"Uploading file {local_path} to S3 with key {s3_key}")
                ...
                try:
                    ...
                    logger.info(f"Successfully uploaded file {local_path} to S3")
                ...
                if not self._store_local:
                    try:
                        ...
                        logger.info(f"Deleted local file {local_path} after upload")
                    except OSError as e:
                        logger.warning(f"Failed to delete local file {local_path}: {e}")
                else:
                    logger.info(f"Keeping local file {local_path} (store_local is true)")
