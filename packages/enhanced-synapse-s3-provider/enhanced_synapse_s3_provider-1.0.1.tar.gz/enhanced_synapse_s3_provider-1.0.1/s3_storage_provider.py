...
class S3StorageProviderBackend(StorageProvider):
    ...
    async def store_file(self, path: str, file_info: Dict[str, Any]) -> None:
        ...
        # Upload to S3 first
        await self._store_file(path, local_path)
        
        # Wait a short period to allow Synapse to complete its processing
        await asyncio.sleep(2)
        
        # Now attempt to remove the local file
        try:
            os.remove(local_path)
            logger.info("Removed local file after S3 upload and processing: %s", local_path)
        except OSError as e:
            logger.warning("Failed to remove local file %s after S3 upload: %s", local_path, str(e))
            # Don't raise the error - let Synapse's cleanup job handle it later
            pass
