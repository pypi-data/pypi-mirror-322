    ...
    def store_file(self, path, file_info):
        ...
        def _store_file():
            with LoggingContext(parent_context=parent_logcontext):
                ...
                logger.info(f"Processing file {local_path} with store_local={self._store_local}")
                ...
                try:
                    ...
                    logger.info(f"Successfully uploaded file {local_path} to S3 with key {s3_key}")

                    # If store_local is false, delete the local file immediately
                    if not self._store_local:
                        try:
                            os.remove(local_path)
                            logger.info(f"Deleted local file {local_path} after S3 upload (store_local=false)")
