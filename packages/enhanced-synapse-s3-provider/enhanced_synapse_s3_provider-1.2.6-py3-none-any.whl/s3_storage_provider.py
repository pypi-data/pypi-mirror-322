...
class S3StorageProviderBackend(StorageProvider):
    ...
    def __init__(self, hs, config):
        ...
        # Store the store_local setting from config
        self._store_local = config.get("store_local", True)
        logger.info(f"store_local setting: {self._store_local}")
    ...
    def store_file(self, path, file_info):
        ...
        def _store_file():
            with LoggingContext(parent_context=parent_logcontext):
                ...
                logger.info(f"Starting S3 upload for {local_path} with store_local={self._store_local}")
                ...
                try:
                    ...
                    try:
                        ...
                        # Only delete local file if store_local is false and S3 upload is verified
                        if not self._store_local:
                            try:
                                os.remove(local_path)
                                logger.info(f"Deleted local file {local_path} after S3 upload")
