...
class S3StorageProviderBackend(StorageProvider):
    ...
    async def store_local_media(self, path: str, file_info: Dict[str, Any]) -> None:
        """Override to handle local media storage differently when store_local is false."""
        if not self.store_local:
            # If store_local is false, we still need to store temporarily for S3 upload
            await self.store_temporary_file(path, file_info)
        else:
            await super().store_local_media(path, file_info)

    async def store_temporary_file(self, path: str, file_info: Dict[str, Any]) -> None:
        """Store file temporarily, upload to S3, then delete."""
        ...
        try:
            ...
            if os.path.exists(local_path):
                ...
                logger.info("Removed temporary local file: %s", local_path)
        except Exception as e:
    ...
    def store_file(self, path: str, file_info: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Handle retention for local files
            ...
            if retention_period:
                ...
                logger.info("File %s scheduled for deletion at %s", local_path, delete_time)
