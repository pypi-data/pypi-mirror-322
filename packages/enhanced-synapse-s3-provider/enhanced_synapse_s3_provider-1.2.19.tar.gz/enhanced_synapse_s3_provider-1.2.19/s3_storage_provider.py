...
class S3StorageProviderBackend(StorageProvider):
    ...
    def _start_cleanup_task(self):
        """Start a periodic task to cleanup expired files."""
                    ...
                    for path, is_local in files_to_delete:
                        try:
                            if os.path.exists(path):
                                ...
                                logger.debug("File already deleted: %s", path)
                        except Exception as e:
                            logger.error("Error deleting expired file %s: %s", path, str(e))
                            
            except Exception as e:
                logger.error("Error in cleanup task: %s", str(e))
            finally:
                # Schedule next cleanup
                reactor.callLater(5, _cleanup_expired_files)
    ...
    async def store_file(self, path: str, file_info: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # If store_local is false or retention is 0, delete the local file immediately
            ...
            if not self.store_local or retention_period == 0:
                try:
            ...
            # If we get here, store_local is true and retention is not 0
            if retention_period:
