    ...
    async def store_file(self, path: str, file_info: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if self.store_local is False:
                # If store_local is explicitly False, delete immediately after S3 upload
                try:
                    ...
                    raise
            elif self.store_local is True:
                # Handle retention only if store_local is explicitly True
                ...
                if retention_period is not None and retention_period >= 0:
                    ...
                    logger.info(
                        "File %s will be kept indefinitely (no retention period)",
                        local_path
                    )
            else:
                # If store_local is None, treat as False for safety
                try:
                    os.remove(local_path)
                    logger.info("Removed local file (store_local=None): %s", local_path)
                    self._cleanup_empty_directories(local_path)
                except Exception as e:
                    logger.error("Failed to remove local file %s: %s", local_path, str(e))
                    raise
