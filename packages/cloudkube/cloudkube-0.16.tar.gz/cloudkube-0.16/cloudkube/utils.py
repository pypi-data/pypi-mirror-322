...
def apply_terraform():
    ...
    return_code, stdout, stderr = tf.apply(
        auto_approve=True, capture_output=False, var={"config_file": config_file_path}
    )
    if return_code != 0:
    ...
    # Debug: Check the raw output of Terraform

    raw_output = tf.output("cluster_name", full_value=True)
    print(f"Raw Terraform output for 'cluster_name': {raw_output}")

    # Extract the cluster name
    if isinstance(raw_output, dict) and "value" in raw_output:
        cluster_name = raw_output["value"]
    else:
        cluster_name = raw_output  # Fallback for string output

    # Debug: Check the raw output of Terraform
    raw_output = tf.output("region", full_value=True)
    print(f"Raw Terraform output for 'region': {raw_output}")

    # Extract the cluster name
    if isinstance(raw_output, dict) and "value" in raw_output:
        region = raw_output["value"]
    else:
        region = raw_output  # Fallback for string output
