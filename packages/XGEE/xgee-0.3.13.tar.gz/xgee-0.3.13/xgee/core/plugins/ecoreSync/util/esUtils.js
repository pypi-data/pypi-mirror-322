...
class EsUtilities{
    ...
    async getAllContents(eObject,filter=null)
    {
        ...
        var results=[];
        let oid=this._esDomain.rlookup(eObject);
        if(!oid) throw new Error('Cannot get contents: the supplied eObject is unknown to this ecoreSync instance');
        if(filter!=null){
            if(filter(eObject)){
    ...
    async decode(obj){
        ...
        else
        {
            if(obj!=null)
            {
                if(obj.qry=="OBJ")
                {   try{
                        if(!Number.isInteger(obj.v)) throw new Error('ecoreSync: Invalid input format during decode: object id is invalid')
        ...
        if(obj!=null)
        {
            ...
            { 
                case "EObject":
                    ...
                    if(oid==null) throw new Error('cannot encode: the supplied eObject of type '+obj.eClass.get('name')+' is unknown to this ecoreSync instance')
                    res={qry:"OBJ",v:oid}
                    break;
                default:
                    res=obj
