    ...
    try:
        ...
        if output_format == 'csv':
            ...
            response_data = {
                ...
                'total': len(result),
                'pkey': pkey,
                'data': collection.serialize(result)
            }

            if 'gzip' in accept_encoding:
                response_json = json.dumps(response_data, cls=DateTimeEncoder).encode('utf-8')
                response_compressed = gzip.compress(response_json, compresslevel=1)
                response = Response(response_compressed, mimetype='application/json')
                response.headers['Content-Encoding'] = 'gzip'
                return response
            else:
                return Response(json.dumps(response_data, cls=DateTimeEncoder), mimetype='application/json')
...
def news():
    try:
        ...
        return Response(json.dumps(docs, cls=DateTimeEncoder), mimetype='application/json')
