...
vercel_url = os.getenv("VERCEL_URL", "jay.so")
platform_url = f"https://{vercel_url}"
if "localhost" in platform_url:
    platform_url = f"http://{vercel_url}"
...
def ensure_valid_agent_path(agent_path: str):
    if agent_path.endswith("main.py"):
        ...
        raise Exception("Agent config must be stored in a file named main.py")
...
@click.option(
    ...
    type=str,
    required=True,
    help="Path to the functions.py file to load dynamically.",
)
@click.option("--agent-id", type=str, required=True, help="The id of the agent.")
@click.option(
    "--host", type=str, default="localhost", help="Host to run the Agent API on."
)
@click.option("--port", type=int, default=8000, help="Port to run the Agent API on.")
def run(agent, agent_id, host, port):
    """
    Run the FastAPI server with dynamic functions and precompiled playground.
    """
    agent_path = os.path.abspath(agent)
    if not os.path.isfile(agent_path):
        click.secho(
            f"The functions path {agent_path} is not a file.", fg="red", bold=True
        )
        sys.exit(1)

    if not api_key:
        raise Exception("No api key detected, please define JAY_API_KEY")

    if not developer_id:
        raise Exception("No developer id detected, please define JAY_DEVELOPER_ID")

    os.environ["JAY_INTERNAL__AGENT_API_KEY"] = api_key

    ensure_valid_agent_path(agent_path)

    print(f"Loading agent configuration from: {agent_path}")
...
@cli.command()
@click.option(
    "--agent-id", type=str, required=True, help="ID of the agent to connect to"
)
@click.option(
    "--host", type=str, default="localhost", help="Host to run the Agent API on."
)
@click.option("--port", type=int, default=8000, help="Port to run the Agent API on.")
def playground(agent_id, host, port):
    """
    Serve only the playground files (for an existing deployed or local agent).
    """
    app = FastAPI()
...
@cli.command()
@click.option("--agent", type=str, required=True, help="Path to the functions.py file.")
@click.option(
    "--agent-id", type=str, required=True, help="ID of the project to deploy."
)
def deploy(agent, agent_id):
    """
    Build and deploy the agent.

    Steps:
    1. Build Docker image
    2. Run container locally & check /health
    3. Save image to tarball
    4. Get a pre-signed S3 URL & upload tarball
    5. Trigger CodeBuild to push image to ECR
    ...
    try:
        create_deployment_payload = {"agent_id": agent_id}
    ...
    agent_dir = os.path.dirname(agent)
