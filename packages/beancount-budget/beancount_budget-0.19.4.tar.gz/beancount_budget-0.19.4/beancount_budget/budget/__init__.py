...
from beancount_budget.beancount import ZERO, BeancountData, PerCategory
...
class Budget:
    ...
    def blank_budget(self) -> PerCategory[PerMonth[Decimal]]:
        ...
        return {c: {m: ZERO for m in self.months} for c in self.categories}
    ...
    def extend(self, month: Month) -> None:
        ...
        for cat in self.categories:
            self.data[cat][month] = ZERO
        self._income[month] = ZERO
    ...
    def load_csv(self, path: Path) -> None:
        ...
        if new_categories := set_difference(self.categories, keys_from_file):
            zeroes = {month: ZERO for month in self.months}
    ...
    def write(self) -> None:
        ...
        with open(self.path, "w", encoding="utf-8") as f:
            ...
            for cat in self.categories:
                ...
                if set(self.data[cat].values()) != {ZERO}:
    ...
    def balances(self, month: Month) -> PerCategory[Decimal]:
        ...
        def prev_balances(self, month: Month):
            if month == self.months[0]:
                return {c: ZERO for c in self.categories + [AVAIL]}
        ...
        for category in budgeted:
            ret[category] += budgeted.get(category, ZERO)
            ret[category] -= spent.get(category, ZERO)
    ...
    @cache
    @check_bounds
    def quotas(self, month: Month) -> PerCategory[CategoryQuota]:
        "Calculate budget quotas for the given month."
        out = (
            load_quotas(self.path_quotas, month, self.quantize)
    ...
    def show_quotas(self, month: Month) -> str:
        ...
        quotas: PerCategory[CategoryQuota] = self.quotas(month)
        if not quotas:
            return ""

        headers = ("Name", "Type", "Req'd balance", "Req'd budget")
        table: list[tuple] = [headers]
        for category, cquota in quotas.items():
            table.extend(cquota.show(category))
    ...
    def deviations(self, month: Month) -> PerCategory[Decimal]:
        ...
        for category, balance in self.balances(month).items():
            budgeted = budget.get(category, ZERO)
            cquota = self.quotas(month).get(category, CategoryQuota({}, month))
            ret[category] = cquota.deviation(budgeted, balance)

        return ret

    def quantize(self, amount: Decimal | None) -> Decimal:
    ...
    # Implicitly bounds-checked by `self.income`.
    def table(
        self,
        month: Month,
        depth: int | None = None,
        ...
        add_row("Total", BudgetRow(*totals))
        add_row("Available", BudgetRow(None, None, available, None))
