...
fn safe_parse_fastq(filepath: &str) -> FastqStats {
    let mut reader = BufReader::new(MultiGzDecoder::new(File::open(filepath).unwrap()));
...
mod tests {
    ...
    fn test_fastq_format_checks() {
        // Non-gzipped data should fail
        // Non-gzipped invalid fastq
        assert_panics!(safe_parse_fastq("test_data/not_fastq.fastq"));
        // Non-gzipped valid fastq
        assert_panics!(safe_parse_fastq("test_data/basic_illumina_1.fastq"));
