...
def call_internal_module(self_obj, file_path, module_name, function_name, i):
        """
        Dynamically calls a function from an internal module.

        :param self_obj: The calling object or `None` if not required.
        :param file_path: Path of the current file, typically __file__.
        ...
        """
        try:
            # Compute the directory containing the module
            current_dir = os.path.dirname(file_path)
            module_path = os.path.join(current_dir, module_name)
            module_name_cleaned = module_name.replace('/', '.').replace('\\', '.')

            # Dynamically import the module
            spec = importlib.util.spec_from_file_location(module_name_cleaned, module_path + '.py')
            ...
            if not callable(func):
                raise AttributeError(f"'{function_name}' is not a callable function in module '{module_name}'")

            # Call the function with self_obj if required, else just pass args
            if self_obj:
                return func(self_obj, i)
            else:
                return func(i)
        except Exception as e:
            raise ImportError(f"Error occurred while calling function '{function_name}' in module '{module_name}': {e}")
