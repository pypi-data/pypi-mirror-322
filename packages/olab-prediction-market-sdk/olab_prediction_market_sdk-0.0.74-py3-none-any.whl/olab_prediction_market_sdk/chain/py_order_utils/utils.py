...
def calculate_order_amounts(price: float, maker_amount: int, side: OrderSide) -> tuple[int, int]:
    """
    ...
    if side == OrderSide.BUY:
        ...
        exact_taker = maker_amount / price
        taker_amount = int(round(exact_taker))
        
        recalculated_maker_amount = int(round(taker_amount * price))
    else:  # SELL
        # For SELL: price = taker/maker
        # Example: if maker_amount = 111000000 and price = 0.29
        # taker_amount should be 111000000 * 0.29 = 32190000
        exact_taker = maker_amount * price
        taker_amount = int(round(exact_taker))  # Use round instead of truncating
        recalculated_maker_amount = maker_amount
