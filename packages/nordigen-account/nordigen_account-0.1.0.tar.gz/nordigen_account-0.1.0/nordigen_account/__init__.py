...
def create_nordigen_client(secret_id: str, secret_key: str, refresh_token: Optional[str] = None) -> Tuple[NordigenClient, str]:
    ...
    Returns:
        Tuple[NordigenClient, str]: Configured Nordigen client instance and the refresh token.

    Raises:
        RuntimeError: If the refresh token is expired or invalid, and a new token cannot be generated.
    """
...
class BankAccountManager:
    ...
    def _initialize_accounts(self, fetch_data: bool) -> None:
        ...
        Raises:
            ValueError: If the requisition status is expired or the refresh token may have expired or bank authorization may be incomplete.
        ...
        try:
            ...
            # Check if the refresh token may be expired
            response_code = accounts_response.get("status_code")
            if response_code == self.STATUS_INVALID:
                detail = accounts_response.get("detail")
