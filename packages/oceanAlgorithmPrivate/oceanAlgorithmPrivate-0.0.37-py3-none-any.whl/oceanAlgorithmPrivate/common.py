...
class Private(LoggerHelper):
    ...
    def validate_target_ticker(self, INPUT): # 0.0.37
        ...
        target_var = INPUT['model_par'][output_target]
    ...
    def handle_target_var(self, params, output_target): # 0.0.37
        ...
        for item in output_target_values:
            if isinstance(item, dict) and 'dep_var' in item:
                dep_vars.append(item['dep_var'])
        return dep_vars

    def get_column_value(self, schema_json, column_name): # 0.0.37
    ...
    def generate_schema_json(self, params, schema_json):  # 0.0.37
        ...
        for _, entry in self.target_schema.iterrows():
            ...
            for output_target_value in output_target_values:
                output_column_target_value, ticker, ticker_type, parent_property = self.get_column_value(schema_json, output_target_value)
    ...
    def validate_data(self, parsed_data): # 0.0.36
    ...
    def get_date(self, df, input_param):
        s_date = input_param['model_par']['start_date']
        e_date = input_param['model_par']['end_date']
