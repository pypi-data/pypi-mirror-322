...
def resample_xarray_grid_to_other_grid_resolution(grid_to_resample: xr.DataArray, grid_to_use: xr.DataArray, interp_method="linear") -> xr.DataArray:
...
def measure_grid_similarity(grid1: xr.DataArray, grid2: xr.DataArray):
    """
    This returns a value describing how similar two grids are on their non-nan values.
    It is equal to:

    1 + sqrt((grid1 - grid2)^2 ) / sqrt(grid1^2)
    Where grid1 and grid2 in the above equation is equal to the element wise comparison of their grid values.

    This means:
    - a value of 1 means the grids are identical
    - a value of 1.1 means that grid1 and grid2 differ by 10% of the magnitude of grid1
    - a value of 2 means that grid1 and grid2 differ by one of magnitude of grid1
    ...
    return 1 + np.linalg.norm(diff1d) / np.linalg.norm(grid1_data)
