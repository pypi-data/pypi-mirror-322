...
def run_sca_typescript(
    ...
    try:
        ...
        shadow_results = run_sca_prime_shadow(client, project_root)
        # The sca CLI prints the results to stdout,and everything else to trace/warn/debug/error
...
def run_sca_prime_shadow(
    client: GableAPIClient, project_root: str, post_metrics=True
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            if post_metrics:
                ...
                if os.path.exists(metrics_file):
                    with open(metrics_file, "r") as f:
                        ...
                        post_metrics_response, post_metrics_success = (
                            client.post_telemetry(post_metrics_request)
                        )
            ...
            findings_file = os.path.join(
                temp_dir, f"{project_root.split('/')[-1]}.json"
            )
            if os.path.exists(findings_file):
                ...
                logger.error("SCA prime generated findings file not found")
                return {}
        except Exception as e:
            logger.opt(exception=e).debug(
                f"Error running Gable Code in shadow mode {e}", e
            )
            return {}
