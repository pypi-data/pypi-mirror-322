...
file_method = StaticMethod(f"{project_name}_file_read", is_form_log_file=ul.is_form_log_file, log_file=ul.log_file_path)
log = Logger(f"{project_name}_file_read")
...
def barcodes_add_anno(annotation_file: path, cell_anno: DataFrame, clusters: str = None) -> DataFrame:
    ...
    if clusters is not None:
        ...
        if cell_anno[cell_anno[clusters].isna()].shape[0] > 0:
            log.warn(f"Due to the presence of `NA` in the `{clusters}`, it is forcibly assigned as `unknown`.")
...
def read_variants(
    ...
    log.info("Create multiple trait-variants data")
