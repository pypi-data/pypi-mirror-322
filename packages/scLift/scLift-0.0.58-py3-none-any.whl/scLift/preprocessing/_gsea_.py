...
from ..util import check_adata_get, to_dense, project_name

file_method = StaticMethod(f"{project_name}_preprocessing_gsea")
log = Logger(f"{project_name}_preprocessing_gsea")
...
def get_gene_enrichment(
    ...
    for cluster in tqdm(cluster_list):
        ...
        if len(_cluster_gene_list_) == 0:
            log.warn(f"The gene list for cluster {cluster} is empty.")
            continue
