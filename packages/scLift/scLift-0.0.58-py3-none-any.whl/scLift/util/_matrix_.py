...
file_method = StaticMethod(f"{project_name}_util_matrix")
log = Logger(f"{project_name}_util_matrix")
        ...
        if data[i] not in new_data:
            ...
            while True:
                ...
                if not isinstance(data[i], str) and not is_warn:
                    log.warn("Convert non string types to string types.")
                    is_warn = True
...
def merge_matrix(datas: list, axis: Literal[0, 1] = 0) -> list:
    ...
    for i in range_size:
        ...
        # judge size
        if judge != constant:
            log.error("Inconsistent traits in the input dataset set.")
...
def numerical_bisection_step(min_value: float, max_value: float, step_length: float) -> Tuple[collection, int]:
    if min_value > max_value:
        log.error(f"`min_value` ({min_value}) must be smaller than `max_value` ({max_value}).")
...
def matrix_dot_block_storage(
    ...
    if m != p:
        log.error(f"Need to meet the principle of matrix multiplication. ({m} != {p})")
    ...
    file_method.makedirs(_cache_path_)

    # Store block data
    log.info("Store block data...")
...
def matrix_multiply_block_storage(
    ...
    if n != n1 or m != m1:
        log.error(f"Need to satisfy the multiplication principle of Hadamard products in matrices. ({(n, m)} != {n1, m1})")
...
def matrix_division_block_storage(
    ...
    if isinstance(value, collection):
        ...
        if q != n and q != m:
            log.error(f"Inconsistent dimensions cannot be divided. {(n, m)} != {q}")
