...
from scLift.util import collection, matrix_data, to_sparse, to_dense, number, check_adata_get, project_name

log = Logger(f"{project_name}_preprocessing_anndata")
...
def adata_group(
    ...
    if "median" in method:
        ann_data.layers["matrix_median"] = to_sparse(matrix_min)
