...
from scLift.util import path, project_name, collection

file_method = StaticMethod(f"{project_name}_plot_core")
log = Logger(f"{project_name}_plot_core")
...
def group_heatmap(
    ...
    # plot
    def _show_plot_(_label_: str, _output_tail_: str, _layer_: str = None) -> None:
        try:
            heatmap_annotation(
                adata_cluster.T,
                layer=_layer_,
                ...
                y_name="Trait",
                show=show,
                output=os.path.join(new_path, f"{layer}_{_output_tail_}.pdf") if new_path is not None else None
            )
    ...
    if "matrix_sum" in layers:
        _show_plot_("Sum TRS", "sum", "matrix_sum")

    if "matrix_median" in layers:
        _show_plot_("Median TRS", "median", "matrix_median")

    if "matrix_max" in layers:
        _show_plot_("Max TRS", "max", "matrix_max")

    if "matrix_min" in layers:
        _show_plot_("Min TRS", "min", "matrix_min")
