...
class RandomWalk:
    ...
    def __init__(
        ...
        if epsilon > 0.1:
            ...
            log.error("The parameter of `epsilon` must be greater than zero.")
    ...
    def _get_seed_cell_(self, init_data: AnnData, info: str = None) -> Tuple[collection, matrix_data, matrix_data, matrix_data, matrix_data]:
        ...
        log.info(f"Calculate {len(self.trait_list)} traits/diseases for seed cells information.{f' ({info})' if info is not None else ''}")
    ...
    def run_benchmark(self) -> None:
        ...
        log.info(f"Calculate {len(self.trait_list)} traits/diseases for process `run_benchmark`. (Randomly perturb seed cells. ===> `benchmark`)")
    ...
    def _run_(self, seed_cell_data: matrix_data, label: str) -> matrix_data:
        ...
        log.info(f"Calculate {len(self.trait_list)} traits/diseases for process `{label}`. ({_log_info_} ===> `{_layer_label_}`)")
    ...
    def _run_enrichment_(self, seed_cell_en_weight: matrix_data, label: str) -> None:
        ...
        if label == "run_en":
            ...
            if not self.is_run_ablation_ncw:
                log.warn("Need to run the `run_ablation_ncw` method first in order to run this method.")
            ...
            if not self.is_run_ablation_nsw:
                log.warn("Need to run the `run_ablation_nsw` method first in order to run this method.")
            ...
            if not self.is_run_ablation_ncsw:
                log.warn("Need to run the `run_ablation_ncsw` method first in order to run this method.")
...
def calculate_init_score(
    ...
    if "data" not in input_data:
        log.error("The `data` field needs to be included in parameter `input_data`.")
        raise ValueError("The `data` field needs to be included in parameter `input_data`.")

    if "overlap_data" not in input_data:
        log.error("The `overlap_data` field needs to be included in parameter `input_data`.")
...
def calculate_init_score_weight(
    ...
    log.info("Calculate cell type weight")
    ...
    # calculate
    log.info("Calculate fragment weighted accessibility.")
    input_data: dict = {
        "data": fragments,
    ...
    log.info("Broadcasting the weight factor to the cellular level")
    anno_info = adata.obs
    ...
    log.info("Calculate trait or disease-cell related initial score")
