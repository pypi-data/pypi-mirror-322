...
class PwpInstallerUi(PwpGui):
    ...
    def from_python_to_ui(self):
        ...
        if self.installer.piwipre_version:
            ...
            self.installer.piwipre_exists = m and m.group(1) and m.group(1) >= self.installer.piwipre_target_version

            self.installer.piwipre_exists = self.installer.piwipre_version >= self.installer.piwipre_target_version

        self.pwp_button['text'] = "re install" if self.installer.piwipre_exists else "install"

        # ---------------------------------------- piwipre
        if self.installer.installer_version:
            # m = re.match(r"([\d\\.]*)", self.installer.installer_version)
            # self.installer.installer_exists = m and m.group(1) and m.group(1) >= self.installer.piwipre_target_version

            self.installer.installer_exists = self.installer.installer_version >= self.installer.piwipre_target_version
