...
from .misc import is_running_in_jupyter
...
####################################################
class LineSelector:
    def __init__(self, ax, data, vertical=False):
        """
...
def check_tomogram( tomogram, mask ):
    ...
    x = np.arange(tomogram.shape[0])
    y = np.arange(tomogram.shape[1])
    z = np.arange(tomogram.shape[2])
    X,Y,Z = np.meshgrid(x,y,z)

    X,Y,Z = X.flatten()[mask], Y.flatten()[mask], Z.flatten()[mask]
    points = np.column_stack((X,Y,Z))#[::10]

    points = filter_grid_points( points )
...
def filter_grid_points(points):
    """
    Filter points by keeping only the lowest and highest z-values for each unique (x, y) pair.
    
    Parameters:
    - points: (n, 3) array of 3D points (x, y, z).
    
    Returns:
    - filtered_points: Array of filtered points with only lowest and highest z for each (x, y) pair.
    """
    # Dictionary to store the lowest and highest z for each (x, y)
    z_extremes = {}
    ...
    for point in points:
        ...
        # If this (x, y) combination is seen for the first time, store z
        if xy_key not in z_extremes:
            z_extremes[xy_key] = [z, z]  # Store both min and max z initially
        else:
            # Update the lowest and highest z values for this (x, y) combination
            z_extremes[xy_key][0] = min(z_extremes[xy_key][0], z)  # Lowest z
            z_extremes[xy_key][1] = max(z_extremes[xy_key][1], z)  # Highest z

    # Build the filtered points array
    filtered_points = []
    for (x, y), (z_min, z_max) in z_extremes.items():
        filtered_points.append([x, y, z_min])
        if z_min != z_max:
            filtered_points.append([x, y, z_max])

    return np.array(filtered_points)

    # # Loop through all pairs of points to remove collinear ones
    # for i in range(n):
    #     redundant = False
    #     for j in range(n):
    #         if i == j:
    ...
    #             # Check if point i lies between points j and k
    #             if is_collinear(points[j], points[k], points[i]):
    #                 redundant = True
    #                 break
    #         if redundant:
    #             break
...
#         self.current_rect = Rectangle((self.press[0], self.ymin),
#                                       0, self.ymax - self.ymin,
#                                       facecolor='yellow', edgecolor='yellow', alpha=0.5)
#         self.ax.add_patch(self.current_rect)
#         self.rects.append(self.current_rect)
#         self.ax.figure.canvas.draw()
...
#     ax.set_title(title)
#     ax.legend()
#     selector = PixelSelector(ax, minval, maxval, 
#                     max_regions=max_regions)
#     plt.show(block=True)  # Keeps the plot open until it is manually closed
...
def mask_detector_old( image ):

    dat_plt = image.copy().astype(float)
    ...
    fig, ax = plt.subplots( figsize = (10,10))
    ax.imshow(dat_plt, norm=colors.LogNorm(vmin=maxval*1e-2, vmax=mskval) )
    ...
    def on_release(event):
        ...
        if distance > 0.5:
            steps = np.arange(0, distance, step=0.5)
            points = np.unique(
                ( (start_point[0] + steps*(end_point[0]-start_point[0])/distance).astype(int),
                (start_point[1] + steps*(end_point[1]-start_point[1])/distance).astype(int) ),
                axis=1, 
            ).T
        else:
            points = np.array([end_point],dtype=int)

        if event.button == 1:  # Left mouse button: mask
            for p in points:
                dat_plt[ p[1], p[0] ] = mskval
                mask[ p[1], p[0] ] = 0

        elif event.button == 3:  # Right mouse button: unmask
            for p in points:
                dat_plt[ p[1], p[0] ] = image[ p[1], p[0] ]
                mask[ p[1], p[0] ] = 1

        plt.gca().clear() 
        plt.gca().imshow(dat_plt, norm=colors.LogNorm(vmin=maxval*1e-2, vmax=mskval) )
        fig.canvas.draw_idle()
...
def draw_rectangle_on_image(image, xy = None, title=''):
    ...
    if is_running_in_jupyter():
        input('Press enter after drawing the rectangle...')
    return drawer
