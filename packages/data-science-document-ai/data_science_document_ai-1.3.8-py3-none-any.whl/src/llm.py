...
class LlmClient:
    ...
    async def ask_gemini(self, prompt: str, document: str = None, response_schema: dict = None):
        ...
        try:
            ...
            model_response = await self.geminy_client.generate_content_async(
                contents=inputs,
    ...
    async def ask_chatgpt(self, prompt: str, document=None, response_schema=None):
        ...
        if document is None:
            completion = await self.chatgpt_client.beta.chat.completions.parse(
                model="gpt-4o",
                temperature=0.1,
                messages=[
                    {
            ...
            completion = self.chatgpt_client.beta.chat.completions.parse(
                model="gpt-4o",
                temperature=0.1,
                messages=[
                    {
