...
from ..clients.git_repo_client import GitRepoClient
from ..schemas.enums import CheckoutResult, FetchResult, SetupOption
from ..schemas.github import GithubScope, GithubType
from ..schemas.repo_config import RepoConfig
from ..utils.clone import clone_or_pull_repo, clone_repo, pull_repo
from ..utils.clone_url import get_clone_url_for_repo_config, url_to_github
from ..utils.codeowner_utils import create_codeowners_parser_for_repo
from ..utils.remote_progress import CustomRemoteProgress
from ..utils.stopwatch_utils import stopwatch
...
class RemoteRepoOperator(RepoOperator):
    ...
    def pull_repo(self) -> None:
        ...
        pull_repo(repo=self.repo_config, path=self.base_dir, github_scope=GithubScope.WRITE)
    ...
    def clone_or_pull_repo(self) -> None:
        ...
        if self.repo_exists():
            self.clean_repo()
        clone_or_pull_repo(self.repo_config, path=self.base_dir, shallow=True, github_scope=GithubScope.WRITE)
    ...
    def setup_repo_dir(self, setup_option: SetupOption = SetupOption.PULL_OR_CLONE, shallow: bool = True) -> None:
        ...
        if setup_option is SetupOption.CLONE:
            ...
            clone_repo(repo=self.repo_config, path=self.base_dir, shallow=shallow, github_scope=GithubScope.WRITE)
