    ...
    if os.path.exists(spectrum_location_file):
      ...
      while line:
        ...
        if "BEGIN IONS" in line:
          ...
          scan = re.split('=|\r\n', line)[1]
          self.spectrum_location_dict[scan] = self.spectrum_location
        elif "RTINSECONDS=" in line:
          rtinseconds = float(re.split('=|\r\n', line)[1])
          self.spectrum_rtinseconds_dict[scan] = rtinseconds
      self.spectrum_location_dict = self.spectrum_location_dict
      self.spectrum_rtinseconds_dict = self.spectrum_rtinseconds_dict
      self.spectrum_count = len(self.spectrum_location_dict)
...
if __name__ == "__main__":
    ...
    print(
        ...
        "required for the Transformer_DIA model.\n"
        "Generated features:\n"
        "- Keys: Peptide sequences\n"
        "- Values: List containing [precursor_mz, precursor_charge, scan_list_middle, ms1, mz_list, int_list, neighbor_right_count, neighbor_size_half]"
    )
    feature_file = input("Enter the path to the feature file (.csv): ").strip()
    while not os.path.isfile(feature_file) or not feature_file.endswith(".csv"):
        print("Invalid file. Please ensure the file exists and has a .csv extension.")
        feature_file = input("Enter the path to the feature file (.csv): ").strip()

    # Prompt the user for the spectrum file path
    spectrum_file = input("Enter the path to the spectrum file (.mgf): ").strip()
    while not os.path.isfile(spectrum_file) or not spectrum_file.endswith(".mgf"):
        print("Invalid file. Please ensure the file exists and has a .mgf extension.")
        spectrum_file = input("Enter the path to the spectrum file (.mgf): ").strip()

    # exctract precursor info
    feature_extraction = data_preprocess(spectrum_file, feature_file)
    feature_extraction.open_input()
    feature_extraction.get_location()
    feature_extraction.get_precursor_info()
    feature_extraction.dict_to_pkl(feature_file.strip('.csv')+'.pkl', feature_extraction.precursor_info)