...
.. function:: set_encrypt(passwd, flags=0)
   ...
   .. warning::
      ...
      The password ``passwd`` **MUST BE** an unicode string with
      no **null characters** (that is, no ``"\x00"`` characters).
      Printable ASCII would be fine, since ASCII is a subset of
      unicode. If you want to use a binary password, you **MUST**
      convert it first to unicode. You can use BASE16 or BASE64,
      for instance. You could do something like
      ``bytes.decode('latin-1')``, but **BE SURE** the result has
      no *null bytes*. If that is the case, the actual password
      would be **TRUNCATED** to the first *null byte*. The
      truncation will produce a **WEAK** key.

      To avoid that risk, a password with *null characters* will
      raise an exception.
