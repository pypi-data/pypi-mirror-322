{% macro avrio__get_show_grant_sql(relation) -%}
    select
        grantee,
...
{%- macro avrio__get_grant_sql(relation, privilege, grantees) -%}
    grant {{ privilege }} on {{ relation }} to {{ adapter.quote(grantees[0]) }}
{%- endmacro %}

{%- macro avrio__support_multiple_grantees_per_dcl_statement() -%}
    {#
        -- This macro should return true or false depending on the answer to
        -- following question:
...
{% macro avrio__call_dcl_statements(dcl_statement_list) %}
