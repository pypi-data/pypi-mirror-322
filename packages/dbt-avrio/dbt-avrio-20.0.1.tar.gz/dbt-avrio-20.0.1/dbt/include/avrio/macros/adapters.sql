...
{% macro avrio__get_columns_in_relation(relation) -%}
  {%- set sql -%}
    describe {{ relation }}
  {%- endset -%}
...
{% macro avrio__list_relations_without_caching(relation) %}
...
{% macro avrio__reset_csv_table(model, full_refresh, old_relation, agate_table) %}
    {{ adapter.drop_relation(old_relation) }}
    {{ return(create_csv_table(model, agate_table)) }}
{% endmacro %}


{% macro avrio__create_csv_table(model, agate_table) %}
...
{% macro avrio__create_table_as(temporary, relation, sql) -%}
  {%- set _properties = config.get('properties') -%}
...
{% macro avrio__create_view_as(relation, sql) -%}
...
{%- macro avrio__get_drop_sql(relation) -%}
...
{% macro avrio__information_schema_name(database) -%}
  {%- if database -%}
...
{# On avrio, 'cascade' is not supported so we have to manually cascade. #}
{% macro avrio__drop_schema(relation) -%}
...
{% macro avrio__rename_relation(from_relation, to_relation) -%}
...
{% macro avrio__alter_relation_comment(relation, relation_comment) -%}
  comment on {{ relation.type }} {{ relation }} is '{{ relation_comment | replace("'", "''") }}';
{% endmacro %}


{% macro avrio__alter_column_comment(relation, column_dict) %}
...
{% macro avrio__list_schemas(database) -%}
...
{% macro avrio__check_schema_exists(information_schema, schema) -%}
...
{% macro avrio__alter_relation_add_remove_columns(relation, add_columns, remove_columns) %}
...
{% macro avrio__alter_column_type(relation, column_name, new_column_type) %}
  {#
