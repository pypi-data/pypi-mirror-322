...
class Trainer(ABC):
    ...
    def _load_model(self, ckpt_epoch_dir: Optional[Path]) -> Optional[Dict[str, Any]]:
        ...
        model_g_state_fpath = ckpt_epoch_dir / MODEL_NAME
        if model_g_state_fpath.exists():
            model_g_state = torch.load(model_g_state_fpath, map_location="cpu", weights_only=True)
            self.model.load_state_dict(model_g_state)
    ...
    def _load_trainer_state(self, ckpt_epoch_dir: Optional[Path]) -> Optional[Dict[str, Any]]:
        ...
        if trainer_state_fpath.exists():
            self.state.load_from_json(trainer_state_fpath)
    ...
    def _save_optimizer_and_scheduler(self, ckpt_epoch_dir: Path, is_best_epoch: bool = False):
    ...
    def _save_checkpoint(self, epoch: int, is_best_epoch: bool = False):
        """Save the checkpoint.
        ...
        self._save_optimizer_and_scheduler(ckpt_epoch_dir, is_best_epoch=is_best_epoch)
    ...
    def train(self):
        ...
        for epoch in range(self.state.epochs_trained + 1, num_train_epochs + 1):
            ...
            if self.is_rank_zero and epoch % self.args.save_epoch_interval == 0:
                self._save_checkpoint(epoch=epoch, is_best_epoch=False)
