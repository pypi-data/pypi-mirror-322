...
# Run the configurence cli
start *argv:
  uv run -- configurence {{ argv }}
...
format:
  uv run black './configurence' ./tests
  uv run isort --settings-file . './configurence' ./tests

# Lint with flake8
lint:
  uv run flake8 './configurence' ./tests
...
clean: _clean-venv _clean-compile _clean-test _clean-tox
  rm -rf configurence.egg-info
  rm -f configurence/*.pyc
  rm -rf configurence/__pycache__
