  ...
  if (settings.transitionDensity) {
    ...
    for (auto system : _systems) {
      ...
      try {
        ...
        throw SerenityError("You need to run an LRSCFTask before you can plot transition densities.");
      }
  ...
  if (settings.holeparticleDensity) {
    for (auto system : _systems) {
      ...
      try {
        ...
        throw SerenityError("You need to run an LRSCFTask before you can plot hole and particle densities.");
      }
      ExcitedStatesAnalysis<SCFMode> excAna(settings.excitations, lrscfcontroller);
      settings.gridSpacing = defaultCubeSpacing;
