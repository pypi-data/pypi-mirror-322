...
void BasisFunctionProvider::provideAtomWithBasisFunction(Atom& atom, const std::string libraryPath,
  ...
  const std::string searchString = element + "\\s+" + basisType;
  ...
  /* Turbomole basis files sometimes have a comment along the lines of # o     (7s4p1d) / [3s2p1d]     {511/31/1}
  ...
  std::string modifiedstring = std::regex_replace(substring, std::regex("D(\\+|-)"), "E$1");
