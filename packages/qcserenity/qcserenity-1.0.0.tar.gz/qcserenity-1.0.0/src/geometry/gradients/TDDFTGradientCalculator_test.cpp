...
TEST_F(TDDFTGradientsTest, CIS_UNRESTRICTED_O2) {
  ...
  auto system = SystemController__TEST_SUPPLY::getSystemController(TEST_SYSTEM_CONTROLLERS::O2_MINBAS_TRIP, settings, 0, 2);
...
TEST_F(TDDFTGradientsTest, TDA_UNRESTRICTED) {
  ...
  lrscf.run();
  lrscf.settings.excGradList = {1, 3, 4};
...
TEST_F(TDDFTGradientsTest, TDDFT_UNRESTRICTED) {
  ...
  settings.basis.densFitJ = Options::DENS_FITS::NONE;
  settings.basis.densFitCorr = Options::DENS_FITS::NONE;
  // getting a test SystemController (system name "SomeTestSystem") makes cleaning up the system directory necesssary to
