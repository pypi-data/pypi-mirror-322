...
class IBTWSTConnector(BrokerConnector):
		...
		async with self._lock:
			...
			try:
				...
				for leg in order.legs:
					legContract = Option(symbolData.contract.symbol, expiration, leg.strike, self._mappedRight(leg.right), 'CBOE', tradingClass = self._determineTradingClass(symbolData.contract.symbol))
					...
					comboLeg = ComboLeg(conId=legContract.conId, ratio=1, action=self._mappedOrderAction(leg.action), exchange='CBOE')
	...
	async def placeOrder(self, order: GenericOrder, template: Template) -> bool:
		...
		async with self._lock:
			...
			comboContract = None
			if len(order.legs) > 1:
				comboContract = Contract(symbol=symbolData.contract.symbol, secType='BAG', exchange='CBOE', currency=symbolData.contract.currency, comboLegs=comboLegs)
			else:
