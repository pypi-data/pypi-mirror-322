...
class OptraBot():
	...
	async def _checkMarketData(self):
		...
		chain = next(c for c in chains if c.tradingClass == 'SPXW' and c.exchange == symbolInformation.exchange)
		if chain == None:
		...
		shortPutContract = Option(spx.symbol, expiration, strikePrice, 'P', symbolInformation.exchange, tradingClass = 'SPXW')
