...
class NonLinearNormalization(tf.keras.layers.Normalization):
    ...
    def __init__(
        self,
        vars_in_use: Optional[list[str]] = None,
        scaling_dict: Optional[dict] = None,
        *args: list[Any],
        **kwargs: dict[Any],
    ) -> None:
        ...
        Parameters
        ----------
        vars_in_use : Optional[list[str]]
            The list of variable names that is currently used. If ``None``, no non-linear scalings are applied. (Default value = None)
        scaling_dict : Optional[dict]
            ...
            If ``None``, no non-linear scalings are applied. (Default value = None)
        *args : list[Any]
        ...
        **kwargs : dict[Any]
        ...
        if vars_in_use is not None and scaling_dict is not None:
            self.apply_non_linear_scalings = True
            ...
            self.apply_non_linear_scalings = False

    @tf.function
    def non_linear_transform(self, inputs: tf.Tensor) -> tf.Tensor:
        ...
        ----------
        inputs : tf.Tensor
            Tensor of inputs to be scaled.

        Returns
        -------
        tf.Tensor
            The tensor containing the scaled inputs.
        """
    ...
    def adapt(self, data: tf.data.Dataset, batch_size: Any = None, steps: Any = None) -> None:
        r"""_summary_.

        Parameters
        ----------
        data : tf.data.Dataset
            The Tensorflow dataset containing the input features that are used to adapt the normalization layer by first
            applying the non-linear transformations and then determining the mean and variance.
        batch_size : Any
            Argument of the ``tf.keras.layers.Normalization.adapt`` function. (Default value = None)
        steps : Any
            Argument of the ``tf.keras.layers.Normalization.adapt`` function. (Default value = None)
        """
        if self.apply_non_linear_scalings:
            scaled_data = data.map(self.non_linear_transform)
        else:
            scaled_data = data
    ...
    def call(self, data: tf.Tensor) -> tf.Tensor:
        ...
        ----------
        data : tf.Tensor
            The tensor of input features to transform.
...
class SignificanceMetric(tf.keras.metrics.Metric):
    ...
    def calc_significance(self, inputs, y_pred):
        r"""_summary_.
        ...
        _type_
            _description_
        """
        # get the target labels and sample weights
        y_true = inputs[1]
        sample_weight = inputs[2].reshape(-1, 1)
...
class WeightedBinaryCrossentropy(tf.keras.losses.BinaryCrossentropy):
    ...
    def calc_loss(self, inputs, y_pred):
        ...
        y_pred : _type_
            _description_
        """
        # get the target labels and optional sample weights
        y_true = inputs[1]
        if len(inputs) == 3:
            sample_weight = inputs[2].reshape(-1, 1)
        else:
        ...
        weighted_loss = (y_true * self.signal_weight + (1 - y_true)) * loss

        if sample_weight is not None:
            return np.mean(sample_weight * weighted_loss)
        else:
...
class WeightedCategoricalCrossentropy(tf.keras.losses.CategoricalCrossentropy):
    ...
    def calc_loss(self, inputs, y_pred):
        ...
        if self.class_weights is not None:
            weighted_loss = -np.sum(y_true * self.class_weights * np.log(y_pred), axis=1, keepdims=True)
        else:
            weighted_loss = -np.sum(y_true * np.log(y_pred), axis=1, keepdims=True)
...
class WeightedKLDivergence(tf.keras.losses.KLDivergence):
    ...
    def calc_loss(self, inputs, y_pred):
        ...
        weighted_loss = np.sum(
            y_true * self.class_weights * np.log(np.clip(y_true, 1e-7, 1.0) / y_pred), axis=1, keepdims=True
        )
