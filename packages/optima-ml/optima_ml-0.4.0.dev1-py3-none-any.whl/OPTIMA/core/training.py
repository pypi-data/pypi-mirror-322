...
class EarlyStopperForTuning(ABC):
    ...
    Firstly, `custom metrics` are metrics whose value can be determined from the model inputs and the corresponding
    model predictions. Their values are calculated at the end of each epoch for both the training and the validation
    dataset. Secondly, `composite metrics` are metrics whose values can be calculated from already existing metrics.
    This includes e.g. also metrics whose values are determined by the value of a `custom metric` on both the training
    and validation datasets. Both types of metrics can be used as target metrics and / or to define `overfitting
    conditions`.
    ...
    def __init__(
        ...
        data: Optional[Union[Any, tuple[Any]]] = None,
        ...
        custom_metrics : Optional[list[tuple[str, Callable]]]
            ...
            ``logs`` (with ``'train_'`` and ``'val_'`` prefixes), and a callable with the signature ``(input, y_pred)``,
            i.e. it has to accept the input data (containing e.g. input features, target labels and sample weights, in
            the same structure as defined in the dataset) and the corresponding model predictions. Both the input data
            and the model predictions are expressed as numpy arrays if possible. If the conversion to numpy arrays fails,
            the native representation (e.g. Tensorflow tensors) is used directly. Additionally, if both input data and
            model predictions are converted to numpy arrays, arrays are tried to be concatenated across multiple batches
            if the shapes of inputs/outputs are fixed. The structure of the input / output values is conserved, e.g.
            a list of tuples of numpy arrays will result in a tuple of numpy arrays. If the concatenation is not possible
            a list of inputs and a list of outputs for each batch is provided instead. The callable has to return either
            a number or a bool. The value of this metric is calculated at the end of each epoch separately on the
            training and validation datasets. (Default value = None)
        ...
        data : Optional[Union[Any, tuple[Any]]]
            Data structure containing the training and validation datasets. If a tuple is passed, it is expected to
            contain separate training and validation sets. Otherwise, a common data structure is expected (e.g. for a
            Lightning DataModule). (Default value = None)
    ...
    def at_epoch_end(self, epoch: int, logs: Optional[dict] = None, **kwargs: dict) -> None:
        ...
        containing the results (i.e. calling ``self.build_results_dict``), a checkpoint is built from the
        ``self.checkpoint_dir`` directory and the results dictionary and the checkpoint are reported to `Tune`.
        ...
        **kwargs : dict
        ...
        """
        # wrap the current model state in an abstract model if needed. At the moment, this is only needed when calculating
        # custom metrics
        if len(self.custom_metrics) > 0:
            self.abstract_model = self.get_abstract_model(**kwargs)
        ...
        # build the results dictionary, append to the history and report to Tune if we are in a Tune session
        (
            current_metrics,
        ...
        if self.in_tune_session:
            # report the results back to tune
    ...
    def calc_custom_metrics(self, logs: Optional[dict] = None, **kwargs: dict) -> dict:
        ...
        **kwargs : dict
        ...
        # the abstract model provides a function to calculate custom metrics, which can directly be used here
        custom_metric_values_train, custom_metric_values_val = self.abstract_model.calc_custom_metrics(
            custom_metrics=[metric for (_, metric) in self.custom_metrics],
            data=self.data,
            skip_test_dataset=True,
        )
        for metric_index in range(len(self.custom_metrics)):
            logs[f"train_{self.custom_metrics[metric_index][0]}"] = custom_metric_values_train[metric_index]
            logs[f"val_{self.custom_metrics[metric_index][0]}"] = custom_metric_values_val[metric_index]
    ...
    def build_results_dict(
        ...
        **kwargs : dict
        ...
        # go through all the overtraining conditions and check if all required input metrics have been added to the results;
    ...
    def update_history(self, results: dict) -> None:
        """Helper function to append the current metric values to the history dictionary.

        Parameters
        ----------
        results : dict
            The dictionary of current metric values.
        """
        if self.history == {}:
    ...
    def finalize(self, **kwargs: dict) -> None:
    ...
    state_type = tuple[
        int, int, int, Union[float, int], dict, dict, dict, dict, Any, int, int, bool, Union[float, int], dict
    ]
    ...
    @abstractmethod
    def get_abstract_model(self, **kwargs: dict) -> OPTIMA.core.model.AbstractModel:
        """Returns the current model, wrapped in a ``OPTIMA.core.model.AbstractModel``.
        ...
        **kwargs : dict
        ...
        OPTIMA.core.model.AbstractModel
            The current model, wrapped in an ``AbstractModel``.
        """
        raise NotImplementedError
...
def build_trainable(
    ...
    input_handler: OPTIMA.builtin.inputs.InputHandler,
    dataset_train: ray.data.Dataset,
    dataset_val: ray.data.Dataset,
    ...
    """Returns a ``functools.partial`` of the provided ``training_func``.
    ...
    # instead of tune.with_parameters because with tune.with_parameters, the objects are never cleared from the object
    # store again, thus consuming potentially large amounts of memory.
...
def perform_crossvalidation(
    ...
    The ``get_experiment_inputs``-function (either defined in the run-config or the built-in version) is called to load
    the input data for crossvalidation.
    ...
    - ``training_func``: the provided ``training_func`` is wrapped with a ``ray.remote``-decorator, setting ``num_cpus``
      ...
      ``functools.partial``.
    - ``dataset_train``, ``dataset_val``: the corresponding datasets for each fold returned by ``get_experiment_inputs``
      are passed.
    ...
    placement_group : Optional[PlacementGroup]
        An optional placement group that is applied to the training tasks. (Default value = None)
    custom_metrics : Optional[list[tuple[str, Callable]]]
    ...
    Union[tuple[dict, dict], tuple[dict, dict, list]]
        ...
        (key ``'split'``), the model-config used for the training, i.e. the corresponding entry in ``model_configs``,
        (key ``config``) and the filename of the training history (key ``history``). Since one list entry is created for
        each trained model, the lists have length `k`. The second dictionary contains the training, validation and (if
        used) testing datasets used for each fold, with the number of each fold used as the key. The values are of the same
        form as the return values of ``get_experiment_inputs`` for ``inputs_for_crossvalidation = False``. If
        ``return_futures``, the list of futures of the remote workers are returned as well.
    """
    ...
    dataset_split = get_experiment_inputs(
    ...
    training_func_remote = ray.remote(num_cpus=cpus_per_model, num_gpus=gpus_per_model)(training_func)
    ...
    training_func_remote = OPTIMA.core.tools.apply_placement_group_strategy(
        placement_group=placement_group, task=training_func_remote
    ).remote
    ...
    for k, data_k in enumerate(zip(*dataset_split)):
        crossval_input_data[str(k)] = list(data_k)
        ...
        trainables_list.append(
            build_trainable(
                ...
                input_handler,
                dataset_train=data_k[0],
                dataset_val=data_k[1],
    ...
    for model_config, model_output_dir in zip(model_configs, model_output_dirs):
        ...
        for k, trainable in enumerate(trainables_list):
            ...
            history_name = f"model_crossval_{k}_history.pickle"
            output_path = os.path.join(model_output_dir, output_name)
            history_path = os.path.join(model_output_dir, history_name)
            if model_output_dir not in crossval_model_info.keys():
                crossval_model_info[model_output_dir] = [
                    {"name": output_name, "split": str(k), "config": model_config_k, "history": history_name}
                ]
            else:
            ...
            if not skip_training:
                if OPTIMA.core.model.is_model(run_config, model_config_k, input_handler, output_path):
                    if verbose > 0:
                        print(
                    ...
                    # adding the checkpoint directory, the final model path and the history path to the model's config,
                    # start the training and add the future to the list
                    ...
                    model_config_k["training_history_path"] = history_path
