...
# settings relevant to the model architecture
conventional_batch_axis = True  # signifies if the model inputs and outputs have a normal batch axis. If True, each tuple yielded by the Tensorflow dataset / Lightning data module is assumed to be a batch, axis 0 of every tensor in the model inputs and outputs are assumed to be the batch axis, and the dimension of the first axis is assumed to be the same for every tensor in a batch. If False, each tuple yielded by the Tensorflow dataset / Lightning data module is handles individually, and no assumptions are made on their structure.
fixed_input_shape = True  # signifies if the structure and shape of the model input is fixed across batches (excluding the batch size). If this is the case, inputs across multiple batches are tried to be concatenated when providing input arrays to custom metrics. If False, a list of input batches is provided instead. (only used if conventional_batch_axis is True)
fixed_output_shape = True  # signifies if the structure and shape of the model output is fixed across batches (excluding the batch size). If this is the case, predictions across multiple batches are tried to be concatenated when providing prediction arrays to custom metrics. If False, a list of model predictions per batch is provided instead. (only used if conventional_batch_axis is True)
...
crossvalidation_for_fixed_hps = True  # only used if no hyperparameter optimization is to be performed, i.e. perform_variable_opt, perform_main_hyperopt and perform_PBT_hyperopt are False. If True, a crossvalidation training is started for the fixed hyperparameters, otherwise only a single model is trained for the nominal fold.
...
predict_batch_size = -1  # used for calculating custom metrics. If not all model inputs and predictions can be held in memory, the predictions can be calculated in batches and the custom metrics are evaluated on each batch. The requested batch size is rounded up to the nearest multiple of the training batch size (note: the final batch can be smaller). At the end, the metric values are averaged. A value of -1 corresponds to no batching. (only used if conventional_batch_axis is True)
