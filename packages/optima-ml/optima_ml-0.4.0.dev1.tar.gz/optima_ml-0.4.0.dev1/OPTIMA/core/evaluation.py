...
def evaluate_experiment(
    analysis: Union[tune.ExperimentAnalysis, Union[tuple[dict, dict]]],
    ...
    results_dir: str,
    dataset_split: list[ray.data.Dataset],
    ...
    All aforementioned steps can be skipped by directly providing the dictionaries returned after a crossvalidation run
    as the ``analysis`` input parameter.

    A set of plots showing the evolution of the reported metrics for the 2*k models trained during the crossvalidation
    is drawn by calling the ``plot_training_history``-function.
    ...
    If ``write_results`` is True, the results and a summary of the experiment (the number of training, validation and
    (if used) testing samples, the used input variables as given by the provided ``input_handler``-instance if not
    ``None`` and the search space) are saved to `results.txt` in the ``results_dir``.
    ...
    ----------
    analysis : Union[tune.ExperimentAnalysis, Union[tuple[dict, dict]]]
        A ``tune.ExperimentAnalysis``-object extracted from the ``tune.ResultsGrid`` returned by the ``Tuner`` or the
        return value of a cross-validation run. In the first case, a hyperparameter optimization is to be evaluated, in
        the latter case, a training for a fixed set of hyperparameter is to be evaluated.
    training_func : Callable
    ...
    dataset_split : list[ray.data.Dataset]
        List containing the training, validation and (if used) testing ray datasets.
    input_handler : OPTIMA.builtin.inputs.InputHandler
    ...
    placement_group : Optional[PlacementGroup]
        The placement group to be used for all computing tasks. This is provided to ``perform_crossvalidation`` and
        applied to the remote execution of the ``evaluate``-function. (Default value = None)
    ...
    if not os.path.isfile(os.path.join(results_dir, "evaluation.pickle")):
        ...
        # check if we need to evaluate a hyperparameter optimization or a single training run for fixed hyperparameters
        if isinstance(analysis, tune.ExperimentAnalysis):
            ...
            if not os.path.isfile(os.path.join(results_dir, "configs.pickle")):
                ...
                assert (
                    all_model_configs != {}
                ), "Dictionary of configs could not be loaded, was the optimization deleted?"
            ...
            for metric, best_trial, best_epoch in zip(
                best_trials.index, best_trials["trial"], best_trials["best epoch"]
            ):
            ...
            if not skip_fit_evaluation:
                for metric, best_trial, best_epoch in zip(
                    ...
                    target_folder = os.path.join(
                        results_dir, metric if len(best_trials_fit.index) > 1 else "", "best_fit"
                    )
                    dirs_to_evaluate.append(target_folder)  # mark target_folder to be evaluated later
                    if not os.path.exists(target_folder):
                        os.makedirs(target_folder, exist_ok=True)
            ...
            # only a single training run needs to be evaluated, thus the crossvalidation output has already been provided
            crossval_model_info, crossval_input_data = analysis
            best_trials = None
            best_trials_fit = None
            model_configs_df = None
            optimization_results_string = ""

        # draw the training history
        training_history = {}
        for output_dir, crossval_model_info_p in crossval_model_info.items():
            training_history[os.path.join(output_dir, "plots")] = []
            for crossval_model_info_p_k in crossval_model_info_p:
                with open(os.path.join(output_dir, crossval_model_info_p_k["history"]), "rb") as f:
                    training_history[os.path.join(output_dir, "plots")].append(pickle.load(f))
        plot_training_history(training_history, metric_names)
        ...
        # wrap the evaluation function in a ray task and apply the placement group strategy. Already apply the final
        # .remote so that the remote function only needs to be called later
        evaluate_remote = OPTIMA.core.tools.apply_placement_group_strategy(
            placement_group=placement_group,
            task=ray.remote(num_cpus=cpus_per_model, num_gpus=gpus_per_model)(evaluate_func),
        ).remote
        ...
        for model_dir in crossval_model_info.keys():
            ...
            for model_info in crossval_model_info[model_dir]:
                # get the inputs
                dataset_split_k = crossval_input_data[model_info["split"]]
                ...
                futures.append(
                    evaluate_remote(
                        ...
                        dataset_split=dataset_split_k,
                        input_handler=input_handler,
                        fig_dir=os.path.join(
                            model_dir, "plots", "evaluation", "crossval_{}".format(model_info["split"])
                        ),
                        cpus=cpus_per_model,
                        print_results=False,
                        native_metrics=native_metrics,
        ...
        for model_dir in crossval_model_info.keys():
            ...
            for model_info in crossval_model_info[model_dir]:
                ...
                if hasattr(run_config, "finalize_model"):
                    ...
                    # sequentially
                    remote_finalize = ray.remote(num_cpus=cpus_per_model, num_gpus=gpus_per_model)(
                        run_config.finalize_model
                    )
                    remote_finalize = OPTIMA.core.tools.apply_placement_group_strategy(
                        placement_group=placement_group, task=remote_finalize
                    ).remote
                    f = remote_finalize(
                        run_config,
                        dataset_split_k,
        ...
        optimization_str = f"training samples: {dataset_train.count()}\n"
        optimization_str += f"validation samples: {dataset_val.count()}\n"
        if run_config.use_testing_dataset:
            optimization_str += f"test samples:  {dataset_test.count()}\n"
        if input_handler.get_vars() is not None:
...
def plot_training_history(training_history: Union[dict[list[dict]], dict[dict]], metric_names: list) -> None:
    """Plots the evolution of the metric values over the training.

    If the values of the provided ``metric_values`` dict are lists of dicts, they are assumed to correspond to cross-
    validations. As such, the mean of each metric value per epoch is drawn, with the standard deviation as an
    uncertainty band.

    The keys of the ``metric_values`` are assumed to be paths to directories where the figures are to be saved.

    Parameters
    ----------
    training_history : Union[dict[list[dict]], dict[dict]]
        The dictionary containing the metric values. Each key is a path to a directories where the figures are to be
        saved. The values are either dicts, containing the metrics as keys and a list of metric values as values, or a
        list of such dict. In the latter case, the mean of each metric value per epoch is drawn, and the standard
        deviation is used as an uncertainty band.
    metric_names : list
        The names of the metrics to draw.
    """
    # iterate over e.g. best_value and best_fit
    for fig_dir, metric_values_dicts_list in training_history.items():
        # create the output directory
        os.makedirs(os.path.join(fig_dir, "training"), exist_ok=True)

        # treat a single training as a 1-fold crossvalidation
        if not isinstance(metric_values_dicts_list, list):
            metric_values_dicts_list = [metric_values_dicts_list]

        # for each, plot all metrics
        for metric in metric_names:
            # metrics can be grouped as train_XXX, val_XXX or can be alone
            if not isinstance(metric, tuple) or isinstance(metric, list):
                metric = [metric]
            ...
            # iterate over the metrics in the group and plot the corresponding evolutions
            for i in range(len(metric)):
                # get all metric values across all folds such that rows are different epochs and columns are
                # different folds
                metric_values = np.array(
                    [metric_values_dict[metric[i]] for metric_values_dict in metric_values_dicts_list]
                ).T

                # calculate mean and std across folds
                mean_metric_values = np.mean(metric_values, axis=1)
                std_metric_values = np.std(metric_values, axis=1)

                # plot the evolution of the mean metric value
                ax.plot(
                    list(range(1, len(mean_metric_values) + 1)),
                    mean_metric_values,
                    label=metric[i],
                    color=colors[i],
                )
            ...
            ax.legend()
            plt.savefig(
                os.path.join(os.path.join(fig_dir, "training"), "{}.png".format("+".join(metric))),
                dpi=600,
            )
