...
def get_experiment_inputs(
    ...
    """Prepares the input dataset used for the training during the optimization and crossvalidation.
    ...
    expected to return a ``ray.data.Dataset``. The dataset needs to contain an ``Input``-column containing the input
    features. If a ``Weight``-column is present, it is interpreted as a per-row-weight that is applied during all
    optimization steps in `OPTIMA`. It's entries are expected to be float or integer values. Additional columns are
    allowed.

    Once loaded, the dataset is given to ``get_target_data`` to be split into training, validation and (if requested)
    testing datasets. The method of splitting and the sizes of the respective dataset is controlled via the
    ``splitting_cond`` parameter of ``get_target_data``. Its value is controlled by various options discussed below.
    Depending on the value of ``inputs_for_crossvalidation``, which controls if a simple splitting or a k-fold splitting
    is to be done and is given to ``get_target_data`` as ``do_kfold``, ``get_target_data`` is expected to return tuples
    of ``ray.data.Dataset``s (if ``inputs_for_crossvalidation`` is ``False``) or tuples of lists of ``ray.data.Dataset``s
    (if ``inputs_for_crossvalidation`` is ``True``). Each tuple entry is expected to correspond to a different type of
    dataset (training/validation/testing), thus the tuple is expected to have length ``2`` if no testing dataset is
    requested and ``3`` otherwise. If ``inputs_for_crossvalidation`` is ``True``, each list entry is expected to
    correspond to a different fold, and subsequently all lists are expected to be of the same length.

    If ``run_config.produce_inputs_plots`` is ``True`` and ``inputs_for_crossvalidation`` is ``False``, the
    ``plot_input_data``-function is called and the dataset and a path to a subdirectory ``'inputs'`` in the provided
    ``output_dir`` is given as the directory to save the plots of the input features into.
    ...
    Union[train_val_splitting_type,train_val_test_splitting_type,kfold_train_val_splitting_type,kfold_train_val_test_splitting_type,]
        The split ray datasets are returned.
    """
    ...
    dataset_split = get_training_data(
        dataset,
        splitting_cond,
    ...
    if run_config.produce_inputs_plots and not inputs_for_crossvalidation:
        ...
        plot_input_data(
            run_config, dataset, outdir=os.path.join(output_dir, "inputs"), input_vars_list=input_handler.get_vars()
        )
