...
from aioagi.exceptions import (AGIAppError, AGICommandNotPermittedError,
                               AGIConnectHangup, AGIInvalidCommand,
                               AGIResultHangup, AGIUsageError)
...
class AGI:
    ...
    async def run_command(self, command: AGICommand, *args) -> AGIMessage:
        ...
        if message.status == AGICode.MEMORY_ALLOCATION_FAILURE:
            ...
            raise AGIAppError(
                message="Error executing application, or hangup.".encode()
            )
    ...
    async def database_del(self, family, key):
        ...
        """
        message = await self.run_command(
            AGICommand.DATABASE_DEL, self._quote(family), self._quote(key)
        )
    ...
    async def database_deltree(self, family, key=""):
        ...
        """
        message = await self.run_command(
            AGICommand.DATABASE_DELTREE, self._quote(family), self._quote(key)
        )
        return message.result_to_bool()
    ...
    async def database_get(self, family, key):
        ...
        """
        message = await self.run_command(
            AGICommand.DATABASE_GET, self._quote(family), self._quote(key)
        )
        return message.result_to_bool()
    ...
    async def exec_app(self, application, options=""):
        ...
        """
        message = await self.run_command(
            AGICommand.EXEC, application, self._quote(options)
        )
        if message.result == "-2":
    ...
    async def get_data(self, filename, timeout=DEFAULT_TIMEOUT, max_digits=255):
        ...
        return await self.run_command(
            AGICommand.GET_DATA, filename, timeout, max_digits
        )
    ...
    async def get_variable(self, name):
        ...
        """
        message = await self.run_command(AGICommand.GET_VARIABLE, self._quote(name))
    ...
    async def say_alpha(self, characters, escape_digits=""):
        ...
        escape_digits = self._process_digit_list(escape_digits)
        message = await self.run_command(
            AGICommand.SAY_ALPHA, characters, escape_digits
        )
        return message.result_to_char()
    ...
    async def say_datetime(self, seconds, escape_digits="", format="", zone=""):
        ...
        message = await self.run_command(
            AGICommand.SAY_DATETIME, seconds, escape_digits, format, zone
        )
        return message.result_to_char()
    ...
    async def set_variable(self, name, value):
        ...
        await self.run_command(
            AGICommand.SET_VARIABLE, self._quote(name), self._quote(value)
        )
    ...
    async def stream_file(self, filename, escape_digits="", sample_offset=0):
        ...
        escape_digits = self._process_digit_list(escape_digits)
        message = await self.run_command(
            AGICommand.STREAM_FILE, filename, escape_digits, sample_offset
        )
        return message.result_to_char()
