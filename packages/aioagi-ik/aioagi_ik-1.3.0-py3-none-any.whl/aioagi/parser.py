...
from aioagi.log import agi_client_logger, agi_server_logger

AGIRequestMessage = collections.namedtuple(
    "AGIRequestMessage", ["method", "path", "headers", "url"]
)
...
class AGIMessage:
    ...
    def result_as_char(self):
        ...
        try:
            ...
            raise AGIAppError(
                message="Unable to convert result to char: {}".format(self.result)
            )
    ...
    def result_as_int(self):
        try:
            ...
            raise AGIAppError(
                message="Unable to convert result to int: {}".format(self.result)
            )
    ...
    def as_data(self):
        ...
        data = (
            " ({data})".format(data=self.data.pop("data"))
...
class AGIRequestParser:
    ...
    def parse_message(self, lines):
        ...
        headers = collections.OrderedDict(
            [line.decode().split(": ", 1) for line in lines]
        )
    ...
    def parse_message(cls, lines):
        ...
        if match:
            ...
            try:
                ...
                agi_server_logger.warning(
                    "Unhandled code or undefined response: %s", status
                )
            ...
            if status == AGICode.OK:
                ...
                # AGI Usage error
                if data["response"].endswith("Proper usage not available.") or data[
                    "response"
                ].endswith("520 End of proper usage."):
                    pass
                ...
                AGICode.TRYING,
                AGICode.INVALID_COMMAND,
                AGICode.COMMAND_NOT_PERMITTED,
            ):
                # Without result and data.
                pass

            elif status == AGICode.MEMORY_ALLOCATION_FAILURE:
                agi_server_logger.critical(
                    "Unhandled code or undefined response: %s", status
                )
