...
def save_tokenizer_and_embeddings_model_onnx_batch_size(model_name: str, local_dir: str, model_task: None, device: str = "cpu",
    ...
    if model_task is None:
        command = f"optimum-cli export onnx --opset {opset_version} --trust-remote-code --no-weight-sharing -m {model_name} {local_dir} "
    else:
        command = f"optimum-cli export onnx --task {model_task} --opset {opset_version} --trust-remote-code --no-weight-sharing -m {model_name} {local_dir} "
...
def save_tokenizer_and_embeddings_model_onnx(model_name: str,  local_dir: str, model_task: None, device: str = "cpu",
    ...
    os.remove(os.path.join(local_dir, "model.onnx"))
    logger.info(f"Suboptimal ONNX model {onnx_model_path} removed")
    os.remove(os.path.join(local_dir, "full_model_onnxembeddings.onnx"))
    logger.info(f"Suboptimal ONNX model {refined_onnx_model_path_} removed")
    return
