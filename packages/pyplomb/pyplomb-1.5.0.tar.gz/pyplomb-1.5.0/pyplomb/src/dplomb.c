...
void dmeanvarrange(double *ymean, double *yvar, double *xmax, double *xmin, double *y, double *x, const int n) {
    /*
...
void dspread(double y, double *yy, int n, double x, int m) {
    /*
...
void ddft1(double *x, int nn, int isign) {
    /*
...
void drfft(double *x, int n, int isign) {
    /*
...
int dplomb(double *x, double *y, const int n, double o, double hi, double *w1, double *w2, const int nw) {
    ...
    */
    ...
    dmeanvarrange(&ymean,&yvar,&xmax,&xmin,y,x,n);
    xdiff = xmax - xmin;
    ...
    for (int i=1; i<=n; ++i) {
        ...
        MOD(xkk,fndim);
        ++xkk;
        dspread(y[i] - ymean,w1,ndim,xk,MACC);
        dspread(1.0,w2,ndim,xkk,MACC);
    }

    // Compute the real Fast Fourier Transforms
	drfft(w1,ndim,1); drfft(w2,ndim,1);
	double df = 1./(xdiff*o);
