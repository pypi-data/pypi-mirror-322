...
class LocationGroup:
    ...
    def name(self) -> str:
        ...
        # Add type description if common
        if any(tag in common_tags for tag in ['abandoned', 'ruins', 'disused']):
            name_parts.append("Abandoned")
            
        # Add building/historic type if present
        for key in ['building', 'historic', 'military']:
            if key in common_tags and common_tags[key] not in ['yes', 'no']:
                name_parts.append(common_tags[key].title())
