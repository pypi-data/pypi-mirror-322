...
class OSMSearchPlus:
    def __init__(self, logger=None):
        ...
        self.timeout = 5  # Reduced to 5 seconds for faster failure/retry
        self.max_retries = 3
    ...
    def _load_excluded_tags(self) -> List[str]:
        """Load excluded tags from excluded_tags.txt file or use defaults"""
        # Default exclusions if file not found
        default_exclusions = [
            'demolished=yes',
            'highway=bus_stop',
                ...
                try:
                    with open(file_path, 'r') as f:
                        file_exclusions = [line.strip() for line in f.readlines() 
                                if line.strip() and not line.startswith('#')]
                        if file_exclusions:
                            self.logger.debug(f"Loaded excluded tags from {file_path}")
    ...
    def _build_exclusion_filters(self) -> str:
        """Build Overpass QL filters for excluded tags"""
        filters = []
        for excluded_tag in self.excluded_tags:
            if '=' in excluded_tag:
                # Exact match (e.g., demolished=yes)
                key, value = excluded_tag.split('=', 1)
                filters.append(f'["{key}"!="{value}"]')
            else:
                # Simple tag (e.g., highway) - exclude any value
                filters.append(f'["{excluded_tag}"!~".*"]')
        return ''.join(filters)
    ...
    def search(self, lat: Union[str, float, None] = None, 
        ...
        """
        # Build exclusion filters once
        exclusion_filters = self._build_exclusion_filters()
        ...
        try:
            for tag in search_tags:
                ...
                if '=' in tag:
                    ...
                    (
                      node["{tag_key}"="{tag_value}"]{area_filter}{exclusion_filters};
                      way["{tag_key}"="{tag_value}"]{area_filter}{exclusion_filters};
                    );
                    out body;
                    >;
                    out skel qt;
                    ...
                    (
                      ...
                      node["{tag_key}"]{area_filter}{exclusion_filters};
                      way["{tag_key}"]{area_filter}{exclusion_filters};
                      // Match as value in common keys
                      node["building"="{tag_key}"]{area_filter}{exclusion_filters};
                      way["building"="{tag_key}"]{area_filter}{exclusion_filters};
                      node["historic"="{tag_key}"]{area_filter}{exclusion_filters};
                      way["historic"="{tag_key}"]{area_filter}{exclusion_filters};
                    );
                    out body;
                    >;
                    out skel qt;
                ...
                # Cache nodes first
                if show_logs:
                    self.logger.info(f"Found {len(data.get('elements', []))} elements")
                ...
                if show_logs:
                    self.logger.info(f"Added {locations_found} new locations for tag {tag}")
