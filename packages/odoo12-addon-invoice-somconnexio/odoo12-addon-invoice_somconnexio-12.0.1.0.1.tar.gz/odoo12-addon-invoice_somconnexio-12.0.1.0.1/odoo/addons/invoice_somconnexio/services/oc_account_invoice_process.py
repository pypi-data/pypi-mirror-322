...
class OpenCellAccountInvoiceProcess:
    ...
    def _prepare_create_line(self, line):
        ...
        if not account:
            raise BadRequest("Account code %s not found" % (line["accountingCode"]))
        tax = self.env["account.tax"].search([("oc_code", "=", line["taxCode"])])
        if not tax:
            raise BadRequest("Tax code %s not found" % (line["taxCode"]))

        product_id = (
            self.env["product.product"]
            .search([("default_code", "=", line["invoiceSubCategoryCode"])])
            .id
        )
        if not product_id:
            raise BadRequest(
                "Product with code %s not found" % (line["invoiceSubCategoryCode"],)
            )
        response_line = {
    ...
    def _prepare_create(self, params):
        ...
        invoice_date = (
            datetime.fromtimestamp(int(params["invoiceDate"]) / 1000)
            + timedelta(hours=2)
        ).date()
        account_code = params["billingAccountCode"]
        partner_ref = int(account_code[0 : account_code.index("_")])
        partner = self.env["res.partner"].search([("ref", "=", partner_ref)])
        if not partner:
            raise BadRequest("Partner with ref %s not found" % (partner_ref,))
        ...
        lines = [
            self.env["account.invoice.line"].create(self._prepare_create_line(line)).id
            for line in invoice_lines
            if line.get("amountWithoutTax")
        ]
        oc_taxes_parsed = params["taxAggregates"]
        for oc_tax in oc_taxes_parsed:
            tax = self.env["account.tax"].search([("oc_code", "=", oc_tax["taxCode"])])
            if not tax:
                raise BadRequest(
                    "Tax code %s in Tax Aggregate not found" % (oc_tax["taxCode"],)
                )
        ...
        if self.env["account.invoice"].search(
            ...
            raise UserError(
                _("Duplicated OC Number in invoice %s") % (params["invoiceNumber"])
            )
        return {
    ...
    def _validator_create(self):
        return schemas.S_OC_ACCOUNT_INVOICE_CREATE
