...
def set_numpy_multithreading_method():
    """
    ...
    import numpy as np  # Import **after** settings.
    np.zeros(1)  # Ensures the formatter doesn't remove the unused NumPy import
...
def assert_path(s: str):
    p = Path(s)
    if not p.exists():
        logger.error(f"Path given is not valid: {s}")
        raise ValueError(f"Path given is not valid: {s}")
    return p
...
def validate_args(args):
    ...
    if args.num_threads_axsi > 1 and (
        ...
        raise ValueError("Invalid input parameters: currently, running R packages with multiple threads is not "
                         "supported.")
    args.subj_folder = assert_path(args.subj_folder)
...
def run_from_terminal():
    ...
    parser.add_argument('--subj-folder', type=Path, help='Path to the subject folder', required=True)
    ...
    set_numpy_multithreading_method()
    ...
    try:
        if args.subj_folder:
            ...
            axsi_main(args.subj_folder, file_names, small_delta, big_delta, gmax, gamma_val)
    finally:
        # for Rserver - not in use, it is very slow
...
# ======================================================
# python AxSI_main.py --subj-folder tests/short-output --bval 'tests/test_data/part_times/test_vol_0_1_5_8_bval.txt' --bvec 'tests/test_data/part_times/test_vol_0_1_5_8_bvec.txt' --data 'tests/test_data/part_times/test_vol_0_1_5_8_data.nii' --mask 'tests/test_data/part_times/test_vol_0_1_5_8_mask.nii'

# 6-timeframes (0,1,5,8,80,81) input files (bvals: 0,0, 1,1, 2,2):
# ======================================================
# python AxSI_main.py --subj-folder tests/short-output --bval 'tests/test_data/part_times/test_vol_0_1_5_8_80_81_bval.txt' --bvec 'tests/test_data/part_times/test_vol_0_1_5_8_80_81_bvec.txt' --data 'tests/test_data/part_times/test_vol_0_1_5_8_80_81_data.nii' --mask 'tests/test_data/part_times/test_vol_0_1_5_8_80_81_mask.nii'
...
# python AxSI_main.py --subj-folder 15-voxels-output --bval "toy_tests/test_data/part_voxels/voxels_${s}_${e}_bval.txt" --bvec "toy_tests/test_data/part_voxels/voxels_${s}_${e}_bvec.txt" --data "toy_tests/test_data/part_voxels/voxels_${s}_${e}_data.nii" --mask "toy_tests/test_data/part_voxels/voxels_${s}_${e}_mask.nii"
