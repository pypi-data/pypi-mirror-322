...
from AxSI.toolbox import Values, MyManager, parallel_batches
...
class AXSI_DTI:
    ...
    def main(self) -> DTI_Values:
        ...
        Parallellism depends on hardcoded value of num_of_processed. # TODO: Refael: make it visiable to the user

        :return: struct with dti values (not a shared one in any case)
        """
        if False:  # config.NUM_PROCESSES == 0: # comment out for now. multiprocessing in this step is not efficient.
...
def dti(scan: Scan, bvalue: float, is1000: bool = False, subj_folder: Path = None) -> DTI_Values:
    ...
    """
    # if config.NUM_PROCESSES > 1 or config.NUM_THREADS > 1:
    #     set_log_config()  # Set logger for child process
