...
class Predictions():
    ...
    def __init__(self, scan: Scan, dti1000: DTI_Values) -> None:
        ...
        if config.NUM_PROCESSES == 1:  # in multiprocessing init it with shared memory in predict_hindered() method
    ...
    def predict_hindered(self, bshell: np.ndarray, shell_dti: list[DTI_Values]) -> None:
        ...
        if config.NUM_PROCESSES > 1:
            with Manager() as manager:
                ...
                parallel_batches(self.num_of_vols, self.predict_md0_for_voxel, bshell, sMDi)
...
def predict(scan: Scan, dti1000: DTI_Values, shell_dti: list[DTI_Values]) -> Predictions:
    ...
    toc = time.time()
    logger.debug("Computation time md0 all voxels " + ("with multi-processes/multi-threads "
                 if config.NUM_PROCESSES > 1 or config.NUM_THREADS > 1 else "") + "= " + str((toc - tic)) + " sec.")
