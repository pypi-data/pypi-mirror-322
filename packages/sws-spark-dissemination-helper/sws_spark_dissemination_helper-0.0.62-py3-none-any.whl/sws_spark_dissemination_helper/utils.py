...
def map_codes(
    ...
    partition_columns = [col for col in dimension_columns if col != dest_column] + [
        "partition_column"
    ]
    ...
    domain_mapping_df = df_mapping.filter(
        ...
        & (col("var_type") == lit(col_type))
        & (col("mapping_type").isNull() | (col("mapping_type").isNull() == lit("")))
    )
    ...
    return (
        ...
        .select(
            ...
            F.coalesce(col("m_domain.delete"), col("m_standard.delete")).alias(
                "delete"
            ),
        ...
        .withColumn(
            "partition_column", F.coalesce(col("new_dim_code"), col(dest_column))
        )
        .withColumn(
            "count_obs_per_point", F.count(col("value")).over(partitioning_window)
        )
        ...
        .withColumn(
            "note",
            F.when(
                col("is_duplicate") & col("new_dim_code").isNotNull(),
                F.array_append(
                    col("note"),
                    lit(
