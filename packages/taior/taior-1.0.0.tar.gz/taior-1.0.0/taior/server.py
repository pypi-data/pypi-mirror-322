...
def start_server():
    """Inicia el servidor para escuchar conexiones entrantes en un puerto libre."""
    server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server.bind(("", 0))  # Asigna un puerto libre automáticamente
    port = server.getsockname()[1]  # Obtiene el puerto asignado
    server.listen(5)
    node_id = generate_node_id()  

    # Agregar o actualizar el peer en la lista
