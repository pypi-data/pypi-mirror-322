...
class TestMSDParameter(unittest.TestCase):
    ...
    def test_stringify_with_exact_and_generated_escape(self):
        param = MSDParameter(
        ...
        expected = replace(param, escape_positions=(9, 11))

        self.assertEqual(expected, next(parse_msd(string=result)))

    def test_stringify_with_exact_and_generated_escape_and_comment(self):
        param = MSDParameter(
            ("key", "http://test.example/ \n"),
            preamble="",
            comments=((0, "// foo"),),
