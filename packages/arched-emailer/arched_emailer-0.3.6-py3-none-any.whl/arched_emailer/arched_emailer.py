...
class ArchedEmailer:
    ...
    def __init__(self, app: str, api_key: Optional[str] = None, mail_connection_string: Optional[str] = None,
                 task_id: Optional[int] = None, flask: Optional[bool] = False, log_file: Optional[str] = None,
        ...
        Args:
            ...
            task_id (int, optional): The task ID associated with the email operations.
            flask (bool, optional): Whether to initialize for Flask integration.
        ...
        # Initialize logger
        self._initialize_logger(log_file, log_level)

        self.logger.info("Initializing ArchedEmailer instance.")

        if not flask:
            self.setup(api_key, mail_connection_string)
        else:
            self.logger.debug("Flask integration enabled; skipping setup.")

        self._cleanup_error_log()

    def _initialize_logger(self, log_file: Optional[str], log_level: Optional[str]):
        """
        Initializes the rotating file logger.

        Args:
            log_file (str, optional): Path to the log file. Defaults to 'arched_emailer.log' in the data directory.
            log_level (str, optional): Logging level.
        """
        if not log_file:
            log_file = os.getenv("ARCHED_EMAILER_LOG_FILE")
            if not log_file:
        ...
        if not self.logger.handlers:
            ...
            self.logger.debug(f"Logger initialized with log file: {log_file} and level: {log_level}")
    ...
    def setup(self, api_key: Optional[str] = None, mail_connection_string: Optional[str] = None):
        ...
        Returns:
            None
        """
        self.logger.info("Setting up ArchedEmailer.")
        ...
        self.logger.debug(f"Arched API Key: {'***' if self.arched_api_key else 'None'}")
        self.logger.debug(f"Mail Connection Details: {'***' if self.connection_details else 'None'}")
        ...
        try:
            ...
            self.logger.info("SMTP mailers initialized successfully.")
        except Exception as e:
            self.logger.error("Failed to initialize SMTP mailers.", exc_info=True)
    ...
    def init_app(self, app, intercept_errors: Optional[bool] = True, add_current_user: bool = True, ignore_url_patterns=None, **kwargs):
        ...
        self.logger.info("Initializing Flask application with ArchedEmailer.")
        app.extensions["arched_emailer"] = self
        self.flask_app = app

        if intercept_errors:
            self.logger.debug("Error interception enabled.")
            if ignore_url_patterns:
                self.ignore_url_patterns = ignore_url_patterns
                self.logger.debug(f"Ignoring URL patterns: {ignore_url_patterns}")
            ...
            def update_visited_urls():
                ...
                if 'visited_urls' not in session:
                    ...
                    self.logger.debug("Initialized 'visited_urls' in session.")
                ...
                if "static" not in current_url and ".js" not in current_url and ".css" not in current_url:
                    ...
                    self.logger.debug(f"Updated visited URLs: {session['visited_urls']}")
    ...
    def _process_error(self, error, add_current_user, request):
        ...
        Returns:
            None
        """
        self.logger.debug("Processing an error to send an email.")
        def format_dict_for_email(data):
            return json.dumps(data, indent=4).replace(" ", "&nbsp;").replace("\n", "<br>")
        ...
        try:
            ...
            self.logger.error("Failed to process and send error email.", exc_info=True)
            ...
            if os.path.exists(error_log_path):
                try:
                    with open(error_log_path, 'r') as f:
                        ...
                        self.logger.debug("Loading existing error log.")
                        return {k: datetime.datetime.fromisoformat(v) for k, v in data.items()}
                except Exception as e:
                    self.logger.error("Failed to load error log.", exc_info=True)
            self.logger.debug("No existing error log found; initializing empty log.")
            return {}
    ...
    def _save_error_log(self):
        ...
        with self._lock:
            ...
            try:
                ...
                self.logger.debug("Error log saved successfully.")
            except Exception as e:
                self.logger.error("Failed to save error log.", exc_info=True)
    ...
    def _cleanup_error_log(self, max_age_minutes: int = 2880):  # 2 days
        ...
        Returns:
            None
        """
        self.logger.debug("Starting cleanup of error log.")
        current_time = datetime.datetime.now()
        keys_to_delete = [k for k, v in self.errors_name_time.items() if
                          (current_time - v).total_seconds() / 60 > max_age_minutes]
        if keys_to_delete:
            self.logger.info(f"Removing {len(keys_to_delete)} old error log entries.")
            for k in keys_to_delete:
                del self.errors_name_time[k]
            self._save_error_log()
        else:
            self.logger.debug("No old error log entries to remove.")
    ...
    def _get_set_user_details(self):
        ...
        Returns:
            None
        """
        self.logger.debug("Fetching user details from API.")
        if self.connection_details:
            self.logger.debug("Connection details already set; skipping API fetch.")
            return
        ...
        try:
            ...
            if data[2] == 200 and data[1]:
                ...
                self.logger.info("User details fetched from API successfully.")
                # Save these details locally as a fallback
                self._save_user_details_locally(data[1])
            else:
                self.logger.warning("API did not return user details; attempting to load locally.")
                raise ValueError(
                    "Server did not respond with user details, either API_KEY is invalid or server is down.")
        except Exception as e:
            self.logger.error("Failed to fetch user details from API; attempting to load from local.", exc_info=True)
    ...
    def _save_user_details_locally(self, user_details):
        ...
        Returns:
            None
        """
        self.logger.debug("Saving user details locally.")
        data_dir = self._get_create_data_dir()
        if not os.path.exists(data_dir):
            os.makedirs(data_dir)
            self.logger.debug(f"Created data directory at {data_dir}.")
        file_path = os.path.join(data_dir, "user_details.json")
        try:
            with open(file_path, 'w') as file:
                json.dump(user_details, file)
            self.logger.info("User details saved locally.")
        except Exception as e:
            self.logger.error("Failed to save user details locally.", exc_info=True)
    ...
    def _load_user_details_from_local(self):
        ...
        Returns:
            None
        """
        self.logger.debug("Loading user details from local file.")
        data_dir = self._get_create_data_dir()
        file_path = os.path.join(data_dir, "user_details.json")
        if os.path.exists(file_path):
            try:
                ...
                self.logger.info("User details loaded from local file successfully.")
            except Exception as e:
                self.logger.error("Failed to load user details from local file.", exc_info=True)
        else:
            self.logger.warning(f"Local user details file does not exist at {file_path}.")
    ...
    def _make_request(self, url: str, method: Optional[str] = "GET", body: Optional[dict] = None) -> Tuple[str, Union[Dict, str], int]:
        ...
        """
        self.logger.debug(f"Making {method.upper()} request to URL: {url} with body: {body}")
        ...
        try:
            ...
            if method == 'get':
                ...
                self.logger.error(f"Unsupported HTTP method: {method.upper()}")
            ...
            if response.status_code == 200:
                try:
                    ...
                    self.logger.debug("Response is not JSON; returning text.")
                    return "Success:", response.text, 200
            else:
                try:
                    error_data = response.json()
                    self.logger.warning(f"API request failed with status {response.status_code}: {error_data}")
            ...
            self.logger.error("HTTP request failed.", exc_info=True)
    ...
    def _load_env(self):
        ...
        Returns:
            None
        """
        self.logger.debug("Loading environment variables from connection details.")
        if self.connection_details:
            try:
                ...
                self.logger.error("Failed to load environment variables from connection details.", exc_info=True)
        else:
            self.logger.warning("No connection details provided; skipping environment variable loading.")
    ...
    def _get_create_data_dir(self):
        ...
        try:
            ...
            self.logger.debug(f"Data directory ensured at: {app_data_dir}")
        except Exception as e:
            self.logger.error(f"Failed to create data directory at {app_data_dir}.", exc_info=True)
    ...
    def _get_email_path(self, typ="error", task_id: Optional[int] = None):
        ...
        """
        self.logger.debug(f"Fetching {typ} email template.")
        data_dir = self._get_create_data_dir()
        ...
        try:
            ...
            if resp_text[2] == 200:
                with open(email_path, "w") as f:
                    f.write(resp_text[1])
                self.logger.info(f"{typ.capitalize()} email template saved at {email_path}.")
            else:
                self.logger.error(f"Failed to fetch {typ} email template: {resp_text[1]}")
        except Exception as e:
            self.logger.error(f"Exception occurred while fetching {typ} email template.", exc_info=True)
    ...
    def log_message(self, log_level: str, text: str, task_id: int = None):
        ...
        """
        self.logger.debug(f"Logging message with level {log_level}: {text}")
        ...
        if response[2] == 200:
            self.logger.info(f"Log message sent successfully: {text}")
        else:
            self.logger.error(f"Failed to send log message: {response[1]}")
        return response
    ...
    def send_email(self, sender_email: str, sender_name: str, recipients: Union[str, list], subject: str,
        ...
        """
        self.logger.debug(f"Sending email from {sender_email} to {recipients} with subject '{subject}'.")
        try:
            ...
            if success:
                ...
                self.logger.warning(f"Email failed to send to {recipients}.")
            return success
        except Exception as e:
            self.logger.error("Exception occurred while sending email.", exc_info=True)
            return False
    ...
    def _allowed_to_send(self, exception: Union[str, Exception], allowed_minutes: int = 60) -> bool:
        ...
        self.logger.debug(f"Checking if allowed to send exception: {exception_text}")
        current_time = datetime.datetime.now()

        last_sent = self.errors_name_time.get(exception_text)
        if last_sent:
            elapsed = (current_time - last_sent).total_seconds() / 60  # minutes
            self.logger.debug(f"Elapsed time since last sent: {elapsed} minutes.")
            if elapsed < allowed_minutes:
                self.logger.info(f"Exception '{exception_text}' sent {elapsed} minutes ago; skipping email.")
                return False

        # Update the timestamp and save
        self.errors_name_time[exception_text] = current_time
        self._save_error_log()
        self.logger.debug(f"Allowed to send exception '{exception_text}'; updated last sent time.")
        return True
    ...
    def _send_to_db(self, success=True, **kwargs: dict):
        ...
        Returns:
            None
        """
        self.logger.debug("Sending email attempt to the database.")
        if kwargs.get("task_id"):
            ...
            if response[2] == 200:
                self.logger.info("Email attempt logged to the database successfully.")
            else:
                self.logger.error(f"Failed to log email attempt to the database: {response[1]}")
    ...
    def _get_html_content(self, message: MIMEMultipart):
        ...
        """
        self.logger.debug("Extracting HTML content from email message.")
        html_content = ""
        for part in message.walk():
            ...
            if part.get_content_type() == 'text/html':
                ...
                self.logger.debug("HTML content extracted successfully.")
                break
        if not html_content:
            self.logger.warning("No HTML content found in the email message.")
        return html_content
    ...
    def send_success_email(self, recipients: Union[str, list], dump_time_taken: Optional[bool] = True,
        ...
        Returns:
            None
        """
        self.logger.debug("Preparing to send success email.")
        try:
            if sender:
                self.success_mailer = SmtpMailer(sender, sender_name)
                self.logger.debug(f"Success mailer updated with sender: {sender}.")
            ...
            if dump_time_taken and self.time_start:
                ...
                self.logger.debug(f"Time taken added to email: {kwargs['time_taken']}.")
            ...
            if not update_db_only:
                ...
                self.logger.info(f"Success email sent to {recipients}.")

                html_content = self._get_html_content(self.success_mailer.message)
            else:
                html_content = None
                self.logger.debug("Update DB only flag is set; skipping email sending.")
            ...
            self.logger.error("Exception occurred while sending success email.", exc_info=True)
    ...
    def send_error_email(self, recipients: Union[str, list], error_text: Optional[str] = None,
        ...
        Returns:
            None
        """
        self.logger.debug("Preparing to send error email.")
        try:
            if sender:
                self.mailer = SmtpMailer(sender, sender_name)
                self.logger.debug(f"Mailer updated with sender: {sender}.")
            ...
            if include_tb:
                ...
                self.logger.debug("Traceback included in error email.")
            if dump_enviro:
                template_data["enviro"] = obfuscate_sensitive_info(dict(os.environ))
                self.logger.debug("Environment variables included in error email.")
            for dump_ok, dump_type, dump_ob in [
                ...
                if dump_ok:
                    ...
                    self.logger.debug(f"{dump_type.capitalize()} variables included in error email.")
            ...
            if issue:
                if self._allowed_to_send(issue, allowed_minutes=allowed_minutes):
                    ...
                    self.logger.info(f"Error email sent to {recipients}.")
                    ...
                    self.logger.info("Error email not sent due to throttling.")
        except Exception as e:
            self.logger.error("Exception occurred while sending error email.", exc_info=True)
    ...
    def try_log_function(self, error_recipients: Union[str, list], send_success: Optional[bool] = False,
        ...
        """
        self.logger.debug("Creating try_log_function decorator.")

        def decorator(func):
            def wrapper(*func_args, **func_kwargs):
                self.logger.debug(f"Function '{func.__name__}' started.")
                result = None

                self._log_timer()
                try:
                    result = func(*func_args, **func_kwargs)
                    self.logger.debug(f"Function '{func.__name__}' executed successfully.")
                    ...
                    if send_success and success_recipients:
                        ...
                        self.logger.info(f"Success email sent to {success_recipients}.")
                    if not send_success and send_to_db:
                        ...
                        self.logger.debug("Success email logged to database without sending.")
                except Exception as e:
                    self.logger.error(f"Exception in function '{func.__name__}': {e}", exc_info=True)
                    self.send_error_email(
                        error_recipients,
                        exception=e,
                    ...
                    self._log_timer(True)
                    self.logger.debug(f"Function '{func.__name__}' finished.")
                    return result
    ...
    def _log_timer(self, reset=False):
        ...
        """
        if reset:
            self.logger.debug("Resetting the timer.")
            self.time_start = None
            return
        self.time_start = datetime.datetime.now()
        self.logger.debug(f"Timer started at {self.time_start}.")
