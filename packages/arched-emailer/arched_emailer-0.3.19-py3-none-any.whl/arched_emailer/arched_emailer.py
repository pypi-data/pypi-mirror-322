...
class ArchedEmailer:
    ...
    def _initialize_logger(self):
        """
        Initializes the rotating file logger and systemctl status logger.
        """
        ...
        self.log("info", "Systemctl status logger is active.")
    ...
    def init_app(self, app, intercept_errors: Optional[bool] = True, add_current_user: bool = True,
                 ignore_url_patterns=None, **kwargs):
        """
        Initialize the application with the ArchedEmailer instance using signals.
        This approach lets custom Flask error pages still run,
        while ArchedEmailer can capture and email exceptions.
        """
        from flask import request, session, got_request_exception

        self._initialize_logger()
        self.log("info", "Initializing Flask application with ArchedEmailer (via signals).")

        app.extensions["arched_emailer"] = self
        self.flask_app = app

        if intercept_errors:
            self.log("debug", "Error interception enabled with signals.")
            ...
            def update_visited_urls():
                ...
                # Skip static, JS, CSS, etc. from visited URLs
            ...
            def capture_exception(sender, exception):
                """
                Signal handler for any exception that Flask sees in a request.
                We do the same process we did in `handle_http_exception`,
                but do *not* re-raise. Let Flask handle the response.
                """
                ...
                # If we have patterns to ignore (for certain routes), skip them
                ...
                self.log("debug", f"got_request_exception signal caught: {exception}")
                self._process_error(exception, add_current_user, request)
                # *Do not* raise or return anything;
                # let Flask continue to whichever error handler is configured.

        # If you need environment setup or other details, do it now:
        self.setup(kwargs.get("api_key"), kwargs.get("mail_connection_string"))
        self.log("info", "Flask application initialization via signals complete.")
