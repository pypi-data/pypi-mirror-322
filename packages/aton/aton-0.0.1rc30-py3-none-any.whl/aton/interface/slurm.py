...
| `scancel_running()` | Scancel all running calculations |
| `scancel_pending()` | Scancel all pending calculations |
| `scancel_here()`    | Scancel all calculations running from a folder |
...
def _squeue(user) -> pd.DataFrame:
    result = call.bash(f'squeue -u {user}')
    data = result.stdout
...
def scancel(user) -> None:
    """Cancel all `user` jobs"""
    df = _squeue(user)
    jobid_list = df['JOBID'].tolist()
    for jobid in jobid_list:
        call.bash(f'echo {jobid}')
