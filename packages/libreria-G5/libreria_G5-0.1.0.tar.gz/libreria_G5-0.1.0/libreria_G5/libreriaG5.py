...
class Structure:
    def __init__ (self, valor_limite = 65, metodo_object = 'moda', metodo_number = 'mean', quantile_min = 0.15, quantile_max = 0.75, mantener_columna = 'no'):
        self.valor_limite = valor_limite
        self.metodo_object = metodo_object
        self.metodo_number = metodo_number
        self.quantile_min = quantile_min
        self.quantile_max = quantile_max
        self.mantener_columna = mantener_columna

    def preprocesado(self, df):
        df = df.loc[:, df.isna().mean()*100 < self.valor_limite]
        columns = df.columns
        num_columns = len(columns)
        for i in columns:
            if df[i].dtype == 'object':
                if self.metodo_object.lower() == 'moda':
                    df[i] = df[i].fillna(df[i].mode()[0])
                elif self.metodo_object.lower() == 'bfill':
                    df[i] = df[i].bfill()
                elif self.metodo_object.lower() == 'ffil':
                    df[i] = df[i].ffill()
                else:
                    print('This model is not valid; please try one of the following: moda, back fill or front fill')
            else:
                if self.metodo_number.lower() == 'mean':
                    df[i] = df[i].fillna(df[i].mean())
                elif self.metodo_number.lower() == 'moda':
                    df[i] = df[i].fillna(df[i].mode()[0])
                elif self.metodo_number.lower() == 'bfill':
                    df[i] = df[i].bfill()
                elif self.metodo_number.lower() == 'ffil':
                    df[i] = df[i].ffill()
                elif self.metodo_number.lower() == 'medain':
                    df[i] = df[i].fillna(df[i].median())
                elif self.metodo_number.lower() == 'max':
                    df[i] = df[i].fillna(df[i].max())
                elif self.metodo_number.lower() == 'min':
    ...
    def string_to_number(self, df):
        ...
        if self.mantener_columna.lower() == 'no':
            for i in df.columns:
                if df[i].dtype == 'object':
                    df[i] = modelo_label.fit_transform(df[i])
        elif self.mantener_columna.lower() == 'si':
            for i in df.columns:
                if df[i].dtype == 'object':
                    nombre = i + '_encoded'
    ...
    def analisis_visual_variables_numericas(self, df_g):
