...
def _execute_query_str(connection, query_str: str, *args, **kwargs) -> List[Tuple[Any]]:
    """
    Execute a raw SQL query string by wrapping it with SQLAlchemy's text() function.

    This wrapper ensures safer execution of string-based SQL queries by:
    1. Making raw SQL usage explicit via text()
    2. Protecting against SQL injection
    3. Maintaining compatibility with newer SQLAlchemy versions that enforce strict typing

    Example:
        execute_raw_sql(conn, "SELECT * FROM my_table")
    """
    return connection.execute(text(query_str), *args, **kwargs)


def _is_dml_query(query: str) -> bool:
    """Check if the query is a DML operation requiring commit."""
    dml_statements = ("INSERT", "UPDATE", "DELETE", "COPY", "TRUNCATE", "MERGE")
    return query.strip().upper().startswith(dml_statements)
...
def query_executor(
    ...
    **kwargs,
    ...
    try:
        ...
        results = _execute_query_str(connection, query, *args, **kwargs).fetchall()
        if _is_dml_query:  # Add DML operations that need commits
...
def ds_query_dataframe(engine, query: str) -> pd.DataFrame:
    return query_executor(engine, query).fetch_pandas_all()
