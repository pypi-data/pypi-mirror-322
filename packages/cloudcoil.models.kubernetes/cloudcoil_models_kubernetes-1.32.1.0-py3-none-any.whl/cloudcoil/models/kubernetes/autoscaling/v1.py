...
class Scale(Resource):
    class Builder(BaseBuilder):
        ...
        def spec(
            self,
            value_or_callback: Union[Optional[ScaleSpec], Callable[[Type[ScaleSpec]], ScaleSpec]],
        ) -> Self:
            value = value_or_callback
            if callable(value_or_callback):
                value = value_or_callback(ScaleSpec)
            return self._set("spec", value)

        def status(
            self,
            value_or_callback: Union[
                Optional[ScaleStatus], Callable[[Type[ScaleStatus]], ScaleStatus]
            ],
        ) -> Self:
            value = value_or_callback
            if callable(value_or_callback):
                value = value_or_callback(ScaleStatus)
            return self._set("status", value)
