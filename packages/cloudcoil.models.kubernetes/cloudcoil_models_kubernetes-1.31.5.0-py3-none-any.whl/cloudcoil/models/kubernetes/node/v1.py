    ...
    class Builder(BaseBuilder):
        ...
        def tolerations(
            ...
            value_or_callback: Union[
                ...
                Callable[[Type[v1.Toleration]], List[v1.Toleration]],
            ],
        ) -> Self:
            value = value_or_callback
            if callable(value_or_callback):
                value = value_or_callback(v1.Toleration)
...
class RuntimeClass(Resource):
    class Builder(BaseBuilder):
        ...
        def overhead(
            self,
            value_or_callback: Union[Optional[Overhead], Callable[[Type[Overhead]], Overhead]],
        ) -> Self:
            value = value_or_callback
            if callable(value_or_callback):
                value = value_or_callback(Overhead)
            return self._set("overhead", value)

        def scheduling(
            self,
            value_or_callback: Union[
                Optional[Scheduling], Callable[[Type[Scheduling]], Scheduling]
            ],
        ) -> Self:
            value = value_or_callback
            if callable(value_or_callback):
                value = value_or_callback(Scheduling)
