...
async def set_cache(client: Redis, key: str, value: Any, 
                    expire: int | timedelta | None = None) -> bool:
    try:
        if expire is not None and (not isinstance(expire, (int, timedelta)) or (isinstance(expire, int) and expire < 0)):
            logger.error("Invalid expire value: must be a non-negative integer, timedelta or None")
            return False
