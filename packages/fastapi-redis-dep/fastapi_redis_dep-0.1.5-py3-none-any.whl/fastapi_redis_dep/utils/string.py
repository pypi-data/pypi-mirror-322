...
class RedisString:
    ...
    async def set(self, key: str, value:StringType, expire: int | timedelta | None = None) -> bool:
        return await self._client.set(key, value, expire)

    async def get(self, key: str) ->StringType| None:
    ...
    async def set_or_get(self, key: str,
                         value_default: StringType | None = None) -> StringType:
        if await self._client.exists(key):
            return await self._client.get(key)
        return await self._client.set(key, value_default) # type: ignore
    ...
    async def set_cache(self, key: str, value:  StringType|dict| BaseModel,
