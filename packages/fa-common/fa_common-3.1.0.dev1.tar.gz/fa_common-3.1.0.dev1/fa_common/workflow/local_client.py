...
class LocalWorkflowClient(WorkflowBaseClient):
    ...
    async def run_job(self, job_base: JobTemplate, verbose: bool = True) -> WorkflowRun:
        ...
        flow_run_state = local_utils.run_prefect_jobs(jobs, flow_name=job_base.custom_id, ignore_clean_up=False, return_state=True)
        # flow_run_name = f"{job_base.module.name}: {flow_run.id}"

        return WorkflowRun(
            workflow_id=job_base.custom_id,
            mode=job_base.submit_mode,
            message=flow_run_state.message,
            status=flow_run_state.type.value,
            detail=None,
        )
        # return
