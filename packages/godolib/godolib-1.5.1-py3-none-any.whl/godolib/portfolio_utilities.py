...
class PortfolioEvaluator:
    def __init__(self, benchmarck_series):
        self.benchmarck_series = benchmarck_series
    ...
    def evaluate_trades(self, simulator):
        ...
        trades_mean_buy_price = self._mean_buy_price(
            trades_df=pd.DataFrame(simulator.trades)
        )
        trades_evaluation["Trade_avg_buy_price"] = trades_evaluation["Trade_ID"].map(
    ...
    def _metrics(self, df):
        ...
        benchmarck_cummulative_return = (
            (
                float(self.benchmarck_series.iloc[-1])
                / float(self.benchmarck_series.iloc[0])
            )
            - 1
        ) * 100
        ...
        metrics["SP500_Cumulative_Return_Percent"] = benchmarck_cummulative_return
