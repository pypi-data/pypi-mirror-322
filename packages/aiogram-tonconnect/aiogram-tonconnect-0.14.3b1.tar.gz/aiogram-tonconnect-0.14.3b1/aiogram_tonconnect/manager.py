...
class ATCManager:
    ...
    def __init__(
        ...
        self.connect_wallet_callbacks_storage = ConnectWalletCallbackStorage(
            storage=tonconnect.storage,
            user_id=user.id,
        )
        self.send_transaction_callbacks_storage = SendTransactionCallbackStorage(
            storage=tonconnect.storage,
            user_id=user.id,
        )
    ...
    async def send_transaction(
        ...
        reply_markup = self.__inline_keyboard.send_transaction(
            self.user.app_wallet.name, self.user.app_wallet.direct_url,
        )
        ...
        try:
            async with self.connector.connect_wallet_context() as result:
                if isinstance(result, WalletInfo):
                    state_data = await self.state.get_data()
                    info_wallet = InfoWallet(**self.connector.wallet.to_dict())
                    account_wallet = AccountWallet.from_dict(self.connector.account.to_dict())
                    ...
                    if state_data.get("check_proof", False):
                        ...
                        if not self.connector.wallet.verify_proof(proof_payload):
                            await self._connect_wallet_proof_wrong()
                            return

                    self.middleware_data["app_wallet"] = self.connector.wallet_app
                    self.middleware_data["account_wallet"] = self.connector.account
                    self.middleware_data["info_wallet"] = self.connector.wallet
