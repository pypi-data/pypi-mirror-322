...
def find_workspace_root(start_path: Optional[Path] = None) -> Path:
    """
    Finds the workspace root by searching for the '.ai' directory starting from
    ...
    Raises:
        WorkspaceError: If the '.ai' directory is not found in any parent directory.
    """
    ...
    for parent in [current_path] + list(current_path.parents):
        ai_dir = parent / ROOT_DIR
        if ai_dir.is_dir():
            return parent

    raise WorkspaceError(f"Workspace root not found. Ensure a '{ROOT_DIR}' directory exists in the workspace root.")
