    ...
    def load(self) -> Iterable[tuple[str, Any]]:
        ...
        for _ in range(self._rounds):
            ...
            for path in sort_files_by_dependency(
                ...
                for name, wants in self._validator(self._cache, module, modname):
...
def static_child_of(*bases: type) -> Validator:
    ...
    def check(
        ...
        for node in ast.walk(module_ast):
            possible_bases = set(base.__name__ for base in bases) | set(cachemap.keys())
            ...
            if tuple_assign is not None:
                for idx, item in enumerate(tuple_assign.elts):
                    if isinstance(item, ast.Call):
                        name = next(filter_isinstance(ast.Name, ast.walk(item)), None)
                ...
                if next_call is None:
                    continue
                name = next(filter_isinstance(ast.Name, ast.walk(next_call.func)), None)
                if not name:
                    continue
        ...
        if mode == 'any':
            ...
            for validator in validators[:-1]:
                for item, _ in validator(*args, **kwargs):
                    yield (item, False)
            yield from validators[-1](*args, **kwargs)
        else:
...
def static_modulename_validator(module_name_filter: str | re.Pattern) -> Validator:
    """
