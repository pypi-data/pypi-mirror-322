...
def update_model_json_schema(model: Type[BaseModel]):
    """
    Update the model_json_schema method to handle Secret fields
    in a unified way with SecretStr
    """
    if getattr(model, "_model_json_schema_patched", False):
        return
    existing_model_json_schema = model.model_json_schema

    def model_json_schema(cls: Type[BaseModel], *args, **kwargs):
        schema = existing_model_json_schema(cls, *args, **kwargs)
        for field in cls.model_fields.values():
            if (
                getattr(field.annotation, "__origin__", None) == Secret
                and len(field.annotation.__args__) == 1
                and issubclass(field.annotation.__args__[0], BaseModel)
            ):
                sub_class = field.annotation.__args__[0]
                for json_field in schema["$defs"][sub_class.__name__][
                    "properties"
                ].values():
                    json_field.update({"writeOnly": True, "format": "password"})
        return schema

    model._model_json_schema_patched = True
...
def publish_action(args):
    ...
    if isinstance(plugin.settings, dict):
        ...
        update_model_json_schema(plugin.settings)
