...
class LogicTree(object):
    ...
    def to_str_as_one_line(self, /, *, _is_root: bool = True) -> str:
        """
        ...
        for ch in self.child_nodes:
            if not isinstance(ch, LogicTree):
                ...
                tmp = ch.to_str_as_one_line(_is_root=False, )
    ...
    def to_str_as_multi_lines(self, /, *, indent: int = 0, _is_root: bool = True, ) -> str:
        """
        ...
        for ch in self.child_nodes:
            if not isinstance(ch, LogicTree):
                ...
                tmp = ch.to_str_as_multi_lines(indent=indent + 4, _is_root=False, )
                ...
                if mid_rows and not new_line_canceled:
                    ...
                    tmp = '\n' + tmp
            mid_rows.append(tmp)
        res = ('\n' + ' ' * indent + str(self.logic) + ' ').join(mid_rows) if len(mid_rows) > 1 else mid_rows[0]
        if self.inverse:
            # 该节点整体有 not
            if self.is_leaf_node():
                # 当该节点是一个叶子节点
