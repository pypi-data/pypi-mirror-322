...
#include "flags.h"
#define KEY_TYPE_TAG TYPE_TAG_I64
...
static PyTypeObject keyIterType_int64_int64 = {
    PyVarObject_HEAD_INIT(NULL, 0)
    .tp_name = "pypocketmap_keys[int64, int64]",
    .tp_doc = "",
...
static PyTypeObject valueIterType_int64_int64 = {
    PyVarObject_HEAD_INIT(NULL, 0)
    .tp_name = "pypocketmap_values[int64, int64]",
    .tp_doc = "",
    .tp_basicsize = sizeof(iterObj),
    .tp_itemsize = 0,
    .tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC,
...
static PyTypeObject itemIterType_int64_int64 = {
    PyVarObject_HEAD_INIT(NULL, 0)
    .tp_name = "pypocketmap_items[int64, int64]",
    .tp_doc = "",
    .tp_basicsize = sizeof(iterObj),
    .tp_itemsize = 0,
    .tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC,
...
static PyObject* _richcmp_(dictObj* self, PyObject* other, int op) {
    ...
    for (uint32_t i = 0; is_equal && i < h->num_buckets; i++) {
        if (_bucket_is_live(h->flags, i)) {
            k_t key = KEY_GET(h->keys, i);
            key_obj = PyLong_FromLongLong(key);
...
static PyObject* _repr_(dictObj* self) {
    h_t* h = self->ht;
    if (h->size == 0) {
        return PyUnicode_FromString("<pypocketmap[int64, int64]: {}>");
    }
    const int REPR_DICT_POS = 1 + 12 + 5 + 2 + 5 + 3;
    //               "<pypocketmap["   k ", "  v   "]: "
    const int REPR_MIN_PAIR = 2 + 1 + 1;
    ...
    if (_PyUnicodeWriter_WriteASCIIString(&writer, "<pypocketmap[int64, int64]: {", REPR_DICT_POS + 1) < 0) {
    ...
    for (uint32_t i = 0; i < h->num_buckets; i++) {
        if (_bucket_is_live(h->flags, i)) {
            ...
            size_t key_len = snprintf(key_repr, 47, "%lld", key);
            if (_PyUnicodeWriter_WriteASCIIString(&writer, key_repr, key_len) < 0) {
                _PyUnicodeWriter_Dealloc(&writer);
                return NULL;
            }
...
static PyObject* keys(dictObj* self) {
    return iter_new(self, &keyIterType_int64_int64);
}
...
static PyObject* values(dictObj* self) {
    return iter_new(self, &valueIterType_int64_int64);
}
...
static PyObject* items(dictObj* self) {
    return iter_new(self, &itemIterType_int64_int64);
}
...
static PyMethodDef methods_int64_int64[] = {
...
static PySequenceMethods sequence_int64_int64 = {
...
static PyMappingMethods mapping_int64_int64 = {
    (lenfunc) _len_, /*mp_length*/
    (binaryfunc)_getitem_, /*mp_subscript*/
    (objobjargproc)_setitem_, /*mp_ass_subscript*/
};

static PyTypeObject dictType_int64_int64 = {
    PyVarObject_HEAD_INIT(NULL, 0)
    .tp_name = "pypocketmap[int64, int64]",
    .tp_doc = "pypocketmap[int64, int64]",
    .tp_as_sequence = &sequence_int64_int64,
    .tp_as_mapping = &mapping_int64_int64,
    .tp_methods = methods_int64_int64,
...
static PyObject* update(dictObj* self, PyObject* args) {
    ...
    if (!is_pydict) {
        ...
        if (PyObject_IsInstance(other, (PyObject *) &dictType_int64_int64) != 1) {
            PyErr_SetString(PyExc_TypeError, "Argument needs to be either a pypocketmap[int64, int64] or compatible Python dictionary");
            return NULL;
        }
    }
...
static struct PyModuleDef moduleDef_int64_int64 = {
    PyModuleDef_HEAD_INIT,
    "int64_int64", // name of module
    "pypocketmap[int64, int64]", // Documentation of the module
    -1,   // size of per-interpreter state of the module, or -1 if the module keeps state in global variables
};

PyMODINIT_FUNC PyInit_int64_int64(void) {
    PyObject* obj;

    if (PyType_Ready(&dictType_int64_int64) < 0)
        return NULL;

    if (PyType_Ready(&keyIterType_int64_int64) < 0)
        return NULL;

    if (PyType_Ready(&valueIterType_int64_int64) < 0)
        return NULL;

    if (PyType_Ready(&itemIterType_int64_int64) < 0)
        return NULL;

    obj = PyModule_Create(&moduleDef_int64_int64);
    if (obj == NULL)
        return NULL;

    Py_INCREF(&dictType_int64_int64);
    if (PyModule_AddObject(obj, "create", (PyObject *) &dictType_int64_int64) < 0) {
        Py_DECREF(&dictType_int64_int64);
        Py_DECREF(obj);
        return NULL;
    }
