...
class Retriever:
    ...
    def load_resources(self, resource_path, chunk_size=512, chunk_overlap=50, chunkers=10, embedders=10, ef_construction=200, M=48):
        ...
        embeddings, metadata = asyncio.run(self._generate_embeddings_async(chunks, SEMAPHORE_CNT=embedders))


        if embeddings:
            index = self._create_faiss_index(np.array(embeddings, dtype=np.float32), ef_construction=ef_construction, M=M)
        else:
    ...
    def _create_faiss_index(self, embeddings, ef_construction=200, M=48):
