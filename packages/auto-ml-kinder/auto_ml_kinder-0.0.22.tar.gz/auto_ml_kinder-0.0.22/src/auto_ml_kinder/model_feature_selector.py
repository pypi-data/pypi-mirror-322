...
def RandomForestFeaturesSelection(X:pd.DataFrame,Y:pd.Series):
    ...
    model = RandomSearch_Selector(*mlh.get_parameters_random_forest_fit_reg(mp.ModelPower.HIGH),X_train=X,y_train=Y)
    model.fit(X, Y)
...
def DecisionTreeClassifierFeaturesSelection(X:pd.DataFrame,Y:pd.Series):
    ...
    model = RandomSearch_Selector(*mlh.get_parameters_decision_tree_fit_reg(mp.ModelPower.HIGH),X_train=X,y_train=Y)
    model.fit(X, Y)
    feature_importance = model.feature_importances_
    feature_importance_dataframe = pd.DataFrame(feature_importance)
    beautified_df = feature_importance_end_model_builder(feature_importance_dataframe,X.columns)
...
def LassoFeaturesSelection(X:pd.DataFrame,Y:pd.Series):
    ...
    model = RandomSearch_Selector(*mlh.get_parameters_lasso_fit(mp.ModelPower.HIGH),X_train=X,y_train=Y)
    model.fit(X, Y)
