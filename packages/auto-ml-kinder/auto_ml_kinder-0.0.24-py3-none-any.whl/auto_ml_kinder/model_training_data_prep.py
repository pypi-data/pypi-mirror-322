...
from auto_ml_kinder.classes import ScalerType as st
...
def get_scaler(scaler_type):
    if not isinstance(scaler_type, st.ScalerType):
        raise ValueError("Invalid scaler type. Please provide a valid ScalerType enum value.")
    scaler = None
    if scaler_type == st.STANDARD_SCALER:
        scaler = StandardScaler()
    elif scaler_type == st.MINMAX_SCALER:
        scaler = MinMaxScaler()
    elif scaler_type == st.MAXABS_SCALER:
        scaler = MaxAbsScaler()
    elif scaler_type == st.ROBUST_SCALER:
        scaler = RobustScaler()
    elif scaler_type == st.QUANTILE_TRANSFORMER:
