...
@click.command()
@click.option("--id", help="The ID of the asset to transcribe.")
@click.option("--url", help="The Nomad URL of the Asset (file or folder) to transcribe (bucket::object-key).")
@click.option("--object-key", help="Object-key of the Asset (file or folder) to transcribe. This option assumes the default bucket that was previously set with the `set-bucket` command.")
@click.option("--transcript-id", required=True, help="The ID of the transcript.")
@click.option("--transcripts", multiple=True, help="The transcript of the transcribe in JSON list format.")
@click.pass_context
