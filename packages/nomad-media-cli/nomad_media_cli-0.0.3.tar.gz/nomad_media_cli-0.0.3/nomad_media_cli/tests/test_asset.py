...
class TestAssetBase(unittest.TestCase):    
    ...
    def setUpClass(cls):
        ...
        test_dir_files = [item for item in test_dir_contents if item["assetTypeDisplay"] == "File"]

        if config["apiType"] == "admin" and len(test_dir_files) == 0:
           ...
           list_dir_result = cls.runner.invoke(cli, [
               "list-assets", 
               "--id", cls.test_dir_id
           ])
           
           list_dir = json.loads(list_dir_result.output)
           asset = (item for item in list_dir["items"] if item["name"] == "Readme.md").__next__()
...
class TestBuildMedia(TestAssetBase):
    ...
    def test_build_media_by_id(self):
        ...
        result = self.runner.invoke(cli, [
            "build-media", 
            "--sources", f'[{{ "sourceAssetId": "{self.asset_id}" }}]',
...
class TestClipAsset(TestAssetBase):
    ...
    def setUpClass(cls):
        ...
        parent_assets = cls.runner.invoke(cli, [
            "list-assets", 
            "--id", cls.test_dir_id
        ])
        
        test_asset = next((item for item in json.loads(parent_assets.output)["items"] if item["name"] == "vid1.mp4"), None)
        cls.vid_asset_id = test_asset["id"]
    ...
    def tearDownClass(cls):
        ...
        print(f"Deleted asset with id: {cls.vid_asset_id}")

    def test_clip_asset_by_id(self):
        """Test asset is clipped successfully"""
        result = self.runner.invoke(cli, [
            "clip-asset", 
            "--id", self.vid_asset_id,
...
class TestCreateAnnotation(TestAssetBase):
    ...
    def setUpClass(cls):
        ...
        parent_asset_list = get_total_asset_list(cls, cls.test_dir_id)
            
        video_assets = [asset for asset in parent_asset_list if asset["mediaTypeDisplay"] == "Video"]
        if not video_assets:
            raise unittest.SkipTest("No video assets found in test directory")
        
        cls.vid_asset_id = video_assets[0]["id"]
    ...
    def test_create_annotation_invalid(self):
        """Test invalid ID returns an error"""
        result = self.runner.invoke(cli, [
            "create-annotation", 
            "--id", "invalid-id",
...
class TestCreateAssetAdBreak(TestAssetBase):
    ...
    def test_create_asset_ad_break(self):
        ...
        ad_breaks = [ad_break for ad_break in ad_breaks if ad_break["id"] != self.vid_asset_id]
    ...
    def test_create_asset_ad_break_invalid(self):
        """Test invalid ID returns an error"""
        result = self.runner.invoke(cli, [
            "create-asset-ad-break", 
            "--id", "invalid-id",
...
class TestCreateFolderAsset(TestAssetBase):
    ...
    def test_create_folder_asset_invalid(self):
...
class TestCreatePlaceholderAsset(TestAssetBase):
    ...
    def test_create_placeholder_asset_invalid(self):
        """Test invalid parent ID returns an error"""
        result = self.runner.invoke(cli, [
            "create-placeholder-asset", 
            "--parent-id", "invalid-id",
...
class TestDeleteAnnotation(TestAssetBase):
    ...
    def test_delete_annotation_invalid(self):
        """Test invalid ID returns an error"""
        result = self.runner.invoke(cli, [
            "delete-annotation", 
            "--id", "invalid-id",
...
class TestDeleteAsset(TestAssetBase):
    ...
    def test_delete_asset(self):
        ...
        asset = json.loads(upload_result.output)
        asset_id = asset["id"]    
...
class TestDuplicateAsset(TestAssetBase):
    ...
    def test_duplicate_asset_invalid(self):
        """Test invalid ID returns an error"""
        result = self.runner.invoke(cli, [
            "duplicate-asset", 
            "--id", "invalid-id"
...
class TestGetAssetScreenshotDetails(TestAssetBase):
    ...
    def setUpClass(cls):
        ...
        video_assets = [asset for asset in parent_asset_list if asset["assetTypeDisplay"] == "Video"]
        if not video_assets:
            raise unittest.SkipTest("No video assets found in test directory")
...
class TestImportAnnotations(TestAssetBase):
    ...
    def test_import_annotations(self):
        ...
        result = self.runner.invoke(cli, [
            "import-annotations", 
            "--annotations", '[{"startTimeCode": "00:00:00;01"},{"startTimeCode": "00:00:00;01","endTimeCode": "00:00:00;02"}]',
            "--id", self.vid_asset_id
        ])
    ...
    def test_import_annotations_by_url(self):
        ...
        result = self.runner.invoke(cli, [
            "import-annotations", 
            "--annotations", '[{"startTimeCode": "00:00:00;02"},{"startTimeCode": "00:00:00;02","endTimeCode": "00:00:00;03"}]',
            "--url", url
        ])
    ...
    def test_import_annotations_by_object_key(self):
        ...
        result = self.runner.invoke(cli, [
            "import-annotations", 
            "--annotations", '[{"startTimeCode": "00:00:00;03"},{"startTimeCode": "00:00:00;03","endTimeCode": "00:00:00;04"}]',
            "--object-key", object_key
        ])
...
class TestIndexAsset(TestAssetBase):
    ...
    def test_index_asset_invalid(self):
        """Test invalid ID returns an error"""
        result = self.runner.invoke(cli, [
            "index-asset", 
            "--id", "invalid-id"
...
class TestLocalRestoreAsset(TestAssetBase):
    ...
    def test_local_restore_asset(self):
...
class TestRecordsAssetTrackingBeacon(TestAssetBase):
    ...
    def setUpClass(cls):
        ...
        cls.live_channel_id = "96540d0a-e8a8-48a3-889a-de6cbdcaa272"
...
#       test_asset = next((item for item in json.loads(parent_assets.output)["items"] if item["name"] == "vid1.mp4"), None)
#       cls.vid_asset_id = test_asset["id"]
        cls.vid_asset_id = "64679a89-7035-4eb9-bdcc-f33ec56b2262"
        ...
        cls.ad_break_id = ad_break_info["id"]
