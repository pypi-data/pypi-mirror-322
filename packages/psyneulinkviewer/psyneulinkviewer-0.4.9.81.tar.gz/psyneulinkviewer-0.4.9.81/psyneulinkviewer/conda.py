...
def check_conda_installation():
    ...
    # Try to detect the Conda installation directory
    conda_path = None
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            shell=False,  # Avoid shell=True for security reasons
            check=True
        )
        conda_path = result.stdout.strip()
    except subprocess.CalledProcessError:
        logging.error("Conda not found in system PATH.")
        conda_path = None

    if conda_path:
        conda_activate_cmd = f"source {os.path.dirname(conda_path)}/activate && conda --version"
    else:
        # Default path for Miniconda/Anaconda on macOS
        possible_paths = [
        ...
        for path in possible_paths:
            ...
            if os.path.exists(expanded_path):
                conda_activate_cmd = f"source {expanded_path} && conda --version"
                break
        else:
            logging.error("Conda installation not found. Please install Conda.")
            sys.exit(1)

    try:
        # Execute the Conda activation and check version
        result = subprocess.run(
            conda_activate_cmd,
            capture_output=True,
            text=True,
    ...
    if conda_version is None:
        ...
        required_version = "4.10"  # Adjust as needed
        if Version(conda_version) >= Version(required_version):
            logging.info("Conda version exists and is valid: %s", conda_version)
        else:
