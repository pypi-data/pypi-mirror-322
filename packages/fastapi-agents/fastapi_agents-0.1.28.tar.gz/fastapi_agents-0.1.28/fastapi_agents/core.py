...
from fastapi_agents.models import BaseAgent, Framework, OpenAIRequestPayload, OpenAIResponsePayload, RequestPayload, ResponsePayload, APIMode, Message, Choice
...
class FastAPIAgents(APIRouter):
    ...
    def register(
        ...
        name: str,
        agent: BaseAgent | Any,
        framework: Optional[Framework] | Literal["auto", "custom"] = "auto",
        ...
        Args:
            ...
            agent (BaseAgent | Any): The agent instance to register.
            framework (Framework | Literal["auto", "custom"], optional): The framework to use for the agent. Defaults to "auto".
        ...
        FRAMEWORKS = {
            Framework.PYDANTIC_AI: ("pydantic_ai", "Agent", "fastapi_agents.pydantic_ai", "PydanticAIAgent"),
            Framework.SMOLAGENTS: ("smolagents.agents", "MultiStepAgent", "fastapi_agents.smolagents", "SmolagentsAgent"),
            Framework.LLAMA_INDEX: ("llama_index.core.agent.runner.base", "AgentRunner", "fastapi_agents.llama_index", "LlamaIndexAgent"),
            Framework.CREWAI: ("crewai", "Crew", "fastapi_agents.crewai", "CrewAIAgent")
        }

                
        def load_agent(agent, framework=None):
            if isinstance(agent, BaseAgent):
                return framework, agent
            
            if framework == "custom":
                if not isinstance(agent, BaseAgent):
                    raise ValueError("Custom agent must inherit from fastapi_agents.models.BaseAgent")
                return framework, agent
            
            for fw, (mod, cls, wrapper_mod, wrapper_cls) in FRAMEWORKS.items():
                if framework and fw != framework:
                    continue
                try:
                    module = __import__(mod, fromlist=[cls])
                    wrapper_module = __import__(wrapper_mod, fromlist=[wrapper_cls])
                    agent_cls, wrapper_cls = getattr(module, cls), getattr(wrapper_module, wrapper_cls)

                    if isinstance(agent, agent_cls):
                        return fw, wrapper_cls(agent)
                    if isinstance(agent, wrapper_cls):
                        return fw, agent
                except ImportError:
                    continue

            raise ImportError("No supported frameworks found. Install pydantic_ai, smolagents, llama_index, or crewai.")

        # Determine framework
        if framework == "auto":
            framework, agent = load_agent(agent)
        elif framework in FRAMEWORKS or framework == "custom":
            framework, agent = load_agent(agent, framework)
        else:
