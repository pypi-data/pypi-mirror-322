...
from fastapi_agents.models import RequestPayload, BaseAgent, APIMode, Framework
...
def test_register_custom_framework():
    agents_router = FastAPIAgents()
    agent = MockAgent()
    agents_router.register("custom_agent", agent, framework="custom")
    assert "custom_agent" in agents_router.agents

def test_register_invalid_framework():
    agents_router = FastAPIAgents()
    agent = MockAgent()
    with pytest.raises(ValueError, match="Invalid framework: invalid_framework"):
        agents_router.register("invalid", agent, framework="invalid_framework")

@pytest.mark.parametrize("framework", [Framework.PYDANTIC_AI, Framework.SMOLAGENTS, Framework.LLAMA_INDEX, Framework.CREWAI, "auto"])
def test_register_different_frameworks(app, mock_agent, framework):
    agents_router = FastAPIAgents()
    agents_router.register("test_agent", mock_agent, framework=framework)
    assert "test_agent" in agents_router.agents
