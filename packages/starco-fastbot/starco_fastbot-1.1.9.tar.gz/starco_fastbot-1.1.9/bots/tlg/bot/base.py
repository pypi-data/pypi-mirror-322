...
class Base:
    def __init__(self, super_self, *args, **kwargs) -> None:
        ...
        self.id = 0
        self.msg_id = 0
        self.back_key = self.super_self.default_text.get('back','back')
        self.back_menu_key = self.super_self.default_text.get('back_menu','back_menu')
        self.menu_keys = []
        self.lang_code=-1
        self.text_list=[]
        ...
        self.go_menu_filter = match_btn(self.back_menu_key)
        self.go_back_filter = match_btn(self.back_key)
    ...
    def menu(self,*args):
        menu_text=self.super_self.default_text.get('menu_text','menu_text')
    ...
    def node(self, name: str, entry: list[Node], states: list[Node] = None, fallbacks: list[Node] = None, db_config: dict[dict] = {}, **kargs):
        ...
        if fallbacks:
            ...
            conversation.fallbacks = [MessageHandler(
                match_btn(self.back_menu_key), self.add_method(self.menu))]
