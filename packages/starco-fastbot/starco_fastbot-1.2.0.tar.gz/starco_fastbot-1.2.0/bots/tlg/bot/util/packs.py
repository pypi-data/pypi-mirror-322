...
def Start(self: Conversation):
    ...
    def act(self: Conversation, *args):
        try:
            start_pm=self.super_self.default_text.get('start_pm','start_pm')
    ...
    def act(self: Conversation, *a):
        try:
            ...
            if start_bot == 'start_bot':
                msg =self.super_self.default_text.get('join_alert_pm','join_alert_pm')
                self.alert(msg)
                return -1
        except:
            pass
...
def Referral(self: Conversation):
    ...
    e1.filters = match_btn('referral')

    def referral_text(self: Base, *args):
        referral_text =self.super_self.default_text.get('referral_text','referral_text')
        invited =self.super_self.default_text.get('invited','invited')
        
        msg = referral_text+'\n\n'
        msg += f'https://t.me/{self.super_self.bot_username}?start=ref{self.id}\n\n'
        msg += f"{invited} : {len(self.db.do('users', condition=f'presenter={self.id}'))}"
...
def ToggleSetting(self: Conversation, **kargs):
    ...
    def act(self: Conversation, *args):
        ...
        res =self.super_self.default_text.get('res',res)
        self.alert(res)
        msg, btn = setting_msg_btn(self)
...
def Status(self: Conversation, **kargs):
    ...
    def setting_msg_btn(self):
        ...
        for item in setting:
            sbtn = [
                telegram.InlineKeyboardButton(item[key], callback_data=f'{item[key]}:{subset}slbl'),
                telegram.InlineKeyboardButton(item[value], callback_data=f'{item[key]}:{subset}set_stat'),
            ]
            btn += [sbtn]
        btn = telegram.InlineKeyboardMarkup(btn)
        return msg, btn
    e = Node()
    e.filters = match_btn(subset)
...
def Setting(self: Conversation, **kargs):
    ...
    def setting_msg_btn(self):
        ...
        for i in setting:
            ...
            btn += [i['key']]
        if msg == '':
            msg =self.super_self.default_text.get('select_option','select_option')
        return msg, btn
    ...
    def act(self: Conversation, *args):
        ...
        if self.set_setting(k, self.get_text()):
            ...
            bad_input =self.super_self.default_text.get('bad_input','bad_input')
            enter_new_value =self.super_self.default_text.get('enter_new_value','enter_new_value')
...
def SettingPro(self: Conversation, **kargs):
    ...
    def setting_msg_btn(self):
        ...
        toggle_msg =self.super_self.default_text.get('click_option','click_option')
        for i in setting:
            if i.get('hide') != 1:
                if i.get('toggle') == 1:
                    sbtn = [
                        telegram.InlineKeyboardButton(
                            i['key'], callback_data=f"{i['key']}:{key}slbl"),
                        telegram.InlineKeyboardButton(
                            i['value'], callback_data=f"{i['key']}:{key}set_stat"),
                    ]
                    toggle_btn += [sbtn]
                else:
                    msg += f"{i['key']} : \n{i['value']} {i['unit']}\n---\n"
    ...
    def act(self: Conversation, *args):
        ...
        if self.set_setting(k, self.get_text()):
            ...
            msg =self.super_self.default_text.get('bad_input','bad_input')
            enter_new_value =self.super_self.default_text.get('enter_new_value','enter_new_value')
            self.send(msg+'\n' +enter_new_value)
            return self.stat_key(key, 1)
...
def SendToAllBasic(self: Conversation):
    ...
    e = Node()
    e.filters = match_btn('send_to_all')
