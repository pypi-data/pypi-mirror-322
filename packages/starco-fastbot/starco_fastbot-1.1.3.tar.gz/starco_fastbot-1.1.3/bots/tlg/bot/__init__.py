...
class TlgBot(Bot):
    def __init__(self, root_dir: str, superAdmins: list[int], token: str, host_id: str = None, base_url: str = None, base_file_url: str = None, request: Request = None, private_key: bytes = None, private_key_password: bytes = None, defaults: Defaults = None, proxy: str = None, **kwargs):
        '''
        ...
        '''
        os.environ['root_path'] = root_dir
        name=root_dir.split('/')[-1].split('\\')[-1]
        request_kwargs=None
        self.proxy=proxy
        if proxy:
