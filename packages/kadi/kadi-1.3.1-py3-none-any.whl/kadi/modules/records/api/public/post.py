...
@status(
    ...
    " following :http:put:`endpoint"
    " </api/records/(int:record_id)/files/(uuid:file_id)>`, which is also returned as"
    " the ``_actions.edit_data`` property of the returned file.",
)
...
def new_upload(id, schema):
    ...
    returned upload. The required size for uploading chunks (except for the final chunk
    for chunked uploads) is returned as the ``_meta.chunk_size`` property of the
    returned upload. For direct uploads, the chunk size is always equal to the total
    upload size.
    """
    record = Record.query.get_active_or_404(id)
    data = schema.load_or_400()
