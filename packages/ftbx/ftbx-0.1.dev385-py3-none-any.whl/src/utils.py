...
def update_toolbox_resources():
    ...
    try:
        logger.info(f"Ensuring bitbucket.org fingerprint is added to SSH known hosts...")
        known_hosts_path = os.path.expanduser("~/.ssh/known_hosts")
        bitbucket_host_entry = subprocess.run(
        ...
        with open(known_hosts_path, "a") as known_hosts_file:
            if bitbucket_host_entry not in open(known_hosts_path).read():
                known_hosts_file.write(bitbucket_host_entry)
                logger.info(f"Added bitbucket.org to SSH known hosts.")
            else:
                logger.info(f"bitbucket.org is already in SSH known hosts.")
