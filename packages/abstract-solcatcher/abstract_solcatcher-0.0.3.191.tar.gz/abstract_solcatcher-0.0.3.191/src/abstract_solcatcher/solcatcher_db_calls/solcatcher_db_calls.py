...
from abstract_solana.rpc_utils import * 
...
def get_result(response):
    ...
    return result
async def get_all_txns(pair_id):
    transactions = await call_solcatcher_db('/api/get_transactions',pair_id)
    return transactions
async def process_logs(transaction):
    ...
    response = await call_solcatcher_ts('process-logs', signature=signature, slot=slot, program_id=program_id, logs=logs)
    return signature
async def get_pair(pair_id=None,mint=None,signature=None):
    try:
        raw_pair_data = await call_solcatcher_db('/api/get_pair',mint=mint,pair_id=pair_id,signature=signature)
        pair_data = clean_pair_values(raw_pair_data)
        return pair_data
    except Exception as e:
        input(e)
async def get_transactions(pair_id=None,txn_id=None,signature=None,log_id=None):
    try:
        raw_transactions = await call_solcatcher_db('/api/get_transactions',pair_id=pair_id,txn_id=txn_id,signature=signature,log_id=log_id)
        return raw_transactions
    except Exception as e:
        input(e)
async def decodeInstructionData(data):
    response = await call_solcatcher_ts('decode-instruction-data', string=data))
    decodedData = get_result(response)
    return decodedData
async def upsertPairData(pairData):
    response = await call_solcatcher_ts('upsert-pair-by-id', pairData=pairData)
    pair_id = get_result(response)
    return pair_id
async def upsert_txn(txData):
    response = await call_solcatcher_ts('upsert-txn', txData=txData)
