...
def decodeInstructionData(data):
    response = asyncio.run(call_solcatcher_ts('decode-instruction-data', string=data))
    decodedData = get_result(response)
    return decodedData
def upsertPairData(pairData):
    response = asyncio.run(call_solcatcher_ts('upsert-pair-by-id', pairData=pairData))
