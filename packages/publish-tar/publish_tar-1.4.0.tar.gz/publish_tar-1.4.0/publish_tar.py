...
class TARPublisher:
    def __init__(self, prod_name: str, disk: str = 'D', artifact: bool = False):
        self._artifact = artifact
        timestamp = self.generate_unique_timestamp()
        self._database = os.path.join(f'{disk}:{os.sep}', 'Flask', 'd', prod_name, timestamp)
        self._features = dict()
...
def main():
    ...
    for i in range(args_len):
        if args[i] == '-p' and i+1 < args_len:
            prod_name = args[i+1]
        elif args[i] == '-d' and i+1 < args_len:
            disk = args[i+1]
        elif args[i] == '-a':
            artifact = True

    if prod_name is None or prod_name not in valid_prods:
        logger.error(f'usage: python publish-tar -p <prodName> [-d <disk>],'
                     f'the valid product names:{" ".join(valid_prods)}')
        sys.exit(-1)
    TARPublisher(prod_name, disk, artifact).publish()
