...
class GoogleVertexAIConnector(LLMConnector.LLMConnector):
  ...
  def auth(self):
    ...
    if not creds or not creds.valid:
      if creds and creds.expired and creds.refresh_token:
        ...
        flow = InstalledAppFlow.from_client_secrets_file(self.config["google_app_credentials_path"], SCOPES)
  ...
  async def request(
    ...
    Args:
        ...
        model_params (list[ModelParam] | None, optional): The list of model parameters. Defaults to None.
        **kwargs: Additional keyword arguments.
    ...
    if self.project_id is None:
      self.console_stderr.print(f"[red]Google VertexAI: A Project ID must be configured for this connector.")

    if self.region is None:
      self.console_stderr.print(f"[red]Google VertexAI: A Region must be configured for this connector.")

    if self.get_model().publisher is None:
      self.console_stderr.print(f"[red]Google VertexAI: The model publisher must be configured for this connector.")

    if creds is None:
      self.console_stderr.print(f"[red]Google VertexAI: Credentials are missing.")

    if creds.token is None:
      self.console_stderr.print(f"[red]Google VertexAI: Credentials token is missing.")
    ...
    try:
      ...
      with httpx.Client() as client:
        ...
        resp = client.post(url, json=params, headers=headers, timeout=None)
      ...
      if self.debug:
        self.console_stderr.print(f"[red]ERROR: {e}")
      return None
