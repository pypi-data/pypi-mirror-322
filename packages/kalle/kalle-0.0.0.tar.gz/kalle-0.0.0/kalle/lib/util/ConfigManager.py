  ...
  try:
    ...
    console_stderr.print(
        Panel(
            ...
            title=f"[red bold]CONFIGURATION PROBLEM",
            style="red",
        )
    )
    if debug:
...
class ConfigManager:
  ...
  def __init__(
    ...
    if not os.path.exists(config_file_path):
      console_stderr.print(f"[red]Configuration file is missing {config_file_path}. Please set it up first.")
      sys.exit(111)
    ...
    with open(config_file_path) as file:
      ...
      from kalle.domain.ToolConfig import ToolsList, ToolConfig
      ...
      models_map = {}
      if "models_map" not in self.yaml_config or self.yaml_config["models_map"] is None:
        console_stderr.print(
            f"[red][bold]Models map is missing from the configuration file [italic]{config_file_path}[/italic]"
        )
        sys.exit(113)
  ...
  def conversation_file(self):
    if self.conversation_key is not None:
      return os.path.normpath(f"{self.conversation_dir}/conversation_{self.conversation_key}.json")
    return None
