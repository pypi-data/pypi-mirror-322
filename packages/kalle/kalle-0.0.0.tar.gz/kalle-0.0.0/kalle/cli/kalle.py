...
class Kalle:
  ...
  def __init__(self, debug: bool = False):
    ...
    self.piped_content = None
    if not sys.stdin.isatty():
      self.piped_content = sys.stdin.read()
    ...
    os.environ["HF_HOME"] = os.path.join(user_cache_dir(self.appname, self.appauthor), "huggingface")
  ...
  def check_dirs(self):
    ...
    self.check_and_create_dir(os.path.join(cache_dir, "huggingface"))  # tiktoken cache dir
  ...
  def show_history(self):
    if self.config.conversation_key is None:
      self.console_stderr.print("[red]NO CONVERSATION TO SHOW HISTORY FOR")
      return

    self.console.print(f"[bold magenta]CONVERSATION HISTORY FOR: {self.config.conversation_key}")
    conversation = self.conversation_tools.load_conversation()
    ac = ""
    al = "center"
  ...
  def load_json_schema(self):
    if self.jsonschema is not None:
      jsonschema_path = os.path.expanduser(self.jsonschema)
      if not jsonschema_path.startswith("/"):
        jsonschema_path = os.path.join(self.base_file_dir, jsonschema_path)
      try:
        with open(jsonschema_path, "r") as f:
          return json.loads(f.read())
      except FileNotFoundError:
        self.console_stderr.print(f"[red]Could not find JSON schema {self.jsonschema}")
        sys.exit(31)
      except IsADirectoryError:
        self.console_stderr.print(f"[red]Specified JSON schema path '{self.jsonschema}' is a directory")
        sys.exit(32)
      except json.decoder.JSONDecodeError as e:
        self.console_stderr.print(f"[red]Invalid JSON schema '{self.jsonschema}': {e}")
        sys.exit(33)
      except Exception as e:
        self.console_stderr.print(f"[red]Unspecified error loading JSON schema '{self.jsonschema}': {e}")
        sys.exit(34)

  def handle_inline_commands(self, content: str = ""):
    if content.startswith("reset conversation"):
      if self.use_memory and self.config.conversation_key is not None and self.conversation.conversation != []:
    ...
    if content.startswith("set conversation"):
      conversation_name = content.split("set conversation ")[1].strip()
    ...
    if content.startswith("unset conversation"):
      conversation_file_path = os.path.join(self.base_file_dir, ".kalle_conversation")
  ...
  def load_pattern(self, pattern_key: str):
    if self.args_pattern_key is not None:
      if self.args_pattern_key in self.config.patterns:
        return self.config.patterns[self.args_pattern_key]
      else:
        self.console_stderr.print(f"[red]Pattern '{self.args_pattern_key}' is not found.")
        exit(17)
  ...
  def show_debug_info(self):
    if self.config.debug:
      self.console_stderr.print(
          Panel(
              Group(
                  f"[magenta][bold]USE MEMORY:[/bold] {self.config.use_memory}",
      ...
      pattern_tree = Tree(f"[magenta][bold]PATTERN:[/bold] {self.args_pattern_key}")
      ...
      self.console_stderr.print(
          Panel(
              Group(
                  f"[magenta][bold]ARGS REGEX:[/bold] {self.regex}",
                  f"[magenta][bold]ARGS JSON SCHEMA PATH:[/bold] {self.jsonschema}",
...
""",
      ...
      if self.tool_handler is not None:
        for t, v in self.tool_handler.get_tools().items():
  ...
  async def run(
      ...
      do_list_conversations: bool = False,
      do_show_history: bool = False,
      regex: str | None = None,
      interactive: bool = False,
      jsonschema: str | None = None,
      format_output: bool = True,
      conversation_key: str | None = None,
      args_system_prompt: str | None = None,
      args_profile_key: str | None = None,
      args_pattern_key: str | None = None,
    ...
    self.do_list_conversations = do_list_conversations
    self.do_show_history = do_show_history
    self.regex = regex
    self.interactive = interactive
    self.jsonschema = jsonschema
    self.format_output = format_output
    self.conversation_key = conversation_key
    self.args_system_prompt = args_system_prompt
    self.args_profile_key = args_profile_key
    self.args_pattern_key = args_pattern_key
    ...
    self.pattern = self.load_pattern(self.args_pattern_key) if self.args_pattern_key else None

    # #############################################################################################################
    # Do things that can cause us to exit early

    # if we're just listing conversations, do that and exit early
    if self.do_list_conversations:
      self.list_conversations()
      sys.exit(0)

    # show history and exit early
    if self.do_show_history:
      self.show_history()
      sys.exit(0)

    # exit early if we've specified multiple constrainer types
    if self.regex is not None and self.jsonschema is not None:
      self.console_stderr.print("[red]Specify only one of JSON schema or regex")
      sys.exit(2)

    # exit early if we haven't specified a request
    if self.param_content == "" and self.piped_content is None and self.pattern is None:
      if self.interactive:
        return

      self.console_stderr.print("[red]Need a request")
      sys.exit(1)

    # #############################################################################################################
    # set up things that depend on the config

    self.profile_keys = self.config.profiles.keys()

    # set the active profile key
    self.profile_key = None
    if self.args_profile_key is not None:
      if self.args_profile_key in self.profile_keys:
        self.profile_key = self.args_profile_key
      else:
        self.console_stderr.print(f"[red]Profile {self.args_profile_key} is not found")
        sys.exit(8)

    # for arg in self.args:
    #    if self.args[arg] and arg in self.profile_keys:
    #        self.profile_key = arg
    #        break
    ...
    if self.use_tools:
      ...
      self.tool_handler = ToolHandler(
          self.config, base_file_dir=self.base_file_dir, tool_list=self.tool_list, console_stderr=self.console_stderr
      )

    # #############################################################################################################
    # load any specified json-schema
    self.jsonschema = self.load_json_schema()
    ...
    self.conversation = self.conversation_tools.load_conversation()

    # reset the conversation if one is active
    self.handle_inline_commands(self.param_content)
    ...
    self.profile_manager = ProfileManager(
        self.config, self.profile_key, model_string=self.model_string, console_stderr=self.console_stderr
    )
    ...
    (compiled_content, uris) = (
        self.uri_handler.parse_content(content)
        if self.uri_handler is not None and self.follow_uris
        else (content, None)
    )
    ...
    constrainer = self.pattern.constrainer if self.pattern is not None else None
    ...
    if self.jsonschema is not None:
      ...
      constrainer = Constrainer(
          ...
          value=json.dumps(self.jsonschema),
      )
    ...
    if self.system_prompt_tokens + self.prompt_tokens > self.profile_manager.model.context_size - 100:
      ...
      if self.use_tools:
        ...
        with self.console_stderr.status(
            f"[bold yellow]Making LLM request ({self.profile_manager.profile.connector.name}/{self.profile_manager.model.key})..."
        ):
    ...
    if self.config.use_memory:
      self.conversation_tools.persist_conversation(self.conversation)
...
def process_args(**kwargs):
  ...
  return {
      ...
      "do_list_conversations": kwargs.get("list_conversations", False),
      "do_show_history": kwargs.get("history", False),
      "regex": kwargs.get("regex", None),
      "interactive": kwargs.get("interactive", None),
      "jsonschema": kwargs.get("jsonschema", None),
      ...
      "base_file_dir": kwargs.get("dir", None),
      "tool_list": kwargs.get("tool", None),
      "use_tools": True if kwargs.get("tool", None) is not None else False,
      "model_string": kwargs.get("model", None),
      "temperature": kwargs.get("temp", None),
