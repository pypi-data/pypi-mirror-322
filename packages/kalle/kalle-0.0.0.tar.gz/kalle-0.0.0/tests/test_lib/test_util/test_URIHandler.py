...
class TestURIHandler(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.uri_handler = URIHandler(self.config_manager.config, self.fixtures_dir)
    self.maxDiff = 1000
    unittest.util._MAX_LENGTH = 2000

  def test_init(self):
    self.assertIsInstance(self.uri_handler.config, Config)
  ...
  @patch("sys.exit")
  def test_replace_placeholders_with_error(self, mock_exit):
    updated_dict = {
        "{URI_0}": Uri(
            uri="http://fe2.net",
            placeholder=None,
  ...
  def test_fetch_file_html(self):
    ...
    self.assertIsNotNone(result)
    self.assertIn("<title>kalle test</title>", result.raw_content)
    self.assertIn("This is a test html file for the kalle unit tests.", result.raw_content)
  ...
  def test_fetch_file_text(self):
    ...
    result = self.uri_handler.fetch_file(uri)
    self.assertIsNotNone(result)
    self.assertIn("This is a test text file for the kalle unit tests.", result.raw_content)
  ...
  def test_fetch_file_pdf(self):
    ...
    result = self.uri_handler.fetch_file(uri)
    self.assertIsNotNone(result)
    self.assertIn("This is a test PDF for the kalle unit tests.", result.raw_content)
