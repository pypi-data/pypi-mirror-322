...
class LocalOrDistributedWorkerBase(WorkerBase):
    ...
    def _get_worker_input_from_broadcast(
        ...
        # logger.info(f"broadcast_tensor_dict: {broadcast_data}")
    ...
    def execute_model(
        ...
        # print("[After {:.2f}s] self.prepare_input".format(time.perf_counter() - start_time))
        if inputs is None:
            return None
...
class WorkerWrapperBase:
    ...
    def update_environment_variables(envs: Dict[str, str]) -> None:
        key = 'CUDA_VISIBLE_DEVICES' # 'ASCEND_RT_VISIBLE_DEVICES'
