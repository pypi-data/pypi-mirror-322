...
from vllm.worker.npu_cache_engine import CacheEngine
...
class NPUWorker(LocalOrDistributedWorkerBase):
    ...
    def determine_num_available_blocks(self) -> Tuple[int, int]:
        ...
        # of the model.
        block_num_pre = self.model_runner.profile_run()

        self.max_memory = NpuHbmInfo.get_hbm_capacity(self.local_rank, self.parallel_config.world_size, self.model_runner.model.soc_info.need_nz)
        peak_memory = int(
            self.max_memory * NpuHbmInfo.get_hbm_usage(self.local_rank, self.parallel_config.world_size, self.model_runner.model.soc_info.need_nz))

        cache_block_size = self.get_cache_block_size_bytes() #
        num_npu_blocks = int(
            (self.max_memory * self.cache_config.gpu_memory_utilization - peak_memory) // #
            cache_block_size)
        num_npu_blocks += block_num_pre
    ...
    def _init_cache_engine(self):
        ...
        self.cache_engine = [
            CacheEngine(self.cache_config, self.model_config,
                        self.parallel_config, self.device_config, self.model_runner.model.soc_info)
        ...
        self.model_runner.set_total_slots(self.cache_config.num_gpu_blocks)
