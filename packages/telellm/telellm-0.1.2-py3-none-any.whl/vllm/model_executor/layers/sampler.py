...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        if do_top_p_top_k and flashinfer_top_k_top_p_sampling is None:
            logits, logits_idx, top_k_p_mask = _apply_top_k_top_p(logits, sampling_tensors.top_ps,
...
def _apply_top_k_top_p(
    ...
    # at least one
    top_k_p_mask = top_k_mask | top_p_mask
    top_k_p_mask[:, -1] = False
    logits_sort.masked_fill_(top_k_p_mask, -float("inf"))
    ...
    return logits_sort, logits_idx, top_k_p_mask
...
def _random_sample(
    ...
    logits_idx: Optional[torch.Tensor] = None
) -> SampleResultType:
    ...
    assert logits_idx is not None
    random_samples = logits_idx.gather(1, random_samples)
...
def _multinomial(
    ...
    top_k_p_mask: Optional[torch.Tensor] = None
) -> torch.Tensor:
    if num_samples > 1:
    ...
    if seq_groups is None:
        ...
        for seq_group in seq_groups:
            ...
            sample_idx += stride
    return probs.div_(q).masked_fill_(top_k_p_mask, -float("inf")).argmax(dim=1).view(-1, num_samples)
...
def _sample_with_torch(
    ...
    if not sampling_metadata.skip_sampler_cpu_output:
        for sampling_type in SamplingType:
            ...
            if sampling_type == SamplingType.GREEDY:
                assert logits_idx is None
                sample_results = _greedy_sample(seq_groups, greedy_samples)
            elif sampling_type in (SamplingType.RANDOM,
                                   SamplingType.RANDOM_SEED):
                assert logits_idx is not None
                sample_results = _random_sample(
                    seq_groups, multinomial_samples[sampling_type], logits_idx)
            elif sampling_type == SamplingType.BEAM:
                assert logits_idx is None
