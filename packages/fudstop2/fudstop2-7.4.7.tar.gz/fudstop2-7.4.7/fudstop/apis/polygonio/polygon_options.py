...
class PolygonOptions:
    ...
    async def batch_upsert_dataframe(
        ...
        No use of `is_datetime64tz_dtype`, which is deprecated.
        Instead, we:
          1) Check if a column is datetime with `is_datetime64_any_dtype`.
          2) Use `.dt.tz` to see if it's naive or aware.
        """
        try:
            ...
            # 2) Convert or localize datetime columns
            for col in df.columns:
                if is_datetime64_any_dtype(df[col]):
                    # If the entire column is recognized by Pandas as datetime64
                    # Check if it has a tz
                    try:
                        # If df[col].dt.tz is None => naive => localize
                        # Otherwise => convert
                        ...
                        # If we can't access df[col].dt.tz, force a safe convert
                        df[col] = pd.to_datetime(df[col], utc=True).dt.tz_convert('America/New_York')

            # 3) If 'insertion_timestamp' doesn't exist, create it
            if 'insertion_timestamp' not in df.columns:
                ...
                # If it exists, ensure it's recognized and localized to America/New_York
                if is_datetime64_any_dtype(df['insertion_timestamp']):
                    try:
                        if df['insertion_timestamp'].dt.tz is None:
                        ...
                        # As a fallback, safely convert
                        df['insertion_timestamp'] = pd.to_datetime(
                            df['insertion_timestamp'], utc=True
                        ).dt.tz_convert('America/New_York')
                else:
                    # If not recognized as datetime, force it
                    df['insertion_timestamp'] = pd.to_datetime(
                        df['insertion_timestamp'], utc=True
                    ).dt.tz_convert('America/New_York')
            ...
            # 5) Convert unique_columns to list if needed
            if isinstance(unique_columns, str):
                unique_columns = [uc.strip() for uc in unique_columns.split(",")]

            async with self.pool.acquire() as connection:
                # 6) Fetch DB column types
                column_types = await connection.fetch(
                    f"""
                    SELECT column_name, data_type
                    FROM information_schema.columns
                ...
                type_mapping = {
                    col['column_name']: col['data_type'].lower() for col in column_types
                }

                # 7) Prepare INSERT ... ON CONFLICT query
                insert_query = f"""
                INSERT INTO {table_name} (
                    {', '.join(f'"{col}"' for col in df.columns)}
                )
                ...
                for record in records:
                    ...
                    for col, val in record.items():
                        ...
                        pg_type = type_mapping.get(col, 'text')

                        # If column is some flavor of timestamp
                        if 'timestamp' in pg_type:
                            # If it's a pandas Timestamp
                            if isinstance(val, pd.Timestamp):
                                row_tuple.append(val.to_pydatetime())
                            elif isinstance(val, datetime):
                                # Ensure tz-aware in America/New_York
