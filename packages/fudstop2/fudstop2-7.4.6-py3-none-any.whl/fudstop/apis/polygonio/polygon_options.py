...
class PolygonOptions:
    ...
    async def create_table(
        ...
        Stores insertion timestamps in Eastern Time (America/New_York).
        """
        ...
        # Dynamically handle potential BIGINT upgrade for int64 columns
        ...
        async with self.pool.acquire() as connection:
            ...
            if table_exists is None:
                # Handle unique columns (could be comma-separated)
                ...
                # Create main table with Eastern Time default for insertion_timestamp
                create_query = f"""
            ...
            # Optional creation of history table with triggers
            if create_history:
                # History table with changed_at in Eastern Time
            ...
            # Ensure all DataFrame columns exist in the main table
            for col, col_dtype in zip(df.columns, df.dtypes):
                alter_query = f"""
                DO $$
                BEGIN
    ...
    async def batch_upsert_dataframe(
        ...
        """
        WEBSITE SIDE:
        Batch upsert a DataFrame into a table.
        On conflict, updates only non-unique columns.
        
        IMPORTANT FIX:
          - Make sure all timestamp columns are consistently tz-aware or tz-naive
            to avoid "Cannot subtract tz-naive and tz-aware" errors.
          - In this example, we store everything in "America/New_York" as a tz-aware datetime.
        """
        try:
            # async with lock:  # If you have an actual async lock
            ...
            # 2) Convert all datetime columns to "America/New_York" tz-aware
            #    This prevents mixed naive vs. aware issues.
            for col in df.select_dtypes(include=['datetime64[ns]', 'datetime64[ns, tz]']):
                # If a column is tz-naive, localize to America/New_York
                # If already tz-aware, convert to America/New_York
                if df[col].dt.tz is None:
                    df[col] = df[col].dt.tz_localize('America/New_York')
                else:
                    df[col] = df[col].dt.tz_convert('America/New_York')

            # 3) Ensure insertion_timestamp is also set as Eastern
            #    (If the DataFrame doesn't have insertion_timestamp, create one.)
            if 'insertion_timestamp' not in df.columns:
                df['insertion_timestamp'] = [
                    datetime.now(pytz.timezone('America/New_York')) for _ in range(len(df))
                ]
            else:
                # If it already exists, ensure it's tz-aware in America/New_York
                if df['insertion_timestamp'].dtype == 'datetime64[ns]':
                    # localize naive
                    df['insertion_timestamp'] = df['insertion_timestamp'].dt.tz_localize('America/New_York')
                else:
                    # convert if already tz-aware
                    df['insertion_timestamp'] = df['insertion_timestamp'].dt.tz_convert('America/New_York')

            # 4) Convert the dataframe to records
            records = df.to_dict(orient='records')

            # 5) Convert unique_columns (comma-delimited string) to a list if necessary
            if isinstance(unique_columns, str):
                unique_columns = [uc.strip() for uc in unique_columns.split(",")]

            async with self.pool.acquire() as connection:
                # 6) Fetch column info from DB
                column_types = await connection.fetch(
                    f"""
                    SELECT column_name, data_type
                    FROM information_schema.columns
                ...
                # 7) Prepare the SQL with ON CONFLICT update
                insert_query = f"""
                INSERT INTO {table_name} (
                    {', '.join(f'"{col}"' for col in df.columns)}
                )
                ...
                for record in records:
                    ...
                    for col, val in record.items():
                        ...
                        pg_type = type_mapping.get(col, 'text').lower()

                        # 8) Handle timestamp columns
                        if 'timestamp' in pg_type:
                            # If it's a pandas Timestamp
                            if isinstance(val, pd.Timestamp):
                                # Convert/ensure tz-aware in America/New_York
                                if val.tz is None:
                                    val = val.tz_localize('America/New_York')
                                else:
                                    val = val.tz_convert('America/New_York')
                                # Convert to Python datetime (still keeps tz info)
                                val = val.to_pydatetime()

                            # If it's a normal datetime
                            elif isinstance(val, datetime):
                                # Localize or convert to Eastern if naive
                                if val.tzinfo is None:
                                    val = pytz.timezone('America/New_York').localize(val)
                                else:
                                    val = val.astimezone(pytz.timezone('America/New_York'))

                            new_record.append(val)

                        # 9) Convert float/double
                        elif pg_type in ['double precision', 'real', 'float8']:
                            # If it's numeric, cast to float
                            new_record.append(float(val))

                        # 10) Convert int
                        elif pg_type in ['integer', 'bigint', 'int', 'smallint']:
                            new_record.append(int(val))

                        else:
                            # For everything else, just add the value
                            new_record.append(val)

                    # Collect one row
                    batch_data.append(tuple(new_record))

                    # 11) Once we hit batch_size, execute
                    if len(batch_data) == batch_size:
                        try:
                            await connection.executemany(insert_query, batch_data)
                            batch_data.clear()
    ...
    async def batch_upsert_dataframe_without_unique(
        self,
        df: pd.DataFrame,
        table_name: str,
        unique_columns,
        ...
        """
        Batch insert a DataFrame into a table WITHOUT unique constraints or conflict resolution.
        """
        try:
            async with lock:
                if not await self.table_exists(table_name):
                ...
                async with self.pool.acquire() as connection:
                    ...
                    # Simple insert query without conflict handling
                    insert_query = f"""
                    INSERT INTO {table_name} (
                        {', '.join(f'"{col}"' for col in df.columns)}
                    ) 
