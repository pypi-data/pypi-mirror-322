...
class PolygonOptions:
    ...
    async def batch_upsert_dataframe(
        ...
        Ensures insertion_timestamp is timezone-aware (America/New_York).
        """
        try:
            # Use your actual async lock if needed
            # async with lock:
            ...
            # Force insertion_timestamp to be a TZ-aware datetime in America/New_York
            ...
            # Convert a string of columns to a list if needed
            if isinstance(unique_columns, str):
                unique_columns = [uc.strip() for uc in unique_columns.split(",")]

            async with self.pool.acquire() as connection:
                # Fetch the types from the database for each column
                column_types = await connection.fetch(
                    f"""
                    SELECT column_name, data_type
                    FROM information_schema.columns
                ...
                # Prepare the SQL insert with ON CONFLICT
                insert_query = f"""
                INSERT INTO {table_name} (
                    {', '.join(f'"{col}"' for col in df.columns)}
                )
                ...
                for record in records:
                    ...
                    for col, val in record.items():
                        ...
                        # If the column is a timestamp of some sort
                        if pg_type in [
                            'timestamp', 
                            'timestamp without time zone', 
                            'timestamp with time zone'
                        ]:
                            # If it's a numpy datetime64, localize to Eastern Time if naive
                            if isinstance(val, np.datetime64):
                                ts = pd.Timestamp(val)
                                # If it's tz-naive, localize it
                                if ts.tzinfo is None:
                                    ts = ts.tz_localize('America/New_York')
                                new_record.append(ts.to_pydatetime())
                            elif isinstance(val, datetime):
                                # If it's tz-naive, localize it
                                ...
                                # Just append the value if we can't identify it
                                new_record.append(val)

                        # If it's a floating point or double precision
                    ...
                    # Once we reach batch_size, execute a batch insert
                    if len(batch_data) == batch_size:
                        try:
                            await connection.executemany(insert_query, batch_data)
                            batch_data.clear()
