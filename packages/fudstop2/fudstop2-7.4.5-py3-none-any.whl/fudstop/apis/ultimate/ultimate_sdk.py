...
class UltimateSDK:
    ...
    # ---------------------------------------------------------------
        self.wb_headers = {
            ...
            "reqid": "og5z5xzrzef89d6epd42hy2uovwkl_81",
            ...
            "ver": "5.2.1",
            "x-s": "2c416512a807121b4a8b3d08f837028cbcc1b2578f0c1e46eb1107d75e3c2afb",
            "x-sv": "xodp2vg9"
        }
    ...
    async def get_candle_data(
        ...
        async with self.semaphore:
            try:
                ...
                try:
                    async with client.get(base_fintech_gw_url,headers=self.wb_headers) as response:
                        response.raise_for_status()
                        ...
                        if data_json and isinstance(data_json, list) and 'data' in data_json[0]:
                            ...
                            print(df['ts'].head())  # Inspect the first few rows of 'ts'
    ...
    async def check_candles_for_pretd9(self, ticker, timespan):
        """
        ...
        """
        try:
            df = await ta.get_candle_data(ticker, interval=timespan, headers=self.wb_headers, count='800')
