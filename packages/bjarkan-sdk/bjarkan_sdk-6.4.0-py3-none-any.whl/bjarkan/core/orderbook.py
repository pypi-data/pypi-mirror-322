...
class OrderbookManager:
    ...
    def __init__(self, orderbook_config: OrderbookConfig):
        ...
        # Map of exchange -> symbols that specific exchange supports
        self.exchange_symbols = {}
        self.fees = self._initialize_fees()
        self.orderbooks = {}  # Will be initialized after checking symbols
    ...
    async def _get_final_orderbook(self, symbol: str) -> Dict:
        ...
        for exchange_id, exchange_obs in self.orderbooks.items():
            ...
            for sym in group_symbols:
                ...
                if sym in supported_symbols and (ob := exchange_obs.get(sym)):
                    print(f"Found orderbook with {len(ob['bids'])} bids, {len(ob['asks'])} asks")
        ...
        if self.config.aggregated:
            ...
            print(f"Aggregation result: {book is not None}")
        else:
            book = books[0].copy()

        if book is None:
            print("Final book is None!")
            return None
        ...
        if symbol in self._last_hash and self._last_hash[symbol] == current_hash:
            print("Same hash as last time, returning None")
            return None
        ...
        print(f"Returning final book with {len(final_book['bids'])} bids and {len(final_book['asks'])} asks")
        return final_book
