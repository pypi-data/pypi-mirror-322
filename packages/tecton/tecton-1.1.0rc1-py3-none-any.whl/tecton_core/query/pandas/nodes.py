...
class PandasFeatureViewPipelineNode(ArrowExecNode):
    ...
    def _node_to_value(
        ...
        if pipeline_node.HasField("transformation_node"):
            ...
            # Override secrets and resources with mock context if provided
            secrets = (
                self.mock_context.secrets
                if self.mock_context and self.mock_context.secrets
                else self._get_secrets(self.feature_definition_wrapper.fv_spec.secrets, context)
            )
    ...
    def _get_resources(
        ...
        for key, resource_provider_spec in resource_provider_specs.items():
            ...
            if resource_provider_spec.secrets:
                if mock_secrets:
                    function_args[RESOURCE_CONTEXT_PARAM_NAME] = ResourceProviderContext(secrets=mock_secrets)
                elif execution_context.secret_resolver:
