...
def _enrich_realtime_context(
    ...
    mock_context_provided = any(isinstance(data, RealtimeContext) for data in input_data.values())
    key = context_parameter_name or DEFAULT_CONTEXT_PARAMETER_NAME

    if mock_context_provided:
        input_data[key].set_request_timestamp(request_timestamp)
    else:
        input_data[key] = RealtimeContext(_request_timestamp=request_timestamp)
