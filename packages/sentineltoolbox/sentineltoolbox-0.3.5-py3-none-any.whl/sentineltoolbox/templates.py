...
def create_tree(parent: DataTree[Any], path: str, content: Dict[str, Any]) -> None:
    ...
    for i, part in enumerate(parts):
        ...
        if existing_child:
            ...
            if i == len(parts) - 1:
                ...
                DataTree(name=part, parent=parent, data=dataset)
            else:
                # Create a new child node if it doesn't exist
                new_child: DataTree[Any] = DataTree(name=part, parent=parent)
                parent = new_child


def create_from_template(data_variables: Dict[str, Any]) -> DataTree[Any]:
    """
    ...
    root: DataTree[Any] = DataTree(name="root")
