...
def pytest_addoption(parser, pluginmanager):
    ...
    if options.OTEL_EXPORTER_OTLP_ENDPOINT is None:
        ...
        cert_bundle = os.path.join(os.path.dirname(__file__), "silabs-net.pem")
        options.OTEL_EXPORTER_OTLP_CERTIFICATE = cert_bundle
        if not os.path.exists(cert_bundle):
            with open(cert_bundle, "wb") as bundle:
                r = requests.get(
                    "https://vault.dev.silabs.net:443/v1/pki/ca/pem", verify=False
                )
                cert_file = os.path.join(os.path.dirname(__file__), "vault-ca-dev.crt")
                with open(cert_file, "wb") as f:
                    f.write(r.content)
                    bundle.write(r.content + b"\n")
                r = requests.get(
                    "https://vault.silabs.net:443/v1/pki/ca/pem", verify=False
                )
                cert_file = os.path.join(os.path.dirname(__file__), "vault-ca.crt")
                with open(cert_file, "wb") as f:
                    f.write(r.content)
                    bundle.write(r.content)
