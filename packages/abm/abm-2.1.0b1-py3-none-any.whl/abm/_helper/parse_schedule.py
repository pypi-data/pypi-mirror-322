...
def parse_schedule(dose: dict[str, int | float | str]) -> Schedule:
    match PartialRepeatScheduleParser.times_column.parse(str(dose["times"])):
        case Success(times_column):
            ...
            unitted_amount = f"{dose['amounts']}:{maybe_add_parentheses(dose['amount_unit'])}"
        ...
        case _:  # Failure: make a list schedule
            ...
            max_length = max(len(amounts), len(dose_times), n_durations)
            ...
            match durations:
                ...
                case _:  # list[str]
                    duration_unit = dose["duration_unit"]  # duration unit expected if durations are present
