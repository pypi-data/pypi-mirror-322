...
def listA(html, baseUrl=None, start=None, end=None, before=None):
    ...
    patternLst = _getRegex(r'<a[\s]+[^>]*>[\s\S]*?</a>')
    patternUrl = _getRegex(r'href[\s]*=[\s\'"]*(?P<url>.*?)[\'"\s>]')
    patternTitle1 = _getRegex(r'title[\s]*=[\s\'"]*(?P<title>.*?)[\'"\s>]')
    patternTitle2 = _getRegex(r'<a[\s]+[^>]*>(?P<title>.*?)</a>')
...
def listImg(html, baseUrl=None, start=None, end=None, before=None):
    ...
    patternLst = _getRegex(r'<img[\s]+[^>]*>')
    patternUrl = _getRegex(r'src[\s]*=[\s\'"]*(?P<url>.*?)[\'"\s>]')
    patternTitle = _getRegex(r'alt[\s]*=[\s\'"]*(?P<title>.*?)[\'"\s>]')
...
__re1 = re.compile(r'[\s]*>')
__re2 = re.compile(r'<[\s]*')
__re3 = re.compile(r'[\s]*/>')
__re4 = re.compile(r'[\s]+')
...
def removeScripts(html):
    ...
    html = _getRegex(r'<!--[\s\S]*?-->').sub('', html)
    html = _getRegex(r'<[\s]*script[^>]*>[\s\S]*?</script>').sub('', html)
    html = _getRegex(r'<[\s]*style[^>]*>[\s\S]*?</style>').sub('', html)
    html = _getRegex(r'<[\s]*link [\s\S]*?>').sub('', html)
    html = _getRegex(r'<[\s]*meta [\s\S]*?>').sub('', html)
    html = _getRegex(r'<[\s]*object[^>]*>[\s\S]*?</object>').sub('', html)
    html = _getRegex(r'<[\s]*iframe[^>]*>[\s\S]*?</iframe>').sub('', html)
    html = _getRegex(r'</object>').sub('', html)
    html = _getRegex(r'<param [\s\S]*?>').sub('', html)
    return html
...
__reh0 = re.compile(r'<!--[\s\S]*?-->')
__reh1 = re.compile(r'<[^<>]+>')
__reh2 = re.compile(r'(\s|&nbsp;)+')
__reh3 = re.compile(r'(\s*_yazz_\s*)+')
...
def _getElement(tag,
    ...
    while True:
        ...
        strP = u'<' + _dealRegChar(tag) + r'[\s]+[^>]*?' + _dealRegChar(
            attr
        ) + r'[=\'"\s]+([\w\-\.]+[\s\w\-\.]*[ ]|\s*|)' + value + r'([\s\'"][\s\S]*?|)>'
        ...
        if not singleTag:
            strP = u'<' + _dealRegChar(tag) + r'[\s]+[^>]*?' + _dealRegChar(
                attr
            ) + r'[=\'"\s]+([\w\-\.]+[\s\w\-\.]*[ ]|\s*|)' + value + r'([\s\'"][\s\S]*?|)>[\s\S]*?</' + _dealRegChar(
                tag) + '>'
            pattern = _getRegex(strP)
        m = pattern.search(html)
        if m:
...
def _getNextTag(html, start):
    pattern = _getRegex(r'<[\s]*(?P<tag>[a-zA-Z0-9:_.-]+?)[/>\s]')
