        ...
        while retry_count <= self.max_retries:
            try:
                ...
                if retry_count > self.max_retries:
                    logger.error(
                        f"Streaming translation failed after {self.max_retries} retries: {str(e)}")
                    raise AIAPIError(f"流式翻译失败: {str(e)}")
                logger.warning(
                    f"Retry {retry_count}/{self.max_retries} due to: {str(e)}")
    ...
    async def __aiter__(self):
        ...
        if self.src == 'auto':
            try:
                ...
                # 如果检测失败，尝试使用备用检测方法
                try:
                    detected = await self.translator.ai_detect_enhanced(text)
                    self.src = detected.lang
...
class AITranslatorSync:
        ...
        if not stream:
            ...
            def stream_generator():
                ...
                try:
                    ...
                    logger.error(f"Stream translation error: {str(e)}")
    ...
    def _create_sync_generator(self, async_gen):
        ...
        def sync_generator():
            try:
                ...
                # 使用run_until_complete运行生成器
                gen = run_generator()
                while True:
                    try:
                        next_value = loop.run_until_complete(gen.__anext__())
                        yield next_value
    ...
    def clear_cache(self):
        """清理翻译缓存"""
        self._ensure_translator()
        self.translator._response_cache.clear()
        self.translator._context_cache = {}

    def get_performance_profile(self, mode='balanced'):
        """获取指定模式的性能配置"""
        return PERFORMANCE_PROFILES.get(mode, PERFORMANCE_PROFILES['balanced'])

    def get_available_performance_modes(self):
        """获取所有可用的性能模式"""
    ...
    def get_current_performance_mode(self):
        """获取当前性能模式"""
        self._ensure_translator()
        current_config = self.translator.perf_config
        for mode, profile in PERFORMANCE_PROFILES.items():
            if current_config == profile:
                return mode
        return "custom"
    ...
    def translate_batch_sync(self, texts: List[str], dest='en', src='auto') -> List[AITranslated]:
        ...
        for text in texts:
            try:
                ...
                results.append(AITranslated(src=src, dest=dest, origin=text,
                                            text=f"Translation failed: {str(e)}"))
        return results

    def translate_with_context(self, text, context, dest='en', src='auto', style_guide=None, stream=False):
        """带上下文的同步翻译方法，支持流式翻译
        ...
        if not stream:
            result = self._loop.run_until_complete(
                self.translator.translate_with_context(
                    text, context, dest=dest, src=src, style_guide=style_guide
                )
            )
            return result
        else:
            async def stream_generator():
                result = await self.translator.translate_with_context(
                    text, context, dest=dest, src=src, style_guide=style_guide, stream=True
                )
    ...
    def translate_with_style(self, text, dest='en', src='auto', style='formal', context=None, max_versions=3, stream=False):
        """带风格的同步翻译方法，支持流式翻译
        ...
        if not stream:
            result = self._loop.run_until_complete(
                self.translator.translate_with_style(
                    text, dest=dest, src=src, style=style,
                    context=context, max_versions=max_versions
                )
            )
            return result
        else:
            async def stream_generator():
                result = await self.translator.translate_with_style(
                    text, dest=dest, src=src, style=style,
                    context=context, max_versions=max_versions, stream=True
                )
                async for partial_result in result:
                    yield partial_result
...
def test_sync_translator():
    ...
    try:
        ...
        for i in range(len(result.text)):
            partial_text = result.text[:i+1]
            print(f"部分结果: {partial_text}", end='\r')
            time.sleep(0.1)
        print()  # 换行
