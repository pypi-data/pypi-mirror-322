...
from diffusion.internal.serialisers.generic_model import GenericModel, ModelFields
...
class Event(typing.Generic[VT], MarshalledModel):
    ...
    @typing_extensions.overload
    def with_value(
        self, new_value: VT_other, type: typing.Type[VT_other]
    ) -> Event[VT_other]: ...
...
class OriginalEventFields(ModelFields):
    value: bytes
...
class OriginalEvent(typing.Generic[VT], Event[VT]):
    ...
    @classmethod
    def from_fields(
        cls: typing.Type[OriginalEvent_T],
        **kwargs,
    ) -> OriginalEvent_T:
        fields = OriginalEventFields.from_fields(kwargs)

        metadata = EventMetadata(
            sequence=fields.original_sequence,
            timestamp=fields.original_timestamp,
            author=fields.original_author,
        )

        decoded_value = typing.cast(
            typing.Type[TimeSeriesValueType], cls.value_type_real()
        ).from_bytes(fields.value)
...
@dataclasses.dataclass
class EditEventFields(OriginalEventFields):
    edit_sequence: int
    edit_timestamp: int
    edit_author: str
...
class EditEvent(typing.Generic[VT], Event[VT]):
    ...
    @classmethod
    def from_fields(cls: typing.Type[EditEvent_T], **kwargs) -> EditEvent_T:
        fields = EditEventFields.from_fields(kwargs)
        ...
        edit_metadata = EventMetadata(
            sequence=fields.edit_sequence,
            timestamp=fields.edit_timestamp,
            author=fields.edit_author,
        )
        return cls(
            original_event=metadata,
            metadata=edit_metadata,
            value=cls.value_type_real().from_bytes(fields.value),
            **kwargs,
        )
