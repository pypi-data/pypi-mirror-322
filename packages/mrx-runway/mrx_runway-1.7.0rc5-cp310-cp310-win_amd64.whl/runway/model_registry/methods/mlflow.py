...
def log_model(
    ...
    **kwargs: Unpack[Dict[str, Any]],
    ...
    """
    Log a model with inference logic and optional data dependencies as an MLflow artifact for the current run. 
    ...
    model : Any
        The model to log
    input_samples : ModelInputsSample
        Sample inputs to test the model with
    model_name : str, default="model"
        Name to give the logged model
    include_snapshot_image : bool, default=True
        Whether to include a container image snapshot
    image_url : Optional[str], default=None
        URL of existing container image to use
    **kwargs : Dict[str, Any]
    ...
    ------
    RuntimeError
        If no active MLflow run is found
    """
    if not mlflow.active_run():
        raise RuntimeError(f"not found active run: {mlflow.active_run()=}")
