        ...
        if not result or "status" not in result:
            ...
            # When the error code is CZLH-57015, jobid needs to be generated to resubmit the job
                ...
                if client.wait_continue_submit(job_id.id, tried):
                    ...
                    raise Exception(
                        f"Job need re-execute status no need resubmit, job_id:{job_id.id}, origin sql:{sql}, error:"
        ...
        def _handle_result_check(sql, context, tried, exception=None, **kwargs):
            ...
            if not Client.verify_result_dict_finished(result, sql):
                ...
                if sleep_time_ms * 1.5 <= 30000:
                    ...
                    _log.debug(
                        "Get async sql job:%s result pending, retry after %d seconds, tried:%d, time cost:%d, next"
                        " sleep time:%d", job_id.id, second, tried, begin_time - datetime.now(), sleep_time_ms)
                return False
            else:
                return True
