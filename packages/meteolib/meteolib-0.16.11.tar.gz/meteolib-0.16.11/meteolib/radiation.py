...
def _clear_sky_ashrae1997(time, lat, lon, heading, slant, albedo=None):
    r"""
    Return direct normal and diffuse irradiance to a surface
    on a clear day
...
def clear_sky_direct_radiation(time, lat, lon, model="ashrae1997"):
    """
    ...
    if model == "ashrae1997":
        return _clear_sky_ashrae1997(time, lat, lon, 0., 0.)[0]
    else:
...
def clear_sky_surface_irradiance(time, lat, lon, heading, slant,
...
def clear_sky_longwave_downwelling(t_k: float, e=0.,
                                   method="angstrom"):
    r"""
    ...
    :type e: float
    :param method: name of the function to use
    :type method: str
