...
def get_scaffolds(session: Session) -> list:
    ...
    with open("binary.txt", "w") as handle:
        session.format("binary.txt", fp = handle)
...
def main():
    ...
    scaffolds = get_scaffolds("binary.txt")
    ...
    updated_status = recover_hits("binary.txt", genome_cluster_composition, no_revisit_by_content, no_revisit_by_score, outlier_z, min_score_diff)
