...
SUMMARY_FILE = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'summary.txt')
BINARY_FILE = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'binary.txt')
...
def load_session(path_to_session: str) -> dict:
    """
    This function loads the cblaster session, converts it to a dictionary and generates the output tables
    
    :param str path_to_session: Path to the cblaster session json file
    :rtype dict: A dictionary export of the cblaster Session object
    """
    print("--- STEP 0: Loading session file. ---")
    
    session = Session.from_file(path_to_session).to_dict()
    
    with open("binary.txt", "w") as binary_handle:
        session.format(form = "binary", fp = binary_handle)
    with open("summary.txt", "w") as summary_handle:
        session.format(form = "summary", fp = summary_handle)
...
def generate_output(dereplicated_scaffolds:list, session:Session) -> None:
    ...
    for org_idx, org in reversed(list(enumerate(session['organisms']))):
...
def main():
    ...
    scaffolds = get_scaffolds(BINARY_FILE)
    ...
    updated_status = recover_hits(BINARY_FILE, genome_cluster_composition, no_revisit_by_content, no_revisit_by_score, outlier_z, min_score_diff)
    ...
    generate_output(dereplicated_scaffolds, path_to_session, session)
