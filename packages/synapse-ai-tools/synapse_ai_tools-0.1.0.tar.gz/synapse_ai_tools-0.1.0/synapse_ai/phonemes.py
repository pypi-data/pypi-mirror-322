        ...
        try:
            if len(word) == 1:
                ...
                if word[1] not in "aeiou":
                    ...
                    temp_text = accents_dictioaccented_vowel + word_without_vowel  
                elif word[-1] in "aeiou" and word[-2] == "e":
                    accents_dictioaccented_vowel = accents_dictionary[word[-2]]
                    temp_text = word[0] + accents_dictioaccented_vowel + word[-1]
                else: 
                    temp_text = word
                ...
                if word[-2] in 'aeiou':
                    ...
                    accents_dictioaccented_vowel = accents_dictionary[word[-2]] 
                    temp_text =  word_without_vowel + accents_dictioaccented_vowel + word[-1] 
                else:
                    word_without_vowel = word[:-3] 
                    accents_dictioaccented_vowel = accents_dictionary[word[-3]] 
                    temp_text =  word_without_vowel + accents_dictioaccented_vowel + word[-2:]
            else: 
                if word[-1] in 'aeiou' and word[-2] in 'aeiou': 
                    if word[-2:] != 'ia' and word[-2:] != 'io': 
                        if word[-4] in 'aeiou':
                            word_without_vowel = word[:-4]
                            accents_dictioaccented_vowel = accents_dictionary[word[-4]]
                            temp_text =  word_without_vowel + accents_dictioaccented_vowel + word[-3:]
                    else:  
                        if word[-4] in 'aeiou': 
                            word_without_vowel = word[:-4] 
                            accents_dictioaccented_vowel = accents_dictionary[word[-4]] 
                            ...
                            accents_dictioaccented_vowel = accents_dictionary[word[-5]]
                            temp_text =  word_without_vowel + accents_dictioaccented_vowel + word[-4:] 
                        else:
                            word_without_vowel = word[:-6]
                            accents_dictioaccented_vowel = accents_dictionary[word[-6]]
                            temp_text =  word_without_vowel + accents_dictioaccented_vowel + word[-5:]
...
def dictionaries(text:str) -> tuple[dict[str, int], dict[str, int]]:
    ...
    Returns:
        tuple[dict[str, int], dict[str, int]]: A tuple containing:
            - diccionario_fonemas (dict[str, int]): A dictionary where keys are unique characters (phonemes) 
              from the text and values are their respective indices. The special key "pad" maps to 0.
            - cantidad_fonemas (dict[str, int]): A dictionary where keys are unique characters (phonemes) 
    ...
    Notes:
        ...
        - Frequencies in `cantidad_fonemas` are initialized to 0 and updated iteratively for each character in the text.
    """
...
def phoneme_graphs(quantity:dict, tokens:dict, fig_size:tuple[int]=(8,8), color:str='violet') -> None:
    """
    ...
    Examples:
        ...
        >>> grafico_fonema(quantity, tokens)
    ...
    data = pd.DataFrame({'fonema': quantity.keys(), 'frecuencia': quantity.values(), 'orden': [tokens[fonema] for fonema in quantity.keys()]})
    data = data.sort_values(by='orden')
    plt.figure(figsize=fig_size)
    sns.barplot(data=data, x='fonema', y='frecuencia', color=color)
    plt.title('Frecuencia de Fonemas', fontsize=16)
    plt.xlabel('Fonemas', fontsize=10)
...
if __name__ == '__main__':
    texto_sin_acentos = phoneme(text)
    diccionario_fonemas_sin_acento, cantidad_fonemas_sin_acento = dictionaries(texto_sin_acentos)
    phoneme_graphs(cantidad_fonemas_sin_acento, diccionario_fonemas_sin_acento)
    texto_con_acentos = accent(texto_sin_acentos)
    diccionario_fonemas_con_acento, cantidad_fonemas_con_acento = dictionaries(texto_con_acentos)
    phoneme_graphs(cantidad_fonemas_con_acento, diccionario_fonemas_con_acento)
