...
async def getAlbumList2(request: web.Request, conn: Connection, _user: User):
...
async def _transcoded_audio(request: web.Request, conn: Connection):
    track = Track.by_relpath(conn, from_id(request.query["id"]))
    assert track
    audio_bytes = await track.transcoded_audio(AudioFormat.WEBM_OPUS_HIGH)
    return web.Response(body=audio_bytes, content_type=AudioFormat.WEBM_OPUS_HIGH.content_type)
...
async def stream(request: web.Request, conn: Connection, _user: User):
    ...
    return await _transcoded_audio(request, conn)
...
async def scrobble(request: web.Request, conn: Connection, user: User):
    ...
    await activity.set_now_playing(conn, user, player_id, relpath, False, 0, 0, False)
