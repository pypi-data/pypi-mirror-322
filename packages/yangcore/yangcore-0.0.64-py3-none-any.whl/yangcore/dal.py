...
class DataAccessLayer:
	...
	def _init_new_db_with_factory_default(A):
		a='import-only-module';Z='module';V='local-address';U='endpoint';O='datastore';N='local-bind';K='ietf-yang-library:yang-library';J='schema';I='module-set'
		...
		b=A._load_yang_dir_file('factory-default.json');c=A._load_yang_dir_file('yang-library-nbi.json');d={'yangcore:audit-log':{'audit-log-record':[]},'yangcore:notification-log':{'notification-log-record':[]}};H=b|c|d;R=H['ietf-restconf-server:restconf-server']['listen']['endpoints'];W={U:R[U].pop(0)};R=W[U]['http']['tcp-server-parameters'];P={N:R[N].pop(0)};L=[]
		while len(H[K][I]):e=H[K][I].pop(0);L.append({I:e})
		Q=[]
		while len(H[K][J]):f=H[K][J].pop(0);Q.append({J:f})
		X=[]
		while len(H[K][O]):g=H[K][O].pop(0);X.append({O:g})
		if(M:=os.environ.get('YANGCORE_INIT_ADDR')):P[N][V]=M
		if(M:=os.environ.get('YANGCORE_INIT_PORT')):
			try:S=int(M)
			except ValueError as h:raise ValueError('Invalid "YANGCORE_INIT_PORT" value ('+M+').')from h
			if S<=0 or S>2**16-1:raise ValueError('The "YANGCORE_INIT_PORT" value ('+M+') is out of range [1..65535].')
			P[N]['local-port']=S
		i=H;B=_B;C=A.schema_path_to_real_table_name[B];D=A.metadata.tables[C]
		with A.engine.begin()as E:F=E.execute(D.insert().values(name=_l,jsob=i))
		...
		with A.engine.begin()as E:T=E.execute(D.insert().values(parent_id=A.global_root_id,name='default startup endpoint',jsob=W))
		assert T.inserted_primary_key[0]==T.lastrowid;j=T.inserted_primary_key[0];G={};G[_H]=j;G[V]=P[N][V];G[_K]=P;B='/ietf-restconf-server:restconf-server/listen/endpoints/'+'endpoint/http/tcp-server-parameters/local-bind';C=A.schema_path_to_real_table_name[B];D=A.metadata.tables[C]
		...
		with A.engine.begin()as E:G={_H:A.global_root_id,_I:L[0][I][_I],Z:L[0][I][Z],a:L[0][I][a]};F=E.execute(D.insert().values(**G));assert F.inserted_primary_key[0]==F.lastrowid;k=F.inserted_primary_key[0]
		...
		with A.engine.begin()as E:
			for Y in X:G={_H:A.global_root_id,_I:Y[O][_I],J:Y[O][J]};F=E.execute(D.insert().values(**G))
