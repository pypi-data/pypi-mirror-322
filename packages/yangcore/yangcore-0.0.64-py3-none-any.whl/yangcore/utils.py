...
def encoded_str_to_obj(estr,enc,dm,sn):
	...
	try:G=yangson.instance.RootNode(C,sn,dm.schema_data,C.timestamp)
...
async def insert_notification_log_record(nvh,notif_payload):
	...
	G=J['yangcore:relay-notification-log-record-callout'];E=_S+G;A=await B.dal.handle_get_config_request(E,{});A=A[_T][0];assert G==A[_C];K=A[_H][_U];L=A[_H][_V];P=next(iter(C));M={'notification-log-record':{_G:F,H:C}};N=_A;O=B.plugins[K][_W][L](M,N);assert O is _A
async def insert_audit_log_record(dal,plugins,audit_log_record):
	...
	E=G['yangcore:relay-audit-log-record-callout'];D=_S+E;B=await C.handle_get_config_request(D,{});B=B[_T][0];assert E==B[_C];H=B[_H][_U];I=B[_H][_V];A[_G]=A[_G].strftime(_R);A.pop('parent_id');J={'audit-log-record':A};K=_A;L=plugins[H][_W][I](J,K);assert L is _A