...
class RestconfServer:
	...
	def __init__(A,loop,dal,endpoint_config,view_handler):
		v='client-certs';u='central-truststore-reference';t='ca-certs';s='client-authentication';r='utf-8';q='\n-----END CERTIFICATE-----\n';p='-----BEGIN CERTIFICATE-----\n';o=False;n='cert-data';m='private-key-format';l='/ietf-keystore:keystore/asymmetric-keys/asymmetric-key=';k='central-keystore-reference';j='server-identity';i='http';V='ASCII';U='ietf-keystore:asymmetric-key';M='certificate';L='tls-server-parameters';K='/ds/ietf-datastores:running{tail:.*}';D='https';C=endpoint_config;B=view_handler;A.len_prefix_running=len(A.root+_B);A.len_prefix_operational=len(A.root+_C);A.loop=loop;A.dal=dal;A.name=C['name'];A.view_handler=B;A.app=web.Application(client_max_size=33554432)
