...
class AutoSlicer:
    ...
    def _calculate_inertia_parameters(self, seg_file: str) -> dict:
        # If actual spacing is known, set it instead of (1,1,1)
        voxel_size = (1, 1, 1)
    ...
    def visualize_with_coordinate_axes(self, original_com=None, decimate_ratio=0.0):
        """
        Reads the self.vtk_path VTK model and visualizes it with:
          ...
          3) If original_com is provided, displays a black sphere at the center of mass
        ...
        if not os.path.exists(self.vtk_path):
            print(f"VTK file not found: {self.vtk_path}")
            return
        ...
        if decimate_ratio > 0:
            polydata = self._decimate_polydata(polydata, decimate_ratio)

        transformed_polydata, dx, dy, dz, transform = self._transform_to_origin(polydata)

        mapper = vtk.vtkPolyDataMapper()
        mapper.SetInputData(transformed_polydata)
        actor = vtk.vtkActor()
        actor.SetMapper(mapper)
        actor.GetProperty().SetOpacity(0.2)  # half-transparent
        ...
        # Some VTK versions require SetFlyModeToStaticEdges() instead of SetFlyModeToStatic()
        ...
        if original_com is not None:
            ...
            sphere_actor.GetProperty().SetColor(0, 0, 0)  # black
            renderer.AddActor(sphere_actor)

            line_com_x = self._create_line_actor((cx_new, cy_new, cz_new), (0, cy_new, cz_new))
            line_com_y = self._create_line_actor((cx_new, cy_new, cz_new), (cx_new, 0, cz_new))
            line_com_z = self._create_line_actor((cx_new, cy_new, cz_new), (cx_new, cy_new, 0))
        ...
        print("VTK window started. Press 'q' or close to exit.")
        interactor.Start()

    def _decimate_polydata(self, polydata, reduction=0.3):
    ...
    def _transform_to_origin(self, polydata):
    ...
    def _create_line_actor(self, pt1, pt2, color=(0.7, 0.7, 0.7), width=2.0):
...
if __name__ == "__main__":
    slicer = AutoSlicer("TEST_WORKSPACE")
    # Example usage:
    dicom_folder = r"D:\Curtin_Hive_Data\head-male female\1-1"
    results = slicer.run_automation(dicom_folder)
    print("Final results:", results)

    # If you already have the final NIfTI or VTK, you can directly visualize:
    com = results["T4"]["value"]  # center of mass
