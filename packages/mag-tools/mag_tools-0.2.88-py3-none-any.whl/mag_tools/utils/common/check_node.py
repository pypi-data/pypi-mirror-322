...
class CheckNode:
    ...
    def log(self):
        ...
        :return: 节点的日志信息
        """
        total = f"{self.__total_cost}纳秒" if self.__total_cost > 1000 else f"{self.__total_cost/1000}毫秒"
        last  = f"{self.__last_cost}纳秒" if self.__last_cost > 1000 else f"{self.__last_cost/1000}毫秒"
