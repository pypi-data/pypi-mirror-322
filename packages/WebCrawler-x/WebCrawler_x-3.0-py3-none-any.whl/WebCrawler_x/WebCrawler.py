...
def get_page_info(url, page_param=None, step=1, first_num=1, mode='direct', max_attempts=10, use_cache=False, cache_file=None, proxy=None,stop_max_attempt_number=3,sleep=1,has_index=False):
    """
        ...
        try:
            if data == None:
                if params == None:
                    ...
                    response = requests.get(url, headers=headers, params=params, proxies=proxy, timeout=10,verify=False)
                if response.status_code == 200:
                    response.encoding = response.apparent_encoding
                    return response
            else:
