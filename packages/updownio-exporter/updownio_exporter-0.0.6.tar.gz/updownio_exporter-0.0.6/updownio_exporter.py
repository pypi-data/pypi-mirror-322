...
def get_from():
    return datetime.now(timezone.utc) - timedelta(hours=conf.loop.lookback)
...
def main():
    ...
    while True:
        ...
        if datetime.now(timezone.utc) - reset_interval < from_:
            ...
            logger.warning(msg, from_, conf.loop.reset, get_from())
            Cache.clear()
...
if __name__ == "__main__":
    ...
    if conf.loop.lookback > conf.loop.reset:
        raise ValueError(
            "Configuration error, can't have lookback superior to reset"
        )
