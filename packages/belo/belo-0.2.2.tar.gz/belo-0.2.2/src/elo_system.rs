...
impl EloSystem {
    ...
    /// Adds a new player to the system with a default rating of 1500.
    pub fn add_player(&mut self, id: &str, verbose: Option<bool>) -> Result<()> {
        if id.trim().is_empty() {
    ...
    pub fn record_game(&mut self, id1: &str, result: GameResult, id2: &str, verbose: Option<bool>) -> Result<()> {
    ...
    #[allow(dead_code)]
    pub fn get_info(&self, id: &str) -> Option<(String, f64, u32, u32, u32)> {
        self.players.get(id).map(|p| (p.id.clone(), p.rating.rating, p.wins, p.losses, p.ties))
    }
