...
def anayze_device_name(log_data:str):
    device_name = ""
    pattern = r"(PerfBoard .*?)$"
    for line in log_data:
        match = re.search(pattern, line)
        if match:
            device_name = line.split(" ")[-1].replace(".","").strip()
            break
    return device_name
...
def re_render_page(html_path:str, task_name:str):
    ...
    for p in soup.find_all("table"):
        p["style"] = (
            "margin: 0 auto; margin-top: 0px; width: 1200px; text-align: left"
        )
...
def get_algo_perf_page(algo_perf_log: str, task_name: str):
    ...
    device_name = anayze_device_name(lines)
    core_num= {}
    if device_name == "BM1684X":
        core_num["TPU"]=1
        core_num["CPU"]=8
        core_num["JPU"]=2
        core_num["VPU_DECODER"]=2
        core_num["VPU_ENCODER"]=1
        core_num["VPP"]=2
    elif device_name == "BM1688":
        core_num["TPU"]=2
        core_num["CPU"]=8
        core_num["JPU"]=4
        core_num["VPU_DECODER"]=2
        core_num["VPU_ENCODER"]=1
    elif device_name == "CV186AH":
        core_num["TPU"]=1
        core_num["CPU"]=6
        core_num["JPU"]=4
        core_num["VPU_DECODER"]=2
    ...
    if(len(core_num) > 0):
        perf_info.add(
            get_basic_info_table_cols(core_num,"核心数量"),
        )
