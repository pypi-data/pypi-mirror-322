...
def format_readable_output(blocks: List[Dict]) -> DocumentContent:
    """Format blocks into structured content with geometric validation"""
    try:
        queries = []
        form_fields = []
        lines = []
        ...
        # Process QUERY blocks with geometric validation
        for block in blocks:
            if block["BlockType"] == "QUERY":
                query = block.get("Query", {})
                query_text = query.get("Text", "")
                query_alias = query.get("Alias", "")

                # Get answer through relationships only
                answer_from_rel = None
                if "Relationships" in block:
                    for rel in block["Relationships"]:
                        if rel["Type"] == "ANSWER":
                            for answer_id in rel["Ids"]:
                                ...
                                if answer_block:
                                    # Check if Geometry exists before trying to find matching blocks
                                    matching_blocks = []
                                    if "Geometry" in answer_block:
                                        matching_blocks = (
                                            find_blocks_with_matching_geometry(
                                                answer_block["Geometry"], blocks
                                            )
                                        )

                                    # Get highest confidence among matching blocks or use answer block confidence
                                    max_confidence = max(
                                        (b["confidence"] for b in matching_blocks),
                                        default=answer_block.get("Confidence", 0.0),
                                    )
                                    answer_from_rel = {
                ...
                # Choose the best answer and include matching blocks info
                answer_text = ""
                answer_source = "none"
                matching_blocks_info = []

                if answer_from_rel:
                    answer_text = answer_from_rel["text"]
                    # Get the original QUERY_RESULT confidence
                    original_confidence = answer_block.get(
                        "Confidence", 0.0
                    )  # This will be 37.00
                    answer_source = "relationship"
                    matching_blocks_info = answer_from_rel.get("matching_blocks", [])
                    # Calculate accumulated score from all matching blocks
                    accumulated_score = (
                        sum(b["confidence"] for b in matching_blocks_info)
                        / len(matching_blocks_info)
                        if matching_blocks_info
                        else 0.0
                    )
                    ...
                    queries.append(
                        {
                            ...
                            "answer": answer_text,
                            "alias": query_alias,
                            "confidence": original_confidence,  # Use original confidence (37.00)
                            "accumulated_score": accumulated_score,  # Average of all blocks (including 99.82%)
                        ...
                        if rel["Type"] == "VALUE":
                            for value_id in rel["Ids"]:
                                ...
                                if value_block:
                                    ...
                                    # Find all blocks with matching geometry
                                    matching_blocks = (
                                        find_blocks_with_matching_geometry(
                                    ...
                                    # Find LINE block containing this key-value pair
                                    ...
                                    original_confidence = value_block.get(
                                        "Confidence", 0.0
                                    )
                                    accumulated_score = (
                                        sum(b["confidence"] for b in matching_blocks)
                                        / len(matching_blocks)
                                        if matching_blocks
                                        else 0.0
                                    )
        ...
        for block in blocks:
            if block["BlockType"] == "TABLE":
                table_data = parse_table(block, blocks)
...
def find_text_by_geometry(
    ...
    This function analyzes spatial relationships between text blocks to identify
    nearby text elements. It uses both Euclidean distance and polygon overlap
    calculations for accurate proximity detection.

    Args:
        block (Dict): Reference block containing:
            - Geometry: Block's geometric information
                - BoundingBox: Box coordinates
                - Polygon: Detailed shape points
            - BlockType: Type of the block
            - Id: Unique identifier
        blocks (List[Dict]): All blocks to search through
        max_distance (float): Maximum Euclidean distance to consider (default: 0.05)
            - Distance is normalized to page dimensions (0-1 scale)
            - Smaller values = stricter proximity requirement

    Returns:
        List[Dict[str, Any]]: List of nearby text blocks, each containing:
            - text: The text content
            - block_type: Type of block (WORD/LINE)
            - confidence: Detection confidence (0-100)
            - position: Relative position (LEFT/RIGHT/ABOVE/BELOW/OVERLAP)
            - distance: Euclidean distance from reference block
            - polygon_overlap: Overlap score if blocks intersect
            - geometry: Detailed geometric information
            - block_id: Unique identifier

    Notes:
        - Results are sorted by proximity (closest first)
        - Overlapping blocks take precedence
        - Distances are normalized to page dimensions
        - Position is determined by center point relationships

    Example:
        >>> reference_block = {"Geometry": {...}, "BlockType": "KEY"}
        >>> nearby = find_text_by_geometry(reference_block, all_blocks)
        >>> for text in nearby:
        ...     print(f"{text['text']} ({text['position']}: {text['distance']:.3f})")
    """
