...
def outline(img,df,w=1):
    for i in df.index.to_list():
        ...
        cv2.drawContours(img, contours, -1, (255,255,255), w)
    return img    
...
def outline_mask_color(path_img,path_mask,algo,min_size,max_size):
    if os.path.isdir("./segmentation_quality/outline_color")==False:
        os.mkdir("./segmentation_quality/outline_color")
    if os.path.isdir("./segmentation_quality/outline_color/"+algo)==False:
        os.mkdir("./segmentation_quality/outline_color/"+algo)
    ...
    for file_img,file_mask in zip(os.listdir(path_img),os.listdir(path_mask+"/"+algo)):
        ...
        cv2.imwrite("./segmentation_quality/outline_color/"+algo+"/"+file_img[:-4]+".png",normalize_255(img))
...
def distribution_number_object(path_img="./img",path_mask="./mask",path_file="./segmentation_quality/",dico_algo_min={},dico_algo_max={},title=""):
    ...
    for k in dico_algo_nb.keys():
        list_mean.append(np.mean(dico_algo_nb[k]))
    plt.figure()
    x_axis=np.arange(len(dico_algo_nb.keys()))
    width=0.8/(len(dico_algo_nb.keys()))
    ...
    plt.xticks(x_axis,dico_algo_nb.keys(),rotation=60)
    plt.savefig(path_file+"mean_"+title)


#Make an histograme of the size of the object in the ground truth mask
def size_distribution(path_mask="./mask",dico_algo_min={},dico_algo_max={},title="size_distribution.png"):
    if os.path.isdir("./segmentation_quality")==False:
        os.mkdir("./segmentation_quality")
    dico_algo_size={}
    for algo in os.listdir(path_mask):
    ...
    plt.savefig("./segmentation_quality/"+title)
...
def calcul_max_size(path_img,path_mask,step=0):
    if os.path.isdir("./segmentation_quality")==False:
        os.mkdir("./segmentation_quality")
    dico_algo_size_iou={}
    for algo in os.listdir(path_mask):
    ...
    plt.savefig("./segmentation_quality/IOU_max_size.png")
    return dico_algo_best

def calcul_min_size(path_img,path_mask,step=100):
    if os.path.isdir("./segmentation_quality")==False:
        os.mkdir("./segmentation_quality")
    dico_algo_size_iou={}
    for algo in os.listdir(path_mask):
    ...
    plt.legend()
    plt.savefig("./segmentation_quality/IOU_min_size.png")
    return dico_algo_best

#calcul the auc
#make a plot of the precision according the IOU
def calcul_auc(path_img,path_mask,dict_min_size={},dict_max_size={}):
    if os.path.isdir("./segmentation_quality")==False:
        os.mkdir("./segmentation_quality")
    dico_iou={}
    ...
    plt.xlim(0)
    plt.legend(fontsize=20)
    plt.savefig("./segmentation_quality/precision_iou.png")
    return dico_algo_ap
...
def script_ground_truth(path_img,path_mask,threshold,step):
 ...
 with open("./summary.txt",'w') as f:
    ...
    size_distribution(path_mask,title="size_distribution.png")

    distribution_number_object(path_img=path_img,path_mask=path_mask,title="number_objects.png")

    #iou calculation without filter
    f.write("Evaluation of the algorithm without size filter")
    f.write("\n")
    for algo in os.listdir(path_mask):
        if algo!="truth":
            list_iou=[]
            ...
            f.write(algo)
            f.write("*"*50)
            f.write("Mean IOU= "+str(np.mean(list_iou))+" (without size filter)")
            f.write("Average precision: "+str(dico_algo_ap[algo]))
            f.write("")
            f.write("Number of predicted objects: "+str(np.sum(list_true_pos)+np.sum(list_false_pos)))
            f.write("Real object number: "+str(np.sum(list_true_pos)+np.sum(list_false_neg)))
            #recall, precision, true positives
            f.write("")
            f.write("For an IOU threshold= "+str(threshold)+" :")
            f.write("   Recall= "+str(np.mean(list_recall)))
            f.write("   Precision= ",str(np.mean(list_precision)))
            f.write("   Number of true positive= ",str(np.sum(list_true_pos)))
            f.write("   Number of false positive= ",str(np.sum(list_false_pos)))
            f.write("   Number of false negative= ",str(np.sum(list_false_neg)))
            f.write("")

    #calculation of the minimum and maximum size of the predicted mask objects that must be filtered
    #max_size,iou=
    dico_algo_max=calcul_max_size(path_img,path_mask,step)
    #calculation of the minimum and maximum size of the predicted mask objects that must be filtered
    dico_algo_min=calcul_min_size(path_img,path_mask,step)

    dico_algo_ap=calcul_auc(path_img,path_mask,dico_algo_min,dico_algo_max)

    #distribution of the size of mask objects with size filter
    size_distribution(path_mask,dico_algo_min,dico_algo_max,title="size_distribution_filtered.png")
    distribution_number_object(path_img=path_img,path_mask=path_mask,dico_algo_max=dico_algo_max,dico_algo_min=dico_algo_min,title="number_objects_filtered.png")

    f.write("\n"*3)
    f.write("Evaluation of the algorithm with a size filter")
    f.write("\n")
    for algo in os.listdir(path_mask):
        if algo!="truth":
            list_iou=[]
            ...
            outline_iou("./segmentation_quality/outline_iou/"+algo+"/"+file_img[:-4]+".png",file_img[:-4]+".png",algo,df_truth,threshold,c=255)

            #Display of predicted masks colored and the outline of the ground truth
            outline_mask_color(path_img,path_mask,algo,dico_algo_min[algo],dico_algo_max[algo])
            f.write("Algorithme: "+algo)
            f.write("Best IOU= "+str(np.mean(list_iou))+" by filtering objects <"+str(dico_algo_min[algo])+" pixels and >"+str(dico_algo_max[algo]))
            f.write("Average precision: "+str(dico_algo_ap[algo]))
            f.write("")
            f.write("Number of predicted objects: "+str(np.sum(list_true_pos)+np.sum(list_false_pos)))
            f.write("Real object number: "+str(np.sum(list_true_pos)+np.sum(list_false_neg)))
            ...
            f.write("for a IOU threshold= "+str(threshold)+" :")
            f.write("   Recall= ",str(np.around(np.mean(list_recall),2)))
            f.write("   Precision= ",str(np.around(np.mean(list_precision),2)))
            f.write("   Number of true positive= ",str(np.sum(list_true_pos)))
            f.write("   Number of false positive= ",str(np.sum(list_false_pos)))
            f.write("   Number of false negative= ",str(np.sum(false_neg)))
            f.write("")
