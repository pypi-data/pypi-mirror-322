...
import matplotlib.pyplot as plt''',
            ...
            'bike-b':'''# Определение нейронной сети с BatchNorm1d
class BikeDemandModelWithBatchNorm(nn.Module):
    def __init__(self, input_dim):
        super(BikeDemandModelWithBatchNorm, self).__init__()
        self.fc = nn.Sequential(
            nn.Linear(input_dim, 64),
            nn.BatchNorm1d(64),  # Добавляем BatchNorm1d после первого полносвязного слоя
            nn.ReLU(),
            nn.Linear(64, 32),
            nn.BatchNorm1d(32),  # Добавляем BatchNorm1d после второго полносвязного слоя
            nn.ReLU(),
            nn.Linear(32, 1)
        )
...
# Параметры обучения с новой моделью
model_with_bn = BikeDemandModelWithBatchNorm(input_dim)
...
for epoch in range(epochs):
    ...
    for batch_X, batch_y in train_loader:
        optimizer.zero_grad()
        predictions = model_with_bn(batch_X)
    ...
    train_losses_with_bn.append(epoch_loss)

    # Оценка на тестовом наборе
    model_with_bn.eval()
    with torch.no_grad():
        test_predictions = model_with_bn(X_test)
        test_loss = criterion(test_predictions, y_test).item()
    test_losses_with_bn.append(test_loss)

    if (epoch + 1) % print_every == 0:
        print(f'With BatchNorm - Epoch {epoch+1}/{epochs}, Train Loss: {epoch_loss:.4f}, Test Loss: {test_loss:.4f}')

# Результаты на обучающем наборе
with torch.no_grad():
    y_train_pred_bn = model_with_bn(X_train).numpy()
train_mse_bn = mean_squared_error(y_train_true, y_train_pred_bn)
train_mae_bn = mean_absolute_error(y_train_true, y_train_pred_bn)
train_r2_bn = r2_score(y_train_true, y_train_pred_bn)

# Результаты на тестовом наборе
with torch.no_grad():
    y_test_pred_bn = model_with_bn(X_test).numpy()
test_mse_bn = mean_squared_error(y_test_true, y_test_pred_bn)
test_mae_bn = mean_absolute_error(y_test_true, y_test_pred_bn)
test_r2_bn = r2_score(y_test_true, y_test_pred_bn)
...
print("С BatchNorm:")
print(f"Train - MSE: {train_mse_bn:.4f}, MAE: {train_mae_bn:.4f}, R2: {train_r2_bn:.4f}")
print(f"Test - MSE: {test_mse_bn:.4f}, MAE: {test_mae_bn:.4f}, R2: {test_r2_bn:.4f}")

# Построение графиков
plt.figure(figsize=(10, 6))
plt.plot(range(1, epochs + 1), train_losses, label="Train Loss (No BatchNorm)")
plt.plot(range(1, epochs + 1), test_losses, label="Test Loss (No BatchNorm)", linestyle='--')
plt.plot(range(1, epochs + 1), train_losses_with_bn, label="Train Loss (With BatchNorm)")
plt.plot(range(1, epochs + 1), test_losses_with_bn, label="Test Loss (With BatchNorm)", linestyle='--')
plt.xlabel("Epochs")
plt.ylabel("Loss")
plt.title("Сравнение процесса обучения с и без BatchNorm")
plt.legend()
plt.show()''',
...
plt.show()''',
            ...
            'gold-b':'''# Определение нейронной сети с BatchNorm1d
class GoldPredictorModelWithBatchNorm(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(GoldPredictorModelWithBatchNorm, self).__init__()
        self.fc = nn.Sequential(
            nn.Linear(input_dim, 64),
            nn.BatchNorm1d(64),  # BatchNorm1d после первого слоя
            nn.ReLU(),
            nn.Linear(64, 32),
            nn.BatchNorm1d(32),  # BatchNorm1d после второго слоя
            nn.ReLU(),
            nn.Linear(32, output_dim)
        )
...
# Параметры обучения для модели с BatchNorm
model_with_bn = GoldPredictorModelWithBatchNorm(input_dim, output_dim)
criterion = nn.MSELoss()
optimizer = torch.optim.Adam(model_with_bn.parameters(), lr=0.001)
...
for epoch in range(epochs):
    ...
    train_losses_with_bn.append(epoch_loss / len(train_loader))
    ...
    if (epoch + 1) % print_every == 0:
        print(f'With BatchNorm - Epoch {epoch+1}/{epochs}, Train Loss: {epoch_loss / len(train_loader):.4f}, Test Loss: {test_loss:.4f}')

# Оценка модели с BatchNorm
with torch.no_grad():
    y_train_pred_bn = scaler_y.inverse_transform(model_with_bn(X_train).numpy())
    y_train_true_bn = scaler_y.inverse_transform(y_train.numpy())
    y_test_pred_bn = scaler_y.inverse_transform(model_with_bn(X_test).numpy())
    y_test_true_bn = scaler_y.inverse_transform(y_test.numpy())

train_mse_bn = mean_squared_error(y_train_true_bn, y_train_pred_bn)
train_mae_bn = mean_absolute_error(y_train_true_bn, y_train_pred_bn)
train_r2_bn = r2_score(y_train_true_bn, y_train_pred_bn)
test_mse_bn = mean_squared_error(y_test_true_bn, y_test_pred_bn)
test_mae_bn = mean_absolute_error(y_test_true_bn, y_test_pred_bn)
test_r2_bn = r2_score(y_test_true_bn, y_test_pred_bn)
...
plt.xlabel("Epochs")
plt.ylabel("Loss")
plt.title("Сравнение обучения с и без BatchNorm")
plt.legend()
plt.show()''',
...
print("Confusion Matrix (Тестовая выборка):")
print(confusion_matrix(y_test.numpy(), y_test_pred_class.numpy()))''',
            'bank-b':'''# Определение модели с BatchNorm
class ChurnModelWithBN(nn.Module):
    def __init__(self, input_size):
        super(ChurnModelWithBN, self).__init__()
        self.fc1 = nn.Linear(input_size, 128)
        self.bn1 = nn.BatchNorm1d(128)  # BatchNorm слой
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(p=0.5)
        self.fc2 = nn.Linear(128, 64)
        self.bn2 = nn.BatchNorm1d(64)  # BatchNorm слой
        self.fc3 = nn.Linear(64, 1)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        x = self.fc1(x)
        x = self.bn1(x)  # Применение BatchNorm
        x = self.relu(x)
        x = self.dropout(x)
        x = self.fc2(x)
        x = self.bn2(x)  # Применение BatchNorm
        x = self.relu(x)
        x = self.dropout(x)
        x = self.fc3(x)
        return self.sigmoid(x)
...
# Определение модели, функции потерь и оптимизатора
input_size = X_train.shape[1]
model_bn = ChurnModelWithBN(input_size)
criterion = nn.BCELoss()
...
# Цикл обучения модели с BatchNorm
for epoch in range(epochs):
    model_bn.train()
    epoch_loss = 0
    ...
    for batch_X, batch_y in train_loader:
        ...
        outputs = model_bn(batch_X).squeeze()
        loss = criterion(outputs, batch_y)
        loss.backward()
        optimizer_bn.step()
        epoch_loss += loss.item()
    ...
    average_train_loss = epoch_loss / len(train_loader)
    train_losses_bn.append(average_train_loss)

    # Оценка на тестовом наборе
    model_bn.eval()
    with torch.no_grad():
        test_outputs = model_bn(X_test).squeeze()
        test_loss = criterion(test_outputs, y_test).item()
        test_losses_bn.append(test_loss)

    if (epoch + 1) % print_every == 0:
        print(f'[BatchNorm] Epoch {epoch + 1}/{epochs}, Train Loss: {average_train_loss:.4f}, Test Loss: {test_loss:.4f}')

# Построение графиков потерь
plt.figure(figsize=(12, 6))
plt.plot(range(1, epochs + 1), train_losses_bn, label="Train Loss with BatchNorm")
plt.plot(range(1, epochs + 1), test_losses_bn, label="Test Loss with BatchNorm", linestyle='--')
plt.xlabel("Epochs")
plt.ylabel("Loss")
plt.title("График потерь с BatchNorm")
plt.legend()
plt.show()
...
with torch.no_grad():
    ...
    y_test_pred = model_bn(X_test).squeeze()
    y_test_pred_class = (y_test_pred >= 0.5).float()

    # Предсказания на обучающей выборке
    y_train_pred = model_bn(X_train).squeeze()
    y_train_pred_class = (y_train_pred >= 0.5).float()

# Вывод результатов для обучающей выборки
print("Classification Report (Обучающая выборка):")
print(classification_report(y_train.numpy(), y_train_pred_class.numpy()))
print("Confusion Matrix (Обучающая выборка):")
print(confusion_matrix(y_train.numpy(), y_train_pred_class.numpy()))
...
print(f"Test F1 Score - Color: {test_color_f1:.4f}, Item: {test_item_f1:.4f}")''',
            ...
            'sign-graph':'''transform = transforms.Compose([
...
plt.show()''',
            ...
            'sign-pca':'''# 1. Загрузка данных
data_dir = "/content/sign_language"
transform = transforms.Compose([
...
# 4. Обучение модели
def train_model(num_blocks):
    model = CNN(num_blocks, num_classes).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=0.001)
...
def evaluate_model(model):
    ...
    return micro_f1, classification_report(y_true, y_pred, target_names=full_dataset.classes), cm

# 6. Проведение экспериментов
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
results = []

for blocks in range(3, 4):
    print(f"Training with {blocks} convolutional blocks...")
    model, train_losses = train_model(blocks)
    micro_f1, report, cm = evaluate_model(model)
    results.append((blocks, micro_f1))

    # График функции потерь
    plt.plot(train_losses, label=f"{blocks} blocks")

    # Вывод матрицы ошибок
    plt.figure(figsize=(10, 8))
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", xticklabels=full_dataset.classes, yticklabels=full_dataset.classes)
    plt.title(f"Confusion Matrix for {blocks} Blocks")
...
# График функции потерь для всех моделей
...
plt.plot(blocks, scores, marker='o')
plt.title("F1 Score vs Number of Convolutional Blocks")
plt.xlabel("Number of Convolutional Blocks")
plt.ylabel("Micro F1 Score")
plt.grid()
plt.show()

# 8. Визуализация скрытых представлений через PCA
model.eval()
features, labels = [], []
