...
from agptools.containers import walk, soft, overlap, flatten, collect, exclude_dict
...
#                 break  # generate a edge
    ...
    async def dispatch(self, edge):
        ...
        ikeys = set(
            [k for k in edge if k.endswith("__") or not isinstance(edge[k], (List, Dict))]
        )
...
class XParticle(iGeo, Particle):
    ...
    LAMDAS = set( ["max", "min", "average", "freq"]) # TODO: instrospection
        ...
        for lambda_name in re.findall(r"\w+", func_names):
            for prefix in "pre", "post":
                ...
                if func:
                    ...
                    self._lambdas[prefix].append(func)
    ...
    # lambda methods
    # --------------------------------------------------------
    def _pre_average(self, key, old_value, new_value, edge):
        if old_value is None:
            ...
            if isinstance(new_value, dict):
                # TODO: agp: use loop with operator
                _aux = old_value.setdefault("average", {})
                for attr, value in new_value.items():
                    M = self._cached_M.get(attr) or self._find_M(attr)
                    # ignore any other scaffolding values
                    _new_value = new_value[attr]

                    _old_value = _aux.setdefault(attr, _new_value)
                    _old_value[0] += (_new_value[0] - _old_value[0]) / M
    ...
    def _pre_average_old(self, key, old_value, new_value, edge):
        ...
        if isinstance(item, Dict):
            for _key, _new_value in item.items():
                M = self._cached_M.get(_key) or self._find_M(_key)
                if isinstance(_new_value, list):
                    # ignore any other scaffolding values
                    _new_value = _new_value[0]
                _old_value = _old_item.setdefault(_key, _new_value)
                _old_item[_key] += (_new_value - _old_value) / M
        else:
            M = self._cached_M.get(key) or self._find_M(key)
            old_value += (item - _old_item) / M
        return old_value

    def _pre_max(self, key, old_value, new_value, edge):
        if old_value is None:
            old_value = {
                "max": new_value,
            }
            ...
            if isinstance(new_value, dict):
                # TODO: agp: use loop with operator
                _max = old_value.setdefault("max", {})
                for attr, value in new_value.items():
                    if _max.setdefault(attr, new_value[attr])[0] < new_value[attr][0]:
                        _max[attr] = new_value[attr]
    ...
    def _pre_min(self, key, old_value, new_value, edge):
        if old_value is None:
            old_value = {
                "min": new_value,
            }
            ...
            if isinstance(new_value, dict):
                # TODO: agp: use loop with operator
                _min = old_value.setdefault("min", {})
                for attr, value in new_value.items():
                    if _min.setdefault(attr, new_value[attr])[0] > new_value[attr][0]:
                        _min[attr] = new_value[attr]
    ...
    def _pre_sum(self, key, old_value, new_value, edge):
        if old_value is None:
            old_value = {
                "sum": new_value,
            }
            ...
            if isinstance(new_value, dict):
                # TODO: agp: use loop with operator
                _min = old_value.setdefault("sum", {})
                for attr, value in new_value.items():
                    _min[attr][0] += new_value[attr][0]
                    # TODO: check
                    # _min[attr][1:]= new_value[attr][1:]

            elif isinstance(new_value, list):
                old_value["sum"][0] += new_value[0]
                # TODO: check
                # old_value[attr][1:]= new_value[attr][1:]

        return old_value

    def _pre_freq(self, key, old_value, new_value, edge):
        """
        collect all data by attribute
        so we can create a historogram later on
        """
        if old_value is None:
            old_value = self._last_output.setdefault(key, {})

        if isinstance(new_value, dict):
            _aux = old_value.setdefault("freq", {})
            for attr, value in new_value.items():
                _aux.setdefault(attr, []).append(new_value[attr][0])

        elif isinstance(new_value, list):
            if old_value["freq"][0] < new_value[0]:
                old_value["freq"] = new_value

        return old_value

    def _post_freq(self, key, values, edge):
        """
        create a historogram

        import pandas as pd
        import numpy as np

        # Example data
        data = np.random.randint(1, 100, 50)  # Generate 50 random integers between 1 and 100
        df = pd.DataFrame({'values': data})

        # Define bins (e.g., 5 bins)
        ...
        max_frequency = bin_counts.max()
        bin_with_max_frequency = bin_counts.idxmax()

        print("Bin with the highest frequency:", bin_with_max_frequency)
        print("Highest frequency:", max_frequency)

        """
        if isinstance(values, dict):
            _aux = values.setdefault("freq", {})
            for attr, samples in list(values.items()):
    ...
    async def _wip_feed(self, buffer, context):
        ...
        for wave__, uid, data in self._ordered_streaming(buffer):
            ...
            _data = stream.pop(0)
            assert id(data) == id(_data)  # TODO: REMOVE check
            log.debug("stream: [%s], %s", len(stream), data)
            if point := await self._wip_build_point(data):
                self._wip_add_point(uid, point)
        ...
        *: <geo_key_hash__>, <value_values>
        ...
        for keys, new_values in universe:
            ...
            for func in lambdas:
                ...
                value = func(keys, old_values, value, edge)
