...
class XParticle(iGeo, Particle):
    ...
    def __init__(self, uid, sources, broker, storage, **kw):
        ...
        for lambda_name in re.findall(r"\w+", func_names):
            func = getattr(self, f"_calc_{lambda_name}", None)
            if func:
    ...
    def _calc_min(self, key, old_value, new_value, edge):
        if old_value is None:
            old_value = {
                "min": new_value,
            }
    ...
    def _calc_max_freq(self, key, old_value, new_value, edge):
        raise NotImplementedError()
        if old_value is None:
            old_value = {
                "max": new_value,
    ...
    def _calc_min_freq(self, key, old_value, new_value, edge):
        raise NotImplementedError()
        if old_value is None:
            old_value = {
                "max": new_value,
