...
class XParticle(iGeo, Particle):
    ...
    def _pre_max(self, key, old_value, new_value, edge):
        ...
        if isinstance(new_value, dict):
            # TODO: agp: use loop with operator
            aux: Dict = old_value.setdefault("max", {})
            for attr, value in new_value.items():
                if aux.setdefault(attr, new_value[attr])[0] < new_value[attr][0]:
    ...
    def _pre_min(self, key, old_value, new_value, edge):
        ...
        if isinstance(new_value, dict):
            # TODO: agp: use loop with operator
            aux: Dict = old_value.setdefault("min", {})
            for attr, value in new_value.items():
                if aux.setdefault(attr, new_value[attr])[0] > new_value[attr][0]:
                    aux[attr] = new_value[attr]
        else:
            ...
            for key in self.available_lambdas.intersection(data):
                ...
                if isinstance(sub_data, list):
                    ...
                    for lambda_mapper in self._lambda_mapper:
                        ...
                        attr = mapping_keys.get(attr, attr)
                    value, ts, topic = sub_data[:3]
