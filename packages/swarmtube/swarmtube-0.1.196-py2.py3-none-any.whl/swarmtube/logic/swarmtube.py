...
class XParticle(iGeo, Particle):
    ...
    def _pre_average(self, key, old_value, new_value, edge):
        ...
        new_value
        [27.9, '2024-07-09 14:39:38+00:00', 'T']
        ...
        if isinstance(new_value, dict):
            ...
            aux: Dict = old_value.setdefault("average", {})
            for attr, value in new_value.items():
                ...
                _old_value = aux.setdefault(attr, _new_value)
                _old_value[0] += (_new_value[0] - _old_value[0]) / M
        else:
            raise RuntimeError(f"Lambda can't handle this value: {new_value}")
    ...
    def _pre_sum(self, key, old_value, new_value, edge):
        ...
        if isinstance(new_value, dict):
            # TODO: agp: use loop with operator
            aux: Dict = old_value.setdefault("sum", {})
            for attr, value in new_value.items():
                if attr in aux:
                    aux[attr][0] += new_value[attr][0]
                else:
                    aux[attr] = new_value[attr]
    ...
    def _pre_freq(self, key, old_value: Dict, new_value, edge):
        """
        ...
        if isinstance(new_value, dict):
            aux: Dict = old_value.setdefault("freq", {})
            for attr, value in new_value.items():
                aux.setdefault(attr, []).append(new_value[attr][0])
        else:
            raise RuntimeError(f"Lambda can't handle this value: {new_value}")
    ...
    async def _compute(self, edge, ekeys):
        ...
        async for data in super()._compute(edge, ekeys):
            ...
            for key in self.available_lambdas.intersection(data):
                ...
                # 1st structural transformations
                if isinstance(sub_data, list):
                    ...
                    for lambda_mapper in self._lambda_mapper:
                        mapping_keys = lambda_mapper.mapping_keys({}, only_defined=True)
                        sub_data[-1] = mapping_keys.get(sub_data[-1], sub_data[-1])
                else:
                    assert key in (
                        "average",
                        "freq",
                    ...
                    # 2nd mappers
                    for lambda_mapper in self._lambda_mapper:
                        sub_data = lambda_mapper.remap(sub_data, only_defined=True)

                # 3rd simple layout transformation
                # reflect the data in the 'root' layout
                for attr, info in sub_data.items():
                    if isinstance(info, list):
                        # TODO: use calleables
                        value, ts, topic = info[:3]
                        if key in (
                            "max",
                            "min",
                        ):
                            data[f"{attr}_{key}_value"] = value
                            data[f"{attr}_{key}_ts"] = ts
                        elif key in (
                            "average",
                            "freq",
                            "sum",
