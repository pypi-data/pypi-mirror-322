...
class CurlParser:
    ...
    def parse_curl(curl_command: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if post_data:
                method = 'post'
                # Special case: don't include data for sunexpress journeys endpoint
                if 'sunexpress.com/pricing/api/v1/journeys' in parsed_args.url:
                    data_dict = None
                    data_as_json = False
                else:
                    try:
                        ...
                        data_dict = dict(parse_qsl(post_data))
                        data_as_json = False
            else:
                method = 'get'
                data_dict = None
