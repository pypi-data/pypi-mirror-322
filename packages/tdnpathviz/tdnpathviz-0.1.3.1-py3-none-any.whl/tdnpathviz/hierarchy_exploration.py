...
class ProductHierarchySelector:
    def __init__(self, dataset, product_hierarchy, max_depth=3, figure_height=800, figure_width=600):
        ...
        hierarchical_levels = self.create_hierarchy(self.dataset, self.product_hierarchy[0:self.max_depth])
    ...
    def reset(self):
        """
        Reset the dataset to its original unfiltered state and update the hierarchy display.
        """
        self.dataset = self.unfiltered_dataset.copy()  # Restore the original dataset

        # Recreate the initial hierarchy
        hierarchical_levels = self.create_hierarchy(self.dataset, self.product_hierarchy[0:self.max_depth])

        # Create initial tiles based on the original hierarchy
        self.columns = self.create_tiles(hierarchical_levels, 0, self.figure_height, self.figure_width)
        ...
        with self.output:
            self.output.clear_output()
            print("Filter reset to original dataset.")