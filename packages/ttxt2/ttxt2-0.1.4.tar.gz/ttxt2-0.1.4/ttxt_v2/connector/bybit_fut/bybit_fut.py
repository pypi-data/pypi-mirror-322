...
from ttxt_v2.core.api.position_update import Position, PositionUpdate
...
from .bybit_fut_auth import BybitFutAuth


class BybitFut(ConnectorBase):
    """
    ...
    def _process_private_data(self, response: WSResponse) -> Optional[List[BaseEvent]]:
        ...
        try:
            ...
            if topic == self.WALLET_CHAN:
                ...
                pos_up = self.convert_exchange_position_update_to_position_update(
                    msg_data
                )
                return [
                    BaseEvent(
    ...
    def authenticator(self) -> BaseAuth:
        ...
        return BybitFutAuth(api_key=self._api_key, api_secret=self._api_secret)
        ...
        if kl_config.on:
            ...
            for ticker in kl_config.tickers:
                for frame in kl_config.timeframes:
                    ...
                    logger.debug(f"Kline sub str: {arg_str}")
                    args.append(arg_str)
                ws_reqs.append(WSJSONRequest({"op": self.SUBSCRIBE_OP, "args": args}))
    ...
    async def fetch_wallet_balance(
        ...
        logger.debug(f"wallet HTPP res: {response}")
    ...
    def convert_exchange_position_update_to_position_update(
        self, msg_data: dict
    ) -> PositionUpdate:
        positions: List[Position] = []
        for data in msg_data:
            symbol = self._symbols_to_trading_pairs_table[data.get("symbol", "")]
            side = self.convert_exchange_side_to_side(data.get("side", "Buy"))
            size = int(data.get("size", "0"))
            position_value = float(data.get("positionValue", "0.0"))
            entry_price = float(data.get("entryPrice", "0.0"))
            position_im = float(data.get("positionIM", "0.0"))
            position_mm = float(data.get("positionMM", "0.0"))
            tp_price = float(data.get("takeProfit", "0.0"))
            sl_price = float(data.get("stopLoss", "0.0"))
            unrealised_pnl = float(data.get("unrealisedPnl", "0.0"))
            cur_pnl = float(data.get("curRealisedPnl", "0.0"))
            create_time = int(data.get("createdTime", "0"))
            update_time = int(data.get("updatedTime", "0"))
            positions.append(
                Position(
                    symbol=symbol,
                    side=side,
        ...
        return PositionUpdate(positions=positions, timestamp=int(time.time()))
