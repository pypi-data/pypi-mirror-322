...
class MentionPairsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        dot_product = torch.dot(A_embedding, B_embedding).unsqueeze(0)  # Add a dimension for concatenation

        # Concatenate embeddings, dot product, and features
        X = torch.cat([A_embedding, B_embedding, dot_product, feature], dim=0)
        print(X.shape)
        return X, label
