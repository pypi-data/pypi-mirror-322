...
def handle_virustotal(change):
    ...
    report = True
    if isinstance(change["virustotal"], list) and len(change["virustotal"]) == 2:
        if change["virustotal"][1]["community_score"] == 0 and change["virustotal"][1]["total_malicious"] == 0:
            report = False
...
def handle_abuseipdb(change):
    ...
    report = True
    if isinstance(change["abuseipdb"], list) and len(change["abuseipdb"]) == 2:
