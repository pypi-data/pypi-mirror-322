...
class GMM:
    ...
    def fit(self, X):
        ...
        self.convergence_iters = torch.full((B,), -1, dtype=int)
        mean_log_prob_norm = torch.full((B,), -np.inf, dtype=self._dtype)
    ...
    def _m_step(self, X, r, N_actual, N_actual_total, converged):
        ...
        for k in range(self._n_components):
            ...
            bad_covs = errors > 0
            if bad_covs.any():
                eigvals, eigvecs = torch.linalg.eigh(self.covs[k][~converged][bad_covs].float())
