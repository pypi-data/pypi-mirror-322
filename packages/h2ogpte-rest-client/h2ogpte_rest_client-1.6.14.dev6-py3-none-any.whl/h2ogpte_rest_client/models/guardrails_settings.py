...
class GuardrailsSettings(BaseModel):
    ...
    GuardrailsSettings
    """ # noqa: E501
    disallowed_regex_patterns: List[StrictStr] = Field(description="A list of regular expressions that match custom PII.")
    presidio_labels_to_flag: List[StrictStr] = Field(description="A list of entities to be flagged as PII by the built-in Presidio model.")
    pii_labels_to_flag: List[StrictStr] = Field(description="A list of entities to be flagged as PII by the built-in PII model.")
    pii_detection_parse_action: StrictStr = Field(description="What to do when PII is detected during parsing of documents. The 'redact' option will replace disallowed content in the ingested documents with redaction bars.")
    pii_detection_llm_input_action: StrictStr = Field(description="What to do when PII is detected in the input to the LLM (document content and user prompts). The 'redact' option will replace disallowed content with placeholders.")
    pii_detection_llm_output_action: StrictStr = Field(description="What to do when PII is detected in the output of the LLM. The 'redact' option will replace disallowed content with placeholders.")
    prompt_guard_labels_to_flag: List[StrictStr] = Field(description="A list of entities to be flagged as safety violations in user prompts by the built-in prompt guard model.")
    guardrails_labels_to_flag: List[StrictStr] = Field(description="A list of entities to be flagged as safety violations in user prompts. Must be a subset of guardrails_entities, if provided.")
    ...
    __properties: ClassVar[List[str]] = ["disallowed_regex_patterns", "presidio_labels_to_flag", "pii_labels_to_flag", "pii_detection_parse_action", "pii_detection_llm_input_action", "pii_detection_llm_output_action", "prompt_guard_labels_to_flag", "guardrails_labels_to_flag", "guardrails_entities"]
