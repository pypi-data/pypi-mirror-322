...
def summary(
    ...
    layer_width = max(len(attr) for attr in module_names) + 3
    ...
    # print summary
    for i, (module_name, input_shape, output_shape) in enumerate(
        zip(module_names, input_shapes, output_shapes)
    ):
        print(f"{f'({i})':<{idx_width}}{module_name:<{layer_width}}{input_shape}")
        print(f"{' ' * idx_width}{f'({module_name})':<{layer_width}}-> {output_shape}")
        print("-" * (idx_width + layer_width + input_output_width))
