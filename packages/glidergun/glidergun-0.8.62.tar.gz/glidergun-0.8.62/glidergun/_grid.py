...
class Grid(GridCore, Interpolation, Prediction, Focal, Zonal):
    ...
    def density(self, points: Optional[Sequence[Tuple[float, float]]] = None):
        if points is None:
            points = [(p.x, p.y) for p in self.to_points()]
        return density(points, self.extent, self.crs, self.cell_size)

    def distance(self, points: Optional[Sequence[Tuple[float, float]]] = None):
        if points is None:
            points = [(p.x, p.y) for p in self.to_points()]
        return distance(points, self.extent, self.crs, self.cell_size)
    ...
    def interp_idw(
        ...
        return idw(points, self.extent, self.crs, cell_size or self.cell_size, radius)
