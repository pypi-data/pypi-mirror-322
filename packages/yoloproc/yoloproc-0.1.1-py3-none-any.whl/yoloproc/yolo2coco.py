...
def convert_yolo_to_coco(image_dir: str,yolo_class_txt:Optional[str]=None):
    ...
    if not yolo_class_txt:
        yolo_class_txt = osp.join(yolo_label_dir,'classes.txt')
        if not osp.exists(yolo_class_txt):
            raise Exception("you must identity classes.txt ")
    ...
    skipped = 0
    converted = 0
    for idx, image in enumerate(tqdm(indices, desc="Converting")):
