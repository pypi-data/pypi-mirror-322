...
from yta_multimedia.audio.edition.audio_frame_editor import AudioFrameEditor
...
class SubClip:
    ...
    _resizes: list[float] = None
    _positions: list[float] = None
    _rotations: list[float] = None
    ...
    def __init__(self, video: Clip):
        ...
        self._resizes = [1 for _ in range(video.n_frames)]
        self._rotations = [0 for _ in range(video.n_frames)]
        # TODO: Use constants
        self._positions = [(1920 / 2, 1080 / 2) for _ in range(video.n_frames)]
    ...
    def number_of_frames(self):
        ...
        # the warnings
        # TODO: Use SMALL_FIX_AMOUNT general const
    ...
    def number_of_audio_frames(self):
        ...
        """
        # TODO: Use SMALL_FIX_AMOUNT general const
        return int(self.video.audio.duration * self.video.audio.fps + 0.000001)
    ...
    def set_volume(self, start: float, end: Union[float, None] = None, rate_function: mRateFunction = mRateFunction.LINEAR):
        """
    ...
    def _set_attribute(self, attribute: str, start: int, end: Union[int, None] = None, rate_function: mRateFunction = mRateFunction.LINEAR):
        setattr(
            self,
            attribute,
    ...
    def _apply_speed_factor(self, video):
        ...
        def _get_frame_ts_applying_speed_factors(self: SubClip):
            ...
            while current_frame_index < self.number_of_frames:
                ...
                if current_speed_factor < 1:
                    ...
                    resizes.extend([self._resizes[current_frame_index]] * times_to_append)
                    positions.extend([self._positions[current_frame_index]] * times_to_append)
                    rotations.extend([self._rotations[current_frame_index]] * times_to_append)
                    ...
                    # Any other array that also has to be recalculated
                    resizes.append(self._resizes[current_frame_index])
                    positions.append(self._positions[current_frame_index])
                    rotations.append(self._rotations[current_frame_index])
        ...
        #video = video.with_duration(len(final_video_frame_ts) * 1 / video.fps + 0.000001)
    ...
    def _process(self):
        ...
        resizes = self._resizes.copy()
        positions = self._positions.copy()
        rotations = self._rotations.copy()
        ...
        if self.zoom is not None and self.zoom != 1:
            resizes = [resize * self.zoom for resize in resizes]
        ...
        video = video.rotated(lambda t: rotations[video_handler.frame_time_to_frame_index(t, video_handler.fps)], expand = True)
        ...
        """
        # TODO: Here I am getting the rotations that are
        # different from the first one to build a frame
        # size array so I can use that array when 
        # processing the position and it runs fast but
        # correctly
        frame = video.get_frame(0)
        frame_sizes = [
            (frame.shape[1], frame.shape[0]) for _ in range(len(rotations))
        ]
        for index, rotation in enumerate(rotations):
            # If rotation or resize is different we need to
            # recalculate it
            if rotation != rotations[0] or resizes != resizes[0]:
                frame = video.get_frame(video_handler.frame_index_to_frame_time(index, self.fps))
                frame_sizes[index] = frame.shape[1], frame.shape[0]

        def with_position(t):
            # This is very slow but allows me to obtain each
            # frame size after resizing and rotating it.
            # I'm not using it now because if I have the
            # first frame size it is enough
            #frame = video.get_frame(t)
            #frame_size = frame.shape[1], frame.shape[0]
            current_frame_index = video_handler.frame_time_to_frame_index(t, video_handler.fps)
            
            # Adjust position to its upper left corner
            upper_left_corner_position = [
                positions[current_frame_index][0] - frame_sizes[current_frame_index][0] / 2,
                positions[current_frame_index][1] - frame_sizes[current_frame_index][1] / 2
            ]
    ...
    def copy(self):
        ...
        copy._color_temperature = self._color_temperature.copy()
        copy._brightness = self._brightness.copy()
        copy._contrast = self._contrast.copy()
        copy._sharpness = self._sharpness.copy()
        copy._white_balance = self._white_balance.copy()
        copy._speed_factor = self._speed_factor.copy()

        copy._zoom = self._zoom.copy()
        copy._x_movement = self._x_movement.copy()
        copy._y_movement = self._y_movement.copy()
        copy._rotation = self._rotation.copy()

        copy._resizes = self._resizes.copy()
        copy._rotations = self._rotations.copy()
...
def _validate_x_movement(x_movement: int):
    # TODO: Use constant
    if not NumberValidator.is_number_between(x_movement, -1920 * 4, 1920 * 4):
        raise Exception(f'The "x_movement" parameter provided is not a number between [{-1920 * 4}, {1920 * 4}].')
    
def _validate_y_movement(y_movement: int):
    # TODO: Use constant
    if not NumberValidator.is_number_between(y_movement, -1920 * 4, 1920 * 4):
        raise Exception(f'The "y_movement" parameter provided is not a number between [{-1920 * 4}, {1920 * 4}].')

def _validate_rotation(rotation: int):
    if not NumberValidator.is_number_between(rotation, -360, 360):
        raise Exception(f'The "rotation" parameter provided is not a number between [-360, 360].')
