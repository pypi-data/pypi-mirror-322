...
class WithPositionArgument:
    ...
    def __init__(self, argument: Union[tuple[int, int], 'Map'], extra_argument: any):
        ...
        if (
            (not PythonValidator.is_tuple(argument) and len(argument) != 2) and
    ...
    def get_values(self, n: int):
        ...
        if PythonValidator.is_instance(self.argument, 'Map'): return self.argument.get_values(n)
...
class RotatedArgument:
    ...
    def get_values(self, n: int):
        ...
        if PythonValidator.is_number(self.argument): return [self.argument] * n
        if PythonValidator.is_instance(self.argument, 'SubClipSetting'): return self.argument.get_values(n)
        if PythonValidator.is_instance(self.argument, 'Graphic'): return [self.argument.get_xy_from_normalized_d(d)[1] for d in Progression(0, 1, n).values]
...
class ResizedArgument:
    ...
    def get_values(self, n: int):
        ...
        if PythonValidator.is_instance(self.argument, 'SubClipSetting'):
            values = self.argument.get_values(n)
            return [[value, value] for value in values]
        if PythonValidator.is_instance(self.argument, 'Graphic'):
            values = [self.argument.get_xy_from_normalized_d(d)[1] for d in Progression(0, 1, n).values]
            return [[value, value] for value in values]
...
class MoviepyWith:
    ...
    def get_arrays(
        ...
        the effect.
        """
        if with_position is None or resized is None or rotated is None:
            # TODO: Improve this exception
            raise Exception('Parameters missing')
        
        from yta_multimedia.video.edition.video_frame_t_helper import VideoFrameTHelper

        ts = VideoFrameTHelper.get_video_frames_ts_from_number_of_frames(number_of_frames)

        resized = [
            resized.get_values(len(ts))
        ] if resized is not None else None
        ...
        with_position = [
            ...
            # TODO: Maybe this has to accept a map instead
            with_position.get_values(len(ts))
        ] if with_position is not None else None

        rotated = [
            rotated.get_values(len(ts))
        ] if rotated is not None else None
