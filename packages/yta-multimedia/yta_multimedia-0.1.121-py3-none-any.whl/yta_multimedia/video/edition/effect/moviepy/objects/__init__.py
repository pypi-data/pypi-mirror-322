...
class MoviepyWith:
    ...
    def get_arrays(
        ...
        ts = VideoFrameTHelper.get_video_frames_ts_from_number_of_frames(number_of_frames, video_fps)

        resized = resized.get_values(len(ts)) if resized is not None else None
        ...
        with_position = with_position.get_values(len(ts)) if with_position is not None else None
        rotated = rotated.get_values(len(ts)) if rotated is not None else None
