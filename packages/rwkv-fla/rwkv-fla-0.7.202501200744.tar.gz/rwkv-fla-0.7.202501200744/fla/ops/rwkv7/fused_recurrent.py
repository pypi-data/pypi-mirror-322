...
class FusedRecurrentRWKV7Function(torch.autograd.Function):
    ...
    @contiguous
    @autocast_custom_fwd
    def forward(ctx, q, k, v, w, a, b, scale, initial_state, output_final_state:bool=True, training: bool = True):
        """        
        Args:
    ...
    def backward(ctx, do, dht):
        raise NotImplementedError(
            ...
            "Please use chunk_wrkv7 for training!"
        )
...
def fused_recurrent_rwkv7(
    ...
    if not training:
        ...
        return FusedRecurrentRWKV7Function.apply(r, k, v, w, a, b, scale, initial_state, output_final_state, training)
    else:
        if use_log_w:
            log_w = w
        else:
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        result, state = fused_recurrent_rwkv7(q, k, v, w, a, b, initial_state=h.transpose(-1, -2), 
                                              training=False, use_log_w=False, head_first=True)
