...
import sqlalchemy
from sqlalchemy import func, or_, cast, and_, union, distinct as distinct_func
    ...
    if columns_type == 'search':
        if is_json_column:
            ...
            return logic_function(
                *[
                    or_(*[func.lower(cast(column, sqlalchemy.String)).like(func.lower(f"%\"{x}\"%")) for x in filter_value])
                    for column in columns
                ]
            )
        else:
            ...
            return logic_function(
                *[
                    func.lower(cast(column, sqlalchemy.String)).like(func.lower(f"%{filter_value}%"))
                    for column in columns
                ]
            )
