...
class SelfAskCategoryScorer(Scorer):
    ...
    async def score_async(self, request_response: PromptRequestPiece, *, task: Optional[str] = None) -> list[Score]:
        ...
        # logging.info(f"System prompt..: {self._system_prompt}")
    ...
    async def _send_chat_target_async(self, request: PromptRequestResponse, request_response_id: uuid.UUID) -> Score:
        ...
        #     raise InvalidJsonException(message=f"Invalid JSON response, missing Key: {response_json}")
            ...
            logging.warning(f"Invalid JSON response, skipping: {response_json}")
            return None

        except KeyError:
            logging.warning(f"Invalid JSON response, missing Key, skipping: {response_json}")
            return None
        return score
