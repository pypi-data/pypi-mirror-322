...
async def combination_method(name, session):
    async with aiohttp.ClientSession() as session:
        ...
        for i in x:
            new_word = (i[0] + " " + i[1])
            url = f"https://el.wikipedia.org/w/api.php?action=query&list=search&srsearch={new_word}&srlimit=20&srprop=&srenablerewrites=True&format=json"
            json_data = await fetch_json(url, session)
            suggestion = json_data.get('query', {}).get('search', {})
            for pageid in suggestion:
                data.add(urllib.parse.unquote(pageid.get('title')))
    return data
...
async def single_method(name, session):
    async with aiohttp.ClientSession() as session:
        ...
        for i in new_name:
            url = f"https://el.wikipedia.org/w/api.php?action=query&list=search&srsearch={i}&srlimit=20&srprop=&srenablerewrites=True&format=json"
            json_data = await fetch_json(url, session)
            suggestion = json_data.get('query', {}).get('search', {})
            for pageid in suggestion:
                data.add(urllib.parse.unquote(pageid.get('title')))
...
async def mains(name, single, combination):
    ...
    async with aiohttp.ClientSession() as session:
        url = f"https://el.wikipedia.org/w/api.php?action=query&list=search&srsearch={name}&srlimit=20&srprop=&srenablerewrites=True&format=json"
        json_data = await fetch_json(url, session)
        suggestion = json_data.get('query', {}).get('search', {})
        for pageid in suggestion:
            data.add(urllib.parse.unquote(pageid.get('title')))
