...
def read_gdx_symbol(gdx_path: Union[str, dict], gdx_symbol: str, sel_dict=None, fill_ts=True) -> pd.DataFrame:
    ...
    if fill_ts and ("timeModel" in df.index.names) and isinstance(df.index, pd.MultiIndex):
        if "timeModelToCalc" in cont.listSymbols():
            ...
            timetocalc_records.columns = [re.sub(r"_\d+$", "", c) for c in timetocalc_records.columns]
...
def write_gdx_symbol_as_csv(gdx: str, symbol: str, directory: str):
    ...
    if gams_older or gams_43:
        ...
        if len(output) > 0:
            with open(os.path.join(directory, f"set_{symbol}.csv"), "w", encoding="UTF-8") as out_data:
...
def fill_missing_values_in_timeseries(df: pd.DataFrame, timemodel: pd.DataFrame) -> pd.DataFrame:
    ...
    mi = pd.MultiIndex.from_product([timemodel.index.to_list(), df.index.droplevel(lvl_time).unique().to_list()])
    mi_time = mi.get_level_values(0)
    mi_sub = pd.MultiIndex.from_tuples(mi.get_level_values(1))

    idx_new = pd.MultiIndex.from_arrays([mi_time] + [mi_sub.get_level_values(i) for i in range(len(mi_sub.levels))])
...
class GDXEval:
    ...
    def get_symbol(self, key: str, scen_names: list[str], sel_dict: Optional[dict] = None) -> pd.DataFrame:
        ...
        if self._gdx_dict:
            ...
            arg_list = tuple((pth, key, sel_dict, self._fill_ts) for pth in list(self._gdx.values()))
            ...
            if self._parallel:
                ...
                df_list = [read_gdx_symbol(pth, key, sel_dict, fill_ts) for pth, key, sel_dict, fill_ts in arg_list]
