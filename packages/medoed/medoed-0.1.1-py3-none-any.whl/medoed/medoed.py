...
class MDECalculatorConfig(BaseModel):
    ...
    outliers_type: Optional[str] = "upper"
...
class MDECalculator:
    ...
    def _get_strat_weights(self, df: pd.DataFrame, strat_column: str) -> pd.Series:
        """
        ...
        """
        strat_weights = df[strat_column].value_counts() / df[strat_column].count()
        strat_weights = (
    ...
    def _prepare_stratification_weights(
        ...
        df_strat_weights = self._get_strat_weights(df=df_historical, strat_column="strat")
    ...
    def _calc_binary_mde(self, metric: str, sample_size: int) -> float:
        ...
        """
        base_rate = float(self.df[metric].mean())
    ...
    def _calc_continuous_mde(self, metric: str, sample_size: int) -> float:
        ...
        absolute_change = min_effect_size * std_dev / np.sqrt(2)
    ...
    def _calc_stratified_mde(self, metric: str, sample_size: int) -> float:
        ...
        weights_dict, df_pre_experiment = self._prepare_stratification_weights(
            df_historical=df_historical, df_pre_experiment=df_pre_experiment
        )
