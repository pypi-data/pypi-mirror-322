...
def generate_per_run_metrics(
    ...
    result = {
        # Compute average for metrics with numeric, boolean, or pass-fail values
        **{
            f"{metric_name}{_AVERAGE_SUFFIX}": avg_value
            for metric_name, avg_value in _compute_avg_for_metric_results(
                eval_results
            ).items()
        },
...
def _compute_avg_for_metric_results(
    eval_results: List[entities.EvalResult],
) -> Dict[str, float]:
    """
    Compute the average value across all eval results for metrics with numeric, boolean, or pass-fail values.
    ...
    for eval_result in eval_results:
        for metric_result in eval_result.metric_results:
            metric_value = (
                metric_result.metric_value.value
                if isinstance(metric_result.metric_value, mlflow_eval.Assessment)
                else metric_result.metric_value
            )
            metric_full_name = metric_result.metric_full_name
            if isinstance(metric_value, (int, float, bool)):
                metric_value_sums[metric_full_name] += metric_value
                metric_value_counts[metric_full_name] += 1
            elif (
                isinstance(metric_value, str)
                and entities.CategoricalRating(metric_value)
                != entities.CategoricalRating.UNKNOWN
            ):
                metric_value_sums[metric_full_name] += (
