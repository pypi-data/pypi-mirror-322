...
def _get_assessments_to_run(
    ...
    # If no guidelines column and no global guidelines, disable all guideline adherence judges
    global_guideline_adherence_assessment = None
    if eval_item.guidelines is None and config.global_guidelines is None:
        ...
        if (
            ...
            and assessment_config.GUIDELINE_ADHERENCE.assessment_name in enabled_metrics
        ):
            # If both global and per-question are defined or only global is defined, use global guideline adherence
            # Note: global guideline adherence is a client-only modification of guideline adherence so we need to do
            #       this special logic here instead of in the service (which has no notion of "global").
            ...
            global_guideline_adherence_assessment = BuiltInAssessmentRunner(
        ...
        if eval_item.guidelines is None:
            # If only global guidelines is defined, disable per-question guideline adherence
            enabled_metrics -= {assessment_config.GUIDELINE_ADHERENCE.assessment_name}
    ...
    if global_guideline_adherence_assessment is not None:
        final_assessments_to_run.append(global_guideline_adherence_assessment)

    return final_assessments_to_run
    ...
    *,
    ...
    with ThreadPoolExecutor(max_workers=len(assessments)) as executor:
        futures = [
            ...
            # Global guideline adherence is a special case which requires modifying the eval item
            if assessment.config != assessment_config.GLOBAL_GUIDELINE_ADHERENCE
        ]
        ...
        global_guideline_adherence_assessment = next(
            (
                assessment
                for assessment in assessments
                if isinstance(assessment, BuiltInAssessmentRunner)
                and assessment.config == assessment_config.GLOBAL_GUIDELINE_ADHERENCE
            ),
            None,
        )
        if global_guideline_adherence_assessment is not None:
            ...
            futures.append(
                executor.submit(
                    ...
                    assessment=global_guideline_adherence_assessment,
                    current_session=session.current_session(),
                )
            )
