...
async def test_cli_run(mock_compress_source_code, mock_get_repo_info, mock_session, test_params, mock_repo_info):
...
async def test_generate_issues(mock_github, test_params):
    ...
    test_results = [{"bug_id": "BUG-1", "bug_title": "Test Bug", "bug_description": "This is a test bug"}]
...
def test_hackbot_run(mock_cli_run, mock_auth, mock_exists, mock_args):
    ...
    mock_cli_run.return_value = [{"bug_id": "TEST-1"}], True
