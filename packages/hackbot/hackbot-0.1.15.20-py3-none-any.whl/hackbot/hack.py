...
async def cli_scope(
    ...
    async for report in do_post(invocation_args, address, port, api_key, Endpoint.SCOPE, source_path, output):
...
async def cli_run(
    ...
    async for report in do_post(invocation_args, address, port, api_key, Endpoint.RUN, source_path, output):
        result_json = json.loads(report)
        log.info("Got result_json:")
        log.debug(str(result_json))
        if result_json.get("message") is not None:
            log.info(result_json.get("message"))
            pbar.set_postfix_str(colored(result_json.get("message"), "yellow"))
