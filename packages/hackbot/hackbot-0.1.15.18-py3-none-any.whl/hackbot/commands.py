...
def get_args() -> argparse.Namespace | int:
    ...
    scope_parser = subparsers.add_parser(Endpoint.SCOPE.value, help="Analyze the scope of the source code")
...
def hackbot_run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.command == Endpoint.RUN.value:
            ...
            if not success:
                log.error("❌ Error: Hackbot analysis failed")
                return 1
        ...
        if str(e) == "Hack request failed: 413":
            ...
            log.error(f"❌ Error: {str(e)}. Traceback: {traceback.format_exc()}")
        return 1
