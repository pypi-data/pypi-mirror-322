...
def generate_avro_and_mapping(sql_file_path, avro_output_path, json_output_path, sql_output_path, date_format):
    ...
    # 1) Buscar el bloque CREATE TABLE con o sin comillas.
    #    Queremos aislar EXACTAMENTE lo que va entre paréntesis (columnas).
    create_table_regex = re.compile(
        r'''CREATE\s+TABLE\s+
            (?:             # Esquema opcional
              "?([A-Za-z0-9_$]+)"?\.
            )?
            "?([A-Za-z0-9_$]+)"?  # nombre tabla
            \s*\(\s*(.*?)\s*\)
            \s*;?
        ''',
        re.IGNORECASE | re.DOTALL | re.VERBOSE
    )
    match = create_table_regex.search(sql_content)
    if not match:
        print(f"[WARN] No se encontró un CREATE TABLE válido en {sql_file_path}. Se omite.")
        return

    schema_name = match.group(1)  # podría ser None si no hay esquema
    table_name = match.group(2).lower()  # Nombre de la tabla
    table_def = match.group(3)          # Contenido dentro de ( ... ), es decir la definición

    # 2) Partir `table_def` línea por línea
    lines = table_def.strip().split('\n')

    # 3) Recorremos cada línea, descartamos las que son constraints, etc.
    columns = []
    for line in lines:
        line_stripped = line.strip().rstrip(',')  # quitar espacios y una coma final si existe
        
        # Si la línea menciona constraint, primary, foreign, check, etc., ignorarla
        if re.search(r'(?i)\b(CONSTRAINT|PRIMARY|FOREIGN|CHECK)\b', line_stripped):
            continue

        # Tratar de parsear "NOMBRE_COL" TIPO ...
        # Permitimos comillas opcionales en el nombre
        col_match = re.match(
            r'^"?([A-Za-z0-9_]+)"?\s+([A-Za-z0-9\(\)_,\s]+)',  # col_name, col_type
            line_stripped,
            flags=re.IGNORECASE
        )
        if col_match:
    ...
    if not columns:
        print(f"[WARN] No se encontraron columnas en la tabla {table_name} en {sql_file_path}.")
        return

    # 4) Generar Avro, JSON, y la consulta SQL
    avro_schema = {
        "type": "record",
        "name": table_name,
        "fields": []
    ...
    for (column_name, column_type) in columns:
        # Evitar columnas que coincidan EXACTO con el nombre de la tabla
        if column_name.lower() == table_name:
            continue

        # Usar tu map_oracle_to_avro, asumiendo que ya lo tienes importado
        avro_type = map_oracle_to_avro(column_type)

        # Ajuste al tipo que pondrás en JSON
        if isinstance(avro_type, dict) and avro_type.get("logicalType") == "decimal":
            json_type = "numeric"
        elif avro_type in ["long", "int"]:
            json_type = "int"
        else:
            json_type = "string"
        ...
        column_names_types.append((column_name.upper(), column_type.upper()))

    # 5) Guardar AVRO
    os.makedirs(avro_output_path, exist_ok=True)
    ...
    with open(sql_query_file_path, "w", encoding="utf-8") as query_file:
        query_file.write(sql_query)

    print(f"[OK] Generado Avro: {avro_file_path}")
    print(f"[OK] Generado JSON: {json_file_path}")
    print(f"[OK] Generado SQL: {sql_query_file_path}")
