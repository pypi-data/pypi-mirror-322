...
def generate_avro_and_mapping(sql_file_path, avro_output_path, json_output_path, sql_output_path, date_format):
    ...
    # 1) Limpieza de contenido (similar a lo que ya tenías)
    ...
    # 2) Busca el bloque CREATE TABLE con o sin comillas
    create_table_regex = re.compile(
        r'''CREATE\s+TABLE\s+
            (?:             # esquema opcional
              "?([A-Za-z0-9_$]+)"?\.
            )?
            "?([A-Za-z0-9_$]+)"?\s*  # nombre tabla
            \(\s*(.*?)\s*\)
            \s*;?
        ''',
    ...
    if not match:
        print(f"No se encontró un CREATE TABLE válido en {sql_file_path}. Se omite.")
        return

    schema_name = match.group(1)  # Puede ser None si no viene esquema
    table_name = match.group(2).lower()  # Nombre de la tabla
    table_def = match.group(3)           # Contenido dentro de ( ... )

    # 3) Extraer columnas solamente (evitando constraints)
    #    Buscar líneas con la forma:
    #         NOMBRE_COLUMNA [o "NOMBRE_COLUMNA"]  tipo (...)  [NOT NULL...], 
    #    y descartar las que empiecen con CONSTRAINT, CHECK, etc.
    col_pattern = re.compile(
        r'''^\s*
            (?!CONSTRAINT|PRIMARY|FOREIGN|CHECK)      # (?!...) = negative lookahead, no emparejar si empieza con esas palabras
            "?([A-Za-z0-9_]+)"?                       # nombre de columna con o sin comillas
            \s+([A-Za-z0-9\(\),\s]+)                  # tipo: NUMBER(9,0), VARCHAR2(40), DATE, etc.
            (?:\s+NOT\s+NULL\s+ENABLE)?               # opcional
            \s*(?:,|$)                                # seguida de coma o fin de línea
        ''',
        re.IGNORECASE | re.MULTILINE | re.VERBOSE
    )

    columns = col_pattern.findall(table_def)
    if not columns:
        print(f"No se encontraron columnas en la tabla {table_name} en el archivo {sql_file_path}.")
        return

    # 4) Generar Avro, JSON y SQL
    avro_schema = {
        "type": "record",
        "name": table_name,
        "fields": []
    ...
    for (column_name, column_type) in columns:
        # Evitar columnas que coincidan con el nombre de la tabla (caso raro)
    ...
    # -- Generar y guardar la consulta SQL (similar a tu versión actual) --
    select_columns = []
