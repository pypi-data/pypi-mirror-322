...
def handle_outliers(
    ...
    def process_group(data: pd.DataFrame) -> pd.DataFrame:
        ...
        if handling_method == "replace_threshold":
            if outlier_type == "upper":
                data.loc[outliers_mask, target_column] = upper_threshold
            elif outlier_type == "lower":
                data.loc[outliers_mask, target_column] = lower_threshold
            else:  # two-sided
                data.loc[data[target_column] > upper_threshold, target_column] = upper_threshold
                data.loc[data[target_column] < lower_threshold, target_column] = lower_threshold
        elif handling_method == "replace_median":
            data.loc[outliers_mask, target_column] = data[target_column].median()
