...
//		assert(minfoo>0 && minfoo<=maxfoo) : minfoo+", "+maxfoo;
	...
	private static String toShortName(String s) {
	...
	private String rename(final String old, final int scaflen) {
		...
		if(scaf==null) {return old+" cov_0";}
		
		final int[] keys=scaf.map.keys();
		final long[] values=scaf.map.values();
		final int invalid=scaf.map.invalid();
		int taxid=-1;
		...
		final int len=Tools.max(scaflen, scaf.length);
		assert(len>0);
		assert(scaflen==scaf.length);
		final float cov=(float)(maxMappedBases/len);
		
		ByteBuilder bb=new ByteBuilder(old.length()+40);
		bb.append(old);
		if(oldTaxid<0 && taxid>0) {bb.append(" tid_").append(taxid);}
		bb.append(" cov_").append(cov, 2);
		return bb.toString();
	}
