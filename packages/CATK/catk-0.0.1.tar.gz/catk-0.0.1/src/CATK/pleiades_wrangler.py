...
def make_pleiades_objects(download_latest_data=False):
    ...
    if download_latest_data:
        ...
        data = utils.get_pleiades_data(
            r"http://atlantides.org/downloads/pleiades/json/pleiades-places-latest.json.gz")
    else:
        raw_data_file = utils.unzip_gz(r"CATK\src\CATK\data\pleiades-places-latest.json.gz")
        data = utils.get_df(raw_data_file)
    data = data['@graph']
    pleiades = []
