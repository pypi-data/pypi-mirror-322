...
class CustomLogger:
    ...
    def print(self, level, msg):
        for handler, flush_me in self.handlers.items():
            ...
            if flush_me:
                handler.flush()
            # Make sure to flush if this is a file handler
        # Update last level
    ...
    def setup_logger(self, dir = '.', log_to_file=False, log_file_path=None, **kwargs):
        ...
        if log_to_file:
            ...
            self.add_handler(file_handler, True)

        #return logger

    # Custom add_handler method 
    def add_handler(self, handler, flush_me = False):
        self.handlers[handler] = flush_me
