...
class PyRexBot:
    """
    PyRexBot simplifies Telegram bot creation, making it easy to build both basic and advanced bots.
    """
    ...
    def add_message_handler(self, callback, text_only=True):
        """
    ...
    def add_inline_query_handler(self, callback):
        """
    ...
    async def send_message(self, chat_id: int, text: str, buttons=None):
        ...
        :param buttons: Inline buttons as a list of tuples [(text, callback_data)].
        """
        markup = InlineKeyboardMarkup(self._build_buttons(buttons)) if buttons else None
    ...
    async def send_document(self, chat_id: int, document, caption=None, buttons=None):
        """
        Send a document with optional caption and buttons.

        :param chat_id: The ID of the chat to send the document to.
        :param document: The document file (file ID, URL, or file object).
        :param caption: Optional caption for the document.
        :param buttons: Optional inline buttons.
        """
        markup = InlineKeyboardMarkup(self._build_buttons(buttons)) if buttons else None
        await self.app.bot.send_document(chat_id=chat_id, document=document, caption=caption, reply_markup=markup)

    async def send_video(self, chat_id: int, video, caption=None, buttons=None):
        """
        Send a video with optional caption and buttons.
    ...
    async def reply(self, update: Update, text: str, buttons=None):
        """
        Reply to a message with optional buttons.
    ...
    async def edit_message(self, chat_id: int, message_id: int, text: str, buttons=None):
        """
        Edit an existing message with new text and buttons.
        ...
        """
        markup = InlineKeyboardMarkup(self._build_buttons(buttons)) if buttons else None
        await self.app.bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=text, reply_markup=markup)
    ...
    def _build_buttons(self, button_data):
        """
        Helper method to build inline keyboard buttons.

        :param button_data: A list of tuples [(text, callback_data)].
        :return: A list of InlineKeyboardButton objects.
        """
        return [[InlineKeyboardButton(text, callback_data=data)] for text, data in button_data] if button_data else None
