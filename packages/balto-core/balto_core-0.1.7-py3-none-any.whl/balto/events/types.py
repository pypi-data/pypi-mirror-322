...
from balto.constants import MAXIMUM_SEED_SIZE_NAME, PIN_PACKAGE_URL
from balto.events.base_types import (
    DebugLevel,
    DynamicLevel,
    ErrorLevel,
    InfoLevel,
...
class MainReportVersion(InfoLevel):
    ...
    def message(self) -> str:
        return f"Running with balto{self.version}"
...
class MainReportArgs(DebugLevel):
    ...
    def message(self) -> str:
        return f"running balto with arguments {str(self.args)}"
...
class ConfigFolderDirectory(InfoLevel):
    ...
    def message(self) -> str:
        return f"Creating balto configuration folder at {self.dir}"
...
class ProfileWrittenWithTargetTemplateYAML(InfoLevel):
    ...
    def message(self) -> str:
        return (
            ...
            "profile_template.yml and your supplied values. Run 'balto debug' to "
...
class ProjectCreated(InfoLevel):
    ...
    def message(self) -> str:
        return f"""
Your new balto project "{self.project_name}" was created!
...
class SeedExceedsLimitSamePath(WarnLevel):
    ...
    def message(self) -> str:
        msg = (
            f"Found a seed ({self.package_name}.{self.name}) "
            f">{MAXIMUM_SEED_SIZE_NAME} in size at the same path, balto "
...
class UnversionedBreakingChange(WarnLevel):
    ...
    def message(self) -> str:
        ...
        msg = (
            ...
            "\nWhile comparing to previous project state, balto detected a breaking change to an unversioned model."
            f"\n  - {reasons}\n"
        )
...
class DepsNotifyUpdatesAvailable(InfoLevel):
    ...
    def message(self) -> str:
        return f"Updates available for packages: {self.packages} \
                \nUpdate your versions in packages.yml, then run balto deps"
...
class DepsScrubbedPackageName(WarnLevel):
    ...
    def message(self) -> str:
        return f"Detected secret env var in {self.package_name}. balto will write a scrubbed representation to the lock file. This will cause issues with subsequent 'dbt deps' using the lock file, requiring 'dbt deps --upgrade'"
...
class LogScheduleResult(DynamicLevel):
    def code(self) -> str:
        return "Q017"
...
class InternalErrorOnRun(DebugLevel):
    ...
    def message(self) -> str:
        ...
        internal_error_string = """This is an error in balto. Please try again. If \
the error persists, open an issue at https://github.com/balto-data/balto
""".strip()
