...
from balto.adapters.contracts.connection import AdapterRequiredConfig, Credentials
from balto.adapters.events.types import (
    AdapterImportError,
    PluginLoadError,
    AdapterRegistered,
)
from balto.include.global_project import (
    PACKAGE_PATH as GLOBAL_PROJECT_PATH,
    PROJECT_NAME as GLOBAL_PROJECT_NAME,
)
from balto.adapters.protocol import AdapterConfig, AdapterProtocol, RelationProtocol
...
class AdapterContainer:
    ...
    def load_plugin(self, name: str) -> Type[Credentials]:
        ...
        try:
            ...
            mod: Any = import_module("." + name, "balto.adapters")
        except ModuleNotFoundError as exc:
            # if we failed to import the target module in particular, inform
            # the user about it via a runtime error
            if exc.name == "balto.adapters." + name:
    ...
    def _adapter_version(self, adapter_name: str) -> str:
        try:
            raw_version = import_module(f".{adapter_name}.__about__", "balto.adapters").version
        except ModuleNotFoundError:
            raw_version = import_module(f".{adapter_name}.__version__", "balto.adapters").version
