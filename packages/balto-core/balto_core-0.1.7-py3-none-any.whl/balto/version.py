...
PYPI_VERSION_URL = "https://pypi.org/pypi/balto-core/json"
...
def _get_core_msg_lines(
    ...
    if latest is None:
        update_info = (
            "  The latest version of balto-core could not be determined!\n"
    ...
    if installed > latest:
        ...
        update_info = (
            "  Your version of balto-core is out of date!"
        )
...
def get_package_pypi_url(package_name: str) -> str:
    return f"https://pypi.org/pypi/balto-core/json"
...
def _get_dbt_plugins_info() -> Iterator[Tuple[str, str]]:
    for plugin_name in _get_adapter_plugin_names():
        if plugin_name == "core":
            continue
        try:
            mod = importlib.import_module(f"balto.adapters.{plugin_name}.__version__")
        except ImportError:
            # not an adapter
            continue
...
def _get_adapter_plugin_names() -> Iterator[str]:
    spec = importlib.util.find_spec("balto.adapters")
    # If None, then nothing provides an importable 'balto.adapters', so we will
    ...
    for adapters_path in spec.submodule_search_locations:
        ...
        for version_path in glob.glob(version_glob):
            # the path is like .../balto/adapters/{plugin_name}/__version__.py
