...
from balto.events.types import FoundStats, WritingInjectedSQLForNode
...
def _generate_stats(manifest: Manifest) -> Dict[NodeType, int]:
    ...
    stats[NodeType.Schedule] += len(manifest.schedules)
...
class Linker:
    ...
    def link_graph(self, manifest: Manifest):
        ...
        for schedule in manifest.schedules.values():
            self.link_node(schedule, manifest)
