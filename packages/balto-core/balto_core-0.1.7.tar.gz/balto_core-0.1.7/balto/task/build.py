...
from balto.events.types import LogNodeNoOpResult
from balto.exceptions import DbtInternalError
from balto.graph import Graph, GraphQueue, ResourceTypeSelector
from balto.node_types import NodeType
from balto.task.base import BaseRunner, resource_types_from_args
from balto.task.test import TestSelector
from dbt_common.events.functions import fire_event

from .run import ModelRunner as run_model_runner
from .run import RunTask
from .schedule import ScheduleRunner as schedule_runner
...
class BuildTask(RunTask):
    ...
    RUNNER_MAP = {
        ...
        NodeType.Schedule: schedule_runner,
    ...
    def handle_job_queue_node(self, node, pool, callback):
        ...
        if runner.node.unique_id in self._skipped_children:
            cause = self._skipped_children.pop(runner.node.unique_id)
            runner.do_skip(cause=cause)
        # skip balto app specific resources
        if self.requires_balto_app(runner.node):
            runner.do_skip()
        args = [runner]
        if self.config.args.single_threaded:
    ...
    def requires_balto_app(self, node):
        # TODO figure out a less hacky way to mark these
        return node.resource_type == "schedule" or node.config.materialized == "trigger"
