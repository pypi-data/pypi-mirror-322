...
from balto.adapters.exceptions import (
    DuplicateMacroInPackageError,
    DuplicateMaterializationNameError,
)
from balto.adapters.factory import get_adapter_package_names

# to preserve import paths
from balto.artifacts.resources import BaseResource, DeferRelation, NodeVersion, RefArgs
from balto.artifacts.resources.v1.config import NodeConfig
from balto.artifacts.schemas.manifest import ManifestMetadata, UniqueID, WritableManifest
from balto.clients.jinja_static import statically_parse_ref_or_source
from balto.contracts.files import (
    AnySourceFile,
    FileHash,
    FixtureSourceFile,
    SchemaSourceFile,
...
from balto.contracts.graph.unparsed import SourcePatch, UnparsedVersion
from balto.contracts.util import SourceKey
from balto.events.types import UnpinnedRefNewVersionAvailable
...
if os.environ.get('DBT_DISABLE_MP'):
    import threading
...
def get_lock_factory():
    if os.environ.get('DBT_DISABLE_MP'):
        ...
        return get_mp_context().Lock

def get_lock():
    return get_lock_factory()()
...
# This handles both models/seeds/snapshots and sources/schedules/metrics/exposures/semantic_models
...
class Manifest(MacroMethods, dbtClassMixin):
    ...
    schedules: MutableMapping[str, Schedule] = field(default_factory=dict)
    ...
    _lock: CONFIGURED_LOCK = field(
        default_factory=get_lock_factory(),
        metadata={"serialize": lambda x: None, "deserialize": lambda x: None},
    )
    ...
    def build_flat_graph(self):
        ...
        self.flat_graph = {
            ...
            "schedules": {k: v.to_dict(omit_none=False) for k, v in self.schedules.items()},
        }
    ...
    def _get_parent_adapter_types(self, adapter_type: str) -> List[str]:
        ...
        from balto.adapters.factory import get_adapter_type_names
    ...
    def from_writable_manifest(cls, writable_manifest: WritableManifest) -> "Manifest":
        manifest = Manifest(
            ...
            schedules=cls._map_resources_to_map_nodes(writable_manifest.schedules),
        )
    ...
    def writable_manifest(self) -> "WritableManifest":
        ...
        return WritableManifest(
            ...
            schedules=self._map_nodes_to_map_resources(self.schedules),
        )
    ...
    def expect(self, unique_id: str) -> GraphMemberNode:
        ...
        elif unique_id in self.schedules:
            return self.schedules[unique_id]
        else:
    ...
    def add_schedule(self, source_file: SchemaSourceFile, schedule: Schedule):
        _check_duplicates(schedule, self.schedules)
        self.schedules[schedule.unique_id] = schedule
        source_file.schedules.append(schedule.unique_id)
