...
import balto.deprecations
import balto.exceptions
import balto.utils
import dbt_common.utils
from balto import plugins
from balto.adapters.factory import (
    get_adapter,
    get_adapter_package_names,
    get_relation_class_by_name,
    register_adapter,
)
from balto.artifacts.resources import FileHash, NodeRelation, NodeVersion
from balto.artifacts.schemas.base import Writable
from balto.clients.jinja import MacroStack, get_rendered
from balto.clients.jinja_static import statically_extract_macro_calls
from balto.config import Project, RuntimeConfig
from balto.constants import (
    MANIFEST_FILE_NAME,
    PARTIAL_PARSE_FILE_NAME,
    SEMANTIC_MANIFEST_FILE_NAME,
)
from balto.context.configured import generate_macro_context
from balto.context.docs import generate_runtime_docs_context
from balto.context.macro_resolver import MacroResolver, TestMacroNamespace
from balto.context.providers import ParseProvider, generate_runtime_macro_context
from balto.context.query_header import generate_query_header_context
from balto.contracts.files import ParseFileType, SchemaSourceFile
from balto.contracts.graph.manifest import (
    Disabled,
    MacroManifest,
    Manifest,
    ManifestStateCheck,
...
from balto.contracts.graph.nodes import (
    Exposure,
    GenericTestNode,
    Macro,
    ManifestNode,
...
from balto.contracts.graph.semantic_manifest import SemanticManifest
...
from balto.mp_context import get_mp_context
from balto.node_types import AccessType, NodeType
from balto.parser.analysis import AnalysisParser
from balto.parser.base import Parser
from balto.parser.docs import DocumentationParser
from balto.parser.fixtures import FixtureParser
from balto.parser.generic_test import GenericTestParser
from balto.parser.hooks import HookParser
from balto.parser.macros import MacroParser
from balto.parser.models import ModelParser
from balto.parser.partial import PartialParsing, special_override_macros
from balto.parser.read_files import (
    FileDiff,
    ReadFiles,
    ReadFilesFromDiff,
...
from balto.parser.schemas import SchemaParser
from balto.parser.search import FileBlock
from balto.parser.seeds import SeedParser
from balto.parser.singular_test import SingularTestParser
from balto.parser.snapshots import SnapshotParser
from balto.parser.sources import SourcePatcher
from balto.parser.unit_tests import process_models_for_unit_test
from balto.version import __version__
...
class ManifestLoader:
    ...
    def load(self) -> Manifest:
        ...
        if self.skip_parsing:
            ...
            if not semantic_manifest.validate():
                raise balto.exceptions.ParsingError("Semantic Manifest validation failed.")
    ...
    def check_valid_group_config_node(
        ...
        if groupable_node_group and groupable_node_group not in valid_group_names:
            raise balto.exceptions.ParsingError(
    ...
    def write_perf_info(self, target_path: str):
        ...
        write_file(path, json.dumps(self._perf_info, cls=balto.utils.JSONEncoder, indent=4))
    ...
    for ver_unver_dict in name_resources.values():
        ...
        if intersection_versioned:
            for name in intersection_versioned:
                ...
                raise balto.exceptions.DuplicateVersionedUnversionedError(
...
def _process_refs(
    ...
    for ref in node.refs:
        ...
        if len(ref.positional_args) < 1 or len(ref.positional_args) > 2:
            raise balto.exceptions.DbtInternalError(
        ...
        if target_model is None or isinstance(target_model, Disabled):
            ...
            raise balto.exceptions.DbtReferenceError(
                unique_id=node.unique_id,
