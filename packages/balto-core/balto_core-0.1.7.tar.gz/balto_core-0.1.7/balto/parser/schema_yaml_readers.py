...
from balto.clients.jinja import get_rendered
from balto.context.context_config import (
    BaseContextConfigGenerator,
    ContextConfigGenerator,
    UnrenderedConfigGenerator,
)
from balto.context.providers import (
    generate_parse_exposure,
    generate_parse_semantic_models,
)
from balto.contracts.files import FileHash, SchemaSourceFile
from balto.contracts.graph.nodes import Exposure, Group, Metric, SavedQuery, Schedule
...
from balto.exceptions import JSONValidationError, YamlParseDictError
from balto.node_types import NodeType
from balto.parser.common import YamlBlock
from balto.parser.schemas import ParseResult, SchemaParser, YamlReader
...
class ScheduleParser(YamlReader):
    def __init__(self, schema_parser: SchemaParser, yaml: YamlBlock) -> None:
        super().__init__(schema_parser, yaml, NodeType.Schedule.pluralize())
        self.schema_parser = schema_parser
        self.yaml = yaml

    def parse_schedule(self, unparsed: UnparsedSchedule) -> None:
        package_name = self.project.project_name
        unique_id = f"{NodeType.Schedule}.{package_name}.{unparsed.name}"
        ...
        parsed = Schedule(
            resource_type=NodeType.Schedule,
            package_name=package_name,
            path=path,
            original_file_path=self.yaml.path.original_file_path,
            unique_id=unique_id,
            fqn=fqn,
            database=self.root_project.credentials.database,
            schema=self.root_project.credentials.schema,
            name=unparsed.name,
            alias=unparsed.name,
            checksum=FileHash.empty(),
            config=self._generate_schedule_config(unparsed, fqn, package_name, True),
            schedule=unparsed.schedule,
        ...
        self.manifest.add_schedule(self.yaml.file, parsed)

    def _generate_schedule_config(self, target: UnparsedSchedule, fqn: List[str], package_name: str, rendered: bool):
    ...
    def parse(self) -> None:
        for data in self.get_key_dicts():
            try:
                UnparsedSchedule.validate(data)
                unparsed = UnparsedSchedule.from_dict(data)
            except (ValidationError, JSONValidationError) as exc:
                raise YamlParseDictError(self.yaml.path, self.key, data, exc)

            self.parse_schedule(unparsed)
