...
from balto.events.types import PartialParsingEnabled, PartialParsingFile
...
class PartialParsing:
    ...
    def schedule_nodes_for_parsing(self, unique_ids):
        for unique_id in unique_ids:
            ...
            elif unique_id in self.saved_manifest.schedules:
                schedule = self.saved_manifest.schedules[unique_id]
                self._schedule_for_parsing(
                    "schedules", schedule, schedule.name, self.delete_schema_schedule
                )
    ...
    def handle_schema_file_changes(self, schema_file, saved_yaml_dict, new_yaml_dict):
        ...
        handle_change("schedules", self.delete_schema_schedule)
    ...
    # schedules are created only from schema files, so just delete
    # the schedule or the disabled schedule.
    def delete_schema_schedule(self, schema_file, schedule_dict):
        schedule_name = schedule_dict["name"]
        schedules = schema_file.schedules.copy()
        for unique_id in schedules:
            if unique_id in self.saved_manifest.schedules:
                schedule = self.saved_manifest.schedules[unique_id]
                if schedule.name == schedule_name:
                    self.saved_manifest.schedules.pop(unique_id)
                    schema_file.schedules.remove(unique_id)
