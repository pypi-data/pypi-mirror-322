...
from balto.contracts.graph.unparsed import UnparsedVersion
...
class SelectorMethod(metaclass=abc.ABCMeta):
    ...
    def schedule_nodes(
        ...
        for key, schedule in self.manifest.schedules.items():
            unique_id = UniqueId(key)
            if unique_id not in included_nodes:
                continue
            yield unique_id, schedule
...
class ScheduleSelectorMethod(SelectorMethod):
    def search(self, included_nodes: Set[UniqueId], selector: str) -> Iterator[UniqueId]:
        parts = selector.split(".")
        target_package = SELECTOR_GLOB
        if len(parts) == 1:
            ...
            msg = (
                'Invalid schedule selector value "{}". Schedules must be of '
                "the form ${{schedule_name}} or "
                "${{schedule_package.schedule_name}}"
            ).format(selector)
            raise DbtRuntimeError(msg)

        for unique_id, node in self.schedule_nodes(included_nodes):
            if not fnmatch(node.package_name, target_package):
                continue
...
class MethodManager:
    SELECTOR_METHODS: Dict[MethodName, Type[SelectorMethod]] = {
        ...
        MethodName.Schedule: ScheduleSelectorMethod,
