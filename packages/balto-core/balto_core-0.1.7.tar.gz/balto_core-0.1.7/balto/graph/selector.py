...
from balto.contracts.graph.nodes import GraphMemberNode
from balto.contracts.state import PreviousState
from balto.events.types import NoNodesForSelectionCriteria, SelectorReportInvalidSelector
from balto.exceptions import DbtInternalError, InvalidSelectorError
...
class NodeSelector(MethodManager):
    ...
    def _is_graph_member(self, unique_id: UniqueId) -> bool:
        ...
        elif unique_id in self.manifest.schedules:
            return True
    ...
    def _is_match(self, unique_id: UniqueId) -> bool:
        ...
        if unique_id in self.manifest.nodes:
            ...
            node = self.manifest.schedules[unique_id]
        else:
