...
class ElementInfo:
    ...
    def __init__(self, element_type:ControlType, name_path:Optional[str]=None, id_path:Optional[str]=None,
        ...
        """
        self.main_element = Control(control_type=element_type, class_name=class_name)
        self.parent: Optional[Control] = None
        self.child: Optional[Control] = None
        self.pop_menu: Optional[Control] = None

        if parent_id or parent_name or parent_type or parent_class:
            self.parent = Control(name=parent_name, automation_id=parent_id, control_type=parent_type, class_name=parent_class)
    ...
    def control_type(self):
        return self.main_element.control_type
    # @property
    ...
    def need_to_find_parent(self)->bool:
        return self.main_element and self.main_element.is_composite and self.parent
    ...
    def is_virtual_control(self) -> bool:
        return self.main_element.control_type.is_virtual()
    ...
    def __parse_name_path(self, name_path:Optional[str]) -> None:
        if name_path:
            if name_path.count('/') == 2:
                ...
                self.main_element.get_from_name_path(main_item)

                if child_item:
                    child_name, child_action, _ = StringUtils.split_by_keyword(child_item, '{}')
                    self.child = Control(child_name, action=ActionType.of_code(child_action))
                ...
                self.main_element.get_from_name_path(name_path)
    ...
    def __parse_id_path(self, id_path:Optional[str]) -> None:
        if id_path:
            if id_path.count('/') == 2:
                ...
                self.main_element.get_from_id_path(main_item)
                ...
                self.main_element.get_from_id_path(id_path)
