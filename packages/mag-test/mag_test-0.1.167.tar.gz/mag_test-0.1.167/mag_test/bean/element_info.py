...
class ElementInfo:
    ...
    def __init__(self, element_type:ControlType, name_path:Optional[str]=None, id_path:Optional[str]=None,
        ...
        if element_type == ControlType.MENU:
            ...
            self.__parse_name_id_path(id_path)
    ...
    def __parse_name_id_path(self, name_path:Optional[str]) -> None:
        if name_path:
            if name_path.count('/') == 2:
                ...
                self.__main_element.get_from_path(main_item)
                self.__main_element.is_composite = True

                if child_item:
                    child_name_id, child_action_str, _ = StringUtils.split_by_keyword(child_item, '{}')
                    child_name = child_name_id if child_name_id and not child_name_id.isdigit() else None
                    child_id = child_name_id if child_name_id and child_name_id.isdigit() else None

                    child_action = ActionType.of_code(child_action_str)
                    # 当子控件动作为空且其为最终控件时，设置缺省CLICK动作
                    if child_action is None and not menu_item:
                        child_action = ActionType.CLICK
                    self.__child = Control(child_name, control_type=self.main_type.child, automation_id=child_id, action=child_action)

                if menu_item:
                    menu_item_id_name, menu_type_str, _ = StringUtils.split_by_keyword(menu_item, '{}')
                    menu_item_name = menu_item_id_name if menu_item_id_name and not menu_item_id_name.isdigit() else None
                    menu_item_id = menu_item_id_name if menu_item_id_name and menu_item_id_name.isdigit() else None
                ...
                self.__main_element.get_from_path(name_path)
    ...
    #
    #             if menu_item:
    #                 menu_item_name, menu_type_str, _ = StringUtils.split_by_keyword(menu_item, '{}')
    #                 menu_type = MenuType.of_code(menu_type_str) if menu_type_str else MenuType.CONTEXT
    #                 self.pop_menu = Control(menu_item_name, menu_type=menu_type)
    #         elif '/' not in id_path:
    #             self.__main_element.get_from_path(id_path)
