...
class DriverFinderUtils:
    ...
    def find_element_by_type(driver: webdriver.Remote, name: Optional[str], control_type: Optional[ControlType] = None):
        if name:
            exp = ElementFinderUtils.global_expression(name, control_type)
            return driver.find_element(By.XPATH, exp)
        else:
            elements = DriverFinderUtils.find_elements_by_type(driver, None, control_type)
    ...
    def find_element_by_class(driver: webdriver.Remote, name: Optional[str], class_name: Optional[str] = None):
        ...
        for element in elements:
            ...
            print(f"attribute_name: {element.get_attribute("attribute_name")}")
