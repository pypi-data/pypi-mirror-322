    ...
    def __parse_name_id_path(self, name_path:Optional[str]) -> None:
        ...
        if name_path:
            if name_path.count('/') == 2:
                ...
                main_name, main_id, main_action, main_init_status = Control.parse_name_id_path(main_item)
                ...
                if child_item:
                    child_name, child_id, child_action, _ = Control.parse_name_id_path(child_item)
                ...
                if menu_item:
                    menu_item_name, menu_item_id, menu_type_str, _ = Control.parse_name_id_path(menu_item)
                ...
                main_name, main_id, main_action, main_init_status = Control.parse_name_id_path(name_path)
                self.__main_element.name = main_name
                self.__main_element.automation_id = main_id
                self.__main_element.action = main_action
                self.__main_element.init_status = main_init_status
    ...
    def __parse_id_path(self, id_path:Optional[str]) -> None:
        ...
        if id_path:
            if id_path.count('/') == 2:
                ...
                main_id, main_action, main_init_status = Control.parse_id_path(main_item)
                self.__main_element.automation_id = main_id
                self.__main_element.action = main_action
                self.__main_element.init_status = main_init_status
                self.__main_element.is_composite = True
                ...
                if child_item:
                    child_id, child_action, _ = Control.parse_id_path(child_item)

                    # 当子控件动作为空且其为最终控件时，设置缺省CLICK动作
                    if child_action is None and not menu_item:
                        child_action = ActionType.CLICK
                    self.__child = Control(control_type=self.main_type.child, automation_id=child_id, action=child_action)
                if menu_item:
                    menu_item_id, menu_type_str, _ = Control.parse_name_id_path(menu_item)

                    menu_type = MenuType.of_code(menu_type_str) if menu_type_str else MenuType.CONTEXT
                    self.pop_menu = Control(automation_id=menu_item_id, menu_type=menu_type)
            elif '/' not in id_path:
                main_id, main_action, main_init_status = Control.parse_name_id_path(id_path)
                self.__main_element.automation_id = main_id
                self.__main_element.action = main_action
                self.__main_element.init_status = main_init_status
