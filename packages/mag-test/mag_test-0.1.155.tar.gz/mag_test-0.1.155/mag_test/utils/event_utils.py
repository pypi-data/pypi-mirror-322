...
class EventUtils:
    ...
    @staticmethod
    def process_single_element(driver: AppDriver, element: WebElement, control: Control, value: Optional[str]):
    ...
    def __process_tree(driver:AppDriver, element: WebElement, action:ActionType, value: Optional[str] = None):
        if action == ActionType.SELECT:
            offset = ElementUtils.get_child_offset(value) if value else (-20, 8)
