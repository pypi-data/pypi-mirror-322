...
class ElementFinder:
    ...
    def find(driver:AppDriver, element_info:ElementInfo):
        ...
        if element_info.main_type == ControlType.MENU:
            element = ElementFinder.__find_menu(driver, element_info)
        # 窗口
        ...
        # 日期时间
            ...
            element = ElementFinder.__find_datetime(driver, element_info)
        else:
            # 查找父控件并处理响应事件
            parent = ElementFinder.__find_parent(driver, element_info)

            # 查找主控件并处理响应事件
            main_element = ElementFinder.__find_main_element(driver, parent, element_info)

            # 查找子控件并处理响应事件
            if element_info.parent_info == ControlType.TABLE:
                child_element = ElementFinder.__find_table(driver, main_element)
            else:
                child_element = ElementFinder.__find_child_element(driver, main_element, element_info)

            # 查找弹出菜单并处理响应事件
            element = ElementFinder.__find_context_menu(driver, child_element, element_info.pop_menu)

            # # 按钮/拆分按钮
            # if element_info.main_type in {ControlType.BUTTON, ControlType.SPLIT_BUTTON}:
            #     element = ElementFinder.__find_split_button(driver, main_element, element_info)
            # # 组合框
            # elif element_info.main_type == ControlType.COMBO_BOX:
            #     element = ElementFinder.__find_combox_listbox_listview_treeview(driver, main_element, element_info, ControlType.LIST_ITEM, parent)
            # # 列表框
            # elif element_info.main_type == ControlType.LIST:
            #     element = ElementFinder.__find_combox_listbox_listview_treeview(driver, main_element, element_info, ControlType.LIST_ITEM, parent)
            # # 列表视图
            # elif element_info.main_type == ControlType.LIST_VIEW:
            #     element = ElementFinder.__find_combox_listbox_listview_treeview(driver, main_element, element_info, ControlType.LIST_ITEM, parent)
            # # 树视图
            # elif element_info.main_type == ControlType.TREE:
            #     element = ElementFinder.__find_combox_listbox_listview_treeview(driver, main_element, element_info, ControlType.TREE_ITEM, parent)
            # # 表格
            # elif element_info.main_type == ControlType.TABLE:
            #     element = ElementFinder.__find_table(driver, main_element, element_info)
            # # 简单控件（包括：工具栏、Pane、GROUP、TAB项等）
            # else:
            #     element = ElementFinder.__find_main_element(driver, element_info)
    ...
    # @staticmethod
    # def __find_split_button(driver:AppDriver, main_element, element_info:ElementInfo):
    #     """
    #     查找按钮控件（包括：普通按钮和拆分按钮）
    #     参数：
    #     name 按钮名
    #     parent 父控件
    #     """
    #     return ElementFinder.__find_context_menu(driver, main_element, element_info.pop_menu)
    ...
    # @staticmethod
    # def __find_combox_listbox_listview_treeview(driver:AppDriver, main_element: WebElement, element_info:ElementInfo, child_type: ControlType, parent:Optional[WebElement] = None):
    #     """
    #         查找树视图
    #         参数：
    ...
    #         if element_info.child_action == InitStatus.EXPANDED:
    ...
    @staticmethod
    def __find_table(main_element, element_info:ElementInfo):
        """
            查找表格
            参数：
            ...
            返回：TableRow的数组或Edit
        """
        if element_info.has_child and ConvertType.of_code(element_info.child_name) is None:
            element = None

            table_rows = ElementFinderUtils.find_elements_by_type(main_element, None, ControlType.TABLE_ROW)
    ...
    @staticmethod
    def __find_child_element(driver: AppDriver, main_element: WebElement, element_info: ElementInfo):
        """
        查找树视图
        参数：
        name 树视图名，格式：树视图名/树节点名/菜单名，树视图名和菜单名可为空，只支持菜单模式的弹出菜单
        """
        if element_info.has_child:
            if element_info.child_action == InitStatus.EXPANDED:
                TreeUtils.expand_all(driver, main_element)

            element = ElementFinderUtils.find_element_by_type(main_element, element_info.child_name, element_info.child_type)
        else:
            element = main_element
    ...
    @staticmethod
    def __find_main_element(driver: AppDriver, parent: WebElement, element_info: ElementInfo):
        try:
            if element_info.main_id:
                if parent:
                    element = ElementFinderUtils.find_element_by_automation(parent, element_info.main_id)
                else:
                    element = DriverFinderUtils.find_element_by_automation_wait(driver, element_info.main_id)
            else:
                if parent:
                    element = ElementFinderUtils.find_element_by_type(parent, element_info.main_name, element_info.main_type)
                else:
                    element = DriverFinderUtils.find_element_by_type_wait(driver, element_info.main_name, element_info.main_type)

            if element and element_info.is_composite:
                offset = ElementUtils.get_parent_offset(element_info.value, element)
        ...
        if element_info.need_to_find_parent:
            try:
                ...
                if element_info.parent_id:
                    ...
                    parent = DriverFinderUtils.find_element_by_type_wait(driver, element_info.parent_name, element_info.parent_type)
