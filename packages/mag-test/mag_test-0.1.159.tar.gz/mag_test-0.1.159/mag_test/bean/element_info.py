    ...
    def __parse_name_path(self, name_path:Optional[str]) -> None:
        if name_path:
            if name_path.count('/') == 2:
                ...
                if child_item:
                    child_name, child_action_str, _ = StringUtils.split_by_keyword(child_item, '{}')
                    child_action = ActionType.of_code(child_action_str)
                    # 当子控件动作为空且其为最终控件时，设置缺省CLICK动作
                    if child_action is None and menu_item is None:
                        child_action = ActionType.CLICK
                    self.__child = Control(child_name, control_type=self.main_type.child, action=child_action)
