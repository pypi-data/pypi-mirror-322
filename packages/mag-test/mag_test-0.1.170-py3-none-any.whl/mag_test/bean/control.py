...
class Control:
    ...
    @classmethod
    def parse_name_id_path(cls, name_id_path: str, main_type: Optional[ControlType] = None):
        ...
        if name_id_path:
            ...
            if main_type in {ControlType.TREE, ControlType.TABLE}:
                init_status = InitStatus.of_code(action_name)
            else:
                action = ActionType.of_code(action_name) if action_name else ActionType.default_action(main_type)

        return name, automation_id, action, init_status

    @classmethod
    def parse_id_path(cls, id_path: str, main_type: Optional[ControlType] = None):
