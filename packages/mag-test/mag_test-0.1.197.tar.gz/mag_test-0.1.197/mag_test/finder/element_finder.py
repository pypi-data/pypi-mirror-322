    ...
    def __find_context_menu(driver:AppDriver, element: WebElement, menu_Info: Optional[Control], probe: Optional[TimeProbe]=None):
        if menu_Info and menu_Info.menu_type:
            ...
            try:
                ...
                elif menu_Info.menu_type == MenuType.MENU_ITEM:
                    actions.move_to_element(element).click(element).perform()
                    menu = DriverFinderUtils.find_element_by_class(driver, None, 'Menu')
                    # menu = DriverFinderUtils.find_element_by_type(driver, None, ControlType.MENU)
