...
class Control:
    ...
    def get_from_path(self, name_id_path: str):
        if name_id_path:
            name_id, action_name, _ = StringUtils.split_by_keyword(name_id_path, '{}')
            self.name = name_id if name_id and not name_id.isdigit() else None
            self.automation_id = name_id if name_id and name_id.isdigit() else None
