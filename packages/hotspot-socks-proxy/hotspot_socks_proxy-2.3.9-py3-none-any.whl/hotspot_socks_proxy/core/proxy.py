...
class ProxyUI:
    def __init__(self, server_ip: str):
        ...
        self.output = Window(
            content=FormattedTextControl(self._get_stats_text), always_hide_cursor=True
        )

        self.container = HSplit([self.output])
    ...
    def resolve_dns(self, domain: str) -> str:
        ...
        try:
            ...
            try:
                ...
                raise Exception(f"Both custom and system DNS resolution failed: {e!s}")
                ...
                if cmd == b"\x01":  # CONNECT
                    ...
                    if atyp == b"\x03":
                        try:
                            ...
                            console.print(
                                f"[green]Successfully resolved {addr} to {resolved_ip}"
                            )
                            addr = resolved_ip
                    ...
                    if not bound_ip.startswith(wifi_ip.rsplit(".", 1)[0]):
                        console.print(
                            "[red]Warning: Connection not using WiFi interface"
                        )
                        remote.close()
                        return

                    reply = struct.pack("!BBBB", 5, 0, 0, 1)
                    reply += socket.inet_aton(bound_ip) + struct.pack(
                        ">H", remote.getsockname()[1]
                    )
