...
async def common_parameters():
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to decode CUSTOMER_TENANTS_B64: {str(decode_err)}")
            sys.exit(1)    
       
        try:
            customer_tenants = json.loads(customer_tenants_json)
            logger.debug("Successfully parsed JSON from decoded Base64 string.")
        except json.JSONDecodeError:
            # Possibly double-escaped (like in .env)
            logger.debug("Initial JSON parsing failed. Attempting to replace escaped quotes.")
            customer_tenants_json = customer_tenants_json.replace('\\\"', '"')
            try:
                customer_tenants = json.loads(customer_tenants_json)
                logger.debug("Successfully parsed JSON after replacing escaped quotes.")
            except json.JSONDecodeError as e:
                logger.error(f"Failed to parse CUSTOMER_TENANTS_JSON after replacement: {str(e)}")
                sys.exit(1)
        ...
        display_names = [tenant.get('displayName') for tenant in customer_tenants]
        ...
        if not display_names:
            logger.error("Error: No valid display names found in CUSTOMER_TENANTS_JSON.")
            sys.exit(1)
