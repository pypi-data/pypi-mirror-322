...
def validate_iac_command(args):
    if args.publish_result and (args.branch == "" or args.repo_url == "" or args.repo_full_name == "" or args.provider == ""):
        raise MissingRequiredFlags(
            "Please provide mandatory flags (--branch, --repo-url, --repo-full-name, --provider) to publish results to SentinelOne CNS.")

    if args.provider != "" and args.provider not in SUPPORTED_GIT_PROVIDERS:
        raise InvalidInput(f"Provider not supported. Supported providers: {', '.join(SUPPORTED_GIT_PROVIDERS)}")
