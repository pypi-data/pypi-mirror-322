...
class Entity(object):
    ...
    def show(self, show_json=False, out=sys.stdout, ndjson=False):
        """
        ...
        if show_json:
            ...
            if ndjson:
                ...
                indent = 2
            out.write('{}\n'.format(dumps(json_obj, indent=indent,
                                          sort_keys=True)))
        else:
            out.write('{}\n'.format(self))
    ...
    def __eq__(self, other):
        return type(other) is type(self) and self._dict == other._dict
...
class ArchivingStatus(Entity):
    """
    Object containing information about Archiving State
    """

    @staticmethod
    def from_xml(response_xml):
        archivingStatus = response_xml.get("ArchivingStatus")
        if archivingStatus == "normal":
            ...
            return ArchivingStatus({
                "status": archivingStatus,
                "pause_id": response_xml.get("ArchivingPauseId"),
                "auto_resume_in": response_xml.get("ArchivingAutoResumeIn")
            })
    ...
    @property
    def auto_resume_in(self):
        """
        :returns int
        """
        return self.get('auto_resume_in')
