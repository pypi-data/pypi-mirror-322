...
async def save_task_result(request: TaskCompleteRequest):
    task_results_repo:TaskResultsRepository = api.state.task_results_repo
    await task_results_repo.update(agent_name, request.prompt_id, request.task_type, request.tool_call_id, TaskStatus.COMPLETED, json.dumps(request.result))
...
async def save_task_result(request: TaskCompleteRequest, background_tasks: BackgroundTasks):
    background_tasks.add_task(save_task_result, request)
    return Response(status_code=200)
