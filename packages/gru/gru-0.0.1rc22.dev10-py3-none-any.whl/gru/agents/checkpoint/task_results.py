...
class TaskResultsRepository:
    ...
    async def update(self, agent_name:str, prompt_id:str, task_type:str, run_id: str, status:TaskStatus, result:str=None):
        connection_pool = PostgresAsyncConnectionPool().get()
        async with connection_pool.connection() as conn:
            async with conn.cursor() as cur:
                await cur.execute(INSERT_QUERY, (agent_name, prompt_id, task_type, run_id, status, result))

    async def get_result(self, agent_name: str, prompt_id: str, task_type:str, run_id: str):
        connection_pool = PostgresAsyncConnectionPool().get()
        async with connection_pool.connection() as conn:
            async with conn.cursor() as cur:
                await cur.execute(select_query, (agent_name, prompt_id, task_type, run_id))
