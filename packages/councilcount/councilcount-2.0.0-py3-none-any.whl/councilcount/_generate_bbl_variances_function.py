...
from _gen_proportion_MOE_function import _gen_proportion_MOE
...
def _generate_bbl_variances(acs_year, demo_dict, pop_est_df, census_api_key, total_pop_code = None, total_house_code = None):
    ...
    variance_df = get_census(dataset = "acs/acs5/profile", # acs 5yr link
                     variables = var_code_list + MOE_code_list, 
                     year = acs_year, # 5-year acs
                     params = {
                           "for": "tract:*", # tract-level
                           "in": "state:36&in=county:005,047,081,085,061", # NYC
                           "key": census_api_key}, # census user key
                     guess_dtypes = True,
                     return_geoid = True)

    # adding unique identifier column: '2020_TRACT_ID'

    variance_df['TRACTCE'] = variance_df['GEOID'].str[5:]
    variance_df['COUNTYFP'] = variance_df['GEOID'].str[3:5]

    variance_df['COUNTYFP'] = variance_df['COUNTYFP'].astype(int) # removing leading zeroes
    variance_df['COUNTYFP'] = variance_df['COUNTYFP'].astype(str) 
    variance_df['TRACTCE'] = variance_df['TRACTCE'].astype(int) # removing leading zeroes
    variance_df['TRACTCE'] = variance_df['TRACTCE'].astype(str) 

    variance_df[str(census_year) + '_tract_id'] = variance_df['TRACTCE'] +'-'+ variance_df['COUNTYFP'] # adding unique identifier
    variance_df = variance_df.drop(columns=['GEOID','TRACTCE','COUNTYFP'])
