...
from _gen_percent_estimate_function import _gen_percent_estimate
...
def _generate_bbl_estimates(acs_year, demo_dict, pop_est_df, census_api_key, total_pop_code = None, total_house_code = None):
    ...
    # making api call
    demo_df = get_census(dataset = "acs/acs5/profile", # acs 5yr link
                 variables = var_code_list,
                 year = acs_year, # 5-year acs
                 params = {
                   "for": "tract:*", # tract-level
                   "in": "state:36&in=county:005,047,081,085,061", # NY
                   "key": census_api_key}, # census user key
                 guess_dtypes = True,
                 return_geoid = True)
    
    # adding unique identifier column for demo_df

    demo_df['TRACTCE'] = demo_df['GEOID'].str[5:]
    demo_df['COUNTYFP'] = demo_df['GEOID'].str[3:5]

    demo_df['COUNTYFP'] = demo_df['COUNTYFP'].astype(int) # removing leading zeroes
    demo_df['COUNTYFP'] = demo_df['COUNTYFP'].astype(str) 
    demo_df['TRACTCE'] = demo_df['TRACTCE'].astype(int) # removing leading zeroes
    demo_df['TRACTCE'] = demo_df['TRACTCE'].astype(str) 

    demo_df[str(census_year) + '_tract_id'] = demo_df['TRACTCE'] +'-'+ demo_df['COUNTYFP'] # adding unique identifier
    demo_df = demo_df.drop(columns=['GEOID','TRACTCE','COUNTYFP'])
