...
async def record(
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # The play event is always expected to occur within 3 seconds, so we never wait any
            ...
            while waiting:
                async with page.expect_console_message(timeout=0) as msg_info:
                    ...
                    if message.type == "log":
                        for arg in message.args:
                            ...
                            if any(
                                [
                                    ("/session" in line and "stage=first" in line),
                                    "action=p0" in line,
                                ]
                            ):
                ...
                if (datetime.datetime.now(datetime.UTC) - start).total_seconds() >= 3:
                    raise TimeoutError("Giving up waiting for console message")
