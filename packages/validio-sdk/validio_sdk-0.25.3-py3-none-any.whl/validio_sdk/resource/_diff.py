...
async def check_for_secret_changes(
    ...
    for field in DiffContext.fields():
        ...
        for name, manifest_object in manifest_resources.items():
            ...
            has_changed = await _check_secret_change(
            ...
            to_update[name] = ResourceUpdate(
                manifest_object, server_object, secrets_changed=has_changed
            )
...
async def _check_secret_change(
    ...
    return_fields = resource._api_secret_change_response_fields()
    ...
    # TODO(VR-3095): Change this to be more granular by indicating which field changed
    return any(response[return_field] for return_field in return_fields)
