...
class FintuneAltumAgeModel_EMB_MLM(pl.LightningModule):
    ...
    def on_test_epoch_end(self):  
        with open(f'{self.model_args["output_dir"]}/prediction_age_results_intervention.pkl', 'wb') as f:
            pickle.dump(self.infer_step_outputs, f)
        print(f"Prediction age results saved to {self.model_args['output_dir']}/prediction_age_results_intervention.pkl")
    ...
    def from_pretrained(self, model_args, vocab):
        model = TransformerModel(
            ...
            use_batch_labels=False,
            num_batch_labels=None,
            domain_spec_batchnorm=model_args["domain_spec_batchnorm"],
            n_input_bins=None,
            ecs_threshold=model_args["ecs_thres"],
            explicit_zero_prob=False,
            use_fast_transformer=model_args["fast_transformer"],
