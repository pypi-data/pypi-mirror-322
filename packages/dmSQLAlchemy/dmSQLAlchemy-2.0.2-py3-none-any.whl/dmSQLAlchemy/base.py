...
class DMDialect(default.DefaultDialect):
    ...
    def do_execute(self, cursor, statement, parameters, context=None):
        ...
        version_info = pkg_resources.get_distribution('dmPython').version.split(".")
    ...
    def _get_all_objects(self, connection, schema, scope, kind, filter_names, dblink, **kw):
        ...
        sql_str += (
            ...
            "AND TABLE_NAME IN(\'" + self.denormalize_name(filter_names[0]) + "\'")
    ...
    def get_multi_columns(self, connection, schema, filter_names, scope, kind, dblink=None, **kw):
        ...
        query = """SELECT a_tab_cols.table_name, a_tab_cols.column_name, a_tab_cols.data_type,\n"""\
                    ...
                    """WHERE a_tab_cols.column_name = a_col_comments.column_name\n"""\
    ...
    def _get_indexes_rows(self, connection, schema, filter_names, scope, dblink=None, **kw):
        ...
        if hasattr(self, 'multi_constraint_data_cache'):
            schema = self.denormalize_name(schema or self.default_schema_name)
            if self.multi_constraint_data_cache['schema'] == schema:
                if self.multi_constraint_data_cache['all_objects'] == filter_names:
                    result = self.multi_constraint_data_cache['result']
                elif self.denormalize_name(filter_names[0]) in self.multi_constraint_data_cache['all_objects']:
                    position = self.multi_constraint_data_cache['all_objects'].index(self.denormalize_name(filter_names[0]))
                    result = [self.multi_constraint_data_cache['result'][position]]

        if result is None:
            result = self.get_multi_constraint_data(connection, schema, filter_names, scope, None, dblink)
    ...
    def get_multi_constraint_data(self, connection, schema, filter_names, scope, kind, dblink=None, **kw):
        ...
        self.multi_constraint_data_cache = {}
        self.multi_constraint_data_cache['result'] = result
        self.multi_constraint_data_cache['schema'] = schema
        self.multi_constraint_data_cache['all_objects'] = all_objects
        self.multi_constraint_data_cache['dblink'] = dblink
        return result
    ...
    def get_multi_foreign_keys(self, connection, *, schema, filter_names, scope, kind, dblink=None, **kw):
        ...
        if hasattr(self, 'multi_constraint_data_cache'):
            schema = self.denormalize_name(schema or self.default_schema_name)
            if self.multi_constraint_data_cache['schema'] == schema and self.multi_constraint_data_cache[
                'all_objects'] == all_objects:
                result = self.multi_constraint_data_cache['result']

        if result is None:
            result = self.get_multi_constraint_data(connection, schema, all_objects, scope, kind, dblink)
    ...
    def get_multi_unique_constraints(self, connection, *, schema, filter_names, scope, kind, dblink=None, **kw):
        ...
        if result is None:
            result = self.get_multi_constraint_data(connection, schema, all_objects, scope, None, dblink)
