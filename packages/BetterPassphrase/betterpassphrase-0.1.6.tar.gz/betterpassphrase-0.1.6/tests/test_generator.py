...
def test_cli_integration():
    ...
    assert min_length <= len(result.strip().split("-")) <= max_length, "Invalid length of phrase"
    assert min_length <= len(extract_capitals(result)) <= max_length, "Invalid number of capital letters"
    temp_file.unlink()
