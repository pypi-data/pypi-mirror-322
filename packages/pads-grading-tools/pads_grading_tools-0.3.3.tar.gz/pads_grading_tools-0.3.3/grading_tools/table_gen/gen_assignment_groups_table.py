...
from grading_tools.common.utils import read_moodle_groups_csv
...
def gen_group_info_table(output_file: str, moodle_groups_list: str, group_formats: list[str],
                         naming_dictionary: NamingDictionary, grouping_names: list[str] | None = None, **config):
    moodle_df = read_moodle_groups_csv(moodle_groups_list)
    ...
    tables = [
        moodle_df.set_index(naming_dictionary.MOODLE_GROUPS_MATR_COL)[list(naming_dictionary.MOODLE_GROUPS_IDENTITY_COLS)]]
    for grouping in full_groups_lists_df.columns:
        tables.append(
...
def register_gen_group_info(parser: argparse.ArgumentParser, naming_dictionary: NamingDictionary, **defaults):
    ...
    parser.add_argument('-m', '--moodle-groups-list', required=True,
