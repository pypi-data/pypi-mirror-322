...
def call(config_file: str = None, command: list[str] = None, path: list[str] = None, remainder: list[str] = None, **config):
    cfg: FileCFG
    with open(config_file, 'r') as f:
        cfg = FileCFG.model_validate(yaml.full_load(f))

    specific_args = dig_dict(cfg.arguments, itertools.chain(command, path))

    if not specific_args:
        print(f'There is no config for {command} in the cfg file: {config_file}.')

    arg_list = list(command) # the first two commands could be module + command or just command, after that it may be a named config
...
def register_parser(parser: argparse.ArgumentParser, **defaults):
    ...
    parser.add_argument('-p', '--path', type=str, help='Optionally, the path to the specific config for the command.', required=False, nargs='+', default=[])
