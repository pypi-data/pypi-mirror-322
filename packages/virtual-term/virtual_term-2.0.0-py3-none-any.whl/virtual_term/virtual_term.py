...
class AsyncPtyProcess:
    ...
    async def spawn(self):
        ...
        if self.child_pid == 0:
            ...
            raise SystemExit(0)
        self.writer = await self._create_stream_writer(self.fd)
        self.reader = self._create_stream_reader(self.fd)

    @staticmethod
    async def _create_stream_writer(fd: int) -> asyncio.streams.StreamWriter:
        loop = asyncio.get_event_loop()
        transport, protocol = await loop.connect_write_pipe(
            lambda: asyncio.streams.FlowControlMixin(loop=loop), os.fdopen(fd, 'wb')
        )
    ...
    async def write(self, data: str):
        ...
        if not self.writer:
            raise TerminalDeadError('PTY writer not initialized.')
    ...
    async def stop(self):
        ...
        self.fd = self.reader = self.writer = self.child_pid = None
