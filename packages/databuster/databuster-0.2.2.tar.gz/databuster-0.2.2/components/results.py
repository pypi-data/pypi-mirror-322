...
from rdkit.Chem import Descriptors, rdDepictor, Draw, rdMolDescriptors, DataStructs
...
def create_complete_analysis_df(results, uploaded_data, smiles_column):
    ...
    if "Chirality Analysis" in results:
        complete_df = ChemicalAnalyzer.process_chirality_info(
            complete_df, smiles_column)
...
def display_percentage_metric(label,
                              count,
                              total,
                              threshold=15.0,
    ...
    st.metric(label, f"{count}", f"{percentage:.1f}%", delta_color=delta_color)
...
def render_results_section():
    ...
    if not hasattr(st.session_state,
                   'uploaded_data') or st.session_state.uploaded_data is None:
        st.info("Please upload a dataset first.")
        return

    if not hasattr(
            st.session_state,
            'analysis_results') or st.session_state.analysis_results is None:
        st.info(
    ...
    try:
        ...
        complete_df = create_complete_analysis_df(
            results, st.session_state.uploaded_data,
        ...
        st.download_button(
            ...
            mime="text/csv",
            help=
            "Download all analysis results including molecular properties, stereochemistry, duplicates, and salts",
        ...
        unique_compounds = len(
            set(st.session_state.uploaded_data[
                st.session_state.smiles_column]))
        duplicate_rate = (1 - unique_compounds /
                          total_compounds) * 100 if total_compounds > 0 else 0

        # Count valid SMILES
        valid_smiles_count = sum(
            check_valid_smiles(smiles) for smiles in
            st.session_state.uploaded_data[st.session_state.smiles_column])

        # Process detection results
        complete_df = detect_peptide_like(complete_df,
                                          st.session_state.smiles_column)
        complete_df = detect_macrocyclic_like(complete_df,
                                              st.session_state.smiles_column)
        complete_df = detect_sugar_like(complete_df,
                                        st.session_state.smiles_column)
        ...
        compounds_with_salts = sum(
            1 for smiles in st.session_state.uploaded_data[
                st.session_state.smiles_column]
        ...
        with col2:
            display_percentage_metric("Valid SMILES",
                                      valid_smiles_count,
                                      total_compounds,
                                      inverse_colors=True)
        with col3:
            display_percentage_metric("Compounds with Salts",
                                      compounds_with_salts, total_compounds)
        with col4:
            display_percentage_metric("Chiral (Specified)", chiral_with_stereo,
                                      total_compounds)
        with col5:
            display_percentage_metric("Macrocyclic-like", macrocyclic_count,
                                      total_compounds)
        with col6:
            display_percentage_metric("Peptide-like", peptide_count,
                                      total_compounds)
        with col7:
            display_percentage_metric("Sugar-like", sugar_count,
                                      total_compounds)
        with col8:
            display_percentage_metric("Duplicate Rate",
                                      total_compounds - unique_compounds,
                                      total_compounds)

        # Create updated tabs for different analysis sections
        tabs = st.tabs([
            "About Databuster", "Structure Analysis", "Molecular Properties",
            "Drug-like", "Duplicates & Salts", "Other Observations"
        ])
        ...
        with tabs[0]:
            ...
            st.image("DataBuster_LandingImage.png")

        # Structure Analysis Tab
        with tabs[1]:
            with st.expander("Visualization"):
                st.write(
                    "Chirality analysis is performed to identify chiral compounds and their chiral centers."
                )
            ...
            with st.expander("Stereochemistry Insights"):
                st.write(
                    "Carryout the deep dive into the data and provides detailed information about the stereochemistry of the dataset."
                )
                # Structure flags
                st.subheader("Structure Analysis")
                flag_col1, flag_col2, flag_col3 = st.columns(3)

                # Chiral Compounds Metric
                with flag_col1:
                    if isinstance(results,
                                  dict) and "Chirality Analysis" in results:
                        # Calculate chiral center count for each compound
                        chiral_compounds = [
                            compound
                            for compound in results["Chirality Analysis"]
                            if Chem.MolFromSmiles(compound) and Chem.
                            FindMolChiralCenters(Chem.MolFromSmiles(compound),
                                                 includeUnassigned=True)
                        ]

                        chiral_count = len(chiral_compounds)

                        # Display metric for chiral compounds
                        st.metric(
                            "Chiral Compounds", f"{chiral_count}"
                            if chiral_count > 0 else "0 (None)",
                            f"{(chiral_count / total_compounds) * 100:.1f}%"
                            if total_compounds > 0 else "0.0%")

                # Specified and Unspecified Stereochemistry Metrics
                with flag_col2:
                    if isinstance(results,
                                  dict) and "Chirality Analysis" in results:
                        # Separate compounds based on specified and unspecified stereochemistry
                        specified_stereo_compounds = [
                            compound
                            for compound in results["Chirality Analysis"]
                            if Chem.MolFromSmiles(compound) and any(
                                center[1] != '?'
                                for center in Chem.FindMolChiralCenters(
                                    Chem.MolFromSmiles(compound),
                                    includeUnassigned=True))
                        ]

                        # Count for each category
                        specified_stereo_count = len(
                            specified_stereo_compounds)

                        # Display metrics for compounds with specified and unspecified stereochemistry
                        st.metric(
                            "Compounds with Specified Stereo",
                            f"{specified_stereo_count}"
                            if specified_stereo_count > 0 else "0 (None)",
                            f"{(specified_stereo_count / total_compounds) * 100:.1f}%"
                            if total_compounds > 0 else "0.0%")

                with flag_col3:
                    if isinstance(results,
                                  dict) and "Chirality Analysis" in results:
                        # Count compounds without specified stereochemistry
                        without_specified_stereo_compounds = [
                            compound
                            for compound in results["Chirality Analysis"]
                            if Chem.MolFromSmiles(compound) and any(
                                center[1] == '?'
                        ...
                        without_specified_stereo_count = len(
                            without_specified_stereo_compounds)

                        st.metric(
                            "Compounds without Specified Stereo",
                            f"{without_specified_stereo_count}" if
                            without_specified_stereo_count > 0 else "0 (None)",
                            f"{(without_specified_stereo_count / total_compounds) * 100:.1f}%"
                            if total_compounds > 0 else "0.0%")

                # Chirality Details
                if "Chirality Analysis" in results:
                    st.write("### Chiral Compounds")

                    # User-provided title selection
                    compound_title = st.selectbox(
                        "Select compound title field",
                        st.session_state.uploaded_data.columns,
                        key="compound_title_selectbox")

                    chiral_df = pd.DataFrame(results["Chirality Analysis"],
                                             columns=["SMILES"])

                    # Ensure selected title field exists in uploaded data
                    if compound_title in st.session_state.uploaded_data.columns:
                        chiral_df = chiral_df.merge(
                            st.session_state.uploaded_data,
                            left_on="SMILES",
                            right_on=st.session_state.smiles_column,
                            how="left")

                        # Initialize 'Group' and 'Group Size' columns
                        chiral_df["Chiral Group"] = None
                        chiral_df["Group Size"] = None
                        chiral_df["Stereo"] = None

                        # Filter chiral_df where ChiralCenterCount > 0
                        chiral_df = chiral_df[chiral_df['ChiralCenterCount'] >
                                              0]
                        st.dataframe(chiral_df, use_container_width=True)

                        # Download Chirality Analysis
                        csv = chiral_df.to_csv(index=False)
                        st.download_button("Download Chirality Analysis Data",
                                           csv,
                                           "chirality_analysis.csv",
                                           "text/csv",
                                           key="chirality_download_button")

            with st.expander("PAINS Alerts"):
                st.write(
                    "Identify PAINS alerts in the dataset and their corresponding tags."
                )
                # PAINS Alerts section
                if st.session_state.uploaded_data is not None:
                    combined_df = st.session_state.uploaded_data.copy()
                    smiles_column = st.session_state.smiles_column
                    combined_df = compute_pains_alerts(combined_df,
                                                       smiles_column)
                    # Show dataframe where PAINS_alert is True
                    pains_df = combined_df[combined_df['PAINS_alert']]
                    st.subheader("Compounds with PAINS Alerts")
                    st.dataframe(pains_df, use_container_width=True)

                    # Create PAINS alerts bar plot
                    pains_plot = create_pains_alerts_bar_plot(combined_df)
                    if pains_plot:
                        st.plotly_chart(pains_plot, key='pains_plot')

                    # Add mols2grid visualization for PAINS compounds
                    if not pains_df.empty:
                        st.write("### PAINS Compounds Visualization")
                        pains_smiles = pains_df[smiles_column].tolist()
                        grid_view = create_mols2grid_view(pains_smiles[:20])
                        if grid_view:
                            st.write(grid_view)
                else:
        ...
        # Drug-like Tab with mols2grid
        with tabs[3]:
            st.subheader("Drug-like Compounds Analysis")
            drug_like_smiles = [
                smiles
                for smiles in complete_df[st.session_state.smiles_column]
                if ChemicalAnalyzer.is_drug_like(smiles)
            ]
            if drug_like_smiles:
                st.write(f"Found {len(drug_like_smiles)} drug-like compounds")
                grid_view = create_mols2grid_view(drug_like_smiles[:20])
                if grid_view:
                    st.write(grid_view)

            lipinski_fig, violations = create_lipinski_analysis(complete_df)
            if lipinski_fig:
                st.plotly_chart(lipinski_fig)

        # Duplicates & Salts Tab with mols2grid
        with tabs[4]:
            ...
            if "Duplicate Detection" in results:
                ...
                with dup_col1:
                    if 'exact' in duplicates:
                        ...
                        exact_smiles = [
                            st.session_state.uploaded_data.iloc[idx][
                                st.session_state.smiles_column]
                        ...
                        if exact_smiles:
                            st.write(
                                f"Found {len(exact_smiles)} exact duplicates")
                            grid_view = create_mols2grid_view(
                                exact_smiles[:10])
                            if grid_view:
                                st.write(grid_view)
                ...
                with dup_col2:
                    if 'desalted' in duplicates:
                        ...
                        if desalted_smiles:
                            st.write(
                                f"Found {len(desalted_smiles)} desalted duplicates"
                            )
                            grid_view = create_mols2grid_view(
                                desalted_smiles[:10])
                            if grid_view:
                ...
                with dup_col3:
                    if 'similar' in duplicates:
                        ...
                        if similar_smiles:
                            st.write(
                                f"Found {len(similar_smiles)} similar compounds"
                            )
                            grid_view = create_mols2grid_view(
                                similar_smiles[:10])
                            if grid_view:
            ...
            # Salt Analysis Section with mols2grid            st.subheader("Salt Analysis")
            if "Salt Detection" in results:
                salt_compounds = results["Salt Detection"]
                if salt_compounds:
                    st.write(
                        f"Found {len(salt_compounds)} compounds with salts")
                    grid_view = create_mols2grid_view(salt_compounds[:20])
                    if grid_view:
                        st.write(grid_view)

        # Other Observations Tab with mols2grid
        with tabs[5]:
            ...
            peptide_smiles = [
                smiles for smiles, is_peptide in zip(
                    complete_df[st.session_state.smiles_column],
                    complete_df['Peptide_like']) if is_peptide
            ]
            if peptide_smiles:
                st.write(f"Found {len(peptide_smiles)} peptide-like compounds")
                grid_view = create_mols2grid_view(peptide_smiles[:20])
                if grid_view:
                    st.write(grid_view)
            ...
            macro_smiles = [
                smiles for smiles, is_macro in zip(
                    complete_df[st.session_state.smiles_column],
                    complete_df['Macrocyclic_like']) if is_macro
            ]
            if macro_smiles:
                st.write(f"Found {len(macro_smiles)} macrocyclic compounds")
                grid_view = create_mols2grid_view(macro_smiles[:20])
                if grid_view:
                    st.write(grid_view)
            ...
            sugar_smiles = [
                smiles for smiles, is_sugar in zip(
                    complete_df[st.session_state.smiles_column],
                    complete_df['Sugar_like']) if is_sugar
            ]
            if sugar_smiles:
                st.write(f"Found {len(sugar_smiles)} sugar-like compounds")
                grid_view = create_mols2grid_view(sugar_smiles[:20])
                if grid_view:
                    st.write(grid_view)
