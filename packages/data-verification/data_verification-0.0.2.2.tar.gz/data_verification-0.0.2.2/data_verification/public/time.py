...
class Time:
    ...
    def datetime_range(time: str, start_time: str, end_time: str, time_format: str = "%Y-%m-%d %H:%M",
        ...
        try:
            ...
            dt = datetime.strptime(str(time), time_format)
            start_dt = datetime.strptime(str(start_time), time_format)
    ...
    def check_timeliness(timestamp_str, date_format="%Y-%m-%d %H:%M:%S", timeliness_threshold_minutes=60):
        ...
        try:
            ...
            data_timestamp = datetime.strptime(str(timestamp_str), date_format)
        except ValueError:
            return False
        # 获取当前时间
        now = datetime.now()
