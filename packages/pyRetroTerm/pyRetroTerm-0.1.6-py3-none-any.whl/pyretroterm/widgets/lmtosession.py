...
from pyretroterm.themes2 import LayeredHUDFrame, ThemeLibrary
...
class DownloadThread(QThread):
    ...
    def is_valid_IP_Address(self, device_dict):
        try:
            system_ips = next((prop['value'] for prop in device_dict['system_properties']
                               if prop['name'] == 'system.ips'), None)
            if not system_ips:
                return False

            ip_list = system_ips.split(',')
            return any(self.validate_single_ip(ip) for ip in ip_list)
        except (KeyError, AttributeError):
            return False

    def validate_single_ip(self, ip):
        try:
            ...
            for x in range(0, 3):
                try:
                    ...
                    for device in response.items:
                        ...
                        if self.is_valid_IP_Address(device_dict):  # Pass full device dict
...
class LMDownloader(QWidget):
    def __init__(self, parent=None):
        super().__init__()
        self.parent = parent
        self.parent.lmimporter = self
        self.title = 'LogicMonitor to Terminal YAML Exporter'

        # Load LogicMonitor settings
        try:
            with open("logicmonitor_settings.json", "r") as f:
                settings = json.load(f)
            self.default_company = settings.get("company", "")
            self.default_access_id = settings.get("access_id", "")
            self.default_access_key = settings.get("access_key", "")
            self.default_cert_path = settings.get("cert_path", "")
        except FileNotFoundError:
            create_default_settings()
            self.default_company = ""
            self.default_access_id = ""
