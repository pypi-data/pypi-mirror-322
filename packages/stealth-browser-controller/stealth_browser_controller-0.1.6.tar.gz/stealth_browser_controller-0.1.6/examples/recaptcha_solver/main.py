...
def main():
    ...
    browser = StealthBrowserController(
        "google-chrome"
    )  # enter path of the browser here
    browser.open("https://www.google.com/recaptcha/api2/demo")

    ele = browser.find_element_by_image("imgs/1.png", timeout=60)
    print(ele.x, ele.y)
    ele.click()

    is_solved = bool(browser.find_element_by_image("imgs/x.png", timeout=10))
    if is_solved:
        browser.close()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        n += 1
        print(f"Iteration {n}")
        print(f"Success rate: {n_success / n}")
