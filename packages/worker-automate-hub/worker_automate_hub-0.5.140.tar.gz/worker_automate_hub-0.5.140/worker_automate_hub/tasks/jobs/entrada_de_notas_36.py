...
async def entrada_de_notas_36(task: RpaProcessoEntradaDTO) -> RpaRetornoProcessoDTO:
    ...
    try:
        ...
        config_nfe_vencimentos = await get_config_by_name("NFE_Vencimentos")
        console.print(task)
        ...
        for registro in config_nfe_vencimentos.conConfiguracao:
            if registro.get("codigoEmpresa") == nota.get("codigoEmpresa"):
                dias_vencimento = int(registro.get("dias", 20))
        ...
        if '0,00' in valores_informado_text:
            ...
            dt_emissao = data_emissao_dt + timedelta(days=dias_vencimento)
