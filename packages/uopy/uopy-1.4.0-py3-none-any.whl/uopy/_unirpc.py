...
def _get_dump_from_bytes(message, bytes_array):
    ...
    for count, a_byte in enumerate(bytes_array):
        header_str += (
            "{:02x}".format(a_byte) if (count == 0 or (count % 4) != 0) else " "
        )
...
class UniRPCPacket(_UniRPC):
    ...
    def receive(self, rpc_socket):
        ...
        while len(self.data_buffer) < self.data_length:
            self.data_buffer.extend(
                rpc_socket.receive(self.data_length - len(self.data_buffer))
            )
        ...
        if (
            _logger.getEffectiveLevel() >= logging.DEBUG
            and _logging_config[LOG_UNIRPC_DEBUG]
        ):
            self._dump()
    ...
    def _check_index(self, index):
        if index == 0:
            ...
            raise UOError(
                ...
                "All packet arguments must be inserted in order, "
                "starting with zero.",
            )
            ...
            raise UOError(
                ...
                "A maximum of "
                + str(_RPC_MAX_ARGS)
                + " packet arguments are supported.",
            )
    ...
    def _compress(self):
        ...
        if (self.compression_threshold > 0) and (
            self.get_data_length() > self.compression_threshold
        ):
    ...
    def _unpack_data(self):
        ...
        if len(self.data_buffer) > 0:
            ...
            for d_length, d_type in self.data_info:
                ...
                elif (d_type == _PACKET_CHAR) or (d_type == _PACKET_FUNCNAME):
                    tmp_len = d_length + 1
                    tmp_len = (tmp_len + 3) & ~3
    ...
    def _dump(self):
        ...
        for idx, info_tuple in enumerate(self.data_info):
            ...
            if info_tuple[1] == _PACKET_CHAR or info_tuple[1] == _PACKET_STRING:
                try:
                    ...
                    code_list = [
                        str(hex(v)) if type(v) is int else str(hex(ord(v)))
                        for v in value
                    ]
        ...
        if type(value) == str:
            try:
                ...
                code_list = [
                    str(hex(v)) if type(v) is int else str(hex(ord(v))) for v in value
                ]
                value = "".join(code_list)
                pass

        parsed_header += "Arguments[{}] = {} type = {}({}) Length={}\n".format(
            idx, value, info_tuple[1], type_dict[info_tuple[1]], info_tuple[0]
        )
