...
class ImageSequenceProcessor:
    def __init__(self, input_file, output_file, directory, chunk_size=1000, max_workers=4):
        self.input_file = f'{directory}/{input_file}'
    ...
    def process(self):
        try:
            ...
            print("the ordered chunks are ", ordered_chunks)
            
            # Combine all chunks
            if ordered_chunks:
                print("Combining chunks into final video...")
                if self.combine_chunks(ordered_chunks):
                    print(f"Successfully created final video: {self.output_file}")
                else:
...
def build_with_new_design_parallelized(build_card_name, feature_descriptions, intro_text, preview_screen_text, process_mode, feature_images_urls, feature_ids, existing_audios, device, preview_feature_image_urls, feature_names, subscription_key, service_region): 
    ...
    combine_frames_parallely(directory_name, temp_text_file)
...
def combine_frames_parallely(directory, temp_text_file):

    processor = ImageSequenceProcessor(
        input_file=temp_text_file,
        output_file='main_new_new.mp4',
        directory=directory,
        chunk_size=1000,
        max_workers=4
    )
