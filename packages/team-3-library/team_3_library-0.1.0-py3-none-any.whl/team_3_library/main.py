...
class dataset_clean():
    def __init__(self,dataframe):
        self.df=dataframe
    def analyze_nulls(self): #sacar el porcentaje de los valores nulos
    ...
    def analyze_distribution(self, column):
        ...
        sesgo = (3*(mean-median)) / std_dev if std_dev!=0 else 0 #para calcular la asimetría

        if abs(sesgo) < 0.5: #si el sesgo es menor a 0.5 se considera una distribucion simétrica y por eso es normal 
            print('The distribution is normal')
            return 'normal'
        elif abs(sesgo) >= 0.5: #si es mayor o igual se considera una distribución asimétrica, sesgada
    ...
    def clean_column_customized(self, column):
        ...
        if null_count ==  0:
            ...
            if self.df[column].dtype in [np.float64, np.int64]:
                ...
                self.graph_num(column)
            elif self.df[column].dtype in [object]:
    ...
    def clean_dataset_percentage(self, percentage):
        ...
        for c in self.df.columns:
            ...
            print(self.df[c])
            print('Now the number of null values is %i'%self.df[c].isnull().sum())
    def percentage(self):
