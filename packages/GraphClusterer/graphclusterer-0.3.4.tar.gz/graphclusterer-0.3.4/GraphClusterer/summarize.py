...
def summarize_per_color(subgraphs: List[nx.Graph], name: str, vertices: pd.DataFrame, aspects: List, print_info: bool,
    ...
    for i, subgraph in enumerate(subgraphs, start=1):
        ...
        response = co.generate_response(prompt=instructions_command_r + combined_abstracts, max_tokens_=500)
        summary = response
        ...
        output = rep.generate_responses(prompt=input_params['prompt'], max_tokens_=500)
        summary = "".join(output)
        ...
        output = rep.generate_responses(prompt=input_params['prompt'], max_tokens_=300)
        title = "".join(output)
        title = title.replace('"', '')
...
def improve_summary(summary, data, scores, score_names, name, summary_model, refinement_model):
    ...
    for score, name in zip(scores, score_names):  # For each score.
        ...
        # Generate the summary using Cohere's summarize API
        response = co.generate_response(prompt=prompt, max_tokens_=500)
