...
def the_almighty_function(pipeline_kwargs: dict):
    ...
    cohere_key = pipeline_kwargs.get('cohere_key', None)  # coherence key for the summaries
    llama_key = pipeline_kwargs.get('llama_key', None)  # llama key for the summaries
    ...
    for i in range(iteration_num):
        ...
        run_summarization(name, vertices, aspects, print_info, cohere_key, llama_key)
        ...
        sr = sg.starr(name, vertices, G, cohere_key)
    ...
    for iter_ in range(iteration_num):
        rel, coh, con, flu, scores = evaluate.metrics_evaluations(name, vertices, G, cohere_key)
        if print_info:
        ...
        if kill_switch:
            break
        summarize.improve_summaries(name, vertices, scores, cohere_key, llama_key)
