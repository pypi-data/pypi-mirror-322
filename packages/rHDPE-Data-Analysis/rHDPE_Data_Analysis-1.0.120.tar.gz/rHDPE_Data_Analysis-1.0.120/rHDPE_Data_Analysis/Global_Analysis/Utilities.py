...
def predict_escr_from_shm( ip, name_appendage ):
    ...
    array = np.hstack( (features_df["SHM"].to_numpy()[:, np.newaxis], features_df["ESCR_50%Failure"].to_numpy()[:, np.newaxis]) )

    df = gu.array_with_column_titles_and_label_titles_to_df( array, ["SHM", "ESCR"], features_df.index.tolist() )

    df.to_csv( ip.output_directory + "Global/Sandbox/ESCR_Prediction/SHM_ESCR_Correlation" + name_appendage + ".csv", float_format = "%.3f" )
    ...
    if False:
        ...
        gu.plot_scatterplot_of_two_features( ip.directory, prediction, target_escr.to_numpy(), features_df.index, [resin_data.loc[i]["Label"] for i in features_df.index], errorbars = False, std = "", line_of_best_fit = False, exponential_fit = False, xlabel = "Prediction", ylabel = "Target", savefig = False, filename = "" )
...
def escr_predictor( ip, features_df, name_appendage = "" ):

    # 0.8110163920280077 (2, 11, 12) Index(['DSC_C_Onset', 'TGA_Temp_480-500', 'Rhe_100.00'], dtype='object')

    reg_escr_shm = predict_escr_from_shm( ip, name_appendage )
    ...
    print( score, best_combination, X.iloc[:, list( best_combination )].columns )

    X1 = X.iloc[:, list( best_combination )]

    reg = LinearRegression().fit( X1, target )

    prediction = reg.predict( X1 )

    cache = [ip.datasets_to_read, ip.sample_mask]

    ip.datasets_to_read, ip.sample_mask = [7], list( map( int, features_df.index.tolist() ) )

    features_df2, _, _ = Preprocessing.read_files_and_preprocess( ip, normalised = False )

    ip.datasets_to_read, ip.sample_mask = cache[0], cache[1]

    prediction_unnormalised = prediction * (features_df2["SHM"].max() - features_df2["SHM"].min()) + features_df2["SHM"].min()

    prediction_escr = prediction_unnormalised * reg_escr_shm.coef_[0] + reg_escr_shm.intercept_

    array = np.hstack( (prediction_unnormalised[:, np.newaxis], prediction_escr[:, np.newaxis]) )

    df = gu.array_with_column_titles_and_label_titles_to_df( array, ["SHM", "ESCR"], features_df.index.tolist() )

    df.to_csv( ip.output_directory + "Global/Sandbox/ESCR_Prediction/ESCR_Prediction" + name_appendage + ".csv", float_format = "%.3f" )
