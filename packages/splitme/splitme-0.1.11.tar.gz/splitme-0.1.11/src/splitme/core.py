"""Core methods for splitting markdown content into sections."""
...
from splitme.tools.markdown.reflink_extractor import ReferenceLinkExtractor
from splitme.utils.filename_sanitizer import sanitize_filename
...
class MarkdownSplitter:
    ...
    def __init__(self, settings: SplitmeApp | None = None) -> None:
        self.settings = settings or SplitmeApp()
        self._compile_patterns()
        _logger.debug(f"MarkdownSplitter initialized with settings: {self.settings}")

    def process_file(self, content: str) -> List[Section]:
        """Process markdown file, split it, and handle additional steps."""
        _logger.info("Processing markdown content...")
        sections = self.split(content)
        output_dir = Path(self.settings.split.output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)
        _logger.debug(f"Verified output directory: {output_dir}")
        ...
        if hasattr(self.settings, "process_mkdocs"):
            _logger.info(f"Processing mkdocs.yml with settings: {self.settings.mkdocs}")
    ...
    def split(self, content: str) -> List[Section]:
        ...
        _logger.info(f"Successfully split document into {len(sections)} sections.")
        return sections
