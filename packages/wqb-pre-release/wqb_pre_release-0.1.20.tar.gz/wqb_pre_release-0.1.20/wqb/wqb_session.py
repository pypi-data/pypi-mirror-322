...
class WQBSession(AutoAuthSession):
    ...
    def patch_properties(
        ...
        *args,
        ...
        category: Category | Null | None = None,
    ...
    async def retry(
        ...
        **kwargs,
        ...
        value_errors = 0
        if log is not None:
            self.logger.info(f"{self}.{'retry'} [{max_tries}]: {log}")
    ...
    async def concurrent_simulate(
        ...
        **kwargs,
        ...
        if log is not None:
            self.logger.info(
                f"{self}.{'concurrent_simulate'} [{total}, {concurrency._value}]: {log}"
            )
    ...
    async def concurrent_check(
        ...
        **kwargs,
        ...
        if log is not None:
            self.logger.info(
                f"{self}.{'concurrent_check'} [{total}, {concurrency._value}]: {log}"
            )
        resp = await concurrent_await(
            (
                self.check(
