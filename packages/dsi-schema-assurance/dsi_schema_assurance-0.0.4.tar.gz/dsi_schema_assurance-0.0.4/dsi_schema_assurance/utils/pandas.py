...
def df_diff_reporter(df_1: DataFrame, df_2: DataFrame) -> DataFrame:
    """
    ...
    missing_records = len(
        list(
            filter(
                lambda x: x == True, (all(diff.iloc[i]) for i in range(diff.shape[0]))
            )
        )
    )
    ...
    diff_records = len(
        list(
            filter(
                lambda x: x == True,
                (
                    any(diff.iloc[i]) and not all(diff.iloc[i])
