...
class NorthbeamService:
    def __init__(self, northbeam_api_key, northbeam_data_client_id, collection_name, dbname, mongo_connection_url, slack_client, tjc_configuration_s3_path, tjc_transformation_s3_path, job_name, use_production=False):
        ...
        self.tjc_transformation_s3_path = tjc_transformation_s3_path
    ...
    def run_northbeam_data_pipeline(self):
        ...
        try:
            ...
            timezone = self.get_timezone_from_config(self.tjc_transformation_s3_path)
    ...
    def get_timezone_from_config(self, s3_path):
        print('readig config tz')
        configuration = self.read_json_from_s3(s3_path)
        print('look at configuration', configuration)
    ...
    def update_data_load_history(self):
        ...
        try:
            ...
            print('look here', mongo_connection_url, mongo_database_name, transformation_path)

            client = MongoClient(mongo_connection_url)
            db = client[mongo_database_name]

            if not (mongo_connection_url or mongo_database_name):
                raise ValueError(f"Mongodb credential(s) are missing. Stopping job: {self.job_name}")
            ...
            output = self.notify_on_slack(
                status="SUCCESS",
                message=f"Glue job '{self.job_name}' completed successfully!",
            ...
            output = self.notify_on_slack(
                status="FAILURE",
                message=f"Glue job '{self.job_name}' failed.",
                error=error_message,
