class StateMachine(object):
  ...
  def __init__(self, owner, name, state_machine_transitions, state, on_step_callback):
    self.__owner = owner
    self.__name = name
  ...
  def state_machine_step(self):
    ...
    # check that only one transition can be done
    ...
    self.__owner._create_notification(msg=msg)
    self.__owner.P(msg, color='y')
    self.__state = next_state

    if self.__on_step_callback is not None:
...
class _StateMachineAPIMixin(object):
  ...
  def state_machine_api_init(self, name: str, state_machine_transitions: dict, initial_state: str, on_successful_step_callback: callable):
    if name in self.__state_machines:
      raise Exception(f"State machine with name `{name}` already exists!")
    self.__state_machines[name] = StateMachine(self, name, state_machine_transitions, initial_state, on_successful_step_callback)
    return
