...
def qpe(unitary_matrix, t_counting_qubits, namedraw='CU'):
    ...
    :param target_qubit: The qubit containing the eigenstate
    """

    qc = Circuit(t_counting_qubits + 1)
    target_qubit = t_counting_qubits
    ...
    for i in range(t_counting_qubits):
        myCU = CU(unitary_matrix, control_qubits=[i], target_qubits=[target_qubit])
    ...
    #qc.measure(range(t_counting_qubits), range(t_counting_qubits))
