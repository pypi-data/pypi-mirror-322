...
def list_datasets_v2_from_neo(dataset_id: str):
    ...
    if dataset_id:
        ...
        url = "{}/dataset/list/all".format(NEO_DEV_URL)
...
def get_presigned_url(obj: str, expiration: int = 600):
    ...
    if response.status_code != 200:
        raise Exception(f"[{response.status_code}] Failed to get presigned url: {response.text}")
...
def get_tmp_token(database: str):
    ...
    if response.status_code != 200:
        raise Exception(f"[{response.status_code}] Failed to get tmp token: {response.text}")
