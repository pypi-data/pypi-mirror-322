...
class DatasetClient(APIKeyAuth):
        ...
        for msg in messages:
            ...
            if "content" in msg:
                ...
                for col_name in column_refs:
                    ...
                    if not col_id:
                        raise ValueError(f"Referenced column '{col_name}' not found in dataset")
                    referenced_columns.add(col_name)
                    content = content.replace(f"{{{{{col_name}}}}}", f"{{{{{col_id}}}}}")
                msg["content"] = content
    ...
    def add_evaluation(
        ...
        template_classes = {
            cls.__name__: cls 
            for cls in EvalTemplate.__subclasses__()
        }
        ...
        if "expected_response" in required_keys:
            if not expected_column_name:
                raise ValueError(f"Expected response column required for {eval_template}")
            expected_column_id = self.get_column_id(expected_column_name)
            if not expected_column_id:
                raise ValueError(f"Column '{expected_column_name}' not found in dataset")
    ...
    def add_optimization(
        ...
        if not column_id:
            raise ValueError(f"Prompt Column '{prompt_column_name}' not found in dataset")
        ...
        if not eval_template_ids:
            raise ValueError(f"No evaluation templates found for column '{prompt_column_name}'")
