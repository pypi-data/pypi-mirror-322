...
@LastEditTime: 2025-01-20 15:09:57
...
class CdpWorkInfoModel(CacheModel):
    ...
    def start_work(self, store_id, work_name):
        return self.update_table("run_start_date=%s,progress_type=1", "store_id=%s and work_name=%s", params=[TimeHelper.get_now_format_time(), store_id, work_name])
    def end_work(self, store_id, work_name, run_desc='处理结束'):
        return self.update_table("run_desc=%s,run_end_date=%s,progress_type=2", "store_id=%s and work_name=%s", params=[run_desc, TimeHelper.get_now_format_time(), store_id, work_name])
