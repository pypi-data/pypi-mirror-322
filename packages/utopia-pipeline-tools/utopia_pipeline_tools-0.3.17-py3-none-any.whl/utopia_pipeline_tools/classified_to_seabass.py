...
class MakeSeaBASS():
    def __init__(self, metadata_filepath, class_filepath, experiment, cruise, 
        ...
        if doc_list is not None and doc_list is not '':
    ...
    def preview_seabass(self):
        ...
        if self.sample_df is None: 
            ...
            if self.blob is True:
                # extract name from df
                sfn = self.sample_filenames['filepath'][0]
            else:
                # call first item in a list
                sfn = self.sample_filenames[0]
    ...
    def make_seabass_files(self):
        ...
        for fp in self.sample_filenames:
...
! (PMTA)  '''
        ...
        self.filename = f"""{self.header_values['experiment'].replace(" ", "")
                             }-{self.header_values['cruise'].replace(" ", "")}_{
                                 data_type}_IFCB_plankton_and_particles_{date
                                                                         }.sb"""
...
!  
! Files include ML automated taxonomic classification. Any image categorized as   
! 'Inoperative' is either non-living or unclassifiable due to technical issues,    
! so these images have no associated AphiaID.   
!  
...
/end_header  """
    ...
    def extract_metadata_for_header(self, sample_ID, sample_df, 
        ...
        Returns:
        :param data_type: Describes how the data was taken, i.e. in-line.
        :type data_type: str
        ...
        bin_col_name = [x for x in self.metadata_df.columns if 'bin' in 
                        x.lower()][0]
        row = self.metadata_df[self.metadata_df[bin_col_name]
                            ==sample_ID]
        
        # get data type from the sample df
        try:
            data_type = row['sample_type'][0]  # is this the right col name? 
        except:
            data_type = 'in-line'

        # get latitude and longitude from sample df or metadata df
        try:
            lat = sample_df['Latitude'][0]
            long = sample_df['Longitude'][0]
            ...
            if lat == np.nan or lat == None:
                try:
                    ...
                    long_col_name = [x for x in self.metadata_df.columns if 
                                    'lon' in x.lower()][0]
                    lat = row[lat_col_name][0]
                    long = row[long_col_name][0]
                except:
                    lat = -9999
                    long = -9999
        ...
        try:
            vol_sampled = row['VolumeSampleRequested'][0] 
        except:
            ...
            print("""INFO: No 'volume sampled' information found. Using default 
                  value (5mL).""")
        try:
            vol_imaged = row['VolumeSampled'][0] 
        except:
            vol_imaged = -9999
        ...
        if stations_bool is True:
            try:
                ...
                station = row[station_col_name][0]
            except:
                station = 'NA'
        else:
            station = 'NA'
        ...
        try: 
            ...
            if concentration == np.nan or concentration == None:
                ...
                concentration = row[concentration_col_name][0]
        except:
            concentration = -9999
    ...
    def structure_data(self, sample_filename, sample_df):
        ...
        for x in np.arange(len(sample_df.index)):
            ...
            sci_name = upt.aphiaID_dict[key][0]  # scientific name from WoRMS
            ...
            biovolume = sample_df['Biovolume'][x]/(
                upt.calibration_ratio**3)
            area = sample_df['Area'][x]/(upt.calibration_ratio**2)
            length = sample_df['MajorAxisLength'][x]/upt.calibration_ratio
            width = sample_df['MinorAxisLength'][x]/upt.calibration_ratio
            esd = sample_df['EquivalentDiameter'][x]/upt.calibration_ratio
