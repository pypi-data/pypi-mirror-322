...
def get_lines(image_palm_lines: Image) -> List[Any]:
    centers = get_cluster_centers()

    image_array = np.array(image_palm_lines)

    # Check if the image is color:
    if image_array.ndim == 3 and image_array.shape[2] == 3:
        image_array = cv2.cvtColor(image_array, cv2.COLOR_RGB2BGR)

    palmline_img = cv2.cvtColor(image_array, cv2.COLOR_RGB2GRAY)

    skeleton = skeletonize(palmline_img > 0)
