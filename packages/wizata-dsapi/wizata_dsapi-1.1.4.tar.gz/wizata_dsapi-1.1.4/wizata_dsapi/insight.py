...
class Insight(ApiDto):
    ...
    def _validate_condition(self, condition, is_first: bool, is_last: bool):
        ...
        if is_else_condition:
            ...
            if "type" not in condition or condition['type'] not in self.VALID_TYPES:
                raise ValueError(f"'type' must be one of {self.VALID_TYPES} if specified.")
            if "value" not in condition or not isinstance(condition['value'], (float, int)):
    ...
    def to_json(self, target: str = None):
        ...
        if self._conditions is not None:
            if not isinstance(self._conditions, dict):
