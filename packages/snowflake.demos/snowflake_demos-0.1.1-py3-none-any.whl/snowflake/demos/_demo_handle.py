...
class DemoHandle:
    ...
    def _setup_handle(self) -> None:
        ...
        if not create_demo_notebooks(self._name, self._num_steps, self._demo_connection.get_root()):
            return
        if not run_setup_commands(self._demo_connection.get_root().connection.cursor(), self._name):
            return
    ...
    def show(self, step: int = 1) -> None:
        ...
        if status:
            ...
            CONSOLE_MANGAER.print_url(url, display_text="Open Notebook")
