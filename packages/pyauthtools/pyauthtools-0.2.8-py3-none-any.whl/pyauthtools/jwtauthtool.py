...
def encode(payload):
  ...
  try:
    ...
    print('pyauthtools.encode, exception: ', traceback.format_exc())
    return e
...
def auth(func):
  def validate_auth(*args, **kwargs):
    try:
      ...
      result = decode(token)
      if result == 'err':
        print('pyauthtools.auth, error: counter a exception while decoding...')
        return redirect(f'http://{JWT_AUTH_SERVICE}/login?url={url}')
      print('pyauthtools.auth, headers: ', headers)
      return func(*args, **kwargs)
    except Exception as e:
      print('pyauthtools.auth, exception: ', traceback.format_exc())
      return redirect(f'http://{JWT_AUTH_SERVICE}/login?url={url}')
