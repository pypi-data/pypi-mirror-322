...
class AppPlus:
    ...
    def __init__(
        ...
        # 处理OpenAPI和UI相关的路由
        self._setup_openapi_and_ui_route(app)


    def _setup_openapi_and_ui_route(self, app: FastAPI) -> None:
        """
        设置OpenAPI和相关UI的路由
        :param app:FastAPI实例
        :return:None
        ...
        if app.openapi_url:
            ...
            # 设置静态资产路径
            self._setup_static_path(app)

    def _setup_static_path(self, app: FastAPI) -> None:
        """
        设置静态文件路径
        :param app:FastAPI实例
        :return:None
        """
        self.static = Path(__file__).absolute().parent / 'static'
    ...
    def _custom_openapi(r: Request) -> JSONResponse:
        ...
        paths = r.app.openapi_schema.get('paths', {})
...
def app_plus(
    ...
    :return:None
    """
    AppPlus(app, cors, cors_allow_credentials, cors_allow_origins, cors_allow_methods, cors_allow_headers)
