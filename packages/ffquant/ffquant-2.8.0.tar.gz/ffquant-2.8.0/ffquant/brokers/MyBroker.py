...
class MyBroker(bt.BrokerBase):
    ...
    def _update_cashvalue(self, force_update=False):
        # cash
        if force_update or datetime.now().timestamp() - self.last_cash_update_ts > self.MIN_UPDATE_INTERVAL_SECONDS:
        ...
        if self.cash is not None:
            if force_update or datetime.now().timestamp() - self.last_value_update_ts > self.MIN_UPDATE_INTERVAL_SECONDS:
