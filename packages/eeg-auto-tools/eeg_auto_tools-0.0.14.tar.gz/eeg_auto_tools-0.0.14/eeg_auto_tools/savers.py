...
def plot_clusters(clusters, clusters_avg, clusters_max, clusters_min, ch_names, ch_pos, correlation_values):
    ...
    # Plot a histogram of correlation distributions
...
def ASSR_psd_plot(ERP_c, ERP_n, noised_c_idx, noised_n_idx, roi, comp, save=None, vis=False):
    ...
    sf = int(epochs_c.info['sfreq'])
    fig_psd = plt.figure(figsize=(16, 12))  #      4:2
...
def plot_noise_data(raw, noisy_channel_flags, n_channels, threshold_noise, integral_curve):
    ...
    #   
    ax1 = fig.add_subplot(gs[0, 1], sharey=ax0)  #  sharey   Y  ax0  ax1
    ...
    pos = np.array([raw.info['chs'][i]['loc'][:2] for i in range(raw.info['nchan'])])  # X  Y 
