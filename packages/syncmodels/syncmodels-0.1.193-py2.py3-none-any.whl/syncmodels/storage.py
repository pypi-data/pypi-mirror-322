        ...
        try:
            if self._has_change(uri, **data):
                ...
                if not reverse_sort_keys and sort_keys:
                    ...
                    log.debug("including sort_keys: [%s] in excluded MASK: [%s] to find similar registers", sort_keys, MASK)
                ...
                resuming_info = {k: kw.get(k, data.get(k)) for k in kw.get(SORT_KEY) or []}
