...
from params import HasLLM, HasPrompt, HasPropagateError
...
class LLMNer(BaseNer, HasLLM, HasPrompt, HasPropagateError):
    ...
    defaultParams = {
        ...
        "tags": ["PERSON", "LOCATION", "DATE", "EMAIL", "PHONE", "ORGANIZATION"],
        "partitionMap": False,
    }
    ...
    def transform_udf(self, document, params=None):
            ...
            messages=[
                {
                    ...
                    "role": "user",
                    "content": f'Pleas extract NER tags: {",".join(params["tags"])}.From the text:' + document.text
                },
            ],
        ...
        for tag in data["entities"]:
            ...
            word = tag["word"]
            for idx, box in enumerate(document.bboxes):
                if any(word.lower() in box.text.lower() for word in word.split(" ") if len(word) > 2):
                    boxes.append(box)
