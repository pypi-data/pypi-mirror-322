        ...
        with temp_sh_path.open("w", encoding="utf-8") as outfile:
            ...
            cleanup_script = dedent(
                ...
                function cleanup() {
                    ...
                    rm -f "$SCRIPT_DIR/python_stdin" "$SCRIPT_DIR/python_stdout" "$SCRIPT_DIR/python_input" 2>/dev/null || true
                }
...
async def fpk_build(directory: Union[str, None], use_euxo: bool = False):
    ...
    try:
        ...
        os.close(slave_fd)  # 👈 CLOSE SLAVE_FD HERE RIGHT AFTER PROCESS STARTS

    clean_log_path = log_dir / f"clean_{log_filename}"
    ...
    async with (
        ...
        # CHANGE 2: Add closefd=False to prevent double-closing
        transport, _ = await loop.connect_read_pipe(
            lambda: protocol,
            os.fdopen(master_fd, 'rb', closefd=False)
            # 👈 ADD closefd=False HERE
        )
    ...
    # CHANGE 3: Close master_fd AFTER processing is done
    os.close(master_fd)  # 👈 CLOSE MASTER_FD HERE AT THE END
    await process.wait()
...
def sync_hooks_to_db_on_startup():
    """Safely syncs hooks to database, ensuring database is initialized first."""
    try:
...
def main():
    ...
    with SessionManager() as session:
        ...
        if not warning_shown:
            console.print(
                "[bold red]Flatpack does NOT endorse any cryptocurrency and will NEVER do so.[/bold red]"
            )
            console.print("")
            warning_shown = True
