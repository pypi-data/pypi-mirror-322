...
class EntryDataExtrasMenu:
    ...
    def sf2e(self):
        with Session(ENGINE) as session:
            ...
            results_ede=query.all()
            field=None
            msg_ede=[]
            for num,i in enumerate(results_ede):
                msg_ede.append(f"{Fore.cyan}{num}/{Fore.light_yellow}{num+1} of {Fore.light_red}{ct}{Fore.grey_50} -> {i.field_name}:{i.field_type}={i.field_value} ede_id={i.ede_id} doe={i.doe}")
            msg_ede='\n'.join(msg_ede)
            while True:
                try:
                    print(msg_ede)
