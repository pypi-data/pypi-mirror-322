    ...
    def fwd(
        ...
        for i in range(self.n_layers):
            ...
            if self.STORE_OUTPUTS and self.is_decoder:
                lens_scores = self.proj(tensor).view(bs, slen, -1)
                lens_probs = F.softmax(lens_scores, dim=-1)
                lens_token_ids = torch.argmax(lens_probs, dim=-1)

                # convert ids to strings
                lens_tokens = [
                    [self.id2word[token_idx.item()] for token_idx in sample_ids]
                    for sample_ids in lens_token_ids
                ]
