...
def is_version_upgrade(app="nxenv", nxcli_path=".", branch=None):
	upstream_version = get_upstream_version(app=app, branch=branch, nxcli_path=nxcli_path)
	...
	local_version = get_major_version(get_current_version(app, nxcli_path=nxcli_path))
...
def switch_branch(branch, apps=None, nxcli_path=".", upgrade=False, check_upgrade=True):
	import git
	from nxcli.nxcli import Nxcli
	from nxcli.utils import log, exec_cmd
	...
	for app in apps:
		...
		if check_upgrade:
			version_upgrade = is_version_upgrade(app=app, nxcli_path=nxcli_path, branch=branch)
	...
	if switched_apps:
		...
		print(
			"Please run `nxcli update --patch` to be safe from any differences in"
			" database schema"
		)

	if version_upgrade[0] and upgrade:
		Nxcli(nxcli_path).setup.requirements()
		backup_all_sites()
		patch_sites()
		build_assets()
...
def switch_to_branch(branch=None, apps=None, nxcli_path=".", upgrade=False):
	switch_branch(branch, apps=apps, nxcli_path=nxcli_path, upgrade=upgrade)


def switch_to_develop(apps=None, nxcli_path=".", upgrade=True):
	switch_branch("develop", apps=apps, nxcli_path=nxcli_path, upgrade=upgrade)
...
def get_develop_version(app, nxcli_path="."):
	repo_dir = get_repo_dir(app, nxcli_path=nxcli_path)
...
def get_upstream_version(app, branch=None, nxcli_path="."):
	repo_dir = get_repo_dir(app, nxcli_path=nxcli_path)
	if not branch:
...
def get_current_nxenv_version(nxcli_path="."):
	try:
		return get_major_version(get_current_version("nxenv", nxcli_path=nxcli_path))
	except OSError:
		return 0


def get_current_branch(app, nxcli_path="."):
...
def get_app_name(nxcli_path: str, folder_name: str) -> str:
	...
	"""
	app_name = None
	apps_path = os.path.join(os.path.abspath(nxcli_path), "apps")
...
def check_existing_dir(nxcli_path, repo_name):
	cloned_path = os.path.join(nxcli_path, "apps", repo_name)
	dir_already_exists = os.path.isdir(cloned_path)
	return dir_already_exists, cloned_path


def get_current_version(app, nxcli_path="."):
