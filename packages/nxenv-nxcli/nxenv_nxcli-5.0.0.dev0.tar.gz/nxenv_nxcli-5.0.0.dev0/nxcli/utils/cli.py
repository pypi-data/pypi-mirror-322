...
def print_nxcli_version(ctx, param, value):
	"""Prints current nxcli version"""
	if not value or ctx.resilient_parsing:
		return

	import nxcli

	click.echo(nxcli.VERSION)
	ctx.exit()
...
def use_experimental_feature(ctx, param, value):
	...
	if value == "dynamic-feed":
		...
		nxcli.cli.dynamic_feed = True
		nxcli.cli.verbose = True
	else:
		from nxcli.exceptions import FeatureDoesNotExistError

		raise FeatureDoesNotExistError(f"Feature {value} does not exist")

	from nxcli.cli import is_envvar_warn_set

	if is_envvar_warn_set:
		return

	click.secho(
		"WARNING: nxcli is using it's new CLI rendering engine. This behaviour has"
