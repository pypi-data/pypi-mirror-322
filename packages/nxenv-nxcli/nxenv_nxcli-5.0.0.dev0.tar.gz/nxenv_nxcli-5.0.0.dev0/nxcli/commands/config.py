...
from nxcli.config.common_site_config import update_config, put_config

# imports - third party imports
import click


@click.group(help="Change nxcli configuration")
def config():
	pass
...
@click.command(
	"dns_multitenant", help="Enable/Disable nxcli multitenancy on running nxcli update"
)
@click.argument("state", type=click.Choice(["on", "off"]))
...
def set_common_config(configs):
	...
	update_config(common_site_config, nxcli_path=".")


@click.command(
	"remove-common-config", help="Remove specific keys from current nxcli's common config"
)
@click.argument("keys", nargs=-1)
def remove_common_config(keys):
	from nxcli.nxcli import Nxcli

	common_site_config = Nxcli(".").conf
	for key in keys:
