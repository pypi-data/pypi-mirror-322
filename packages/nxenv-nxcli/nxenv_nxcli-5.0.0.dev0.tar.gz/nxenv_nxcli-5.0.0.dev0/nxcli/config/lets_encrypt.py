...
from nxcli.config.production_setup import service
from nxcli.config.site_config import get_domains, remove_domain, update_site_config
from nxcli.nxcli import Nxcli
from nxcli.utils import exec_cmd, which
from nxcli.utils.nxcli import update_common_site_config
from nxcli.exceptions import CommandFailedError


def setup_letsencrypt(site, custom_domain, nxcli_path, interactive):

	site_path = os.path.join(nxcli_path, "sites", site, "site_config.json")
	...
	if not Nxcli(nxcli_path).conf.get("dns_multitenant"):
		print("You cannot setup SSL without DNS Multitenancy")
		return

	create_config(site, custom_domain)
	run_certbot_and_setup_ssl(site, custom_domain, nxcli_path, interactive)
	setup_crontab()
...
def run_certbot_and_setup_ssl(site, custom_domain, nxcli_path, interactive=True):
	service("nginx", "stop")
	...
	if custom_domain:
		remove_domain(site, custom_domain, nxcli_path)
		domains = get_domains(site, nxcli_path)
		ssl_config["domain"] = custom_domain
		domains.append(ssl_config)
		update_site_config(site, {"domains": domains}, nxcli_path=nxcli_path)
	else:
		update_site_config(site, ssl_config, nxcli_path=nxcli_path)

	make_nginx_conf(nxcli_path)
	service("nginx", "start")
...
def setup_wildcard_ssl(domain, email, nxcli_path, exclude_base_domain):
