...
from nxcli.utils import get_nxcli_name


def make_nginx_conf(nxcli_path, yes=False, logging=None, log_format=None):
	conf_path = os.path.join(nxcli_path, "config", "nginx.conf")
	...
	if allow_rate_limiting:
		template_vars.update(
			{
				"nxcli_name_hash": hashlib.sha256(nxcli_name).hexdigest()[:16],
...
def make_nxcli_manager_nginx_conf(nxcli_path, yes=False, port=23624, domain=None):
	from nxcli.config.site_config import get_site_config

	template = nxcli.config.env().get_template("nxcli_manager_nginx.conf")
	nxcli_path = os.path.abspath(nxcli_path)
	sites_path = os.path.join(nxcli_path, "sites")

	config = Nxcli(nxcli_path).conf
	site_config = get_site_config(domain, nxcli_path=nxcli_path)
	nxcli_name = get_nxcli_name(nxcli_path)

	template_vars = {
		"port": port,
		"domain": domain,
		"nxcli_manager_site_name": "nxcli-manager.local",
		"sites_path": sites_path,
		"http_timeout": config.get("http_timeout"),
		"webserver_port": config.get("webserver_port"),
		"socketio_port": config.get("socketio_port"),
	...
	nxcli_manager_nginx_conf = template.render(**template_vars)

	conf_path = os.path.join(nxcli_path, "config", "nginx.conf")

	if not yes and os.path.exists(conf_path):
		click.confirm(
			"nginx.conf already exists and nxcli-manager configuration will be appended to it. Do you want to continue?",
			abort=True,
		)

	with open(conf_path, "a") as myfile:
		myfile.write(nxcli_manager_nginx_conf)


def prepare_sites(config, nxcli_path):
	sites = {
		"that_use_port": [],
		"that_use_dns": [],
		"that_use_ssl": [],
	...
	sites_configs = get_sites_with_config(nxcli_path=nxcli_path)
...
def get_sites_with_config(nxcli_path):
	from nxcli.nxcli import Nxcli
	from nxcli.config.site_config import get_site_config
	...
	for site in sites:
		try:
			site_config = get_site_config(site, nxcli_path=nxcli_path)
	...
	use_wildcard_certificate(nxcli_path, ret)

	return ret


def use_wildcard_certificate(nxcli_path, ret):
	"""
...
def get_error_pages():
	nxcli_app_path = os.path.abspath(nxcli.__path__[0])
	templates = os.path.join(nxcli_app_path, "config", "templates")
