...
def relative_abundance_coverm_(
    ...
    **kwargs: Additional arguments.
    ...
    if container == "none":
        if paired:
            command= f"coverm genome -m relative_abundance -1 {read1} -2 {read2} --genome-fasta-directory {genomes_dir} -o {output_file} -x {genome_extension} "
        
        else:
            command= f"coverm genome -m relative_abundance --single {read1} --genome-fasta-directory {genomes_dir} -o {output_file} -x {genome_extension} "
        ...
        if paired:
            ...
            command= f"docker run -v {bind_path} {config.docker_container} coverm genome -m relative_abundance -1 {read1} -2 {read2} --genome-fasta-directory {genomes_dir} -o {output_file} -x {genome_extension} "
        else:
            ...
            command= f"docker run -v {bind_path} {config.docker_container} coverm genome -m relative_abundance --single {read1} --genome-fasta-directory {genomes_dir} -o {output_file} -x {genome_extension} "
        ...
        if paired:
            ...
            command= f"singularity exec --bind {bind_path} {config.singularity_container} coverm genome -m relative_abundance -1 {read1} -2 {read2} --genome-fasta-directory {genomes_dir} -o {output_file} -x {genome_extension} "
        else:
            ...
            command= f"singularity exec --bind {bind_path} {config.singularity_container} coverm genome -m relative_abundance --single {read1} --genome-fasta-directory {genomes_dir} -o {output_file} -x {genome_extension} "
